...
6. `WALL_LOCK_MVP_RUNTIME_BEHAVIOR.md` - current MVP wall lock behavior, including inventory key use, cursor insertion and `Activated -> Door.Open` connector rule.
7. `06_GRID_EDITOR_UX_SPEC.md` - current editor UX target and implemented UI decisions.
8. `07_GRID_OBJECT_ARCHETYPE_ASSET_AUDIT.md` - audit of archetype fields after UI/runtime cleanup.
9. `11_GRID_OBJECT_ARCHETYPE_PARAMETERS_REFERENCE.md` - practical reference explaining each DataAsset / GridObjectArchetypeAsset parameter.
10. `ITEM_AND_PICKUP_ASSET_CREATION_GUIDE.md` - production guide for item definitions, pickup archetypes, palette entries and receptacle content.
11. `08_GRID_OBJECT_ARCHETYPE_DATA_ASSETS_AUDIT.md` - audit/checklist for concrete DataAssets.
12. `09_GRID_OBJECT_ARCHETYPE_NAMING_NORMALIZATION_PLAN.md` - naming normalization plan.
13. `10_GRID_EDITOR_UI_CONSISTENCY_CHECKLIST.md` - UI/runtime consistency checklist.
14. `04_IMPLEMENTATION_ROADMAP.md` - historical implementation roadmap.
15. `05_CODEX_TASKS.md` - historical Codex task templates.
16. `99_DECISIONS_LOG.md` - chronological decision log.
