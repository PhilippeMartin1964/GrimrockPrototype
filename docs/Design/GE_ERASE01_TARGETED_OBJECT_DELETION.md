...
4. warn when deleting the object also removes incoming/outgoing logical links;
5. delete exactly one placement by `FGuid` through `UGridLevelAsset::RemoveObjectById`;
6. rebuild the preview and clear stale selection/link-source state;
7. wrap the mutation in an Unreal editor transaction so `Ctrl+Z` can restore it.
...
7. Delete an object with logical links and verify that the confirmation warns about the link count.
8. Press `Ctrl+Z` and verify that the deleted object and its links are restored.
9. On a cell without object candidates, verify that the previous wall/cell erase fallback still works.
