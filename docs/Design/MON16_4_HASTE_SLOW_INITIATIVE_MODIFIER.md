...
Pour un Haste/Slow classique, le design pourra utiliser `NoStack`, `RefreshDuration` ou `ReplaceIfStronger` avec `MaxStacks=1`.
...
`UGridStatusEffectLifecycleSubsystem` écoute désormais :
...
- le round suivant utilisera naturellement le modificateur encore actif.
...
```text
AggregateModifier
StackScalingAndSaturation
TurnOrderBroadcastProjection
FutureHasteReorder
FutureSlowReorder
ActiveCombatantStability
TurnExpiration
RoundExpiration
MonsterParity
ReapplicationUpdatesModifier
NoParallelSystem
```
...
MON16.4 sera déclaré **VALIDÉ ET CLOS** uniquement après chargement/compilation UE5.5.4 et succès des tests sur logs utilisateur.

Prochaine étape après clôture : **MON16.5 — Stun / Silence / Immobilize**.
