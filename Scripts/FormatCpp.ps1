...
Write-Host "Formatage de $($Files.Count) fichiers C++ first-party..."

foreach ($File in $Files)
{
    $Arguments = @('-i', '--style=file', '--Werror')
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "clang-format a echoue sur : $($File.FullName)"
    }
}

Write-Host 'Formatage C++ termine.'
