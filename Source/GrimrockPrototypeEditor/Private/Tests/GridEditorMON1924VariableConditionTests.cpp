...
namespace
{
    ...
    FGridLevelObjectData MakeEditorObject1924 (
        FGuid Id,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridEditorMON1924VariableConditionPolicyTest,
    "Grimrock.MON19.2.Editor.VariableConditions.PolicyAndTyping",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridEditorMON1924VariableConditionPolicyTest::RunTest (
    const FString& Parameters)
{
    ...
    const TArray<EGridObjectCondition> DoorConditions =
        GridEditorLinkPolicy::GetSupportedConditionsForTarget (Door);
    TestEqual (TEXT ("Non-receptacle exposes None plus two variable conditions"),
        DoorConditions.Num (), 3);
    TestTrue (TEXT ("Door exposes Bool variable condition"),
        DoorConditions.Contains (EGridObjectCondition::LevelVariableBoolEquals));
    TestTrue (TEXT ("Door exposes Int variable condition"),
        DoorConditions.Contains (EGridObjectCondition::LevelVariableIntCompare));
    ...
    TestEqual (TEXT ("Receptacle keeps eight historical options and adds two variables"),
        ReceptacleConditions.Num (), 10);
    TestTrue (TEXT ("Receptacle still exposes item-tag condition"),
        ReceptacleConditions.Contains (
            EGridObjectCondition::ReceptacleContainsItemTag));
    TestTrue (TEXT ("Receptacle also exposes Bool variable condition"),
        ReceptacleConditions.Contains (
    ...
    Level->Objects.Add (MakeEditorObject1924 (
        SourceId,
        EGridLevelObjectType::Button));
    Level->Objects.Add (MakeEditorObject1924 (
        TargetId,
        EGridLevelObjectType::Door));
    Level->LevelVariables = {
        MakeEditorBoolVariable1924 (TEXT ("Gate")),
        MakeEditorIntVariable1924 (TEXT ("Count"))
    };
    ...
    TestTrue (TEXT ("Declared Bool variable is accepted for Bool condition"),
        GridEditorLinkService::IsLinkSupported (*Level, BoolLink));

    BoolLink.ConditionVariableId = TEXT ("Count");
    TestFalse (TEXT ("Int variable is rejected for Bool condition"),
        GridEditorLinkService::IsLinkSupported (*Level, BoolLink));
    ...
    TestTrue (TEXT ("Declared Int32 variable is accepted for Int condition"),
        GridEditorLinkService::IsLinkSupported (*Level, IntLink));

    IntLink.ConditionIntComparison = static_cast<EGridLogicIntComparison> (255);
    TestFalse (TEXT ("Invalid Int comparator is rejected"),
        GridEditorLinkService::IsConditionConfigurationValid (IntLink));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridEditorMON1924VariableConditionIdentityTest,
    "Grimrock.MON19.2.Editor.VariableConditions.IdentityAndNormalization",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridEditorMON1924VariableConditionIdentityTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    TestFalse (TEXT ("Bool expected value participates in exact identity"),
    ...
    TestFalse (TEXT ("VariableId participates in exact identity"),
        GridEditorLinkPolicy::AreLinksExactlyEquivalent (
            BoolTrue,
            OtherVariable));
    ...
    TestFalse (TEXT ("Int comparator participates in exact identity"),
        GridEditorLinkPolicy::AreLinksExactlyEquivalent (IntA, IntB));
    IntB = IntA;
    IntB.ConditionIntValue = 4;
    TestFalse (TEXT ("Int threshold participates in exact identity"),
        GridEditorLinkPolicy::AreLinksExactlyEquivalent (IntA, IntB));
    ...
    const FGridObjectLink NormalizedBool =
        GridEditorLinkService::NormalizeLink (BoolTrue);
    TestTrue (TEXT ("Bool normalization keeps VariableId"),
        NormalizedBool.ConditionVariableId == FName (TEXT ("Gate")));
    TestTrue (TEXT ("Bool normalization keeps expected Bool"),
        NormalizedBool.ConditionBoolValue);
    TestTrue (TEXT ("Bool normalization clears stale item definition"),
        NormalizedBool.ConditionItemDefinitionId.IsNone ());
    TestTrue (TEXT ("Bool normalization clears stale item tag"),
        NormalizedBool.ConditionItemTag.IsNone ());
    TestEqual (TEXT ("Bool normalization resets Int threshold"),
        NormalizedBool.ConditionIntValue,
        0);
    TestTrue (TEXT ("Bool normalization resets Int comparator"),
        NormalizedBool.ConditionIntComparison == EGridLogicIntComparison::Equal);

    IntA.ConditionItemDefinitionId = TEXT ("StaleItem");
    IntA.ConditionBoolValue = true;
    const FGridObjectLink NormalizedInt =
        GridEditorLinkService::NormalizeLink (IntA);
    TestTrue (TEXT ("Int normalization keeps VariableId"),
        NormalizedInt.ConditionVariableId == FName (TEXT ("Count")));
    TestTrue (TEXT ("Int normalization keeps comparator"),
        NormalizedInt.ConditionIntComparison ==
            EGridLogicIntComparison::GreaterOrEqual);
    TestEqual (TEXT ("Int normalization keeps threshold"),
        NormalizedInt.ConditionIntValue,
        3);
    TestFalse (TEXT ("Int normalization clears stale Bool value"),
        NormalizedInt.ConditionBoolValue);
    TestTrue (TEXT ("Int normalization clears stale item definition"),
        NormalizedInt.ConditionItemDefinitionId.IsNone ());
    return true;
}
