	...
	struct FGridMON1431EditorTestWorld
	{
		...
		FGridMON1431EditorTestWorld()
		{
			...
			World = UWorld::CreateWorld(EWorldType::EditorPreview, false,
				FName(*FString::Printf(TEXT("MON1431EditorTestWorld_%s"), *FGuid::NewGuid().ToString(EGuidFormats::Digits))), nullptr, true,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMON1431PatrolRouteEditingModelTest, "Grimrock.Editor.MON14.3.1.PatrolRouteEditingModel",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMON1431PatrolRouteEditingModelTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	...
	TestTrue(TEXT("Selected MonsterSpawn supports patrol editing"), Fixture.EditorActor->CanEditSelectedMonsterPatrolRoute());

	Fixture.EditorActor->ToggleSelectedMonsterPatrolRouteEditing();
	TestTrue(TEXT("Patrol route edit mode activates"), Fixture.EditorActor->IsPatrolRouteEditModeActive());
	...
	TestEqual(TEXT("Single waypoint keeps patrol disabled"), Spawn->PatrolMode, EGridMonsterPatrolMode::None);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMON1431PatrolRouteGuardTest, "Grimrock.Editor.MON14.3.1.PatrolRouteGuards", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMON1431PatrolRouteGuardTest::RunTest(const FString& Parameters)
{
	...
	TestFalse(TEXT("Loop cannot be enabled without two waypoints"), Fixture.EditorActor->SetSelectedMonsterPatrolMode(EGridMonsterPatrolMode::Loop));

	Fixture.EditorActor->HoveredCellX = -1;
	Fixture.EditorActor->HoveredCellY = 99;
	TestFalse(TEXT("Invalid hovered cell cannot create waypoint"), Fixture.EditorActor->AddOrSelectPatrolWaypointAtHoveredCell());

	TestFalse(TEXT("No waypoint can be removed before one is selected"), Fixture.EditorActor->RemoveSelectedPatrolWaypoint());
	return true;
}
