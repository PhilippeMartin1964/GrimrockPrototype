...
namespace
{
    FGridLuaScriptSource MakeScript (
        FName ScriptId,
        const TCHAR* Source,
        bool bEnabled = true)
    {
    ...
    bool HasMessageContaining (
        const TArray<FGridLevelValidationMessage>& Messages,
        ...
        FName Category = NAME_None)
    {
        ...
        {
            return (Category.IsNone () || Message.Category == Category) &&
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON196LuaScriptAnalysisTest,
    "Grimrock.MON19.6.Editor.LuaScriptAnalysis",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON196LuaScriptAnalysisTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;

    UGridLevelAsset* Level = MakeLevel (GetTransientPackage ());
    Level->LuaScripts.Add (MakeScript (
        TEXT ("Puzzle"),
        TEXT ("function on_trigger(event)\nend\nhelper = function(event)\nend\nvalue = 3\n")));

    FGridEditorLuaAnalysis Analysis;
    TestTrue (TEXT ("Valid level Lua analysis succeeds"),
        GridEditorLuaService::AnalyzeLevel (*Level, Analysis));
    TestTrue (TEXT ("Script definitions are valid"),
        Analysis.bDefinitionsValid);
    TestTrue (TEXT ("Combined VM builds"),
        Analysis.bFullVmValid);
    TestEqual (TEXT ("One script analyzed"),
        Analysis.Scripts.Num (), 1);
    if (Analysis.Scripts.Num () == 1)
    {
        TestTrue (TEXT ("on_trigger callback detected"),
            Analysis.Scripts[0].CallbackNames.Contains (TEXT ("on_trigger")));
        TestTrue (TEXT ("global function assignment detected"),
            Analysis.Scripts[0].CallbackNames.Contains (TEXT ("helper")));
        TestFalse (TEXT ("non-function global excluded"),
            Analysis.Scripts[0].CallbackNames.Contains (TEXT ("value")));
    }

    Level->LuaScripts[0].Source = TEXT ("function broken(\n");
    TestFalse (TEXT ("Syntax error is rejected"),
        GridEditorLuaService::AnalyzeLevel (*Level, Analysis));
    TestFalse (TEXT ("Invalid enabled script is reported"),
        Analysis.Scripts.IsEmpty () || Analysis.Scripts[0].bValid);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON196LuaBindingMutationTest,
    "Grimrock.MON19.6.Editor.LuaBindingAndScriptMutations",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON196LuaBindingMutationTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    FString Error;
    TestTrue (TEXT ("Targetless typed Lua binding is supported"),
        GridEditorLuaService::IsLuaLinkSupported (*Level, Link, Error));
    TestTrue (TEXT ("Lua binding is added"),
        GridEditorLuaService::AddLuaLink (*Editor, Link, Error));
    TestEqual (TEXT ("One link stored"), Level->Links.Num (), 1);
    if (!Level->Links.IsEmpty ())
    {
        TestFalse (TEXT ("Lua binding is canonical targetless"),
            Level->Links[0].TargetObjectId.IsValid ());
    }

    FGridObjectLink InvalidCondition = Link;
    InvalidCondition.Condition = EGridObjectCondition::ReceptacleHasAnyItem;
    TestFalse (TEXT ("Receptacle condition is rejected for Lua"),
        GridEditorLuaService::IsLuaLinkSupported (
            *Level,
            InvalidCondition,
            Error));

    TestTrue (TEXT ("Rename updates script and binding atomically"),
        GridEditorLuaService::RenameScript (
            *Level,
            TEXT ("Puzzle"),
            TEXT ("CryptPuzzle"),
            Error));
    TestEqual (TEXT ("Binding follows renamed ScriptId"),
        Level->Links[0].LuaScriptId,
        FName (TEXT ("CryptPuzzle")));

    TestFalse (TEXT ("Referenced script cannot be disabled"),
        GridEditorLuaService::SetScriptEnabled (
            *Level,
            TEXT ("CryptPuzzle"),
            false,
            Error));
    TestFalse (TEXT ("Referenced script cannot be removed"),
        GridEditorLuaService::RemoveScript (
            *Level,
            TEXT ("CryptPuzzle"),
            Error));

    TestTrue (TEXT ("Exact Lua binding can be removed"),
        GridEditorLuaService::RemoveLuaLink (
            *Editor,
            Level->Links[0]));
    TestTrue (TEXT ("Unreferenced script can be removed"),
        GridEditorLuaService::RemoveScript (
            *Level,
            TEXT ("CryptPuzzle"),
            Error));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON196ValidationAlignmentTest,
    "Grimrock.MON19.6.Editor.ValidationAlignment",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON196ValidationAlignmentTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    const TArray<FGridLevelValidationMessage> Messages =
        GridEditorLuaService::ValidateCurrentLevelWithLua (*Editor);

    TestFalse (TEXT ("Targetless Lua link is not a false legacy error"),
        HasMessageContaining (Messages, TEXT ("invalid TargetObjectId")));
    TestFalse (TEXT ("Different Lua callbacks are not false duplicates"),
        HasMessageContaining (Messages, TEXT ("Link 1 duplicates an identical link")));
    TestFalse (TEXT ("Data-only Relay has no Logic validation error"),
        HasMessageContaining (Messages, TEXT ("Logic object is invalid"), TEXT ("Logic")));
    TestFalse (TEXT ("Data-only Logic does not require an archetype"),
        Messages.ContainsByPredicate ([&Logic] (const FGridLevelValidationMessage& Message)
        {
            return Message.OptionalObjectId == Logic.ObjectId &&
                Message.Message.Contains (TEXT ("no ArchetypeId"));
        }));
    TestFalse (TEXT ("Valid Lua bindings have no Lua validation error"),
        ...
        {
            return Message.Category == TEXT ("Lua") &&
                Message.Severity == EGridLevelValidationSeverity::Error;
        }));
    return true;
}
