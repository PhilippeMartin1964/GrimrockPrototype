	...
	struct FTestWorld
	{
		...
		FTestWorld()
		{
			...
			World = UWorld::CreateWorld(EWorldType::EditorPreview, false,
				FName(*FString::Printf(TEXT("MON2045_%s"), *FGuid::NewGuid().ToString(EGuidFormats::Digits))), nullptr, true, ERHIFeatureLevel::Num, &Values);
			if (!World || !GEngine)
			{
				return;
			}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridEditorMON2045StoryCompanionPalettePlacementTest, "Grimrock.MON20.4.RecruitmentUI.PalettePlacement",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridEditorMON2045StoryCompanionPalettePlacementTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
