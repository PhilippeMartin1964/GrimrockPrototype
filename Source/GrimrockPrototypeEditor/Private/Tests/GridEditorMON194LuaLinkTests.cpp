...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON194LuaLinkIdentityTest,
    "Grimrock.MON19.4.Editor.LuaLinkIdentity",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON194LuaLinkIdentityTest::RunTest (
    const FString& Parameters)
{
    ...
    TestTrue (
        TEXT ("Same Lua callback link is exactly equivalent"),
        GridEditorLinkPolicy::AreLinksExactlyEquivalent (Base, Same));

    FGridObjectLink OtherScript = Base;
    OtherScript.LuaScriptId = TEXT ("OtherPuzzle");
    TestFalse (
        TEXT ("ScriptId participates in exact link identity"),
    ...
    TestFalse (
        TEXT ("CallbackName participates in exact link identity"),
        GridEditorLinkPolicy::AreLinksExactlyEquivalent (
            Base,
            OtherCallback));
