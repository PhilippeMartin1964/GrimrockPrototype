...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridEditorMON192LinkPolicyMatrixTest, "Grimrock.MON19.2.Editor.LinkPolicyMatrix", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridEditorMON192LinkPolicyMatrixTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	...
	TestTrue(TEXT("Lever emits exactly Activated and Deactivated"),
		ContainsExactly(GridEditorLinkPolicy::GetSupportedEventsForSource(Lever), { EGridObjectEvent::Activated, EGridObjectEvent::Deactivated }));
	TestTrue(TEXT("PressurePlate emits exactly Activated and Deactivated"),
	...
	TestTrue(TEXT("MonsterSpawn emits exactly its six lifecycle events"),
		ContainsExactly(GridEditorLinkPolicy::GetSupportedEventsForSource(MonsterSpawn),
			{ EGridObjectEvent::MonsterDied, EGridObjectEvent::MonsterSpawned, EGridObjectEvent::MonsterDespawned, EGridObjectEvent::MonsterTeleported,
				EGridObjectEvent::EncounterWaveStarted, EGridObjectEvent::EncounterCompleted }));
	TestFalse(TEXT("Door is not an event source in the current editor contract"), GridEditorLinkPolicy::CanObjectEmitEvents(Door));

	const TArray<EGridObjectCommand> DoorCommands = GridEditorLinkPolicy::GetSupportedCommandsForTarget(Door);
	TestTrue(TEXT("Door exposes exactly five connector commands"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridEditorMON192ConditionalLinkIdentityTest, "Grimrock.MON19.2.Editor.ConditionalLinkIdentity",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridEditorMON192ConditionalLinkIdentityTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	...
	TestTrue(TEXT("Exact persistent connector data is equivalent"), GridEditorLinkPolicy::AreLinksExactlyEquivalent(Base, ExactCopy));

	FGridObjectLink DifferentCondition = Base;
	DifferentCondition.Condition = EGridObjectCondition::ReceptacleHasAnyItem;
	TestFalse(TEXT("Same historical quadruplet with another condition is distinct"), GridEditorLinkPolicy::AreLinksExactlyEquivalent(Base, DifferentCondition));

	FGridObjectLink DifferentDefinition = Base;
	DifferentDefinition.ConditionItemDefinitionId = TEXT("Item_BlueGem");
	TestFalse(TEXT("A condition parameter participates in connector identity"), GridEditorLinkPolicy::AreLinksExactlyEquivalent(Base, DifferentDefinition));

	FGridObjectLink DifferentCount = Base;
	DifferentCount.ConditionCount = 2;
	TestFalse(TEXT("ConditionCount participates in connector identity"), GridEditorLinkPolicy::AreLinksExactlyEquivalent(Base, DifferentCount));

	FGridObjectLink DifferentWeight = Base;
	DifferentWeight.ConditionWeight = 3.0f;
	TestFalse(TEXT("ConditionWeight participates in connector identity"), GridEditorLinkPolicy::AreLinksExactlyEquivalent(Base, DifferentWeight));

	FGridObjectLink Inverted = Base;
	Inverted.bInvertCondition = true;
	TestFalse(TEXT("Condition inversion participates in connector identity"), GridEditorLinkPolicy::AreLinksExactlyEquivalent(Base, Inverted));
