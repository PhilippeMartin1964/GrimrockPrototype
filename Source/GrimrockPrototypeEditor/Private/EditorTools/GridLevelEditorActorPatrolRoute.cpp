...
void AGridLevelEditorActor::ToggleSelectedMonsterPatrolRouteEditing ()
{
    ...
    if (!CanEditSelectedMonsterPatrolRoute ())
    {
        ...
        UE_LOG (LogTemp, Warning,
            TEXT ("[MON14.3.1] Patrol route edit requires a selected MonsterSpawn."));
        RedrawGridEditorViewports ();
        return;
    }
...
bool AGridLevelEditorActor::SetSelectedMonsterPatrolMode (
    EGridMonsterPatrolMode NewMode)
{
    ...
    {
        UE_LOG (LogTemp, Warning,
            TEXT ("[MON14.3.1] Patrol mode requires at least two waypoints. ObjectId=%s"),
    ...
    const FScopedTransaction Transaction (
        FText::FromString (TEXT ("Set Monster Patrol Mode")));
    LevelAsset->Modify ();
...
bool AGridLevelEditorActor::AddOrSelectPatrolWaypointAtHoveredCell ()
{
    FGridLevelObjectData* Obj = FindSelectedObjectMutable ();
    if (!LevelAsset || !Obj || Obj->Type != EGridLevelObjectType::MonsterSpawn ||
        !LevelAsset->IsValidCoord (HoveredCellX, HoveredCellY))
    {
        return false;
    }
    ...
    const FScopedTransaction Transaction (
        FText::FromString (TEXT ("Add Monster Patrol Waypoint")));
    LevelAsset->Modify ();
...
bool AGridLevelEditorActor::SelectPatrolWaypointByIndex (int32 WaypointIndex)
{
    ...
    if (!Obj || Obj->Type != EGridLevelObjectType::MonsterSpawn ||
        !Obj->PatrolWaypoints.IsValidIndex (WaypointIndex))
    {
        return false;
    }
...
bool AGridLevelEditorActor::RemoveSelectedPatrolWaypoint ()
{
    FGridLevelObjectData* Obj = FindSelectedObjectMutable ();
    if (!LevelAsset || !Obj || Obj->Type != EGridLevelObjectType::MonsterSpawn ||
        !Obj->PatrolWaypoints.IsValidIndex (SelectedPatrolWaypointIndex))
    {
        return false;
    }

    const FScopedTransaction Transaction (
        FText::FromString (TEXT ("Remove Monster Patrol Waypoint")));
    LevelAsset->Modify ();
    ...
    {
        SelectedPatrolWaypointIndex = FMath::Clamp (
            SelectedPatrolWaypointIndex,
            0,
            Obj->PatrolWaypoints.Num () - 1);
    }
...
bool AGridLevelEditorActor::ClearSelectedMonsterPatrolRoute ()
{
    ...
    const FScopedTransaction Transaction (
        FText::FromString (TEXT ("Clear Monster Patrol Route")));
    LevelAsset->Modify ();
...
bool AGridLevelEditorActor::MoveSelectedPatrolWaypoint (int32 IndexDelta)
{
    ...
    const int32 NewIndex = FMath::Clamp (
        SelectedPatrolWaypointIndex + IndexDelta,
        0,
        Obj->PatrolWaypoints.Num () - 1);
    ...
    const FScopedTransaction Transaction (
        FText::FromString (TEXT ("Reorder Monster Patrol Waypoint")));
    LevelAsset->Modify ();
...
bool AGridLevelEditorActor::SetSelectedPatrolWaypointFacing (EGridEdge NewFacing)
{
    ...
    FGridMonsterPatrolWaypoint& Waypoint =
        Obj->PatrolWaypoints[SelectedPatrolWaypointIndex];
    ...
    const FScopedTransaction Transaction (
        FText::FromString (TEXT ("Set Monster Patrol Waypoint Facing")));
    LevelAsset->Modify ();
...
bool AGridLevelEditorActor::SetSelectedPatrolWaypointWaitSeconds (
    float NewWaitSeconds)
{
    FGridLevelObjectData* Obj = FindSelectedObjectMutable ();
    if (!LevelAsset || !Obj || Obj->Type != EGridLevelObjectType::MonsterSpawn ||
        !Obj->PatrolWaypoints.IsValidIndex (SelectedPatrolWaypointIndex) ||
    ...
    const FScopedTransaction Transaction (
        FText::FromString (TEXT ("Set Monster Patrol Waypoint Wait")));
    LevelAsset->Modify ();
...
void AGridLevelEditorActor::CycleSelectedPatrolWaypointFacing ()
{
    ...
    SetSelectedPatrolWaypointFacing (
        GetNextPatrolFacing (
            Obj->PatrolWaypoints[SelectedPatrolWaypointIndex].Facing));
}
...
void AGridLevelEditorActor::IncreaseSelectedPatrolWaypointWait ()
{
    ...
    SetSelectedPatrolWaypointWaitSeconds (
        Obj->PatrolWaypoints[SelectedPatrolWaypointIndex].WaitSeconds + 0.5f);
}
