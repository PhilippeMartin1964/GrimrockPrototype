...
namespace
{
	...
	FSlateColor GetOverviewCellColor(const FGridLevelCellData* CellData, bool bSelectedCell, bool bSelectedObjectCell)
	{
		if (!CellData)
		{
	...
	void AddOverviewOutline(const TSharedRef<SOverlay>& CellOverlay, const FSlateColor& OutlineColor)
	{
		auto AddOutlineStrip = [&CellOverlay, &OutlineColor](float X, float Y, float Width, float Height)
		{
			CellOverlay->AddSlot()
				.HAlign(HAlign_Left)
				.VAlign(VAlign_Top)
				.Padding(FMargin(X, Y, 0.f, 0.f))[SNew(SBox).WidthOverride(Width).HeightOverride(
					Height)[SNew(SBorder).Padding(0.f).BorderImage(FCoreStyle::Get().GetBrush("WhiteBrush")).BorderBackgroundColor(OutlineColor)]];
		};
...
void SGridEditorOverviewMapPanel::Construct(const FArguments& InArgs)
{
	...
	ChildSlot[BuildOverviewMapSection()];
}
...
TSharedRef<SWidget> SGridEditorOverviewMapPanel::BuildOverviewCell(int32 CellX, int32 CellY, const FGridLevelObjectData* SelectedObject)
{
	...
	const FSlateColor OutlineColor = bSelectedCell ? FSlateColor(GetOverviewSelectedCellOutlineColor())
		: bSelectedObjectCell                      ? FSlateColor(GetOverviewSelectedObjectOutlineColor())
		: ObjectCount > 1                          ? FSlateColor(GetOverviewMultiObjectOutlineColor())
												   : FillColor;

	TSharedRef<SOverlay> CellOverlay = SNew(SOverlay);

	CellOverlay->AddSlot()[SNew(SBorder).Padding(0.f).BorderImage(FCoreStyle::Get().GetBrush("WhiteBrush")).BorderBackgroundColor(FillColor)];

	if (bExistingCell)
	{
		CellOverlay->AddSlot()[BuildCellObjectMarkers(CellObjects)];
	}
...
TSharedRef<SWidget> SGridEditorOverviewMapPanel::BuildCellObjectMarkers(const TArray<const FGridLevelObjectData*>& CellObjects) const
{
	...
	auto AddMarker = [&MarkerOverlay](EGridEditorOverviewObjectAnchor MarkerAnchor, bool bDoorMarker)
	{
		...
		MarkerOverlay->AddSlot()
			.HAlign(HAlign_Left)
			.VAlign(VAlign_Top)
			.Padding(GetOverviewMarkerPadding(MarkerAnchor, bDoorMarker))[SNew(SBox)
					.WidthOverride(MarkerSize.X)
					.HeightOverride(MarkerSize.Y)[SNew(SBorder)
							.Padding(0.f)
...
TSharedRef<SWidget> SGridEditorOverviewMapPanel::BuildOverviewColorLegend() const
{
	return SNew(SHorizontalBox)
		...
		+ SHorizontalBox::Slot().AutoWidth().Padding(
			  0.f, 0.f, 6.f, 0.f)[BuildOverviewLegendSwatch(FText::FromString(TEXT("Selected")), GetOverviewSelectedCellFillColor())]

		+ SHorizontalBox::Slot().AutoWidth().Padding(
			  0.f, 0.f, 6.f, 0.f)[BuildOverviewLegendSwatch(FText::FromString(TEXT("Selected Object")), GetOverviewSelectedObjectFillColor())];
}
...
TSharedRef<SWidget> SGridEditorOverviewMapPanel::BuildOverviewLegendSwatch(const FText& Label, const FLinearColor& Color) const
{
	return SNew(SHorizontalBox)
		...
		+ SHorizontalBox::Slot()
			  ...
			  .Padding(0.f, 0.f, 3.f, 0.f)[SNew(SBorder)
					  .Padding(0.f)
					  .BorderImage(FCoreStyle::Get().GetBrush("WhiteBrush"))
					  .BorderBackgroundColor(FSlateColor(Color))[SNew(SBox).WidthOverride(10.f).HeightOverride(10.f)]]
...
TSharedRef<SWidget> SGridEditorOverviewMapPanel::BuildSelectedCellSection()
{
	return SNew(SBorder)
		...
		.BorderImage(FAppStyle::GetBrush("ToolPanel.DarkGroupBorder"))[SNew(SVerticalBox)

			+ SVerticalBox::Slot().AutoHeight().Padding(0.f, 0.f, 0.f, 5.f)[SNew(SWrapBox)

				  + SWrapBox::Slot().Padding(0.f, 0.f, 6.f, 2.f)[SNew(STextBlock)
							.Text(FText::FromString(TEXT("SELECTED CELL")))
							.Font(FCoreStyle::GetDefaultFontStyle("Bold", 9))
							.ColorAndOpacity(FSlateColor(FLinearColor(0.80f, 0.80f, 0.80f, 1.f)))]

				  + SWrapBox::Slot().Padding(0.f, 0.f, 0.f,
						2.f)[SNew(STextBlock).Text(GetSelectedCellSummaryText()).Font(FCoreStyle::GetDefaultFontStyle("Regular", 8)).AutoWrapText(true)]]

			+ SVerticalBox::Slot().AutoHeight().Padding(0.f, 1.f, 0.f, 0.f)[BuildObjectsOnSelectedCellSection()]];
}
...
TSharedRef<SWidget> SGridEditorOverviewMapPanel::BuildObjectsOnSelectedCellSection()
{
	...
	ObjectsBox->AddSlot().AutoHeight().Padding(0.f, 0.f, 0.f, 4.f)[SNew(STextBlock)
	...
	for (const FGridEditorOverviewAnchorObjectGroup& Group : ObjectGroups)
	{
		...
		bHasObjects = true;
		ObjectsBox->AddSlot().AutoHeight().Padding(0.f, 0.f, 0.f, 4.f)[BuildObjectAnchorGroup(Group)];
	}

	if (!bHasObjects)
	{
		ObjectsBox->AddSlot().AutoHeight()[SNew(STextBlock)
...
TSharedRef<SWidget> SGridEditorOverviewMapPanel::BuildObjectAnchorGroup(const FGridEditorOverviewAnchorObjectGroup& Group)
{
	TSharedRef<SVerticalBox> GroupBox = SNew(SVerticalBox);

	GroupBox->AddSlot().AutoHeight().Padding(0.f, 0.f, 0.f, 2.f)[SNew(STextBlock)
...
FText SGridEditorOverviewMapPanel::GetOverviewCellTooltipText(int32 CellX, int32 CellY) const
{
	...
	{
		return FText::Format(FText::FromString(TEXT("Cell X={0} Y={1}\nInvalid cell")), FText::AsNumber(CellX), FText::AsNumber(CellY));
	}
...
FText SGridEditorOverviewMapPanel::GetSelectedCellSummaryText() const
{
	...
	return FText::Format(FText::FromString(TEXT("X={0} Y={1} | Type: {2} | Walls: {3} | Objects: {4}")), FText::AsNumber(CurrentEditorActor->SelectedCellX),
		FText::AsNumber(CurrentEditorActor->SelectedCellY),
		GridEditorWidgetHelpers::GetGridEnumDisplayText(CellTypeEnum, static_cast<int64>(CellData.CellType)), GetCellWallSummaryText(CellData),
...
FText SGridEditorOverviewMapPanel::GetCellWallSummaryText(const FGridLevelCellData& CellData) const
{
	...
	return FText::Format(FText::FromString(TEXT("N={0}, E={1}, S={2}, W={3}")), NorthText, EastText, SouthText, WestText);
}
