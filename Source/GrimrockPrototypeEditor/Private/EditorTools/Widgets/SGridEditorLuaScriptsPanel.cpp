...
namespace
{
    const FGridLuaScriptSource* FindScript (
        const UGridLevelAsset* LevelAsset,
        FName ScriptId)
    {
        return LevelAsset
            ? LevelAsset->LuaScripts.FindByPredicate (
    ...
    template <typename T>
    TSharedPtr<T> FindEnumOption (
        const TArray<TSharedPtr<T>>& Options,
        const TSharedPtr<T>& Current)
    {
        if (!Current.IsValid ())
        {
            return nullptr;
    ...
    TSharedPtr<FName> FindNameOption (
        const TArray<TSharedPtr<FName>>& Options,
        FName Value)
    {
...
void SGridEditorLuaScriptsPanel::SetStatus (
    const FString& Text,
    bool bSuccess)
{
    StatusText = Text;
...
void SGridEditorLuaScriptsPanel::LoadSelectedScriptDraft ()
{
    const FGridLuaScriptSource* Script =
        FindScript (GetLevelAsset (), SelectedScriptId);
    if (!Script)
    {
...
void SGridEditorLuaScriptsPanel::RebuildBindingOptions ()
{
    ...
    const FName PreviousScript = SelectedBindingScript.IsValid ()
        ? *SelectedBindingScript
        : NAME_None;
    const FName PreviousCallback = SelectedCallback.IsValid ()
        ? *SelectedCallback
        : NAME_None;
    const TSharedPtr<EGridObjectCondition> PreviousCondition = SelectedCondition;
    const FName PreviousVariable = SelectedVariable.IsValid ()
        ? *SelectedVariable
        : NAME_None;
    ...
    const FGridLevelObjectData* Source =
        EditorActor ? EditorActor->GetSelectedObjectData () : nullptr;
    if (Source)
    {
        for (EGridObjectEvent Event :
    ...
    if (LevelAsset)
    {
        for (const FGridLuaScriptSource& Script : LevelAsset->LuaScripts)
        {
            ...
            {
                BindingScriptOptions.Add (
                    MakeShared<FName> (Script.ScriptId));
            }
        }
    }
    ...
    SelectedBindingScript = FindNameOption (
        BindingScriptOptions,
        PreviousScript);
    ...
    if (LevelAsset && SelectedBindingScript.IsValid ())
    {
        ...
        if (GridEditorLuaService::GetCallbacksForScript (
                *LevelAsset,
                *SelectedBindingScript,
                CallbackNames,
                CallbackError))
    ...
    if (LevelAsset && SelectedCondition.IsValid ())
    {
        ...
        {
            for (const FGridLevelVariableDefinition& Variable :
                LevelAsset->LevelVariables)
            {
                if (!Variable.VariableId.IsNone () &&
                    Variable.Type == RequiredType)
                {
                    VariableOptions.Add (
                        MakeShared<FName> (Variable.VariableId));
                }
            }
        }
    }
    ...
    {
        ComparisonOptions.Add (
            MakeShared<EGridLogicIntComparison> (Comparison));
    }
    SelectedComparison = FindEnumOption (
        ComparisonOptions,
        PreviousComparison);
        ...
        [
            ...
            [
                ...
                [
                    ...
                    [
                        SNew (STextBlock)
                            .Text (FText::FromString (TEXT ("GRIMROCK LUA — SCRIPTS & BINDINGS")))
                    ...
                    [
                        SNew (SButton)
                            ...
                            .OnClicked (this, &SGridEditorLuaScriptsPanel::OnRefreshClicked)
                    ]
                ]
...
TSharedRef<SWidget> SGridEditorLuaScriptsPanel::BuildStatusSection () const
{
    ...
    const FString LevelText = LevelAsset
        ? LevelAsset->GetPathName ()
        : TEXT ("No UGridLevelAsset found on the current Grid Editor actor.");
...
TSharedRef<SWidget> SGridEditorLuaScriptsPanel::BuildScriptsSection ()
{
    ...
    if (LevelAsset->LuaScripts.IsEmpty ())
    {
        ...
        [
            SNew (STextBlock)
                .Text (FText::FromString (TEXT ("No Lua scripts in this level.")))
        ];
    }

    const FGridLuaScriptSource* Selected =
        FindScript (LevelAsset, SelectedScriptId);
    if (!Selected)
    {
        return Root;
    }
...
TSharedRef<SWidget> SGridEditorLuaScriptsPanel::BuildBindingsSection ()
{
    ...
    [
        SNew (STextBlock)
            .Text (FText::FromString (TEXT ("LUA BINDINGS — SELECTED GRID OBJECT")))
            .Font (FCoreStyle::GetDefaultFontStyle ("Bold", 10))
    ];
    ...
    if (!LevelAsset || !Source)
    {
        ...
        [
            SNew (STextBlock)
                .Text (FText::FromString (TEXT ("Select a Grid object in Grimrock Grid Editor, then press Refresh.")))
                .AutoWrapText (true)
        ];
        return Root;
    }
    ...
    for (int32 LinkIndex = 0; LinkIndex < LevelAsset->Links.Num (); ++LinkIndex)
    {
        ...
        if (Link.SourceObjectId != Source->ObjectId ||
    ...
    if (BindingCount == 0)
    {
        ...
        [
            SNew (STextBlock)
                .Text (FText::FromString (TEXT ("No Lua bindings for the selected object.")))
        ];
    }
    return Root;
}
...
FReply SGridEditorLuaScriptsPanel::OnAddScriptClicked ()
{
    ...
    FString Error;
    const bool bOk = GridEditorLuaService::AddScript (
        *LevelAsset,
        NewId,
...
FReply SGridEditorLuaScriptsPanel::OnApplyScriptClicked ()
{
    ...
    FString Error;
    bool bOk = GridEditorLuaService::RenameScript (
        *LevelAsset,
        SelectedScriptId,
        NewId,
        Error);
    ...
    {
        SelectedScriptId = NewId;
        bOk = GridEditorLuaService::SetScriptSource (
            *LevelAsset,
            SelectedScriptId,
            DraftSource,
            Error);
...
FReply SGridEditorLuaScriptsPanel::OnRemoveScriptClicked ()
{
    ...
    FString Error;
    const bool bOk = GridEditorLuaService::RemoveScript (
        *LevelAsset,
        SelectedScriptId,
        Error);
    if (bOk)
...
FReply SGridEditorLuaScriptsPanel::OnValidateClicked ()
{
    ...
    const bool bValid = GridEditorLuaService::AnalyzeLevel (
        *LevelAsset,
        Analysis);
    LastValidatedCallbacks.Reset ();
    ...
    if (bValid)
    {
        SetStatus (
            FString::Printf (
                TEXT ("Lua validation OK: %d script(s), VM build accepted."),
                Analysis.Scripts.Num ()),
            true);
    }
    else
    {
...
FReply SGridEditorLuaScriptsPanel::OnCreateBindingClicked ()
{
    ...
    {
        Link.ConditionVariableId = SelectedVariable.IsValid ()
            ? *SelectedVariable
            : NAME_None;
    ...
    else if (Link.Condition == EGridObjectCondition::LevelVariableIntCompare)
    {
        Link.ConditionVariableId = SelectedVariable.IsValid ()
            ? *SelectedVariable
            : NAME_None;
        Link.ConditionIntComparison = SelectedComparison.IsValid ()
            ? *SelectedComparison
            : EGridLogicIntComparison::Equal;
    ...
    FString Error;
    const bool bOk = GridEditorLuaService::AddLuaLink (
        *EditorActor,
        Link,
        Error);
...
FReply SGridEditorLuaScriptsPanel::OnRemoveBindingClicked (int32 LinkIndex)
{
    ...
    const FGridObjectLink Link = LevelAsset->Links[LinkIndex];
    const bool bOk = GridEditorLuaService::RemoveLuaLink (
        *EditorActor,
        Link);
...
TSharedRef<SWidget> SGridEditorLuaScriptsPanel::MakeEventOptionWidget (
...
TSharedRef<SWidget> SGridEditorLuaScriptsPanel::MakeNameOptionWidget (
...
TSharedRef<SWidget> SGridEditorLuaScriptsPanel::MakeConditionOptionWidget (
...
TSharedRef<SWidget> SGridEditorLuaScriptsPanel::MakeComparisonOptionWidget (
...
void SGridEditorLuaScriptsPanel::OnEventChanged (
    TSharedPtr<EGridObjectEvent> Item,
    ESelectInfo::Type)
{
    SelectedEvent = Item;
}

void SGridEditorLuaScriptsPanel::OnBindingScriptChanged (
...
void SGridEditorLuaScriptsPanel::OnCallbackChanged (
    TSharedPtr<FName> Item,
    ESelectInfo::Type)
{
...
void SGridEditorLuaScriptsPanel::OnConditionChanged (
    TSharedPtr<EGridObjectCondition> Item,
    ESelectInfo::Type)
{
...
void SGridEditorLuaScriptsPanel::OnVariableChanged (
    TSharedPtr<FName> Item,
    ESelectInfo::Type)
{
...
void SGridEditorLuaScriptsPanel::OnComparisonChanged (
    TSharedPtr<EGridLogicIntComparison> Item,
    ESelectInfo::Type)
{
...
FText SGridEditorLuaScriptsPanel::GetEventText (EGridObjectEvent Event) const
{
    ...
    return Enum
        ? Enum->GetDisplayNameTextByValue (static_cast<int64> (Event))
...
FText SGridEditorLuaScriptsPanel::GetComparisonText (
    EGridLogicIntComparison Comparison) const
{
...
FString SGridEditorLuaScriptsPanel::GetLuaBindingSummary (
    const FGridObjectLink& Link) const
{
    FString Result = FString::Printf (
        TEXT ("%s -> Lua %s.%s"),
        *GetEventText (Link.SourceEvent).ToString (),
        *Link.LuaScriptId.ToString (),
        *Link.LuaCallbackName.ToString ());
    if (Link.Condition == EGridObjectCondition::LevelVariableBoolEquals)
    {
    ...
    else if (Link.Condition == EGridObjectCondition::LevelVariableIntCompare)
    {
        Result += FString::Printf (
            TEXT (" if %s %s %d"),
            *Link.ConditionVariableId.ToString (),
            *GetComparisonText (Link.ConditionIntComparison).ToString (),
