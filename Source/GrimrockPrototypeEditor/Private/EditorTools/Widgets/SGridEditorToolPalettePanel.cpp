...
namespace
{
    FText GetToolGlyph (EGridEditorTool Tool)
    {
        switch (Tool)
        {
            case EGridEditorTool::Select:      return FText::FromString (TEXT ("S"));
            case EGridEditorTool::PaintCell:   return FText::FromString (TEXT ("C"));
            case EGridEditorTool::PaintWall:   return FText::FromString (TEXT ("W"));
            case EGridEditorTool::PaintObject: return FText::FromString (TEXT ("O"));
            case EGridEditorTool::Erase:       return FText::FromString (TEXT ("X"));
    ...
    int32 GetPaletteCategorySortOrder (FName Category)
    {
        ...
        if (Category == FName (TEXT ("Transitions"))) return 3;
        if (Category == FName (TEXT ("Items"))) return 4;
        if (Category == FName (TEXT ("Logic"))) return 5;
        if (Category == FName (TEXT ("Readable"))) return 6;
        if (Category == FName (TEXT ("Wall Decorations"))) return 7;
        if (Category == FName (TEXT ("Floor Decorations"))) return 8;
        if (Category == FName (TEXT ("Lights"))) return 9;
        if (Category == FName (TEXT ("Spawns"))) return 10;
        if (Category == FName (TEXT ("Uncategorized"))) return 11;
        return 100;
    }
}
...
FGridEditorToolPalettePanelState& SGridEditorToolPalettePanel::GetToolPaletteState () const
{
    ...
    return ToolPaletteState.IsValid ()
        ? *ToolPaletteState
        : FallbackToolPaletteState;
}
...
FText SGridEditorToolPalettePanel::GetSelectedPaletteEntryText () const
{
    ...
    {
        return CurrentEditorActor->SelectedPaletteEntryId.IsNone ()
