...
namespace GridEditorWidgetHelpers
{
	...
	{
		return Enum ? Enum->GetDisplayNameTextByValue(Value) : FText::FromString(TEXT("Unknown"));
	}
	...
	TSharedRef<SWidget> BuildGridPropertyRow(const FText& Label, TSharedRef<SWidget> ValueWidget)
	{
		return SNew(SHorizontalBox) +
			SHorizontalBox::Slot()
				...
				.Padding(0.f, 2.f, 8.f, 2.f)[SNew(STextBlock).Text(Label).ColorAndOpacity(FSlateColor(FLinearColor(0.72f, 0.72f, 0.72f, 1.f)))] +
			SHorizontalBox::Slot().FillWidth(0.65f).VAlign(VAlign_Center).Padding(0.f, 2.f)[ValueWidget];
	}

	TSharedRef<SWidget> BuildGridReadOnlyPropertyRow(const FText& Label, const FText& Value)
	{
		return BuildGridPropertyRow(Label, SNew(STextBlock).Text(Value).AutoWrapText(true));
	}

	TSharedRef<SWidget> BuildGridActionButton(const FText& Label, const FOnClicked& OnClicked)
	{
		return SNew(SButton).Text(Label).HAlign(HAlign_Center).ContentPadding(FMargin(8.f, 3.f)).OnClicked(OnClicked);
	}
	...
	TSharedRef<SWidget> BuildGridPanelSection(const FText& Title, TSharedRef<SWidget> Content)
	{
		return SNew(SBorder).Padding(6.f).BorderImage(FAppStyle::GetBrush("ToolPanel.GroupBorder"))[SNew(SVerticalBox)
			...
			+ SVerticalBox::Slot().AutoHeight().Padding(
				  0.f, 0.f, 0.f, 6.f)[SNew(STextBlock).Text(Title).Font(FAppStyle::GetFontStyle("DetailsView.CategoryFontStyle"))]
