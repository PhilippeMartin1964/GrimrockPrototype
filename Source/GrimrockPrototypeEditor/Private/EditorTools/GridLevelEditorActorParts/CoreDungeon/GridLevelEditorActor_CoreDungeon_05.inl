...
FString AGridLevelEditorActor::GetItemWorkflowDiagnostics() const
{
	...
	if (DungeonAsset && DungeonAsset->Levels.Num() > 0)
	{
		for (const FGridDungeonLevelEntry& Entry : DungeonAsset->Levels)
		{
			...
			const FString LevelLabel = Entry.LevelId.IsNone() ? FString(TEXT("None")) : Entry.LevelId.ToString();
...
bool AGridLevelEditorActor::CreateAndAddDungeonLevel(FName NewLevelId, FText DisplayName, FIntVector LogicalPosition, FString& OutError)
{
	...
	for (const FGridDungeonLevelEntry& Entry : DungeonAsset->Levels)
	{
		...
		{
			OutError = FString::Printf(TEXT("Logical Position (%d,%d,%d) is already used by LevelId '%s'."), LogicalPosition.X, LogicalPosition.Y,
				LogicalPosition.Z, *Entry.LevelId.ToString());
			return false;
		}
	}
