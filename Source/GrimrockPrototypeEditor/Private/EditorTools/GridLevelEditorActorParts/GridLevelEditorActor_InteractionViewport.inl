...
bool AGridLevelEditorActor::HasAnyWallInSelectedCell() const
{
	...
	return CellData.NorthWall != EGridWallType::None || CellData.EastWall != EGridWallType::None || CellData.SouthWall != EGridWallType::None ||
...
void AGridLevelEditorActor::EraseAtSelection()
{
	...
	if (FGridLevelCellData* CellData = GetSelectedCellMutable())
	{
		...
		if (CellData->CellType != EGridCellType::Empty && !HasAnyObjectInSelectedCell() && !HasAnyWallInSelectedCell())
		{
			ClearSelectedCell();
			return;
		}
...
void AGridLevelEditorActor::EnsureCoordinateHoverLabel()
{
	...
	CoordinateHoverLabel = NewObject<UTextRenderComponent>(this, UTextRenderComponent::StaticClass(), TEXT("CoordinateHoverLabel"), RF_Transactional);
...
void AGridLevelEditorActor::UpdateCoordinateHoverLabel()
{
	...
	CoordinateHoverLabel->SetText(FText::FromString(FString::Printf(TEXT("X:%d   Y:%d  %s"), HoveredCellX, HoveredCellY, EdgeText)));
	CoordinateHoverLabel->SetRelativeLocation(FVector((HoveredCellX + 0.5f) * CellSize, (HoveredCellY + 0.5f) * CellSize, CoordinateGridZOffset + 4.f));
