        ...
        [
            ...
            [
                SAssignNew (ErrorTextBlock, STextBlock)
                    .Text (FText::GetEmpty ())
                    .AutoWrapText (true)
                    .ColorAndOpacity (FSlateColor (FLinearColor (1.f, 0.20f, 0.12f, 1.f)))
            ]
            ...
            [
                ...
                [
                    SNew (SButton)
                        ...
                        .OnClicked_Lambda (
                            [this, DungeonAsset, LevelIdTextBox, DisplayNameTextBox, LogicalXSpinBox, LogicalYSpinBox, LogicalZSpinBox, ErrorTextBlock, DialogWindow] ()
                            {
                                ...
                                FString LevelIdString = LevelIdTextBox.IsValid ()
                                    ? LevelIdTextBox->GetText ().ToString ()
                                    : FString ();
                                ...
                                FString DisplayNameString = DisplayNameTextBox.IsValid ()
                                    ? DisplayNameTextBox->GetText ().ToString ()
                                    : FString ();
                                DisplayNameString.TrimStartAndEndInline ();
                                const FText DisplayName = DisplayNameString.IsEmpty ()
                                    ? FText::FromName (NewLevelId)
                                    : FText::FromString (DisplayNameString);

                                FString Error;
                                if (!CurrentEditorActor->CreateAndAddDungeonLevel (NewLevelId, DisplayName, LogicalPosition, Error))
                                {
                                    if (ErrorTextBlock.IsValid ())
                                    {
