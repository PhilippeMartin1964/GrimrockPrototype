...
namespace
{
    FGridLuaScriptSource MakeLuaScript (
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON1931LuaVersionLifecycleTest,
    "Grimrock.MON19.3.Lua.Foundation.VersionAndLifecycle",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON1931LuaVersionLifecycleTest::RunTest (
    const FString& Parameters)
{
    ...
    FString Error;
    TestTrue (
        TEXT ("Empty script set creates a ready VM"),
        Vm.Reload ({}, FGridLuaVmConfig (), Error));
    TestTrue (TEXT ("VM is ready"), Vm.IsReady ());
    TestEqual (
        TEXT ("Runtime is pinned to Lua 5.4.8"),
        Vm.GetVersionString (),
        FString (TEXT ("Lua 5.4.8")));
    ...
    TestTrue (
        TEXT ("VM allocation stays below default quota"),
        Vm.GetAllocatedBytes () <= FGridLuaVmConfig ().MemoryLimitBytes);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON1931LuaMultipleScriptsTest,
    "Grimrock.MON19.3.Lua.Foundation.MultipleScriptsIsolated",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON1931LuaMultipleScriptsTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON1931LuaSandboxSurfaceTest,
    "Grimrock.MON19.3.Lua.Foundation.SandboxSurface",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON1931LuaSandboxSurfaceTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    const TArray<FGridLuaScriptSource> Scripts = {
        MakeLuaScript (
            ...
            TEXT ("and require == nil and dofile == nil and loadfile == nil and load == nil ")
            TEXT ("and collectgarbage == nil and coroutine == nil\n")
            TEXT ("safe_present = type(math) == 'table' and type(string) == 'table' ")
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON1931LuaAtomicReloadTest,
    "Grimrock.MON19.3.Lua.Foundation.InvalidDefinitionsAtomicReload",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON1931LuaAtomicReloadTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    FGridLuaVm Vm;
    FString Error;
    const TArray<FGridLuaScriptSource> ValidScripts = {
        MakeLuaScript (
            TEXT ("Stable"),
    ...
    const TArray<FGridLuaScriptSource> DuplicateScripts = {
        MakeLuaScript (TEXT ("Duplicate"), TEXT ("x = 1")),
        MakeLuaScript (TEXT ("Duplicate"), TEXT ("x = 2"), false)
    };
    TestFalse (
        TEXT ("Duplicate ScriptId is rejected even when one entry is disabled"),
        Vm.Reload (DuplicateScripts, FGridLuaVmConfig (), Error));
    ...
    const TArray<FGridLuaScriptSource> SyntaxErrorScripts = {
        MakeLuaScript (TEXT ("Broken"), TEXT ("function broken( this is not lua"))
    };
    TestFalse (
        TEXT ("Syntax error rejects candidate VM"),
        Vm.Reload (SyntaxErrorScripts, FGridLuaVmConfig (), Error));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON1931LuaInstructionBudgetTest,
    "Grimrock.MON19.3.Lua.Foundation.InstructionBudget",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON1931LuaInstructionBudgetTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON1931LuaMemoryQuotaTest,
    "Grimrock.MON19.3.Lua.Foundation.MemoryQuota",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON1931LuaMemoryQuotaTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
