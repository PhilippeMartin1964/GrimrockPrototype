...
namespace
{
    FGridLuaScriptSource MakeMON1971Script (
        ...
        TMap<FName, int32>& IntValues)
    {
            ...
            FString& OutError)
        {
            ...
            OutError = FString::Printf (
                TEXT ("Missing Int32 '%s'."),
                *VariableId.ToString ());
            return false;
        };
        Host.SetInt32 = [&IntValues] (
            FName VariableId,
            int32 Value,
            FString& OutError)
        {
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON1971PersistentTableTest,
    "Grimrock.MON19.7.1.LuaAuthoring.PersistentTable",
...
bool FGridMON1971PersistentTableTest::RunTest (const FString& Parameters)
{
    ...
    const FGridLuaScriptSource Script = MakeMON1971Script (
        TEXT ("Puzzle"),
    ...
    if (!Vm.Reload ({Script}, FGridLuaVmConfig (), Error))
    {
        AddError (FString::Printf (
            TEXT ("Persistent authoring VM failed to load: %s"),
            *Error));
        return false;
    }
    ...
    TestTrue (
        TEXT ("First persistent callback succeeds"),
        Vm.CallEventFunction (
            TEXT ("Puzzle"),
            TEXT ("on_trigger"),
            MakeMON1971Event (),
            Host,
    ...
    TestTrue (
        TEXT ("Second persistent callback succeeds"),
        Vm.CallEventFunction (
            TEXT ("Puzzle"),
            TEXT ("on_trigger"),
            MakeMON1971Event (),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON1971PersistentValidationTest,
    "Grimrock.MON19.7.1.LuaAuthoring.PersistentValidation",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)
...
bool FGridMON1971PersistentValidationTest::RunTest (const FString& Parameters)
{
    ...
    FGridLuaVm Vm;
    TestFalse (
        TEXT ("Unsupported persistent default type is rejected"),
        Vm.Reload (
            {MakeMON1971Script (
                TEXT ("BadType"),
                TEXT ("persistent = { Bad = 'text' }\n"))},
            FGridLuaVmConfig (),
            Error));
    TestTrue (
        TEXT ("Unsupported type is diagnosed"),
        Error.Contains (TEXT ("Bool or Int32")));

    TestFalse (
        TEXT ("Conflicting persistent declarations are rejected"),
        Vm.Reload (
            {
                MakeMON1971Script (
                    TEXT ("A"),
                    TEXT ("persistent = { Shared = false }\n")),
                MakeMON1971Script (
                    TEXT ("B"),
                    TEXT ("persistent = { Shared = 1 }\n"))
            },
            FGridLuaVmConfig (),
            Error));
    TestTrue (
        TEXT ("Conflicting declaration is diagnosed"),
        Error.Contains (TEXT ("conflicting")));

    const FGridLuaScriptSource TypoScript = MakeMON1971Script (
        TEXT ("Typo"),
        TEXT ("persistent = { GateOpen = false }\n")
        TEXT ("function on_trigger(event) persistent.GateOepn = true end\n"));
    TestTrue (
        TEXT ("Typo script loads before callback"),
        Vm.Reload ({TypoScript}, FGridLuaVmConfig (), Error));
    ...
    const FGridLuaHostApi Host = MakeMON1971Host (BoolValues, IntValues);
    TestFalse (
        TEXT ("Undeclared persistent key is rejected after callback"),
        Vm.CallEventFunction (
            TEXT ("Typo"),
            TEXT ("on_trigger"),
            MakeMON1971Event (),
            ...
            Error));
    TestTrue (
        TEXT ("Undeclared persistent key is diagnosed"),
        Error.Contains (TEXT ("was not declared")));
    TestFalse (
        TEXT ("Rejected typo does not mutate authoritative Bool"),
        BoolValues.FindRef (TEXT ("GateOpen")));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON1971LegacyGridVarsCompatibilityTest,
    "Grimrock.MON19.7.1.LuaAuthoring.LegacyGridVarsCompatibility",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON1971LegacyGridVarsCompatibilityTest::RunTest (
    const FString& Parameters)
{
    ...
    FString Error;
    TestTrue (
        TEXT ("Legacy compatibility VM loads"),
        Vm.Reload ({Script}, FGridLuaVmConfig (), Error));
    ...
    const FGridLuaHostApi Host = MakeMON1971Host (BoolValues, IntValues);
    TestTrue (
        TEXT ("Legacy grid.vars callback succeeds"),
        Vm.CallEventFunction (
            TEXT ("Legacy"),
            TEXT ("on_trigger"),
            MakeMON1971Event (),
            ...
            Error));
    TestTrue (
        TEXT ("Unchanged persistent table does not overwrite direct grid.vars mutation"),
        BoolValues.FindRef (TEXT ("GateOpen")));
    return true;
}
