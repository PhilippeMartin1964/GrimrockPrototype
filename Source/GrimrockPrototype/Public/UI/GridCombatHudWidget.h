...
public:
	...
	static void BuildHotbarActions(const TArray<FGridCombatHotbarBinding>& Bindings, const TArray<FGridAvailableCombatAction>& AvailableActions,
		TArray<FGridCombatHudActionView>& OutActions);

	static FGridCombatHudMobilityView BuildMobility(const FGridPartyMobilityState& MobilityState);

	static void BuildInitiative(const TArray<FGridInitiativePreviewEntry>& InitiativePreview, TArray<FGridCombatHudInitiativeView>& OutInitiative,
		int32 MaximumVisibleEntries = MaximumVisibleInitiativeEntries);

	static float CalculateHealthPercent(int32 CurrentHealth, int32 MaximumHealth);
};
...
public:
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Combat|HUD|Visuals", meta = (ClampMin = "0.0", ClampMax = "1.0"))
	...
	void InitializeAction(UGridCombatHudWidget* InOwnerHud, const FGridCombatHudActionView& InView);

	/** Reuses the slot visual as a draggable, non-executable palette entry. */
	void InitializePaletteAction(UGridCombatHudWidget* InOwnerHud, const FGridAvailableCombatAction& InAction);
...
UCLASS()
class GRIMROCKPROTOTYPE_API UGridCombatHudInitiativeSlotWidget : public UUserWidget
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Combat|HUD|Visuals")
	FLinearColor HealthBarFillColor = FLinearColor(0.82f, 0.02f, 0.02f, 1.0f);
	...
	void InitializeInitiativeSlot(const FGridCombatHudInitiativeView& InView);
...
public:
	...
	UPROPERTY(Transient, BlueprintReadOnly, Category = "Combat|HUD")
	TArray<TObjectPtr<UGridCombatHudInitiativeSlotWidget>> InitiativeSlotWidgets;
	...
	bool BeginCombatActionTargeting(const FGridCombatHudActionView& ActionView);
	...
	UFUNCTION(BlueprintCallable, Category = "Combat|HUD|Targeting")
	bool ConfirmCombatActionTarget(FIntPoint TargetCell, FGridCombatActionRequestResult& OutResult);
	...
	bool HandleHotbarDrop(int32 TargetSlotIndex, UDragDropOperation* DragOperation);

	/** Assigns a currently catalogued class/universal action to one slot. */
	UFUNCTION(BlueprintCallable, Category = "Combat|HUD|Hotbar")
	bool AssignCombatActionToHotbarSlot(int32 TargetSlotIndex, const FGridAvailableCombatAction& Action);
