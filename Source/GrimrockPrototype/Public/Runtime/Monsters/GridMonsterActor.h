...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FGridMonsterDiedSignature, AGridMonsterActor*, Monster, FIntPoint, DeathCell);
...
public:
	AGridMonsterActor()
	{
		...
		IdleVariationComponent = CreateDefaultSubobject<UGridMonsterIdleVariationComponent>(TEXT("MonsterIdleVariations"));
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Monster|Components")
	TObjectPtr<UGridMonsterIdleVariationComponent> IdleVariationComponent;
	...
	UFUNCTION(BlueprintCallable, Category = "Monster|Initialization")
	bool InitializeMonster(UGridMonsterDefinitionAsset* InDefinition, FGuid InSpawnObjectId, FIntPoint InCell, EGridEdge InFacing = EGridEdge::North,
	...
	bool CaptureRuntimeMonsterState(FGridRuntimeMonsterState& OutState, FName CurrentLevelId) const;

	bool RestoreRuntimeMonsterState(const FGridRuntimeMonsterState& State, AGridLevelRuntimeActor* RuntimeActor);

	FName ResolveRuntimeDungeonLevelId(FName CurrentLevelId) const;

	UFUNCTION(BlueprintPure, Category = "Monster|Persistence")
	bool IsRuntimeLevelActive() const
	{
		return bRuntimeLevelActive;
	}
	...
	{
		if (MonsterState == EGridMonsterState::Dead && InState != EGridMonsterState::Dead)
		{
			return;
		}
	...
	void ApplyAttackResult(const FGridAttackResult& Result)
	{
		...
		CurrentPhysicalArmor = FMath::Max(0, CurrentPhysicalArmor - Result.PhysicalArmorDamage);
		CurrentMagicalArmor = FMath::Max(0, CurrentMagicalArmor - Result.MagicalArmorDamage);
	...
	void SetMovementAnimationState(bool bInMoving, float InMoveAlpha = 0.0f)
	{
		...
		if (bWasMoving != bIsMoving && IdleVariationComponent)
		{
			IdleVariationComponent->RefreshIdleVariationScheduling();
		}
	}
	...
	void SetTurnAnimationState(int32 InTurnDirection)
	{
		...
		TurnDirection = MonsterState == EGridMonsterState::Dead ? 0 : FMath::Clamp(InTurnDirection, -1, 1);
		bIsTurning = TurnDirection != 0;
		if (bWasTurning != bIsTurning && IdleVariationComponent)
		{
			IdleVariationComponent->RefreshIdleVariationScheduling();
		}
	}
	...
	bool IsDead() const
	{
		return MonsterState == EGridMonsterState::Dead || (DeathComponent && DeathComponent->bDeathCommitted) ||
...
{
	return SkeletalMeshComponent ? Cast<UGridMonsterAnimInstance>(SkeletalMeshComponent->GetAnimInstance()) : nullptr;
}
