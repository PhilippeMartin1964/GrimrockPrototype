...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Transient, Category = "Monster|VFX|Debug")
	bool bNativeSpawnEnabled = true;

	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Monster|VFX")
	TObjectPtr<AGridMonsterActor> OwnerMonster = nullptr;

	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Monster|VFX")
	FGridMonsterVFXSpawnRequest LastSpawnRequest;

	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Monster|VFX|Debug")
	...
	UFUNCTION(BlueprintCallable, Category = "Monster|VFX")
	bool PlayAttackVFX(const FGridMonsterAttackDefinition& Attack);
	...
	UFUNCTION(BlueprintPure, Category = "Monster|VFX")
	int32 GetSpawnRequestCountForEvent(EGridMonsterVFXEvent Event) const;

	UFUNCTION(BlueprintPure, Category = "Monster|VFX|Debug")
	int32 GetActiveNiagaraComponentCount() const;

	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Monster|VFX|Debug")
...
private:
	...
	bool PlayDefinition(EGridMonsterVFXEvent Event, const FGridMonsterVFXEventDefinition& Definition, FName AttackId, const FGridAttackResult* Result,
		FVector ImpactWorldLocation, int32 TargetCharacterIndex);

	const FGridMonsterVFXEventDefinition* GetMonsterDefinition(EGridMonsterVFXEvent Event) const;
