...
public:
	...
	static void BuildAxialFiringCandidates(const FIntPoint& PartyCell, int32 MinDistanceCells, int32 MaxDistanceCells, TArray<FIntPoint>& OutCandidates);

	/** Inclusive preferred-distance predicate with defensive range normalization. */
	static bool IsPreferredDistance(int32 DistanceCells, int32 PreferredMinDistance, int32 PreferredMaxDistance);
     ...
     */
	static bool BuildRepositionTurn(const FGuid& SourceActorId, const FIntPoint& StartCell, EGridEdge StartFacing, const FIntPoint& PartyCell,
		const TArray<FIntPoint>& PathToFiringCell, int32 AvailableActionPoints, const FGridMonsterAttackDefinition& Attack, bool bGoalLineOfSightSatisfied,
		bool bMayAttackThisTurn, TArray<FGridCombatAction>& OutActions);
};
