...
struct FGridMonsterPatrolWaypoint
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Monster|Patrol", meta = (ClampMin = "0.0"))
...
struct FGridMonsterAttackDefinition
{
	...
	bool ValidateDefinition(FString& OutError) const
	{
		...
		if (!FMath::IsFinite(ImpactTimeSeconds) || ImpactTimeSeconds < 0.0f || ImpactTimeSeconds > ExpectedDuration)
		{
		...
		if (!FMath::IsFinite(ProjectileVisualScale.X) || !FMath::IsFinite(ProjectileVisualScale.Y) || !FMath::IsFinite(ProjectileVisualScale.Z) ||
			ProjectileVisualScale.X <= 0.0f || ProjectileVisualScale.Y <= 0.0f || ProjectileVisualScale.Z <= 0.0f)
		{
			Errors.Add(TEXT("ProjectileVisualScale components must be finite and greater than zero."));
		}
		if (!FMath::IsFinite(ProjectileSourceOffset.X) || !FMath::IsFinite(ProjectileSourceOffset.Y) || !FMath::IsFinite(ProjectileSourceOffset.Z))
		{
			Errors.Add(TEXT("ProjectileSourceOffset components must be finite."));
		}

		const auto ValidateAudio = [&Errors](const TCHAR* FieldName, const FGridMonsterAudioEventDefinition& Definition)
		{
			FString Error;
			if (!Definition.ValidateDefinition(Error))
			{
				Errors.Add(FString::Printf(TEXT("%s.%s"), FieldName, *Error));
			}
		};
		...
		const auto ValidateVFX = [&Errors](const TCHAR* FieldName, const FGridMonsterVFXEventDefinition& Definition)
		{
			FString Error;
			if (!Definition.ValidateDefinition(Error))
			{
...
struct FGridMonsterDamageModifier
{
	...
	bool IsValidDefinition() const
	{
		return FMath::IsFinite(DamageMultiplier) && DamageMultiplier >= 0.0f &&
