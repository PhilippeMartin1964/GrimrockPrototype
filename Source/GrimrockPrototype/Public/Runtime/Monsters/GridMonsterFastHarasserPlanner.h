...
public:
     ...
     */
	static int32 ScoreCandidate(const FGridRetreatCandidate& Candidate, int32 CurrentDistanceToParty);

	static bool SelectBestRetreatCell(const TArray<FGridRetreatCandidate>& Candidates, int32 CurrentDistanceToParty, FGridRetreatDecision& OutDecision);

	static bool ShouldRetreat(float RetreatChance, FRandomStream& RandomStream, float* OutRoll = nullptr);

	static void SelectAggroTargets(const FGuid& SourceObjectId, FName SourceMonsterId, FName EncounterGroupId, const FIntPoint& SourceCell,
