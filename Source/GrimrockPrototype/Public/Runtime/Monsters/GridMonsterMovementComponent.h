...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FGridMonsterMoveCompletedSignature, FIntPoint, FromCell, FIntPoint, ToCell);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FGridMonsterTurnCompletedSignature, EGridEdge, FromFacing, EGridEdge, ToFacing);
...
public:
	...
	bool IsBusy() const
	{
		return ActiveMotion != EGridMonsterMotionType::None;
	}
	...
	UFUNCTION(BlueprintPure, Category = "Monster|Movement")
	UGridMonsterOccupancySubsystem* GetOccupancySubsystem() const
	{
		return OccupancySubsystem;
	}
