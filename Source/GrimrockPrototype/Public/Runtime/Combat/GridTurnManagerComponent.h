...
{
public:
	static void RollAndSort(TArray<FGridCombatantInitiativeEntry>& Entries, FRandomStream& RandomStream);

	static void Sort(TArray<FGridCombatantInitiativeEntry>& Entries);

	static bool Precedes(const FGridCombatantInitiativeEntry& Left, const FGridCombatantInitiativeEntry& Right);
};
...
public:
	...
	EGridCombatPhase GetPhase() const
	{
		return Phase;
	}
...
{
public:
	static void BuildMovementTurn(const FGuid& SourceActorId, const FIntPoint& StartCell, EGridEdge StartFacing, const TArray<FIntPoint>& Path,
		int32 AvailableActionPoints, TArray<FGridCombatAction>& OutActions);
     ...
     */
	static void BuildDirectMeleeTurn(const FGuid& SourceActorId, const FIntPoint& StartCell, EGridEdge StartFacing, const FIntPoint& PartyCell,
		const TArray<FIntPoint>& Path, int32 AvailableActionPoints, FName AttackId, int32 AttackActionPointCost, TArray<FGridCombatAction>& OutActions);

	/**
     * Preserves DirectMelee planning and optionally appends a marked retreat
     * after the single planned melee attack.
     */
	static void BuildFastHarasserTurn(const FGuid& SourceActorId, const FIntPoint& StartCell, EGridEdge StartFacing, const FIntPoint& PartyCell,
		const TArray<FIntPoint>& Path, int32 AvailableActionPoints, FName AttackId, int32 AttackActionPointCost, const FGridRetreatDecision& RetreatDecision,
		bool bShouldRetreat, TArray<FGridCombatAction>& OutActions);
};

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridCombatPhaseChangedSignature, EGridCombatPhase, NewPhase);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridCombatRoundStartedSignature, int32, RoundNumber);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridMonsterTurnSignature, AGridMonsterActor*, Monster);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridCombatActionStartedSignature, FGridCombatAction, Action);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FGridCombatActionCompletedSignature, FGridCombatAction, Action, bool, bSucceeded);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(
	FGridMonsterAttackResolvedSignature, AGridMonsterActor*, Monster, int32, TargetCharacterIndex, FGridAttackResult, Result);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridCombatEndedSignature, EGridCombatPhase, ResultPhase);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridCombatLogEntryAddedSignature, FGridCombatLogEntry, Entry);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridPlayerAttackRequestedSignature, FGridPlayerAttackRequest, Request);
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridPartyMobilityStateChangedSignature, FGridPartyMobilityState, MobilityState);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(
	FGridPartyMovementRejectedSignature, int32, CharacterIndex, EGridEdge, Direction, EGridPartyMovementRejectReason, RejectReason);

DECLARE_DYNAMIC_MULTICAST_DELEGATE(FGridTurnOrderChangedSignature);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridActiveCombatantChangedSignature, FGridCombatantInitiativeEntry, ActiveCombatant);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridCombatantStateChangedSignature, FGridCombatantInitiativeEntry, Combatant);
...
struct FGridCombatActionCooldownKey
{
	...
	{
		return CharacterId == Other.CharacterId && ActionId == Other.ActionId;
	}

	friend uint32 GetTypeHash(const FGridCombatActionCooldownKey& Key)
	{
		return HashCombine(GetTypeHash(Key.CharacterId), GetTypeHash(Key.ActionId));
	}
};
...
public:
	...
	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Combat|Turn Manager|Random")
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Combat|Turn Manager|Diagnostics")
	bool bCollectRuntimeMetrics = false;

	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Combat|Turn Manager|Diagnostics")
	FGridCombatRuntimeMetrics RuntimeMetrics;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Combat|Feedback", meta = (ClampMin = "1", ClampMax = "512"))
	int32 MaxCombatLogEntries = 128;

	UPROPERTY(Transient, VisibleInstanceOnly, BlueprintReadOnly, Category = "Combat|Feedback")
	...
	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Combat|Initiative")
	...
	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Combat|Player Attack")
	EGridPlayerAttackRejectReason LastPlayerAttackRejectReason = EGridPlayerAttackRejectReason::None;

	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Combat|Player Attack")
	FGridAttackResult LastPlayerAttackResult;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Combat|Player Turn", meta = (ClampMin = "2", ClampMax = "6"))
	int32 BasePlayerActionPointsPerTurn = 4;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Combat|Player Attack", meta = (ClampMin = "1", ClampMax = "6"))
	int32 PlayerAttackActionPointCost = 2;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Combat|Party Mobility", meta = (ClampMin = "0", ClampMax = "4"))
	int32 BasePartyMobilityActionPointsPerRound = 2;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Combat|Party Mobility", meta = (ClampMin = "1", ClampMax = "6"))
	int32 PartyTranslationActionPointCost = 1;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Combat|Party Mobility", meta = (ClampMin = "1", ClampMax = "4"))
	int32 PartyTranslationMobilityActionPointCost = 1;

	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Combat|Player Turn")
	TArray<FGridPlayerCharacterTurnState> PlayerCharacterTurnStates;

	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Combat|Party Mobility")
	FGridPartyMobilityState PartyMobilityState;

	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Combat|Party Mobility")
	EGridPartyMovementRejectReason LastPartyMovementRejectReason = EGridPartyMovementRejectReason::None;
	...
	FGridPlayerCharacterTurnStateChangedSignature OnPlayerCharacterTurnStateChanged;

	UPROPERTY(BlueprintAssignable, Category = "Combat|Party Mobility")
	FGridPartyMobilityStateChangedSignature OnPartyMobilityStateChanged;

	UPROPERTY(BlueprintAssignable, Category = "Combat|Party Mobility")
	FGridPartyMovementRejectedSignature OnPartyMovementRejected;
	...
	UPROPERTY(VisibleInstanceOnly, BlueprintReadOnly, Transient, Category = "Combat|Player Attack|Diagnostics")
	int32 PlayerAttackRejectedBroadcastCount = 0;

	UFUNCTION(BlueprintCallable, Category = "Combat|Turn Manager")
	bool InitializeTurnManager(AGridLevelRuntimeActor* InRuntimeActor = nullptr, AGrimrockPartyPawn* InPartyPawn = nullptr);
	...
	{
		return PendingPartyMotionType != EGridPendingPartyMotionType::None;
	}

	UFUNCTION(BlueprintCallable, Category = "Combat|Player Attack")
	bool RequestSelectedCharacterAttack(FGridPlayerAttackRequest& OutRequest, FGridAttackResult& OutResult, EGridPlayerAttackRejectReason& OutRejectReason);

	UFUNCTION(BlueprintCallable, Category = "Combat|Player Attack")
	bool RequestCharacterAttack(
		int32 AttackerCharacterIndex, FGridPlayerAttackRequest& OutRequest, FGridAttackResult& OutResult, EGridPlayerAttackRejectReason& OutRejectReason);
     ...
     */
	UFUNCTION(BlueprintCallable, Category = "Combat|Player Attack")
	bool RequestCharacterAttackFromSlot(int32 AttackerCharacterIndex, EGridEquipmentSlot RequestedEquipmentSlot, FGridPlayerAttackRequest& OutRequest,
		FGridAttackResult& OutResult, EGridPlayerAttackRejectReason& OutRejectReason);
	...
	void GetAvailableCombatActions(int32 CharacterIndex, TArray<FGridAvailableCombatAction>& OutActions) const;
	...
	bool BuildCombatActionTargetingPreview(int32 CharacterIndex, FName ActionId, EGridCombatActionSourcePolicy SourcePolicy, FName SourceDefinitionId,
		EGridEquipmentSlot SourceEquipmentSlot, FIntPoint TargetCell, FGridCombatActionTargetingPreview& OutPreview) const;

	/** Executes a previously previewed Cell/Area action at the selected cell. */
	UFUNCTION(BlueprintCallable, Category = "Combat|Action Targeting")
	bool RequestCharacterCombatActionAtCell(int32 CharacterIndex, FName ActionId, EGridCombatActionSourcePolicy SourcePolicy, FName SourceDefinitionId,
		EGridEquipmentSlot SourceEquipmentSlot, FIntPoint TargetCell, FGridCombatActionRequestResult& OutResult);
	...
	UFUNCTION(BlueprintPure, Category = "Combat|Player Turn")
	bool CanCharacterSpendActionPoints(int32 CharacterIndex, int32 ActionPointCost) const;

	/** Returns a copy of the authoritative (or safely derived) turn state. */
	UFUNCTION(BlueprintPure, Category = "Combat|Player Turn")
	bool GetPlayerCharacterTurnState(int32 CharacterIndex, FGridPlayerCharacterTurnState& OutTurnState) const;
	...
	UFUNCTION(BlueprintCallable, Category = "Combat|Initiative")
	void GetUpcomingInitiativeOrder(TArray<FGridCombatantInitiativeEntry>& OutEntries) const;
     ...
     */
	UFUNCTION(BlueprintCallable, Category = "Combat|Initiative")
	void GetInitiativePreview(TArray<FGridInitiativePreviewEntry>& OutEntries, int32 MaximumEntries = 8) const;
     ...
     */
	UFUNCTION(BlueprintCallable, Category = "Combat|Initiative")
	bool SetCombatantInitiativeModifier(EGridCombatantSide Side, FGuid CombatantId, int32 InitiativeModifier);

	UFUNCTION(BlueprintPure, Category = "Combat|Initiative")
	bool GetActiveCombatant(FGridCombatantInitiativeEntry& OutCombatant) const;
	...
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Combat|Turn Manager|Diagnostics")
	...
	UFUNCTION(BlueprintCallable, CallInEditor, Category = "Combat|Feedback|Debug")
	...
	UFUNCTION(BlueprintPure, Category = "Combat|Turn Manager")
	bool IsPartyInputLocked() const
	{
		return bPartyInputLocked;
	}
...
private:
	...
	bool RejectPlayerAttack(int32 AttackerCharacterIndex, EGridPlayerAttackRejectReason RejectReason, EGridPlayerAttackRejectReason& OutRejectReason);
	bool RequestCharacterAttackInternal(int32 AttackerCharacterIndex, EGridEquipmentSlot RequestedEquipmentSlot, bool bRequireRequestedEquipmentSlot,
		const FGridAvailableCombatAction* CombatActionOverride, FGridPlayerAttackRequest& OutRequest, FGridAttackResult& OutResult,
		EGridPlayerAttackRejectReason& OutRejectReason);
	void BuildPlayerCombatActionContributions(int32 CharacterIndex, TArray<FGridCombatActionContribution>& OutContributions) const;
	...
	bool BuildPlayerAttackResolutionInputs(const FGridInventoryCharacterSummary& CharacterSummary, const AGridMonsterActor* TargetMonster,
		const FGridOffensiveEquipmentProfile& OffensiveProfile, FGridAttackSourceStats& OutSource, FGridAttackTargetStats& OutTarget,
	...
	bool BeginPlayerCombatantTurn(FGridCombatantInitiativeEntry& Entry);
	bool BeginMonsterCombatantTurn(FGridCombatantInitiativeEntry& Entry);
	void FinishActivePlayerTurn();
	void FinishInitiativeRound();
	void ClearInitiativeState(bool bBroadcast);
	void SetInitiativeEntryState(FGridCombatantInitiativeEntry& Entry, EGridCombatantTurnState NewState);
	void RefreshInitiativeEntryVitals(FGridCombatantInitiativeEntry& Entry) const;
	...
	AGridMonsterActor* FindCombatMonsterById(const FGuid& CombatantId) const;
	FGridCombatantInitiativeEntry* FindInitiativeEntry(EGridCombatantSide Side, const FGuid& CombatantId);
	const FGridCombatantInitiativeEntry* FindInitiativeEntry(EGridCombatantSide Side, const FGuid& CombatantId) const;
	void BeginPlayerCharacterPhase();
	void CompletePlayerCharacterPhase();
	void ClearPlayerCharacterTurnStates();
	bool SpendPlayerCharacterActionPoints(int32 CharacterIndex, int32 ActionPointCost);
	void RefreshPlayerCharacterVitalState(int32 CharacterIndex);
	FGridPlayerCharacterTurnState* EnsurePlayerCharacterTurnState(int32 CharacterIndex);
	const FGridPlayerCharacterTurnState* FindPlayerCharacterTurnState(const FGuid& CharacterId) const;
	FGridPlayerCharacterTurnState* FindPlayerCharacterTurnState(const FGuid& CharacterId);
	void BroadcastPlayerCharacterTurnState(const FGridPlayerCharacterTurnState& TurnState);
	...
	UFUNCTION()
	void HandleCombatMonsterDied(AGridMonsterActor* Monster, FIntPoint DeathCell);
