...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridPlayerAttackPresentationRequestedSignature, FGridPlayerAttackPresentationRequest, Request);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridPlayerAttackFeedbackRequestedSignature, FGridPlayerAttackFeedbackRequest, Request);

UCLASS(ClassGroup = (Grid), meta = (BlueprintSpawnableComponent))
class GRIMROCKPROTOTYPE_API UGridPlayerAttackPresentationComponent : public UActorComponent
{
	GENERATED_BODY()
...
public:
	...
	FGridPlayerAttackPresentationRequestedSignature OnPresentationRequested;

	UPROPERTY(BlueprintAssignable, Category = "Combat|Player Attack|Feedback")
	FGridPlayerAttackFeedbackRequestedSignature OnFeedbackRequested;

	UFUNCTION(BlueprintCallable, Category = "Combat|Player Attack|Presentation")
	bool InitializePresentation(UGridTurnManagerComponent* InTurnManager = nullptr);

	UFUNCTION(BlueprintCallable, Category = "Combat|Player Attack|Presentation")
	void ResetTransientPresentationState();

	UFUNCTION(BlueprintPure, Category = "Combat|Player Attack|Presentation")
	int32 GetPresentationRequestCountForEvent(EGridPlayerAttackPresentationEvent Event) const;

	UFUNCTION(BlueprintPure, Category = "Combat|Player Attack|Audio")
	int32 GetAudioPlaybackRequestCountForEvent(EGridPlayerAttackPresentationEvent Event) const;

	UFUNCTION(BlueprintPure, Category = "Combat|Player Attack|Motion")
	bool IsHeldItemMotionActive() const
	{
		return bMotionActive;
	}
...
private:
	...
	UFUNCTION()
	void HandlePlayerAttackResolved(FGridPlayerAttackRequest Request, AGridMonsterActor* TargetMonster, FGridAttackResult Result);

	UFUNCTION()
	void HandlePlayerAttackRejected(int32 AttackerCharacterIndex, EGridPlayerAttackRejectReason RejectReason);

	UFUNCTION()
	void HandleCombatEnded(EGridCombatPhase ResultPhase);

	FGridPlayerAttackPresentationProfile ResolveProfile(const FGridPlayerAttackRequest& Request) const;
	AGridMonsterActor* ResolveTargetMonster(const FGridPlayerAttackRequest& Request) const;
	void EmitPresentation(EGridPlayerAttackPresentationEvent Event, const FGridPlayerAttackRequest& Request, const FGridAttackResult* Result,
		AGridMonsterActor* TargetMonster, const FGridPlayerAttackPresentationProfile& Profile);
	void EmitFeedback(const FGridPlayerAttackFeedbackRequest& Feedback);
	void StartHeldItemMotion(const FGridPlayerAttackRequest& Request, const FGridPlayerAttackPresentationProfile& Profile);
	...
	void ConfigureThrownItemOutcome(AGridThrownItemActor* ThrownItem, AGridMonsterActor* TargetMonster, const FGridAttackResult& Result);
