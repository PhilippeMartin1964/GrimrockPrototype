...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGridStatusEffectFeedbackSignature, FGridCombatLogEntry, Entry);
...
public:
     ...
     */
	bool TryApplyStatusEffectToPartyCharacter(int32 CharacterIndex, UGridStatusEffectDefinitionAsset* Definition, const FGuid& SourceId,
	...
	bool TryApplyStatusEffectToMonster(AGridMonsterActor* Monster, UGridStatusEffectDefinitionAsset* Definition, const FGuid& SourceId,
		FGridStatusEffectApplyResult& OutResult, FString& OutError, int32 InitialStackCount = 1, int32 DurationOverride = INDEX_NONE,
		int32 PotencyOverride = INDEX_NONE);
...
private:
	...
	void EmitStatusFeedback(EGridCombatLogEntryType Type, const FGridStatusEffectRuntimeState& State, int32 CharacterIndex, AGridMonsterActor* Monster,
		const FGridAttackResult* DamageResult = nullptr);
	void EmitExpiredFeedback(const TArray<FGridStatusEffectRuntimeState>& PreviousStates, const FGridStatusEffectAdvanceResult& AdvanceResult,
		int32 CharacterIndex, AGridMonsterActor* Monster);
