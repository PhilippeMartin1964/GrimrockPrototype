...
struct GRIMROCKPROTOTYPE_API FRPGSaveMigrationService
{
     ...
     */
	static bool PrepareLoadedSave(UGrimrockPartySaveGame* SaveGame, FText& OutError, FRPGSaveMigrationReport* OutReport = nullptr);

	/** Strict validation of the current save contract. */
	static bool ValidateCurrentSave(UGrimrockPartySaveGame* SaveGame, FText& OutError);
};
