...
struct GRIMROCKPROTOTYPE_API FRPGSkillRequirementProjectionService
{
     ...
     */
	static bool AppendSatisfiedRequirements(const FGridCharacterInventoryState& CharacterState, TSet<FName>& OutRequirements, FString& OutError);

	/**
     * Testable overload with an injected canonical SkillId resolver.
     */
	static bool AppendSatisfiedRequirements(const FGridCharacterInventoryState& CharacterState, TFunctionRef<const URPGSkillAsset*(FName)> DefinitionResolver,
		TSet<FName>& OutRequirements, FString& OutError);
