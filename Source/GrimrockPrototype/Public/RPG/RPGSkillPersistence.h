...
{
	static bool CapturePartySkills(const FGridPartyInventoryState& PartyState, TArray<FRPGCharacterSkillSaveState>& OutSavedStates, FString& OutError);

	/** Test seam and deterministic capture with an injected canonical resolver. */
	static bool CapturePartySkills(const FGridPartyInventoryState& PartyState, TFunctionRef<const URPGSkillAsset*(FName)> DefinitionResolver,
		TArray<FRPGCharacterSkillSaveState>& OutSavedStates, FString& OutError);

	static bool ValidateSavedPartySkills(const FGridPartyInventoryState& PartyState, const TArray<FRPGCharacterSkillSaveState>& SavedStates, FString& OutError);

	/** Test seam and deterministic validation with an injected canonical resolver. */
	static bool ValidateSavedPartySkills(const FGridPartyInventoryState& PartyState, const TArray<FRPGCharacterSkillSaveState>& SavedStates,
     ...
     */
	static bool RestorePartySkills(FGridPartyInventoryState& InOutPartyState, const TArray<FRPGCharacterSkillSaveState>& SavedStates, FString& OutError);

	/** Test seam and deterministic restore with an injected canonical resolver. */
	static bool RestorePartySkills(FGridPartyInventoryState& InOutPartyState, const TArray<FRPGCharacterSkillSaveState>& SavedStates,
		TFunctionRef<const URPGSkillAsset*(FName)> DefinitionResolver, FString& OutError);
