...
struct GRIMROCKPROTOTYPE_API FRPGSkillRequirementGrant
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "RPG|Skills|Requirements", meta = (ClampMin = "1"))
	int32 MinimumRank = 1;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "RPG|Skills|Requirements")
	TArray<FName> GrantedRequirementIds;

	bool IsValid(int32 SkillMaxRank) const
	{
		if (MinimumRank < 1 || MinimumRank > SkillMaxRank || GrantedRequirementIds.IsEmpty())
		{
			return false;
		}
		...
		{
			if (RequirementId.IsNone() || UniqueRequirementIds.Contains(RequirementId))
			{
				return false;
			}
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "RPG|Skills", meta = (MultiLine = "true"))
	FText Description;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "RPG|Skills")
	ERPGSkillGoverningAttribute GoverningAttribute = ERPGSkillGoverningAttribute::None;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "RPG|Skills", meta = (ClampMin = "1"))
	int32 MaxRank = 5;
