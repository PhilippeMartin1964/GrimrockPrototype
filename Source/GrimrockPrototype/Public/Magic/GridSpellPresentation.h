...
struct FGridSpellProjectilePresentationProfile
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Spell|Presentation|Projectile", meta = (ClampMin = "0.01", ClampMax = "5.0"))
	...
	bool IsValid() const
	{
		...
		return FMath::IsFinite(TravelDurationSeconds) && TravelDurationSeconds > 0.0f && !VisualScale.ContainsNaN() && VisualScale.X > 0.0f &&
...
struct FGridSpellPresentationProfile
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Spell|Presentation|Feedback", meta = (ClampMin = "0.1", ClampMax = "10.0"))
	float FeedbackDurationSeconds = 1.25f;

	bool IsValid() const
	{
		return CastAudio.IsValid() && CastVFX.IsValid() && ImpactAudio.IsValid() && ImpactVFX.IsValid() && Projectile.IsValid() &&
			FMath::IsFinite(FeedbackDurationSeconds) && FeedbackDurationSeconds > 0.0f;
	}
};
...
struct FGridSpellPresentationPlan
{
	...
	bool IsValid() const
	{
		return !SpellId.IsNone() && !SourceWorldLocation.ContainsNaN() && !TargetWorldLocation.ContainsNaN() &&
			(!bLaunchProjectile || ProjectileTravelDurationSeconds > 0.0f) && !Events.IsEmpty();
	}
};
...
{
	static bool BuildPlan(const FGridSpellDefinition& Definition, const FGridSpellResolvedTarget& ResolvedTarget, const FGridSpellPresentationProfile& Profile,
		const FVector& SourceWorldLocation, const FVector& TargetWorldLocation, FGridSpellPresentationPlan& OutPlan);

	static float CalculateProjectileLaunchDelay(float ImpactTimeSeconds, const FGridSpellPresentationProfile& Profile);

	static FVector EvaluateProjectileTrajectory(const FVector& Source, const FVector& Target, float Alpha);
};
