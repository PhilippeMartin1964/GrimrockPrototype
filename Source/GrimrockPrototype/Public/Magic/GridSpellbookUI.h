...
public:
	...
	static FGridCombatActionDefinition MakeSpellCombatActionDefinition(const FGridSpellDefinition& SpellDefinition);

	UFUNCTION(BlueprintPure, Category = "Magic|Spellbook|UI")
	static FGridCombatHotbarBinding MakeSpellHotbarBinding(FName SpellId, int32 SlotIndex);

	UFUNCTION(BlueprintPure, Category = "Magic|Spellbook|UI")
	static bool IsSpellHotbarBinding(const FGridCombatHotbarBinding& Binding, FName SpellId);

	UFUNCTION(BlueprintPure, Category = "Magic|Spellbook|UI")
	static int32 FindAssignedSpellSlot(const TArray<FGridCombatHotbarBinding>& HotbarBindings, FName SpellId);

	UFUNCTION(BlueprintCallable, Category = "Magic|Spellbook|UI")
	static EGridSpellHotbarAssignmentResult AssignKnownSpellToHotbar(UGridPartyInventoryComponent* InventoryComponent, int32 CharacterIndex,
		const FGridCharacterSpellbookState& Spellbook, FName SpellId, int32 TargetSlotIndex);
