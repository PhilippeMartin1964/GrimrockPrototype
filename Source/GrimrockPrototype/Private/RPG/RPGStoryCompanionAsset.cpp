...
bool URPGStoryCompanionAsset::IsValidDefinition() const
{
	...
	if (Level < URPGCharacterRulesLibrary::GetMinimumLevel() || Level > URPGCharacterRulesLibrary::GetMaximumLevel())
	{
		return false;
	}

	const FRPGAttributes FinalAttributes = URPGCharacterRulesLibrary::AddAttributes(ClassDefinition->BaseAttributes, RaceDefinition->AttributeBonuses);
	if (!URPGCharacterRulesLibrary::AreAttributesInRange(FinalAttributes))
	{
		return false;
	}

	return RPGStoryCompanionAssetPrivate::AreStartingEquipmentIdsValid(StartingEquipmentIds);
}
