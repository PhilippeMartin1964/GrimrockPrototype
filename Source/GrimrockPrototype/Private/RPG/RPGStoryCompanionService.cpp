...
namespace RPGStoryCompanionServicePrivate
{
    ...
    {
        return Character.CharacterId == Definition.CharacterId &&
    ...
    bool BuildCandidate (
        ...
        FGridCharacterInventoryState& OutCandidate)
    {
        ...
        const FRPGAttributes FinalAttributes =
            URPGCharacterRulesLibrary::AddAttributes (
                Definition.ClassDefinition->BaseAttributes,
                Definition.RaceDefinition->AttributeBonuses);
        ...
        Candidate.RaceDisplayName =
            Definition.RaceDefinition->DisplayName;
        Candidate.ClassId = Definition.ClassDefinition->ClassId;
        Candidate.ClassDisplayName =
        ...
        Candidate.Strength =
            static_cast<float> (FinalAttributes.Strength);
    ...
    FRPGStoryCompanionRegistrationResult& OutResult)
{
    ...
    {
        OutResult.Status =
            ERPGStoryCompanionRegistrationStatus::InvalidInventory;
    ...
    if (!State.bInitialCharacterCreationCompleted ||
        ...
        !State.ActiveCharacters.IsValidIndex (
    ...
    for (int32 CharacterIndex = 0;
        CharacterIndex < State.ActiveCharacters.Num ();
        ++CharacterIndex)
    {
        if (State.ActiveCharacters[CharacterIndex].CharacterId !=
    ...
    for (int32 PoolIndex = 0;
        ...
        ++PoolIndex)
    {
        if (State.CharacterPool[PoolIndex].CharacterId !=
            CompanionDefinition->CharacterId)
        {
            continue;
        }
    ...
    {
        const FGridCharacterInventoryState& Existing =
            State.ActiveCharacters[ActiveMatchIndex];
        if (!RPGStoryCompanionServicePrivate::MatchesDefinition (
                Existing,
                *CompanionDefinition))
        {
            OutResult.Status =
    ...
    {
        const FGridCharacterInventoryState& Existing =
            State.CharacterPool[PoolMatchIndex];
        if (!RPGStoryCompanionServicePrivate::MatchesDefinition (
                Existing,
                *CompanionDefinition))
        {
    ...
    FGridCharacterInventoryState Candidate;
    if (!RPGStoryCompanionServicePrivate::BuildCandidate (
            *CompanionDefinition,
    ...
    const int32 NewPoolIndex = State.CharacterPool.Add (
        MoveTemp (Candidate));
