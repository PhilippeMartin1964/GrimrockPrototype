...
namespace MON156SaveMigrationPrivate
{
	...
	void ResetLegacyPartyStatusEffects(FGridPartyInventoryState& PartyState)
	{
	...
	void ResetLegacyPartySkillRanks(FGridPartyInventoryState& PartyState)
	{
		for (FGridCharacterInventoryState& Character : PartyState.ActiveCharacters)
		{
	...
	void ResetLegacyMonsterStatusEffects(FGridDungeonRuntimeState& DungeonState)
	{
		for (TPair<FName, FGridLevelRuntimeState>& LevelPair : DungeonState.LevelStates)
		{
			for (TPair<FGuid, FGridRuntimeMonsterState>& MonsterPair : LevelPair.Value.Monsters)
			{
				MonsterPair.Value.StatusEffects.Reset();
			}
			for (TPair<FGuid, FGridRuntimeMonsterPlacementState>& PlacementPair : LevelPair.Value.MonsterPlacements)
			{
	...
	bool MigrateLegacyCharacter(FGridCharacterInventoryState& Character, int32& InOutReconciledCount, FText& OutError)
	{
		...
		const int32 SafeStoredLevel = FMath::Clamp(OriginalLevel, URPGCharacterRulesLibrary::GetMinimumLevel(), URPGCharacterRulesLibrary::GetMaximumLevel());
		const int32 NormalizedExperience = URPGCharacterRulesLibrary::NormalizeExperience(OriginalExperience);
		const int32 ExperienceFloorForStoredLevel = URPGCharacterRulesLibrary::GetCumulativeExperienceRequiredForLevel(SafeStoredLevel);
		const int32 ReconciledExperience = FMath::Max(NormalizedExperience, ExperienceFloorForStoredLevel);
		const int32 ReconciledLevel = URPGCharacterRulesLibrary::GetLevelForExperience(ReconciledExperience);

		const bool bNeedsReconciliation = OriginalLevel != ReconciledLevel || OriginalExperience != ReconciledExperience;
		...
		if (OriginalLevel != ReconciledLevel)
		{
			...
			if (!ClassDefinition)
			{
				OutError = FText::FromString(
					FString::Printf(TEXT("La sauvegarde legacy du personnage %s nécessite un recalcul de niveau mais sa définition de classe est invalide."),
						Character.CharacterId.IsValid() ? *Character.CharacterId.ToString(EGuidFormats::Digits) : TEXT("Unknown")));
				return false;
			}

			FRPGDerivedStats RecalculatedStats = URPGCharacterRulesLibrary::CalculateDerivedStats(Character.Attributes, ClassDefinition, ReconciledLevel);
			RecalculatedStats.CurrentHealth = PreserveHealthDeficit(OriginalStats, RecalculatedStats.MaxHealth);
			RecalculatedStats.CurrentMana = PreserveManaDeficit(OriginalStats, RecalculatedStats.MaxMana);
	...
	bool ValidateSnapshot(const FGridPartyInventoryState& PartyState, const TArray<FRPGCharacterProgressionSaveState>& ProgressionStates,
...
bool FRPGSaveMigrationService::PrepareLoadedSave(UGrimrockPartySaveGame* SaveGame, FText& OutError, FRPGSaveMigrationReport* OutReport)
{
	OutError = FText::GetEmpty();
	if (OutReport)
	{
	...
	if (SourceVersion < UGrimrockPartySaveGame::MinimumCompatibleSaveVersion || SourceVersion > UGrimrockPartySaveGame::CurrentSaveVersion)
	{
		OutError = FText::FromString(FString::Printf(TEXT("Version de sauvegarde %d hors de la plage compatible %d-%d."), SourceVersion,
	...
	if (SourceVersion == 7)
	{
		if (!ValidateSnapshot(SaveGame->PartyInventoryState, SaveGame->ClassProgressionStates, SaveGame->PendingLevelUpNotifications, OutError) ||
			!ValidateSpellbooks(*SaveGame, OutError) || !ValidateLevelVariables(*SaveGame, OutError))
		{
			return false;
		}
	...
	for (FGridCharacterInventoryState& Character : MigratedPartyState.ActiveCharacters)
	{
		if (!MigrateLegacyCharacter(Character, ReconciledCharacterCount, OutError))
		{
			return false;
		}
	}
	for (FGridCharacterInventoryState& Character : MigratedPartyState.CharacterPool)
	{
		if (!MigrateLegacyCharacter(Character, ReconciledCharacterCount, OutError))
		{
			return false;
	...
	MigratedProgressionStates.Reserve(MigratedPartyState.ActiveCharacters.Num());
	for (const FGridCharacterInventoryState& Character : MigratedPartyState.ActiveCharacters)
	{
	...
	if (!ValidateSnapshot(MigratedPartyState, MigratedProgressionStates, MigratedPendingNotifications, OutError))
	{
		return false;
	}
...
bool FRPGSaveMigrationService::ValidateCurrentSave(UGrimrockPartySaveGame* SaveGame, FText& OutError)
{
	OutError = FText::GetEmpty();
	if (!SaveGame)
	{
	...
	{
		OutError = FText::FromString(FString::Printf(
			TEXT("La validation stricte attend la version %d et reçoit la version %d."), UGrimrockPartySaveGame::CurrentSaveVersion, SaveGame->SaveVersion));
		return false;
	}
