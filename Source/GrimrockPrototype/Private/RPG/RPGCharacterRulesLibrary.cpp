...
namespace
{
	...
	int32 CalculateCumulativeExperienceUnchecked(int32 Level)
	{
		...
		const int64 RequiredExperience = static_cast<int64>(RPGExperienceStepPerLevel) * PreviousLevels * (PreviousLevels + 1) / 2;
...
FRPGAttributes URPGCharacterRulesLibrary::AddAttributes(const FRPGAttributes& BaseAttributes, const FRPGAttributes& Bonuses)
{
...
bool URPGCharacterRulesLibrary::AreAttributesInRange(const FRPGAttributes& Attributes, int32 MinimumValue, int32 MaximumValue)
{
	...
	return Attributes.Strength >= MinimumValue && Attributes.Strength <= MaximumValue && Attributes.Dexterity >= MinimumValue &&
		Attributes.Dexterity <= MaximumValue && Attributes.Constitution >= MinimumValue && Attributes.Constitution <= MaximumValue &&
		Attributes.Intelligence >= MinimumValue && Attributes.Intelligence <= MaximumValue && Attributes.Wisdom >= MinimumValue &&
		Attributes.Wisdom <= MaximumValue && Attributes.Charisma >= MinimumValue && Attributes.Charisma <= MaximumValue;
}

float URPGCharacterRulesLibrary::CalculateMaxCarryWeight(const FRPGAttributes& Attributes, float WeightPerStrength)
{
	return FMath::Max(0, Attributes.Strength) * FMath::Max(0.0f, WeightPerStrength);
}

FRPGDerivedStats URPGCharacterRulesLibrary::CalculateDerivedStats(const FRPGAttributes& Attributes, const URPGClassAsset* ClassDefinition, int32 Level)
{
...
bool URPGCharacterRulesLibrary::IsLevelExperienceConsistent(int32 Level, int32 TotalExperience)
{
