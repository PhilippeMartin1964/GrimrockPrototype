...
{
    bool ResolveTalentContext (
        UGridPartyInventoryComponent* PartyInventoryComponent,
        int32 CharacterIndex,
        const FGridCharacterInventoryState*& OutCharacter,
    ...
    bool BuildSelectionSet (
        const TArray<FName>& SelectedChoiceIds,
        TSet<FName>& OutSelection)
    {
        OutSelection.Reset ();
    ...
    TArray<FRPGTalentRuntimeView>& OutTalents)
{
    ...
    if (!RPGTalentRuntimeServicePrivate::ResolveTalentContext (
    ...
    if (!RPGTalentRuntimeServicePrivate::BuildSelectionSet (
            SelectedChoiceIds,
            SelectedChoiceSet))
    {
        return false;
...
bool FRPGTalentRuntimeService::TryGetSelectedCharacterTalents (
    UGridPartyInventoryComponent* PartyInventoryComponent,
    TArray<FRPGTalentRuntimeView>& OutTalents)
{
    OutTalents.Reset ();
    ...
    return TryGetSelectedTalents (
        PartyInventoryComponent,
        PartyInventoryComponent->GetSelectedCharacterIndex (),
        OutTalents);
}

bool FRPGTalentRuntimeService::HasTalent (
    UGridPartyInventoryComponent* PartyInventoryComponent,
    int32 CharacterIndex,
    FName ChoiceId,
    bool& OutHasTalent)
...
bool FRPGTalentRuntimeService::HasSelectedCharacterTalent (
    UGridPartyInventoryComponent* PartyInventoryComponent,
    FName ChoiceId,
    bool& OutHasTalent)
{
    ...
    FRPGTalentPointBalance& OutBalance)
{
    ...
    return FRPGClassProgressionTransactionService::TryGetChoicePointBalance (
        PartyInventoryComponent,
        CharacterIndex,
        OutBalance.GrantedPoints,
        OutBalance.SpentPoints,
        OutBalance.RemainingPoints);
}
    ...
    FRPGTalentPointBalance& OutBalance)
{
    ...
    return TryGetTalentPointBalance (
        PartyInventoryComponent,
        PartyInventoryComponent->GetSelectedCharacterIndex (),
        OutBalance);
}
