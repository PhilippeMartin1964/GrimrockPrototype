...
bool FGridStatusEffectPeriodicDamageResolver::Resolve (
    ...
    FString& OutError)
{
    ...
    if (!IsValid (State.DefinitionAsset))
    {
        OutError = FString::Printf (
            TEXT ("EffectId '%s' has no runtime definition asset."),
    ...
    {
        OutError = FString::Printf (
            TEXT ("Runtime EffectId '%s' does not match definition EffectId '%s'."),
            *State.EffectId.ToString (),
            *Definition.EffectId.ToString ());
        return false;
    }
    if (!Definition.PeriodicDamage.IsEnabled ())
    {
        OutError = FString::Printf (
            TEXT ("EffectId '%s' does not declare periodic damage."),
            *State.EffectId.ToString ());
        return false;
    }
    ...
    const int64 ScaledDamage =
        static_cast<int64> (Definition.PeriodicDamage.DamagePerStack) *
        static_cast<int64> (State.StackCount);
    const int32 RawDamage = static_cast<int32> (
        FMath::Clamp<int64> (ScaledDamage, 0, MAX_int32));
    if (RawDamage <= 0)
    {
    ...
    OutResolution.DamageResult = FGridCombatResolver::ResolveDirectDamage (
        Target,
        Definition.PeriodicDamage.DamageType,
        RawDamage);
    OutError.Reset ();
    return true;
}
