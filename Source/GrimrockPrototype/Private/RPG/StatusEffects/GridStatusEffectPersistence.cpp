...
namespace
{
    const FPrimaryAssetType StatusEffectPrimaryAssetType (
        TEXT ("GridStatusEffect"));

    bool StatusSaveStateLess (
        const FGridStatusEffectSaveState& Left,
        const FGridStatusEffectSaveState& Right)
    {
...
bool FGridStatusEffectPersistence::ValidateSavedCollection (
    ...
    for (const FGridStatusEffectSaveState& SavedState : SavedStates)
    {
        if (!SavedState.IsStructurallyValid ())
        {
            OutError = FString::Printf (
                TEXT ("Saved status effect '%s' has an invalid runtime snapshot."),
                *SavedState.EffectId.ToString ());
            return false;
        }
        if (EffectIds.Contains (SavedState.EffectId))
        {
            OutError = FString::Printf (
                TEXT ("Saved status effect '%s' is duplicated."),
                *SavedState.EffectId.ToString ());
            return false;
        }
    ...
    FString& OutError)
{
    ...
    for (const FGridStatusEffectRuntimeState& RuntimeState :
        RuntimeCollection.ActiveEffects)
    {
        if (!RuntimeState.IsValid ())
        {
            OutError = FString::Printf (
                TEXT ("Runtime status effect '%s' is invalid and cannot be saved."),
                *RuntimeState.EffectId.ToString ());
            return false;
        }

        const UGridStatusEffectDefinitionAsset* Definition =
            RuntimeState.DefinitionAsset.Get ();
        if (!IsValid (Definition) ||
            !Definition->IsValidDefinition () ||
            Definition->EffectId != RuntimeState.EffectId ||
            Definition->DurationUnit != RuntimeState.DurationUnit ||
            RuntimeState.StackCount > Definition->MaxStacks)
        {
            OutError = FString::Printf (
                TEXT ("Runtime status effect '%s' has no matching valid definition for persistence."),
                *RuntimeState.EffectId.ToString ());
            return false;
        }
    ...
    FString& OutError)
{
    ...
    for (const FGridStatusEffectSaveState& SavedState : SavedStates)
    {
        UGridStatusEffectDefinitionAsset* Definition =
            DefinitionResolver (SavedState.EffectId);
        if (!IsValid (Definition) ||
            !Definition->IsValidDefinition () ||
            Definition->EffectId != SavedState.EffectId)
        {
            OutError = FString::Printf (
                TEXT ("Saved status effect '%s' cannot resolve its canonical definition."),
                *SavedState.EffectId.ToString ());
            return false;
        }
        ...
        {
            OutError = FString::Printf (
                TEXT ("Saved status effect '%s' duration unit no longer matches its definition."),
                *SavedState.EffectId.ToString ());
            return false;
        }
        ...
        {
            OutError = FString::Printf (
                TEXT ("Saved status effect '%s' stack count %d exceeds definition MaxStacks %d."),
                *SavedState.EffectId.ToString (),
                SavedState.StackCount,
        ...
        if (!Definition->BuildRuntimeState (
                SavedState.SourceId,
                SavedState.StackCount,
                SavedState.RemainingDuration,
                SavedState.Potency,
                RuntimeState,
                RehydrateError))
        {
            OutError = FString::Printf (
                TEXT ("Saved status effect '%s' failed runtime rehydration: %s"),
                *SavedState.EffectId.ToString (),
                *RehydrateError);
            return false;
        }
    ...
    FString& OutError)
{
    return RestoreCollection (
        ...
        OutRuntimeCollection,
        OutError);
}

UGridStatusEffectDefinitionAsset*
FGridStatusEffectPersistence::ResolveDefinitionByEffectId (FName EffectId)
{
    if (EffectId.IsNone ())
    {
        return nullptr;
    ...
    const FPrimaryAssetId PrimaryAssetId (
        StatusEffectPrimaryAssetType,
        EffectId);

    UGridStatusEffectDefinitionAsset* Definition =
        AssetManager.GetPrimaryAssetObject<
            UGridStatusEffectDefinitionAsset> (PrimaryAssetId);
    if (!IsValid (Definition))
    {
        ...
        if (!AssetPath.IsValid ())
        {
            ...
            AssetManager.ScanPathsForPrimaryAssets (
                StatusEffectPrimaryAssetType,
                SearchPaths,
                UGridStatusEffectDefinitionAsset::StaticClass (),
                false,
                false,
                true);
        ...
        if (AssetPath.IsValid ())
        {
            Definition = Cast<UGridStatusEffectDefinitionAsset> (
                AssetPath.TryLoad ());
        }
    }
