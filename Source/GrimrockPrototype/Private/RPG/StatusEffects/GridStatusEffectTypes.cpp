...
bool FGridStatusEffectCollection::TryApply(const UGridStatusEffectDefinitionAsset& Definition, const FGuid& SourceId, int32 InitialStackCount,
	int32 DurationOverride, int32 PotencyOverride, FGridStatusEffectApplyResult& OutResult, FString& OutError)
{
	OutResult.Reset();

	FGridStatusEffectRuntimeState Candidate;
	if (!Definition.BuildRuntimeState(SourceId, InitialStackCount, DurationOverride, PotencyOverride, Candidate, OutError))
	{
		return false;
	}
	...
	{
		OutError = FString::Printf(TEXT("EffectId '%s' cannot be reapplied with a different DurationUnit."), *Candidate.EffectId.ToString());
		return false;
	}
	...
	switch (Definition.StackPolicy)
	{
		case EGridStatusEffectStackPolicy::NoStack:
			...
			OutError = FString::Printf(TEXT("EffectId '%s' rejects reapplication because StackPolicy is NoStack."), *Candidate.EffectId.ToString());
			return false;
		...
		case EGridStatusEffectStackPolicy::AddStacks:
			...
			Updated.StackCount = FMath::Min(Definition.MaxStacks, ExistingSnapshot.StackCount + Candidate.StackCount);
		...
		case EGridStatusEffectStackPolicy::ReplaceIfStronger:
			if (Candidate.Potency <= ExistingSnapshot.Potency)
			{
				...
				OutError = FString::Printf(TEXT("EffectId '%s' potency %d is not stronger than active potency %d."), *Candidate.EffectId.ToString(),
					Candidate.Potency, ExistingSnapshot.Potency);
				return false;
			}
			Updated = Candidate;
