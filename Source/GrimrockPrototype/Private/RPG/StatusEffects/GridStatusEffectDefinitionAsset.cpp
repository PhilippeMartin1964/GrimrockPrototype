...
bool UGridStatusEffectDefinitionAsset::ValidateDefinition(FString& OutError) const
{
	...
	if (PeriodicDamage.IsEnabled() && DurationUnit == EGridStatusEffectDurationUnit::Permanent)
	{
...
bool UGridStatusEffectDefinitionAsset::BuildRuntimeState(const FGuid& SourceId, int32 InitialStackCount, int32 DurationOverride, int32 PotencyOverride,
	FGridStatusEffectRuntimeState& OutState, FString& OutError) const
{
	...
	{
		OutError = FString::Printf(TEXT("InitialStackCount %d is outside the valid range [1, %d]."), InitialStackCount, MaxStacks);
		return false;
	}
	...
	Candidate.DefinitionAsset = const_cast<UGridStatusEffectDefinitionAsset*>(this);
