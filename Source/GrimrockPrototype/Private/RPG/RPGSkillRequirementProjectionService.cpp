...
bool FRPGSkillRequirementProjectionService::AppendSatisfiedRequirements(
	const FGridCharacterInventoryState& CharacterState, TSet<FName>& OutRequirements, FString& OutError)
{
...
bool FRPGSkillRequirementProjectionService::AppendSatisfiedRequirements(const FGridCharacterInventoryState& CharacterState,
	TFunctionRef<const URPGSkillAsset*(FName)> DefinitionResolver, TSet<FName>& OutRequirements, FString& OutError)
{
	...
	{
		const URPGSkillAsset* Definition = DefinitionResolver(SkillRank.SkillId);
		if (!IsValid(Definition) || !Definition->IsValidDefinition() || Definition->SkillId != SkillRank.SkillId)
		{
			OutError = FString::Printf(TEXT("Skill '%s' cannot resolve a matching valid definition."), *SkillRank.SkillId.ToString());
			return false;
		}
		if (SkillRank.Rank > Definition->MaxRank)
		{
			OutError =
				FString::Printf(TEXT("Skill '%s' rank %d exceeds definition MaxRank %d."), *SkillRank.SkillId.ToString(), SkillRank.Rank, Definition->MaxRank);
			return false;
		}
...
const URPGSkillAsset* FRPGSkillRequirementProjectionService::ResolveDefinitionBySkillId(FName SkillId)
{
	if (SkillId.IsNone())
	{
		return nullptr;
	...
	const FPrimaryAssetId PrimaryAssetId(RPGSkillPrimaryAssetType, SkillId);

	URPGSkillAsset* Definition = AssetManager.GetPrimaryAssetObject<URPGSkillAsset>(PrimaryAssetId);
