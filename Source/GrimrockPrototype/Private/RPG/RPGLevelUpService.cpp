...
namespace
{
        ...
        int32 NewMaximumHealth)
    {
        ...
        const int32 SafePreviousMaximum =
            FMath::Max (1, PreviousStats.MaxHealth);
        const int32 SafePreviousCurrent =
            FMath::Clamp (
        ...
        const int32 DamageTaken =
            SafePreviousMaximum - SafePreviousCurrent;
        return FMath::Clamp (
        ...
        int32 NewMaximumMana)
    {
        ...
        const int32 SafePreviousMaximum =
            FMath::Max (0, PreviousStats.MaxMana);
        const int32 SafePreviousCurrent =
            FMath::Clamp (
                PreviousStats.CurrentMana,
                0,
                SafePreviousMaximum);
        const int32 ManaSpent =
            SafePreviousMaximum - SafePreviousCurrent;
        return FMath::Clamp (
    ...
    URPGClassAsset* ResolveCharacterClassDefinition (
...
bool FRPGLevelUpService::ApplyPendingLevelUp (
    UGridPartyInventoryComponent* PartyInventoryComponent,
    int32 CharacterIndex,
    bool bNotifyPartyInventoryChanged)
{
    ...
    if (Character.Level < MinimumLevel ||
        Character.Level > MaximumLevel ||
        Character.Experience !=
            URPGCharacterRulesLibrary::NormalizeExperience (
                Character.Experience))
    {
        UE_LOG (
            LogGridLevelUp,
            Warning,
            TEXT ("[GridLevelUp] Character=%d Level=%d Experience=%d Result=Rejected Reason=InvalidProgressionState"),
            CharacterIndex,
            Character.Level,
            Character.Experience);
        return false;
    ...
    if (TargetLevel < Character.Level)
    {
        UE_LOG (
            LogGridLevelUp,
            Warning,
            TEXT ("[GridLevelUp] Character=%d Level=%d Target=%d Experience=%d Result=Rejected Reason=WouldDemote"),
            CharacterIndex,
            Character.Level,
            TargetLevel,
            Character.Experience);
    ...
    URPGClassAsset* ClassDefinition =
        ResolveCharacterClassDefinition (Character);
    if (!ClassDefinition ||
            ...
            ClassDefinition->ClassId != Character.ClassId))
    {
        UE_LOG (
            LogGridLevelUp,
            Warning,
            TEXT ("[GridLevelUp] Character=%d Level=%d Target=%d Experience=%d ClassId=%s Result=Rejected Reason=InvalidClassDefinition"),
            CharacterIndex,
            Character.Level,
            TargetLevel,
            Character.Experience,
    ...
    FRPGDerivedStats NewStats =
        URPGCharacterRulesLibrary::CalculateDerivedStats (
            Character.Attributes,
            ClassDefinition,
            TargetLevel);
    NewStats.CurrentHealth =
        PreserveHealthDeficit (
            PreviousStats,
            NewStats.MaxHealth);
    NewStats.CurrentMana =
        PreserveManaDeficit (
            PreviousStats,
            NewStats.MaxMana);
    ...
    UE_LOG (
        LogGridLevelUp,
        Log,
        TEXT ("[GridLevelUp] Character=%d PreviousLevel=%d NewLevel=%d LevelsGained=%d Experience=%d HP=%d/%d Mana=%d/%d"),
        CharacterIndex,
        PreviousLevel,
        TargetLevel,
        LevelsGained,
...
int32 FRPGLevelUpService::ApplyPendingLevelUps (
    UGridPartyInventoryComponent* PartyInventoryComponent,
    bool bNotifyPartyInventoryChanged)
{
    if (!IsValid (PartyInventoryComponent))
    ...
    for (int32 CharacterIndex = 0;
        CharacterIndex <
            PartyInventoryComponent->PartyInventoryState.ActiveCharacters.Num ();
        ++CharacterIndex)
    {
        const int32 PreviousLevel =
            PartyInventoryComponent->PartyInventoryState.ActiveCharacters[
                CharacterIndex].Level;
        if (ApplyPendingLevelUp (
                PartyInventoryComponent,
                CharacterIndex,
                bNotifyPartyInventoryChanged))
        {
            TotalLevelsGained +=
                PartyInventoryComponent->PartyInventoryState.ActiveCharacters[
                    CharacterIndex].Level - PreviousLevel;
        }
    }
