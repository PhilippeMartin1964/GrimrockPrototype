...
FRPGExperienceRewardService::OnCharacterExperienceAwarded ()
{
    static FGridCharacterExperienceAwardedNativeSignature Delegate;
    return Delegate;
}

int32 FRPGExperienceRewardService::AwardToActiveParty (
    ...
    int32 TotalExperienceReward)
{
    ...
    for (int32 CharacterIndex = 0;
        CharacterIndex < ActiveCharacters.Num ();
        ++CharacterIndex)
    {
        ...
        if (Experience < 0 || Experience > MaximumExperience)
        {
            UE_LOG (
                LogGridExperience,
                Warning,
                TEXT ("[GridExperience] Character=%d Experience=%d Result=Skipped Reason=InvalidStoredExperience"),
                CharacterIndex,
                Experience);
            continue;
        }
    ...
    if (EligibleCharacterIndices.IsEmpty ())
    {
        UE_LOG (
            LogGridExperience,
            Log,
            TEXT ("[GridExperience] Reward=%d Applied=0 Result=NoEligibleActiveCharacter"),
    ...
    for (int32 EligibleIndex = 0;
        EligibleIndex < EligibleCount;
        ++EligibleIndex)
    {
        const int32 CharacterIndex =
            EligibleCharacterIndices[EligibleIndex];
        const FGridCharacterInventoryState& Character =
            ActiveCharacters[CharacterIndex];
        const int32 RequestedShare =
            BaseShare + (EligibleIndex < Remainder ? 1 : 0);
        if (RequestedShare <= 0)
        {
            continue;
        }
        ...
        const int64 CandidateExperience =
            static_cast<int64> (Pending.PreviousExperience) +
            static_cast<int64> (RequestedShare);
        Pending.NewExperience = static_cast<int32> (
            FMath::Min<int64> (
                CandidateExperience,
                static_cast<int64> (MaximumExperience)));
        Pending.AppliedExperience =
            Pending.NewExperience - Pending.PreviousExperience;
    ...
    {
        const FGridCharacterInventoryState& Character =
            ActiveCharacters[Pending.CharacterIndex];
        PartyInventoryComponent->NotifyPartyInventoryChanged (
            Pending.CharacterIndex);
        OnCharacterExperienceAwarded ().Broadcast (
            Pending.CharacterIndex,
            Pending.AppliedExperience,
            Pending.PreviousExperience,
            Pending.NewExperience);

        UE_LOG (
            LogGridExperience,
            Log,
            TEXT ("[GridExperience] Character=%d Requested=%d Applied=%d Previous=%d New=%d LevelStored=%d"),
            Pending.CharacterIndex,
            Pending.RequestedExperience,
            Pending.AppliedExperience,
            Pending.PreviousExperience,
            Pending.NewExperience,
            Character.Level);
    ...
    UE_LOG (
        LogGridExperience,
        Log,
        TEXT ("[GridExperience] Reward=%d Eligible=%d Applied=%d Unapplied=%d"),
        TotalExperienceReward,
        EligibleCount,
        TotalAppliedExperience,
        TotalExperienceReward - TotalAppliedExperience);
