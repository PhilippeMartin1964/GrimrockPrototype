...
namespace RPGCustomRecruitServicePrivate
{
    ...
    bool GenerateUniqueCharacterId (
        const FGridPartyInventoryState& State,
    ...
    FGridCharacterInventoryState BuildCandidate (
        ...
        const FRPGAttributes& FinalAttributes)
    {
        ...
        Candidate.InventorySlots.SetNum (
            FMath::Max (0, InventorySlotCount));
    ...
    FRPGCustomRecruitResult& OutResult)
{
    ...
    if (!PartyInventoryComponent)
    {
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::InvalidInventory;
    ...
    {
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::InitialCharacterMissing;
    ...
    {
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::InvalidPartyState;
    ...
    {
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::PartyFull;
        OutResult.Error = TEXT ("Active party is full.");
        return false;
    }

    const FString NormalizedName =
        RPGCustomRecruitServicePrivate::NormalizeDisplayName (
            Request.DisplayName);
    if (NormalizedName.Len () < 1 || NormalizedName.Len () > 24)
    {
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::InvalidName;
        OutResult.Error = TEXT ("Custom recruit display name must contain between 1 and 24 characters.");
        return false;
    }

    if (!Request.RaceDefinition ||
        !Request.RaceDefinition->IsValidDefinition ())
    {
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::InvalidRace;
        OutResult.Error = TEXT ("A valid race definition is required.");
        return false;
    }

    if (!Request.ClassDefinition ||
        !Request.ClassDefinition->IsValidDefinition ())
    {
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::InvalidClass;
    ...
    if (!CombatActionSourceClass ||
        ...
        CombatActionSourceClass->ClassId != Request.ClassDefinition->ClassId)
    {
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::InvalidCombatActionSourceClass;
    ...
    if (!URPGCharacterRulesLibrary::AreAttributesInRange (
            FinalAttributes))
    {
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::InvalidAttributes;
        OutResult.Error = TEXT ("All custom recruit attributes must be between 6 and 20.");
        return false;
    }

    FGuid CharacterId;
    if (!RPGCustomRecruitServicePrivate::GenerateUniqueCharacterId (
            State,
            CharacterId))
    {
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::CharacterIdGenerationFailed;
        OutResult.Error = TEXT ("A unique custom recruit CharacterId could not be generated.");
        return false;
    }

    FGridCharacterInventoryState Candidate =
        RPGCustomRecruitServicePrivate::BuildCandidate (
            Request,
            CombatActionSourceClass,
            NormalizedName,
            CharacterId,
    ...
    if (!FRPGPartyRecruitmentService::TryRecruitFromPool (
            ...
            CharacterId,
            RecruitmentResult) ||
        !RecruitmentResult.bCommitted)
    {
        State = PreviousState;
        OutResult.RejectReason =
            ERPGCustomRecruitRejectReason::RecruitmentFailed;
        OutResult.Error = RecruitmentResult.Error.IsEmpty ()
            ? TEXT ("Custom recruit activation failed.")
