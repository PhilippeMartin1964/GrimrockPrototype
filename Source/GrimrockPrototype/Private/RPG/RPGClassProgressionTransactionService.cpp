...
namespace
{
    ...
    bool BuildRuntimeStateFromSnapshot (
        const FGridCharacterInventoryState& Character,
        int32 CharacterIndex,
        const TArray<FName>& SavedChoiceIds,
        UGridPartyInventoryComponent* InventoryComponent,
        FRuntimeClassProgressionState& OutState,
        FText& OutError)
    {
        ...
        if (!Character.CharacterId.IsValid ())
        {
            OutError = FText::FromString (
                TEXT ("Un personnage ne possède pas de CharacterId valide."));
            return false;
        }
        ...
        if (!BuildSelectionSet (SavedChoiceIds, SelectedChoiceIds))
        {
            OutError = FText::FromString (
                FString::Printf (
                    TEXT ("Les choix persistants du personnage %s contiennent un identifiant vide ou dupliqué."),
        ...
        if (!ClassDefinition)
        {
            if (!SelectedChoiceIds.IsEmpty ())
            {
                OutError = FText::FromString (
                    FString::Printf (
                        TEXT ("La classe du personnage %s est introuvable alors que des choix de progression sont sauvegardés."),
                        *Character.CharacterId.ToString (EGuidFormats::Digits)));
                return false;
            }
                ...
                RemainingPoints))
        {
            OutError = FText::FromString (
                FString::Printf (
                    TEXT ("Les choix persistants du personnage %s ne sont pas valides pour son niveau et sa classe."),
                    *Character.CharacterId.ToString (EGuidFormats::Digits)));
            return false;
        }
                ...
                SatisfiedRequirements))
        {
            OutError = FText::FromString (
                FString::Printf (
                    TEXT ("La projection des choix persistants du personnage %s a échoué."),
                    *Character.CharacterId.ToString (EGuidFormats::Digits)));
            return false;
        }
        ...
        const FGridCharacterInventoryState& Character)
    {
        ...
        if (State->CharacterId != Character.CharacterId ||
    ...
    int32 CharacterIndex)
{
    ...
    FText RebuildError;
    if (!BuildRuntimeStateFromSnapshot (
            *Character,
            CharacterIndex,
            ExistingChoices,
            PartyInventoryComponent,
    ...
    TSet<FName>& InOutSatisfiedRequirements)
{
    ...
    const FRuntimeClassProgressionState* State =
        RuntimeStates.Find (CharacterId);
    if (!State)
    {
        return;
    }
    ...
    FText& OutError)
{
        ...
        ++CharacterIndex)
    {
        ...
        if (!Character.CharacterId.IsValid () ||
            CharacterIds.Contains (Character.CharacterId))
        {
            OutError = FText::FromString (
                TEXT ("Les personnages actifs possèdent un CharacterId invalide ou dupliqué."));
            OutStates.Reset ();
            return false;
        }
        ...
        if (const FRuntimeClassProgressionState* RuntimeState =
                RuntimeStates.Find (Character.CharacterId))
        {
            if (RuntimeState->ClassId != Character.ClassId)
            {
                OutError = FText::FromString (
                    FString::Printf (
                        TEXT ("Le cache de progression du personnage %s ne correspond plus à sa classe."),
                        *Character.CharacterId.ToString (EGuidFormats::Digits)));
                OutStates.Reset ();
                return false;
            }
    ...
    FText& OutError)
{
    ...
    if (SavedStates.Num () != PartyState.ActiveCharacters.Num ())
    {
        OutError = FText::FromString (
            FString::Printf (
                TEXT ("Le nombre d'états de progression sauvegardés (%d) ne correspond pas au nombre de personnages actifs (%d)."),
                SavedStates.Num (),
    ...
    for (const FRPGCharacterProgressionSaveState& SavedState : SavedStates)
    {
        if (!SavedState.CharacterId.IsValid () ||
            StatesByCharacterId.Contains (SavedState.CharacterId))
        {
            OutError = FText::FromString (
                TEXT ("Un état de progression sauvegardé possède un CharacterId invalide ou dupliqué."));
            return false;
        }
        ...
        ++CharacterIndex)
    {
        ...
        const FRPGCharacterProgressionSaveState* const* SavedStatePtr =
            StatesByCharacterId.Find (Character.CharacterId);
        if (!SavedStatePtr || !*SavedStatePtr)
        {
            OutError = FText::FromString (
                FString::Printf (
                    TEXT ("Aucun état de progression n'est sauvegardé pour le personnage %s."),
                    *Character.CharacterId.ToString (EGuidFormats::Digits)));
            return false;
        }
