...
namespace RPGSkillPersistencePrivate
{
	bool CollectPartyCharacters(
		const FGridPartyInventoryState& PartyState, TArray<const FGridCharacterInventoryState*>& OutCharacters, TSet<FGuid>& OutCharacterIds, FString& OutError)
	{
		OutCharacters.Reset();
		OutCharacterIds.Reset();
		OutError.Reset();

		const auto AppendCharacters = [&OutCharacters, &OutCharacterIds, &OutError](
			...
			for (int32 Index = 0; Index < Characters.Num(); ++Index)
			{
				...
				if (OutCharacterIds.Contains(Character.CharacterId))
				{
					OutError = FString::Printf(
						TEXT("CharacterId %s is duplicated or ambiguous in the party snapshot."), *Character.CharacterId.ToString(EGuidFormats::Digits));
					return false;
				}
	...
	bool ValidateDefinitionAndRank(FName SkillId, int32 Rank, TFunctionRef<const URPGSkillAsset*(FName)> DefinitionResolver, FString& OutError)
	{
		...
		{
			OutError = FString::Printf(TEXT("Skill '%s' persisted rank %d exceeds definition MaxRank %d."), *SkillId.ToString(), Rank, Definition->MaxRank);
			return false;
		}
		return true;
	}
	...
	{
		SkillRanks.Sort(
			[](const FRPGSkillRankSaveState& Left, const FRPGSkillRankSaveState& Right)
			{
...
bool FRPGSkillPersistence::CapturePartySkills(
	const FGridPartyInventoryState& PartyState, TArray<FRPGCharacterSkillSaveState>& OutSavedStates, FString& OutError)
{
	return CapturePartySkills(
		...
		OutSavedStates, OutError);
}

bool FRPGSkillPersistence::CapturePartySkills(const FGridPartyInventoryState& PartyState, TFunctionRef<const URPGSkillAsset*(FName)> DefinitionResolver,
	TArray<FRPGCharacterSkillSaveState>& OutSavedStates, FString& OutError)
{
	TArray<const FGridCharacterInventoryState*> OrderedCharacters;
	TSet<FGuid> CharacterIds;
	if (!CollectPartyCharacters(PartyState, OrderedCharacters, CharacterIds, OutError))
	{
		return false;
	}
	...
	for (const FGridCharacterInventoryState* Character : OrderedCharacters)
	{
		...
		{
			if (!ValidateDefinitionAndRank(RuntimeRank.SkillId, RuntimeRank.Rank, DefinitionResolver, OutError))
			{
				return false;
			}
	...
	Candidate.Sort(
		[](const FRPGCharacterSkillSaveState& Left, const FRPGCharacterSkillSaveState& Right)
		{
...
bool FRPGSkillPersistence::ValidateSavedPartySkills(
	const FGridPartyInventoryState& PartyState, const TArray<FRPGCharacterSkillSaveState>& SavedStates, FString& OutError)
{
	return ValidateSavedPartySkills(
		PartyState, SavedStates,
		[](FName SkillId)
		{
			return FRPGSkillPersistence::ResolveDefinitionBySkillId(SkillId);
		},
...
bool FRPGSkillPersistence::ValidateSavedPartySkills(const FGridPartyInventoryState& PartyState, const TArray<FRPGCharacterSkillSaveState>& SavedStates,
	TFunctionRef<const URPGSkillAsset*(FName)> DefinitionResolver, FString& OutError)
{
	TArray<const FGridCharacterInventoryState*> OrderedCharacters;
	TSet<FGuid> PartyCharacterIds;
	if (!CollectPartyCharacters(PartyState, OrderedCharacters, PartyCharacterIds, OutError))
	{
		return false;
	}
	...
	for (const FRPGCharacterSkillSaveState& SavedCharacter : SavedStates)
	{
		...
		{
			OutError = FString::Printf(TEXT("Saved Skill state duplicates CharacterId %s."), *SavedCharacter.CharacterId.ToString(EGuidFormats::Digits));
			return false;
		}
		if (!PartyCharacterIds.Contains(SavedCharacter.CharacterId))
		{
			OutError = FString::Printf(TEXT("Saved Skill state references orphan CharacterId %s."), *SavedCharacter.CharacterId.ToString(EGuidFormats::Digits));
			return false;
		}
		if (SavedCharacter.SkillRanks.IsEmpty())
		{
			OutError = FString::Printf(TEXT("Saved Skill state %s is empty and must be omitted from the sparse snapshot."),
		...
		for (const FRPGSkillRankSaveState& SavedRank : SavedCharacter.SkillRanks)
		{
			...
			{
				OutError = FString::Printf(
					TEXT("Saved Skill state %s contains an invalid or duplicate SkillId."), *SavedCharacter.CharacterId.ToString(EGuidFormats::Digits));
				return false;
			}
			if (!ValidateDefinitionAndRank(SavedRank.SkillId, SavedRank.Rank, DefinitionResolver, OutError))
			{
				return false;
			}
...
bool FRPGSkillPersistence::RestorePartySkills(
	FGridPartyInventoryState& InOutPartyState, const TArray<FRPGCharacterSkillSaveState>& SavedStates, FString& OutError)
{
	return RestorePartySkills(
		InOutPartyState, SavedStates,
		[](FName SkillId)
		{
			return FRPGSkillPersistence::ResolveDefinitionBySkillId(SkillId);
		},
...
bool FRPGSkillPersistence::RestorePartySkills(FGridPartyInventoryState& InOutPartyState, const TArray<FRPGCharacterSkillSaveState>& SavedStates,
	TFunctionRef<const URPGSkillAsset*(FName)> DefinitionResolver, FString& OutError)
{
	if (!ValidateSavedPartySkills(InOutPartyState, SavedStates, DefinitionResolver, OutError))
	{
		return false;
	}
	...
	for (const FRPGCharacterSkillSaveState& SavedCharacter : SavedStates)
	{
		FGridCharacterInventoryState* TargetCharacter = FindCharacterById(Candidate, SavedCharacter.CharacterId);
		if (!TargetCharacter)
		{
			OutError = FString::Printf(TEXT("Saved Skill state cannot resolve CharacterId %s."), *SavedCharacter.CharacterId.ToString(EGuidFormats::Digits));
			return false;
		}
		...
		for (const FRPGSkillRankSaveState& SavedRank : OrderedRanks)
		{
			...
			if (!FRPGSkillService::TrySetSkillRank(*TargetCharacter, Definition, SavedRank.Rank, MutationResult))
			{
				OutError = FString::Printf(TEXT("Skill '%s' could not be restored for CharacterId %s."), *SavedRank.SkillId.ToString(),
					*SavedCharacter.CharacterId.ToString(EGuidFormats::Digits));
				return false;
			}
		}
