...
namespace GridPartyRecruitmentPrivate
{
        ...
        FString& OutError)
    {
        ...
        {
            Candidate.CombatHotbarSlots.SetNum (
        ...
        if (Candidate.CombatHotbarSlots.Num () !=
            FGridCombatHotbarBinding::SlotCount)
        {
            OutError = FString::Printf (
                TEXT ("Candidate hotbar has %d slots; expected %d."),
        ...
        for (int32 SlotIndex = 0;
            SlotIndex < Candidate.CombatHotbarSlots.Num ();
            ++SlotIndex)
        {
            const FGridCombatHotbarBinding& Binding =
                Candidate.CombatHotbarSlots[SlotIndex];
            if (Binding.SlotIndex != SlotIndex || !Binding.IsValid ())
            {
                OutError = FString::Printf (
                    TEXT ("Candidate hotbar slot %d is invalid."),
                    SlotIndex);
                return false;
            }
        }
    ...
    void NormalizeCandidateInventoryOwnership (
        FGridCharacterInventoryState& Candidate,
        int32 NewCharacterIndex)
    {
    ...
    float CalculateCandidateInventoryWeight (
        const FGridCharacterInventoryState& Candidate)
    {
        ...
        for (const FGridInventorySlot& Slot : Candidate.InventorySlots)
        {
            ...
            TotalWeight +=
                Slot.Item.Weight * FMath::Max (1, Slot.Item.Quantity);
        }
        return TotalWeight;
    }
}
    ...
    FRPGPartyRecruitmentResult& OutResult)
{
    ...
    if (!PartyInventoryComponent)
    {
        OutResult.RejectReason =
            ERPGPartyRecruitmentRejectReason::InvalidInventory;
    ...
    {
        OutResult.RejectReason =
            ERPGPartyRecruitmentRejectReason::InvalidCharacterId;
    ...
    for (int32 PoolIndex = 0;
        PoolIndex < State.CharacterPool.Num ();
        ++PoolIndex)
    {
    ...
    if (CandidateMatchCount != 1 ||
        !State.CharacterPool.IsValidIndex (CandidateIndex))
    {
        OutResult.RejectReason =
            ERPGPartyRecruitmentRejectReason::AmbiguousCandidate;
        OutResult.Error = TEXT ("CharacterPool contains duplicate candidates with the same CharacterId.");
        return false;
    }

    FGridCharacterInventoryState Candidate =
        State.CharacterPool[CandidateIndex];

    FString CandidateError;
    if (!GridPartyRecruitmentPrivate::IsCandidateIdentityValid (
            Candidate,
            CandidateError))
    {
        OutResult.RejectReason =
            ERPGPartyRecruitmentRejectReason::InvalidCandidate;
    ...
    {
        Candidate.InventorySlots.SetNum (
            FMath::Max (
                0,
                PartyInventoryComponent->DefaultInventorySlotCountPerCharacter));
    }

    if (!GridPartyRecruitmentPrivate::NormalizeCandidateHotbar (
            Candidate,
            CandidateError))
    {
        OutResult.RejectReason =
    ...
    GridPartyRecruitmentPrivate::NormalizeCandidateInventoryOwnership (
        Candidate,
        NewCharacterIndex);
    Candidate.MaxCarryWeight =
        URPGCharacterRulesLibrary::CalculateMaxCarryWeight (
            Candidate.Attributes);
    Candidate.CurrentWeight =
        GridPartyRecruitmentPrivate::CalculateCandidateInventoryWeight (
            Candidate);
