...
bool FRPGSkillService::ValidateSkillState(const FGridCharacterInventoryState& CharacterState)
{
...
int32 FRPGSkillService::GetSkillRank(const FGridCharacterInventoryState& CharacterState, FName SkillId)
{
	...
	FGridCharacterInventoryState& CharacterState, const URPGSkillAsset* SkillDefinition, int32 NewRank, FRPGSkillMutationResult& OutResult)
{
	...
	const int32 PreviousRank = ExistingIndex != INDEX_NONE ? CharacterState.SkillRanks[ExistingIndex].Rank : 0;
	...
	FGridCharacterInventoryState& CharacterState, const URPGSkillAsset* SkillDefinition, int32 Delta, FRPGSkillMutationResult& OutResult)
{
	...
	const int32 CurrentRank = GetSkillRank(CharacterState, SkillDefinition->SkillId);
	...
	return TrySetSkillRank(CharacterState, SkillDefinition, CurrentRank + Delta, OutResult);
}
