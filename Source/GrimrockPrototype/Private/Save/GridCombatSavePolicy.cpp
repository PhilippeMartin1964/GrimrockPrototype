...
namespace
{
    bool PartyStatesShareStableCharacter (
        const FGridPartyInventoryState& SavedParty,
        const FGridPartyInventoryState& RuntimeParty)
    {
        TSet<FGuid> SavedCharacterIds;
        for (const FGridCharacterInventoryState& Character :
            SavedParty.ActiveCharacters)
        {
        ...
        for (const FGridCharacterInventoryState& Character :
            RuntimeParty.ActiveCharacters)
        {
            if (Character.CharacterId.IsValid () &&
                SavedCharacterIds.Contains (Character.CharacterId))
            {
                return true;
            }
        }
        for (const FGridCharacterInventoryState& Character :
            RuntimeParty.CharacterPool)
        {
            if (Character.CharacterId.IsValid () &&
                SavedCharacterIds.Contains (Character.CharacterId))
            {
...
FString FGridCombatSavePolicy::BuildPreCombatCheckpointSlotName (
    const FString& BaseSlotName)
{
    return BaseSlotName.IsEmpty ()
        ? FString ()
        : BaseSlotName + TEXT ("_AutoCombat");
}

bool FGridCombatSavePolicy::IsSaveBlockedByCombatState (
    const UGridTurnManagerComponent* TurnManager)
{
    ...
    return TurnManager->CurrentPhase != EGridCombatPhase::Exploration &&
        TurnManager->CurrentPhase != EGridCombatPhase::Victory;
}

bool FGridCombatSavePolicy::IsSaveBlockedForParty (
    const FGridPartyInventoryState& PartyState)
{
    for (TObjectIterator<UGridTurnManagerComponent> It; It; ++It)
    {
        ...
        UWorld* World = IsValid (TurnManager)
            ? TurnManager->GetWorld ()
            : nullptr;
        ...
        if (!World || !World->IsGameWorld () ||
            !IsValid (PartyPawn) || !IsValid (Inventory) ||
            !PartyStatesShareStableCharacter (
                PartyState,
        ...
        {
            UE_LOG (
                LogGridCombatSavePolicy,
                Log,
                TEXT ("[MON18.9.1] SaveBlocked CharacterCount=%d Phase=%s Active=%s"),
...
bool FGridCombatSavePolicy::PreparePreCombatCheckpoint (
    ...
    bool& bOutSkipped)
{
    ...
    if (!IsValid (PartyPawn))
    {
        OutError = FText::FromString (
            TEXT ("Le groupe est indisponible pour le checkpoint pré-combat."));
        return false;
    }

    UGridPartyInventoryComponent* Inventory =
        PartyPawn->PartyInventoryComponent.Get ();
    if (PartyPawn->PartySaveSlotName.IsEmpty () ||
        !IsValid (Inventory) ||
        !Inventory->HasCompletedInitialCharacterCreation ())
    {
        ...
        UE_LOG (
            LogGridCombatSavePolicy,
            Verbose,
            TEXT ("[MON18.9.1] PreCombatCheckpoint Skipped Pawn=%s Reason=TransientOrUninitializedParty"),
    ...
    {
        OutError = FText::FromString (
            TEXT ("Le checkpoint pré-combat doit être créé avant l'activation du combat."));
        return false;
    }

    const FString MainSlotName = PartyPawn->PartySaveSlotName;
    const FString CheckpointSlotName =
        BuildPreCombatCheckpointSlotName (MainSlotName);
    if (CheckpointSlotName.IsEmpty ())
    {
        OutError = FText::FromString (
            TEXT ("Le nom du checkpoint pré-combat est invalide."));
        return false;
    }
    ...
    if (!bSaved)
    {
        UE_LOG (
            LogGridCombatSavePolicy,
            Warning,
            TEXT ("[MON18.9.1] PreCombatCheckpoint Failed Slot=%s Reason=%s"),
            *CheckpointSlotName,
            *OutError.ToString ());
        return false;
    }
    ...
    UE_LOG (
        LogGridCombatSavePolicy,
        Log,
        TEXT ("[MON18.9.1] PreCombatCheckpoint Saved Slot=%s SourceSlot=%s Cell=(%d,%d)"),
        *CheckpointSlotName,
        *MainSlotName,
