...
namespace RPGMON2072TalentRuntimeReadModelTests
{
	...
	FGridCharacterInventoryState MakeCharacter(URPGClassAsset* ClassDefinition, int32 Level, const TCHAR* Name)
	{
	...
	UGridPartyInventoryComponent* MakeParty(int32 Level, URPGClassAsset*& OutClassDefinition)
	{
		...
		Party->PartyInventoryState.ActiveCharacters.Add(MakeCharacter(OutClassDefinition, Level, TEXT("A")));
		Party->PartyInventoryState.ActiveCharacters.Add(MakeCharacter(OutClassDefinition, Level, TEXT("B")));
		Party->PartyInventoryState.ActiveEquipment.SetNum(2);
		Party->PartyInventoryState.SelectedCharacterIndex = 0;
		return Party;
	}

	bool CommitTalents(UGridPartyInventoryComponent* Party, int32 CharacterIndex, std::initializer_list<FName> ChoiceIds)
	{
		...
		return FRPGClassProgressionTransactionService::TryCommitChoices(Party, CharacterIndex, Requested, Result);
	}
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON2072ExplicitSelectedTalentsTest, "Grimrock.MON20.7.Talents.ExplicitSelectedTalents",
...
bool FRPGMON2072ExplicitSelectedTalentsTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Explicit selected talents are readable"), FRPGTalentRuntimeService::TryGetSelectedTalents(Party, 0, Talents));
	...
	if (Talents.Num() == 1)
	{
		...
		TestTrue(TEXT("Selected talent reports AlreadySelected through MON15 rules"),
			Talents[0].AvailabilityReason == ERPGClassProgressionChoiceAvailabilityReason::AlreadySelected);
	}
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON2072SelectedCharacterAuthorityTest, "Grimrock.MON20.7.Talents.SelectedCharacterAuthority",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2072SelectedCharacterAuthorityTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Talent A+B commit for selected character"), CommitTalents(Party, 1, { TalentA, TalentB }));

	TArray<FRPGTalentRuntimeView> SelectedCharacterTalents;
	TestTrue(TEXT("Selected-character talent query succeeds"), FRPGTalentRuntimeService::TryGetSelectedCharacterTalents(Party, SelectedCharacterTalents));
	TestEqual(TEXT("Selected character owns both committed talents"), SelectedCharacterTalents.Num(), 2);

	TArray<FRPGTalentRuntimeView> CharacterZeroTalents;
	TestTrue(TEXT("Explicit character zero query succeeds"), FRPGTalentRuntimeService::TryGetSelectedTalents(Party, 0, CharacterZeroTalents));
	TestEqual(TEXT("Character zero remains independent"), CharacterZeroTalents.Num(), 0);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON2072HasTalentTest, "Grimrock.MON20.7.Talents.HasTalent", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2072HasTalentTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Known acquired talent query succeeds"), FRPGTalentRuntimeService::HasTalent(Party, 0, TalentA, bHasTalent));
	TestTrue(TEXT("Acquired talent is reported"), bHasTalent);

	bHasTalent = true;
	TestTrue(TEXT("Known unacquired talent query succeeds"), FRPGTalentRuntimeService::HasTalent(Party, 0, TalentB, bHasTalent));
	TestFalse(TEXT("Unacquired talent is not reported"), bHasTalent);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON2072TalentPointBalanceTest, "Grimrock.MON20.7.Talents.PointBalance", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2072TalentPointBalanceTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Talent point balance query succeeds"), FRPGTalentRuntimeService::TryGetTalentPointBalance(Party, 0, Balance));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON2072AvailableBeforeSelectionTest, "Grimrock.MON20.7.Talents.AvailableBeforeSelection",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2072AvailableBeforeSelectionTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Available talent query succeeds"), FRPGTalentRuntimeService::TryGetAvailableTalents(Party, 0, Available));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON2072AvailableAfterPrerequisiteTest, "Grimrock.MON20.7.Talents.AvailableAfterPrerequisite",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2072AvailableAfterPrerequisiteTest::RunTest(const FString& Parameters)
{
	...
	TArray<FRPGTalentRuntimeView> Available;
	TestTrue(TEXT("Available talent query succeeds after prerequisite"), FRPGTalentRuntimeService::TryGetAvailableTalents(Party, 0, Available));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON2072SelectedCharacterFacadeTest, "Grimrock.MON20.7.Talents.SelectedCharacterFacade",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2072SelectedCharacterFacadeTest::RunTest(const FString& Parameters)
{
	...
	bool bHasTalent = false;
	TestTrue(TEXT("Selected-character HasTalent succeeds"), FRPGTalentRuntimeService::HasSelectedCharacterTalent(Party, TalentA, bHasTalent));
	TestTrue(TEXT("Selected character owns Talent A"), bHasTalent);

	FRPGTalentPointBalance Balance;
	TestTrue(TEXT("Selected-character balance succeeds"), FRPGTalentRuntimeService::TryGetSelectedCharacterTalentPointBalance(Party, Balance));
	TestEqual(TEXT("Selected character has one point remaining"), Balance.RemainingPoints, 1);

	TArray<FRPGTalentRuntimeView> Available;
	TestTrue(TEXT("Selected-character available talents query succeeds"), FRPGTalentRuntimeService::TryGetSelectedCharacterAvailableTalents(Party, Available));
	TestEqual(TEXT("Selected character can now select Talent B"), Available.Num(), 1);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON2072InvalidIndexNoMutationTest, "Grimrock.MON20.7.Talents.InvalidIndexNoMutation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2072InvalidIndexNoMutationTest::RunTest(const FString& Parameters)
{
	...
	TArray<FName> Before;
	TestTrue(TEXT("Authoritative MON15 selection is readable before invalid calls"),
		FRPGClassProgressionTransactionService::TryGetSelectedChoiceIds(Party, 0, Before));

	TArray<FRPGTalentRuntimeView> Talents;
	Talents.Add(FRPGTalentRuntimeView());
	TestFalse(TEXT("Invalid explicit index is rejected"), FRPGTalentRuntimeService::TryGetSelectedTalents(Party, 99, Talents));
	...
	TestFalse(TEXT("Invalid balance index is rejected"), FRPGTalentRuntimeService::TryGetTalentPointBalance(Party, 99, Balance));
	...
	TestFalse(TEXT("Invalid HasTalent index is rejected"), FRPGTalentRuntimeService::HasTalent(Party, 99, TalentA, bHasTalent));
	TestFalse(TEXT("Failed HasTalent resets output"), bHasTalent);

	Party->PartyInventoryState.SelectedCharacterIndex = 99;
	TestFalse(TEXT("Invalid selected character is rejected"), FRPGTalentRuntimeService::TryGetSelectedCharacterTalents(Party, Talents));

	TArray<FName> After;
	TestTrue(TEXT("Authoritative MON15 selection remains readable"), FRPGClassProgressionTransactionService::TryGetSelectedChoiceIds(Party, 0, After));
