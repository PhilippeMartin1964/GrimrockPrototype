...
namespace
{
    ...
    FGridCharacterSpellbookState MakeMON187Spellbook (
    ...
    TArray<FGridCombatHotbarBinding> ReadMON187Hotbar (
        UGridPartyInventoryComponent& Inventory)
    {
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON187ProductionEntriesTest,
    "Grimrock.Magic.MON18.7a.ProductionEntriesReflectKnowledge",
...
bool FGridMON187ProductionEntriesTest::RunTest (const FString& Parameters)
{
    ...
    UGridSpellbookUILibrary::BuildProductionSpellbookEntries (
        Spellbook,
        ReadMON187Hotbar (*Inventory),
        Entries);

    TestEqual (TEXT ("Only known spells are projected"), Entries.Num (), 2);
    TestEqual (TEXT ("Knowledge order is preserved"),
        Entries[0].SpellId, FGridProductionSpellLibrary::ArcaneBoltId ());
    TestTrue (TEXT ("Arcane Bolt definition resolves"),
        Entries[0].bDefinitionResolved);
    TestTrue (TEXT ("Arcane Bolt can be assigned"),
        Entries[0].bCanAssignToHotbar);
    TestTrue (TEXT ("UI action is a Spell source"),
        Entries[0].CombatActionDefinition.SourcePolicy ==
            EGridCombatActionSourcePolicy::Spell);
    TestEqual (TEXT ("Mana cost is projected"),
        Entries[0].ManaCost, 3);
    TestEqual (TEXT ("AP cost is projected"),
        Entries[0].ActionPointCost, 2);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON187MissingDefinitionTest,
    "Grimrock.Magic.MON18.7a.UnknownDefinitionRemainsVisible",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)
...
bool FGridMON187MissingDefinitionTest::RunTest (const FString& Parameters)
{
    ...
    TestFalse (TEXT ("Definition is explicitly unresolved"),
        Entries[0].bDefinitionResolved);
    TestFalse (TEXT ("Unresolved spell cannot be assigned"),
        Entries[0].bCanAssignToHotbar);
    TestEqual (TEXT ("Stable SpellId is preserved"),
        Entries[0].SpellId, FName (TEXT ("Spell_NotRegistered")));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON187BindingContractTest,
    "Grimrock.Magic.MON18.7a.SpellBindingContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)
...
bool FGridMON187BindingContractTest::RunTest (const FString& Parameters)
{
    ...
    const FGridCombatHotbarBinding Binding =
        UGridSpellbookUILibrary::MakeSpellHotbarBinding (SpellId, 4);
    ...
    TestTrue (TEXT ("SourcePolicy is Spell"),
        Binding.SourcePolicy == EGridCombatActionSourcePolicy::Spell);
    TestEqual (TEXT ("SourceDefinitionId reuses SpellId"),
        Binding.SourceDefinitionId, SpellId);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON187AssignKnownSpellTest,
    "Grimrock.Magic.MON18.7a.AssignKnownSpell",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)
...
bool FGridMON187AssignKnownSpellTest::RunTest (const FString& Parameters)
{
    ...
    TestTrue (TEXT ("Known spell assignment succeeds"),
        UGridSpellbookUILibrary::AssignKnownSpellToHotbar (
            Inventory, 0, Spellbook, SpellId, 3) ==
            EGridSpellHotbarAssignmentResult::Success);

    FGridCombatHotbarBinding Binding;
    TestTrue (TEXT ("Assigned slot can be read"),
        Inventory->GetCharacterCombatHotbarBinding (0, 3, Binding));
    TestTrue (TEXT ("Assigned slot contains the spell identity"),
        UGridSpellbookUILibrary::IsSpellHotbarBinding (Binding, SpellId));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON187DuplicateMoveSwapTest,
    "Grimrock.Magic.MON18.7a.DuplicateAssignmentMovesOrSwaps",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)
...
bool FGridMON187DuplicateMoveSwapTest::RunTest (const FString& Parameters)
{
    ...
    UGridSpellbookUILibrary::AssignKnownSpellToHotbar (
        Inventory, 0, Spellbook, ArcaneBolt, 1);
    UGridSpellbookUILibrary::AssignKnownSpellToHotbar (
        Inventory, 0, Spellbook, Haste, 2);

    TestTrue (TEXT ("Reassigning an existing spell uses move/swap"),
        UGridSpellbookUILibrary::AssignKnownSpellToHotbar (
            Inventory, 0, Spellbook, ArcaneBolt, 2) ==
            EGridSpellHotbarAssignmentResult::Success);
    ...
    TestTrue (TEXT ("Haste is swapped into the old slot"),
        UGridSpellbookUILibrary::IsSpellHotbarBinding (SlotOne, Haste));
    TestTrue (TEXT ("Arcane Bolt occupies the requested slot"),
        UGridSpellbookUILibrary::IsSpellHotbarBinding (SlotTwo, ArcaneBolt));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON187RejectUnknownNoMutationTest,
    "Grimrock.Magic.MON18.7a.UnknownSpellNoMutation",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)
...
bool FGridMON187RejectUnknownNoMutationTest::RunTest (const FString& Parameters)
{
    ...
    TestTrue (TEXT ("Unknown spell is rejected"),
        UGridSpellbookUILibrary::AssignKnownSpellToHotbar (
            ...
            0,
            Spellbook,
            FGridProductionSpellLibrary::CurePoisonId (),
            5) == EGridSpellHotbarAssignmentResult::UnknownSpell);

    FGridCombatHotbarBinding Binding;
    Inventory->GetCharacterCombatHotbarBinding (0, 5, Binding);
    TestTrue (TEXT ("Rejected assignment leaves target slot empty"),
        Binding.IsEmpty ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON187DragDropPayloadTest,
    "Grimrock.Magic.MON18.7a.DragDropPayload",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)
...
bool FGridMON187DragDropPayloadTest::RunTest (const FString& Parameters)
{
    ...
    UGridCombatHotbarDragDropOperation* Operation =
        NewObject<UGridCombatHotbarDragDropOperation> ();
    Operation->InitializeFromSpellbookEntry (0, Entries[0]);
    TestTrue (TEXT ("Payload is marked as Spellbook source"),
        Operation->bFromSpellbook);
    TestFalse (TEXT ("Payload is not an action-palette source"),
        Operation->bFromActionPalette);
    TestEqual (TEXT ("Payload keeps SpellId"),
        Operation->Binding.SourceDefinitionId, SpellId);

    TestTrue (TEXT ("Blueprint drop hook commits through MON12 hotbar"),
        Operation->CommitSpellbookDrop (Inventory, Spellbook, 7) ==
            EGridSpellHotbarAssignmentResult::Success);
    FGridCombatHotbarBinding Binding;
    Inventory->GetCharacterCombatHotbarBinding (0, 7, Binding);
    TestTrue (TEXT ("Drop target contains the spell"),
        UGridSpellbookUILibrary::IsSpellHotbarBinding (Binding, SpellId));
    return true;
}
