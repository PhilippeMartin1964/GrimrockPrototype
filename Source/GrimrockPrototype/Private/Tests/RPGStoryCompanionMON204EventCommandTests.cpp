...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON2044RecruitmentEventCommandContractTest,
    "Grimrock.MON20.4.RecruitmentUI.EventCommandContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON2044RecruitmentEventCommandContractTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;

    TestEqual (
        TEXT ("OfferRecruitment keeps the serialized command value 23"),
    ...
    {
        TestEqual (
            TEXT ("Lua-style enum lookup resolves OfferRecruitment"),
            CommandEnum->GetValueByNameString (TEXT ("OfferRecruitment")),
    ...
    {
        TestEqual (
            TEXT ("StoryCompanion is exposed by the object type enum"),
            TypeEnum->GetValueByNameString (TEXT ("StoryCompanion")),
    ...
    {
        TestNotNull (
            TEXT ("Story companion definition is part of level object data"),
            ObjectDataStruct->FindPropertyByName (
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON2044RecruitmentMissingDefinitionTest,
    "Grimrock.MON20.4.RecruitmentUI.EventCommandMissingDefinition",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON2044RecruitmentMissingDefinitionTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    TestFalse (
        TEXT ("OfferRecruitment rejects a story companion without a definition"),
        Activation->ExecuteLinksFromObjectForEvent (
            SourceId,
            EGridObjectEvent::Activated));
    TestFalse (
        TEXT ("Rejected recruitment does not mark the data-only target active"),
        Activation->GetActiveObjectIds ().Contains (CompanionId));
