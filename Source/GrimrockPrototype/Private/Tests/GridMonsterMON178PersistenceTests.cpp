...
namespace GridMonsterMON178Persistence
{
    ...
    UGridMonsterDefinitionAsset* MakeDefinition (UObject* Outer)
    {
        ...
        Definition->DisplayName =
            FText::FromString (TEXT ("MON17.8 Persistence Monster"));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON178DeadRestorePresentationContractTest,
    "Grimrock.Monsters.MON17.8.DeadRestorePresentationContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON178DeadRestorePresentationContractTest::RunTest (
    const FString& Parameters)
{
    ...
    AGridMonsterActor* Monster = SpawnMonster (
        TestWorld.World,
        Definition,
        PersistenceId,
        DeathCell,
        TEXT ("MON178_DeadRestore"));
    ...
    TestTrue (
        TEXT ("Test setup initially occupies the death cell"),
        Occupancy && Occupancy->RegisterMonster (Monster, DeathCell));

    const FGridRuntimeMonsterState DeadState =
        MakeState (Monster, DeathCell, true);

    TestTrue (
        TEXT ("Dead runtime state restores"),
        Monster->RestoreRuntimeMonsterState (DeadState, Runtime));
    ...
    TestTrue (
        TEXT ("Death commit remains restored"),
        Monster->DeathComponent->bDeathCommitted);
    TestTrue (
        TEXT ("Loot generation remains committed"),
        Monster->DeathComponent->bLootGenerated);
    TestFalse (
        TEXT ("Restored dead mesh is hidden immediately"),
        Monster->SkeletalMeshComponent->IsVisible ());
    TestFalse (
        TEXT ("Actor is retained instead of hidden/destroyed"),
        Monster->IsHidden ());
    TestEqual (
        TEXT ("Restored dead collision stays disabled"),
        Monster->CollisionComponent->GetCollisionEnabled (),
        ECollisionEnabled::NoCollision);
    TestFalse (
        TEXT ("Restored dead occupies no cell"),
        Occupancy && Occupancy->IsCellOccupied (DeathCell));
    TestFalse (
        TEXT ("Death montage presentation is not active after restore"),
        Monster->DeathComponent->bDeathPresentationActive);
    TestFalse (
        TEXT ("Death dissolve is not replayed after restore"),
        Monster->DeathComponent->bDeathDissolveActive);
    TestEqual (
        TEXT ("Transient dissolve alpha is reset"),
        Monster->DeathComponent->DeathDissolveAlpha,
        0.0f);
    TestEqual (
        TEXT ("Restore emits no logical death event"),
        Monster->DeathComponent->LogicalDeathEventCount,
        0);
    TestEqual (
        TEXT ("Restore executes no MonsterDied links"),
        Monster->DeathComponent->LinkExecutionAttemptCount,
        0);
    TestEqual (
        TEXT ("Restore generates no transient loot metadata"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON178LivingRestoreClearsDeathPresentationTest,
    "Grimrock.Monsters.MON17.8.LivingRestoreClearsDeathPresentation",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON178LivingRestoreClearsDeathPresentationTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace GridMonsterMON178Persistence;
    ...
    const FGridRuntimeMonsterState DeadState =
        MakeState (Monster, Cell, true);
    TestTrue (
        TEXT ("Dead precursor state restores"),
        Monster->RestoreRuntimeMonsterState (DeadState, Runtime));
    TestFalse (
        TEXT ("Dead precursor hides the mesh"),
        Monster->SkeletalMeshComponent->IsVisible ());

    const FGridRuntimeMonsterState LivingState =
        MakeState (Monster, Cell, false);
    TestTrue (
        TEXT ("Living state restores after dead presentation state"),
        Monster->RestoreRuntimeMonsterState (LivingState, Runtime));
    TestFalse (
        TEXT ("Living restore clears the death commit"),
        Monster->DeathComponent->bDeathCommitted);
    TestFalse (
        TEXT ("Living restore clears loot generation guard"),
        Monster->DeathComponent->bLootGenerated);
    TestFalse (
        TEXT ("Living restore has no active death presentation"),
        Monster->DeathComponent->bDeathPresentationActive);
    TestFalse (
        TEXT ("Living restore has no active dissolve"),
        Monster->DeathComponent->bDeathDissolveActive);
    TestEqual (
        TEXT ("Living restore resets dissolve alpha"),
        Monster->DeathComponent->DeathDissolveAlpha,
        0.0f);
    TestTrue (
        TEXT ("Living restore makes the mesh visible again"),
        Monster->SkeletalMeshComponent->IsVisible ());
    ...
    UGridMonsterOccupancySubsystem* Occupancy =
        TestWorld.World->GetSubsystem<UGridMonsterOccupancySubsystem> ();
    TestTrue (
        TEXT ("Living restore re-establishes normal occupancy"),
        Occupancy && Occupancy->IsCellOccupied (Cell));
    return true;
}
