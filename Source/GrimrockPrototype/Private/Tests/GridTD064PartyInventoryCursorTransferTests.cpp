...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridTD064PartyInventoryCursorTransferContractTest, "Grimrock.TechnicalDebt.TD06_4.PartyInventoryCursorTransfer.Contract",
...
bool FGridTD064PartyInventoryCursorTransferContractTest::RunTest(const FString& Parameters)
{
	...
	const FName ReflectedFunctions[] = { TEXT("SetCursorItem"), TEXT("ClearCursorItem"), TEXT("HasCursorItem"), TEXT("TryTakeInventorySlotToCursor"),
		TEXT("TryTakeInventorySlotQuantityToCursor"), TEXT("TryPlaceCursorItemInCharacterInventorySlot"), TEXT("TryMoveCharacterInventorySlot"),
		TEXT("TryPlaceCursorItemInCharacterInventory"), TEXT("TryPlaceCursorItemInSelectedCharacterInventory"),
		TEXT("TryClearCursorToSelectedCharacterInventory"), TEXT("TryDropCursorItem"), TEXT("CanEquipCursorItemToCharacterSlot"),
		TEXT("TryEquipCursorItemToCharacterSlot"), TEXT("TryEquipCursorItemToSelectedCharacterSlot"), TEXT("TryTakeEquipmentSlotToCursor"),
	...
	UGridItemDefinitionAsset* StackDefinition = GridTD064CreateItemDefinition(Component, TEXT("Potion_TD064"), EGridItemType::Potion, true);
	...
	if (!TestNotNull(TEXT("The stack definition is created"), StackDefinition) || !TestNotNull(TEXT("Weapon A definition is created"), WeaponADefinition) ||
	...
	TestTrue(TEXT("The split cursor item can be placed into a chosen empty inventory slot"), Component->TryPlaceCursorItemInCharacterInventorySlot(0, 5));
	...
	TestTrue(
		TEXT("The placed item is normalized to inventory ownership"), Character.InventorySlots[5].Item.OwnerType == EGridItemOwnerType::CharacterInventory);
	...
	TestTrue(TEXT("Dropping the cursor onto an occupied inventory slot swaps atomically"), Component->TryPlaceCursorItemInCharacterInventorySlot(0, 1));
	...
	TestTrue(
		TEXT("The swapped target is normalized to inventory ownership"), Character.InventorySlots[1].Item.OwnerType == EGridItemOwnerType::CharacterInventory);
	...
	TestTrue(
		TEXT("A cursor weapon is compatible with its declared main-hand slot"), Component->CanEquipCursorItemToCharacterSlot(0, EGridEquipmentSlot::MainHand));
	TestTrue(TEXT("A cursor weapon equips to an empty main hand"), Component->TryEquipCursorItemToCharacterSlot(0, EGridEquipmentSlot::MainHand));
	...
	TestFalse(TEXT("Equipment to cursor clears the source equipment slot"), Component->IsEquipmentSlotOccupied(0, EGridEquipmentSlot::MainHand));
	...
	TestTrue(
		TEXT("The cursor still owns Weapon A after the independent equipment transaction"), Component->GetCursorItem().RuntimeObjectId == WeaponARuntimeId);
	...
	TestTrue(TEXT("The cursor item can be returned to the first free inventory slot"), Component->TryPlaceCursorItemInCharacterInventory(0));
	...
	TestTrue(TEXT("Moving an inventory item into an empty slot succeeds"), Component->TryMoveCharacterInventorySlot(0, 2, 3));
	...
	TestTrue(TEXT("Moving onto an occupied inventory slot swaps atomically"), Component->TryMoveCharacterInventorySlot(0, 3, 1));
