...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON203DefinitionValidationTest,
    "Grimrock.MON20.3.StoryCompanion.DefinitionValidation",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON203DefinitionValidationTest::RunTest (
    const FString& Parameters)
{
    ...
    URPGStoryCompanionAsset* Companion =
        MakeCompanion (Race, ClassDefinition);

    TestTrue (TEXT ("Complete story companion definition is valid"),
        Companion->IsValidDefinition ());

    Companion->CharacterId = FGuid ();
    TestFalse (TEXT ("Missing stable CharacterId is rejected"),
        Companion->IsValidDefinition ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON203RegisterCandidateTest,
    "Grimrock.MON20.3.StoryCompanion.RegisterCandidate",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON203RegisterCandidateTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace GridMON203StoryCompanionTests;
    ...
    TestTrue (TEXT ("Story companion registers"),
        FRPGStoryCompanionService::EnsureCandidateRegistered (
            Inventory,
            Companion,
            Result));
    TestTrue (TEXT ("Registration reports success"), Result.bSucceeded);
    TestEqual (TEXT ("Registration status is AddedToPool"),
        static_cast<int32> (Result.Status),
        static_cast<int32> (
            ERPGStoryCompanionRegistrationStatus::AddedToPool));
    TestEqual (TEXT ("Exactly one pool candidate exists"),
        Inventory->PartyInventoryState.CharacterPool.Num (),
        1);

    const FGridCharacterInventoryState& Candidate =
        Inventory->PartyInventoryState.CharacterPool[0];
    TestTrue (TEXT ("Stable CharacterId is copied"),
        Candidate.CharacterId == Companion->CharacterId);
    TestEqual (TEXT ("Race identity is copied"),
        Candidate.RaceId,
        Race->RaceId);
    TestEqual (TEXT ("Class identity is copied"),
        Candidate.ClassId,
        ClassDefinition->ClassId);
    TestEqual (TEXT ("Level is copied"), Candidate.Level, 3);
    TestEqual (TEXT ("Level-three XP floor is derived"),
        Candidate.Experience,
        3000);
    TestEqual (TEXT ("Inventory slots use party default"),
        Candidate.InventorySlots.Num (),
        Inventory->DefaultInventorySlotCountPerCharacter);
    TestEqual (TEXT ("Hotbar contains ten empty bindings"),
        Candidate.CombatHotbarSlots.Num (),
        FGridCombatHotbarBinding::SlotCount);
    TestTrue (TEXT ("Candidate RPG state is initialized"),
        Candidate.bRPGAttributesInitialized);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON203IdempotentRegistrationTest,
    "Grimrock.MON20.3.StoryCompanion.IdempotentRegistration",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON203IdempotentRegistrationTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace GridMON203StoryCompanionTests;
    ...
    UGridPartyInventoryComponent* Inventory = MakeParty ();
    URPGStoryCompanionAsset* Companion =
        MakeCompanion (MakeRace (), MakeClass ());

    FRPGStoryCompanionRegistrationResult FirstResult;
    TestTrue (TEXT ("First registration succeeds"),
        FRPGStoryCompanionService::EnsureCandidateRegistered (
            Inventory,
            Companion,
            FirstResult));
    ...
    TestTrue (TEXT ("Second registration resolves idempotently"),
        FRPGStoryCompanionService::EnsureCandidateRegistered (
            Inventory,
            Companion,
            SecondResult));
    TestEqual (TEXT ("Second status is AlreadyInPool"),
        static_cast<int32> (SecondResult.Status),
        static_cast<int32> (
            ERPGStoryCompanionRegistrationStatus::AlreadyInPool));
    TestEqual (TEXT ("Pool still contains one candidate"),
        Inventory->PartyInventoryState.CharacterPool.Num (),
        1);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON203AlreadyActiveRecognitionTest,
    "Grimrock.MON20.3.StoryCompanion.AlreadyActiveRecognition",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON203AlreadyActiveRecognitionTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace GridMON203StoryCompanionTests;
    ...
    TestTrue (TEXT ("Companion enters pool"),
        FRPGStoryCompanionService::EnsureCandidateRegistered (
            Inventory,
            Companion,
            Registration));
    ...
    TestTrue (TEXT ("MON20.2 recruits the registered companion"),
    ...
    TestTrue (TEXT ("Recheck recognizes the active companion"),
        FRPGStoryCompanionService::EnsureCandidateRegistered (
            Inventory,
            Companion,
            Recheck));
    TestEqual (TEXT ("Recheck status is AlreadyActive"),
        static_cast<int32> (Recheck.Status),
        static_cast<int32> (
    ...
    TestEqual (TEXT ("Active party contains two characters"),
        ...
        2);
    TestEqual (TEXT ("Pool is empty after recruitment"),
        Inventory->PartyInventoryState.CharacterPool.Num (),
        0);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON203IdentityCollisionRejectTest,
    "Grimrock.MON20.3.StoryCompanion.IdentityCollisionReject",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON203IdentityCollisionRejectTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace GridMON203StoryCompanionTests;
    ...
    FRPGStoryCompanionRegistrationResult Result;
    TestFalse (TEXT ("Colliding identity is rejected"),
        FRPGStoryCompanionService::EnsureCandidateRegistered (
            Inventory,
            Companion,
            Result));
    TestEqual (TEXT ("Collision status is explicit"),
        static_cast<int32> (Result.Status),
        static_cast<int32> (
            ERPGStoryCompanionRegistrationStatus::IdentityCollision));
    TestEqual (TEXT ("Rejected collision does not mutate pool"),
        Inventory->PartyInventoryState.CharacterPool.Num (),
        0);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON203InvalidDefinitionAtomicRejectTest,
    "Grimrock.MON20.3.StoryCompanion.InvalidDefinitionAtomicReject",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON203InvalidDefinitionAtomicRejectTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace GridMON203StoryCompanionTests;
    ...
    FRPGStoryCompanionRegistrationResult Result;
    TestFalse (TEXT ("Invalid definition is rejected"),
        FRPGStoryCompanionService::EnsureCandidateRegistered (
            Inventory,
            Companion,
            Result));
    TestEqual (TEXT ("Invalid definition status is explicit"),
        static_cast<int32> (Result.Status),
        static_cast<int32> (
            ERPGStoryCompanionRegistrationStatus::InvalidDefinition));
    TestEqual (TEXT ("Rejected definition does not mutate pool"),
        Inventory->PartyInventoryState.CharacterPool.Num (),
        0);
    return true;
}
