...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON144HearingAlarmPropagationTest,
    "Grimrock.Monsters.MON14.4.HearingAlarmPropagation",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON144HearingAlarmPropagationTest::RunTest (
    const FString& Parameters)
{
    ...
    UGridMonsterDefinitionAsset* SourceDefinition = Fixture.MakeDefinition (
        TEXT ("MON14_4_Guard"), 0, 3, true, 2);
    UGridMonsterDefinitionAsset* AllyDefinition = Fixture.MakeDefinition (
        TEXT ("MON14_4_Guard"), 0, 0, false, 0);
    ...
    AGridMonsterActor* Source = Fixture.AddMonster (
        ...
        TEXT ("Alarm_A"));
    AGridMonsterActor* Ally = Fixture.AddMonster (
        AllyDefinition,
        FIntPoint (2, 1),
        EGridEdge::West,
    ...
    TestTrue (TEXT ("Source hearing is processed"),
        Fixture.Patrol->ProcessMonsterNow (Source, TEXT ("MON144Hearing")));
    ...
    TestFalse (TEXT ("Source does not see party"),
        SourceBehavior && SourceBehavior->bCanSeeParty);
    TestEqual (TEXT ("Dormant ally wakes to Alert"),
        Ally->MonsterState,
        EGridMonsterState::Alert);
    TestTrue (TEXT ("Ally receives last known party cell"),
        AllyBehavior && AllyBehavior->bHasLastKnownPartyCell);
    TestTrue (TEXT ("Ally receives correct party cell"),
        AllyBehavior && AllyBehavior->LastKnownPartyCell == FIntPoint (1, 4));
    TestEqual (TEXT ("Ally starts investigation"),
        Fixture.Patrol->GetMonsterActivity (Ally->ResolvePersistenceId ()),
        EGridMonsterExplorationActivity::Investigating);
    TestFalse (TEXT ("Alarm alone does not start combat"),
        Fixture.TurnManager->bCombatActive);

    const int32 RepeatedAlertCount = Fixture.Patrol->HandleExplorationAlert (
        Source,
        FIntPoint (1, 4),
        TEXT ("MON144RepeatSameCell"));
    TestEqual (TEXT ("Same-cell repeated alarm does not restart investigation"),
        RepeatedAlertCount,
        0);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON144AlarmFilteringTest,
...
bool FGridMonsterMON144AlarmFilteringTest::RunTest (const FString& Parameters)
{
    ...
    UGridMonsterDefinitionAsset* SourceDefinition = Fixture.MakeDefinition (
        TEXT ("MON14_4_FilterGuard"), 0, 3, true, 2);
    UGridMonsterDefinitionAsset* SameTypeDefinition = Fixture.MakeDefinition (
        TEXT ("MON14_4_FilterGuard"), 0, 0, true, 2);
    UGridMonsterDefinitionAsset* OtherTypeDefinition = Fixture.MakeDefinition (
        TEXT ("MON14_4_OtherMonster"), 0, 0, true, 2);

    AGridMonsterActor* Source = Fixture.AddMonster (
        SourceDefinition, FIntPoint (1, 1), EGridEdge::East,
        EGridMonsterState::Idle, TEXT ("Alarm_Filter"));
    AGridMonsterActor* ValidAlly = Fixture.AddMonster (
        SameTypeDefinition, FIntPoint (2, 1), EGridEdge::West,
        EGridMonsterState::Dormant, TEXT ("Alarm_Filter"));
    AGridMonsterActor* OutOfRange = Fixture.AddMonster (
        SameTypeDefinition, FIntPoint (5, 1), EGridEdge::West,
        EGridMonsterState::Dormant, TEXT ("Alarm_Filter"));
    AGridMonsterActor* OtherGroup = Fixture.AddMonster (
        SameTypeDefinition, FIntPoint (1, 2), EGridEdge::South,
        EGridMonsterState::Dormant, TEXT ("Alarm_Other"));
    AGridMonsterActor* OtherType = Fixture.AddMonster (
        OtherTypeDefinition, FIntPoint (2, 2), EGridEdge::South,
        EGridMonsterState::Dormant, TEXT ("Alarm_Filter"));
    ...
    TestEqual (TEXT ("Eligible same-group ally is alerted"),
        ValidAlly->MonsterState, EGridMonsterState::Alert);
    TestEqual (TEXT ("Out-of-range ally remains dormant"),
        OutOfRange->MonsterState, EGridMonsterState::Dormant);
    TestEqual (TEXT ("Other encounter group remains dormant"),
        OtherGroup->MonsterState, EGridMonsterState::Dormant);
    TestEqual (TEXT ("Different monster type remains dormant"),
        OtherType->MonsterState, EGridMonsterState::Dormant);
    TestFalse (TEXT ("Filtering scenario does not start combat"),
        Fixture.TurnManager->bCombatActive);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON144SharingDisabledTest,
...
bool FGridMonsterMON144SharingDisabledTest::RunTest (const FString& Parameters)
{
    ...
    UGridMonsterDefinitionAsset* SourceDefinition = Fixture.MakeDefinition (
        TEXT ("MON14_4_SoloGuard"), 0, 3, false, 8);
    UGridMonsterDefinitionAsset* AllyDefinition = Fixture.MakeDefinition (
        TEXT ("MON14_4_SoloGuard"), 0, 0, false, 0);

    AGridMonsterActor* Source = Fixture.AddMonster (
        SourceDefinition, FIntPoint (1, 1), EGridEdge::East,
        EGridMonsterState::Idle, TEXT ("Alarm_Disabled"));
    AGridMonsterActor* Ally = Fixture.AddMonster (
        AllyDefinition, FIntPoint (2, 1), EGridEdge::West,
        EGridMonsterState::Dormant, TEXT ("Alarm_Disabled"));
    if (!Source || !Ally)
    {
        return false;
    }
    ...
    UGridMonsterBehaviorComponent* AllyBehavior =
        Ally->FindComponentByClass<UGridMonsterBehaviorComponent> ();
    TestEqual (TEXT ("Ally remains dormant when sharing is disabled"),
        Ally->MonsterState, EGridMonsterState::Dormant);
    TestFalse (TEXT ("Ally receives no external last-known cell"),
        AllyBehavior && AllyBehavior->bHasLastKnownPartyCell);
