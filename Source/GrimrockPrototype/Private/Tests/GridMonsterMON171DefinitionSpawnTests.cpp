...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON171RangedAttackContractTest,
    "Grimrock.Monsters.MON17.1.RangedAttackContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON171RangedAttackContractTest::RunTest (
    const FString& Parameters)
{
    ...
    FString Error;
    TestTrue (TEXT ("Generic projectile attack validates"),
        Attack.ValidateDefinition (Error));
    TestTrue (TEXT ("Projectile attack is classified as ranged"),
        Attack.IsRangedAttack ());
    TestFalse (TEXT ("Minimum range excludes adjacent target"),
        Attack.SupportsDistance (1));
    TestTrue (TEXT ("Minimum range is inclusive"),
        Attack.SupportsDistance (2));
    TestTrue (TEXT ("Maximum range is inclusive"),
        Attack.SupportsDistance (6));
    TestFalse (TEXT ("Maximum range rejects farther target"),
        Attack.SupportsDistance (7));

    Attack.MinRangeCells = 7;
    TestFalse (TEXT ("Minimum range greater than maximum is rejected"),
        Attack.ValidateDefinition (Error));
    TestTrue (TEXT ("Invalid range reports the generic range contract"),
        Error.Contains (TEXT ("RangeCells must be at least MinRangeCells")));

    Attack.MinRangeCells = 2;
    Attack.CooldownTurns = -1;
    TestFalse (TEXT ("Negative cooldown is rejected"),
        Attack.ValidateDefinition (Error));
    TestTrue (TEXT ("Invalid cooldown is reported"),
        Error.Contains (TEXT ("CooldownTurns")));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON171GoblinDefinitionTest,
    "Grimrock.Monsters.MON17.1.GoblinDefinition",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON171GoblinDefinitionTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    TestEqual (TEXT ("Preferred minimum distance remains data-driven"),
        ...
        3);
    TestEqual (TEXT ("Preferred maximum distance remains data-driven"),
        Definition->PreferredMaxDistance,
        5);
    TestEqual (TEXT ("Definition defaults to the generic native monster actor"),
    ...
    TestEqual (TEXT ("Monster primary asset type is generic"),
        PrimaryId.PrimaryAssetType,
    ...
    if (Definition->Attacks.Num () == 1)
    {
        ...
        TestEqual (TEXT ("Goblin attack delivery is projectile"),
            Attack.Delivery,
            EGridMonsterAttackDelivery::Projectile);
        TestTrue (TEXT ("Goblin projectile requires line of sight"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON171SpawnPersistenceContractTest,
    "Grimrock.Monsters.MON17.1.SpawnPersistenceContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON171SpawnPersistenceContractTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    UGridMonsterDefinitionAsset* Definition =
        MakeMON171GoblinDefinition (Level);
    ...
    TestEqual (TEXT ("Persistent runtime state keeps the MonsterSpawn id"),
        ...
        SpawnId);
    TestEqual (TEXT ("Dormant state remains representable for the second family"),
