...
namespace GridMonsterMON20102DeadRestoreOccupancy
{
        ...
        FName MonsterId)
    {
        ...
        Test.TestNotNull (
            TEXT ("MON20.10.2 fixture loads Rat Giant skeletal mesh"),
            SkeletalMesh);
        Test.TestNotNull (
            TEXT ("MON20.10.2 fixture loads Rat Giant animation class"),
            AnimationClass);
        Test.TestNotNull (
            TEXT ("MON20.10.2 fixture loads Rat Giant actor class"),
            MonsterActorClass);
        ...
        Definition->DisplayName =
            FText::FromString (TEXT ("MON20.10.2 Restore Monster"));
    ...
    FGridRuntimeMonsterState MakeRestoreState (
        const FGuid& SpawnId,
        ...
        const FGridRuntimeMonsterState& MonsterState)
    {
        ...
        LevelState.MonsterPlacements.Add (
            SpawnId,
            PlacementState);
    }

    AGrimrockPartyPawn* SpawnParty (
        UWorld* World,
        AGridLevelRuntimeActor* Runtime,
        FIntPoint Cell)
    {
        ...
        AGrimrockPartyPawn* Party =
            World->SpawnActor<AGrimrockPartyPawn> (
                AGrimrockPartyPawn::StaticClass (),
                FTransform::Identity,
                Params);
        if (Party)
        {
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON20102DeadRestoreOverPartyCellTest,
    "Grimrock.MON20.10.2.DeadRestoreOverPartyCell",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON20102DeadRestoreOverPartyCellTest::RunTest (
    const FString& Parameters)
{
    ...
    if (!TestNotNull (
            TEXT ("MON20.10.2 test world exists"),
            TestWorld.World))
    {
        return false;
    }
    ...
    TestTrue (
        TEXT ("Party occupies the saved monster cell"),
        Runtime->IsPartyOnCell (SharedCell.X, SharedCell.Y));

    Runtime->RebuildLevel ();

    TestEqual (
        TEXT ("Dead restore does not count as a spawn failure"),
        Runtime->GetMonsterSpawnFailureCount (),
        0);
    AGridMonsterActor* Monster =
        Runtime->FindSpawnedMonsterActor (SpawnId);
    if (!TestNotNull (
            TEXT ("Dead restored monster actor is retained"),
            Monster))
    {
        return false;
    }
    ...
    TestTrue (
        TEXT ("Restored monster is logically dead"),
        Monster->IsDead ());
    TestEqual (
        TEXT ("Restored dead health is zero"),
        Monster->CurrentHealth,
        0);
    TestFalse (
        TEXT ("Restored dead mesh is hidden"),
        Monster->SkeletalMeshComponent->IsVisible ());
    TestEqual (
        TEXT ("Restored dead collision is disabled"),
    ...
    TestFalse (
        TEXT ("Restored dead monster does not occupy the shared cell"),
        Occupancy && Occupancy->IsCellOccupied (SharedCell));

    TestTrue (
        TEXT ("Full runtime-state application accepts the retained dead actor"),
        Runtime->ApplyCurrentLevelRuntimeState ());
    TestTrue (
        TEXT ("Dead actor remains available after full state application"),
        Runtime->FindSpawnedMonsterActor (SpawnId) == Monster);
    TestFalse (
        TEXT ("Dead actor remains non-occupying after full state application"),
        Occupancy && Occupancy->IsCellOccupied (SharedCell));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON20102LivingRestoreStillRejectsPartyCellTest,
    "Grimrock.MON20.10.2.LivingRestoreStillRejectsPartyCell",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON20102LivingRestoreStillRejectsPartyCellTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace GridMonsterMON20102DeadRestoreOccupancy;
    ...
    TestEqual (
        TEXT ("Living restore on the party cell remains a spawn failure"),
        Runtime->GetMonsterSpawnFailureCount (),
        1);
    TestNull (
        TEXT ("Living restore does not create an overlapping actor"),
        Runtime->FindSpawnedMonsterActor (SpawnId));
    return true;
}
