...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGCreateInitialCharacterCC2Test, "Grimrock.CharacterCreation.CC2.CreateInitialCharacter",
...
bool FRPGCreateInitialCharacterCC2Test::RunTest(const FString& Parameters)
{
	...
	Component->InitializeDefaultPartyIfNeeded();
	TestEqual(TEXT("Reinitialization preserves the created character"), Component->PartyInventoryState.ActiveCharacters[0].DisplayName.ToString(),
		FString(TEXT("Elias")));
	TestTrue(TEXT("Reinitialization preserves completion state"), Component->HasCompletedInitialCharacterCreation());
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGRejectInvalidInitialCharacterCC2Test, "Grimrock.CharacterCreation.CC2.RejectInvalidRequestAtomically",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGRejectInvalidInitialCharacterCC2Test::RunTest(const FString& Parameters)
{
	...
	FText Error;
	TestTrue(TEXT("A request with an empty normalized name is rejected"), !Component->CreateInitialCharacter(EmptyNameRequest, Error));
	...
	TestTrue(TEXT("A request without race is rejected"), !Component->CreateInitialCharacter(MissingRaceRequest, Error));
	...
	Error = FText::GetEmpty();
	TestTrue(TEXT("Attributes outside 6-20 are rejected"), !Component->CreateInitialCharacter(InvalidAttributesRequest, Error));
	TestTrue(TEXT("The attribute error is returned"), !Error.IsEmpty());

	TestTrue(TEXT("Creation remains incomplete after invalid requests"), !Component->HasCompletedInitialCharacterCreation());
	TestEqual(TEXT("The placeholder count is unchanged"), Component->GetActiveCharacterCount(), 1);
	TestTrue(TEXT("The placeholder runtime id is unchanged"), Component->PartyInventoryState.ActiveCharacters[0].CharacterId == PlaceholderId);
	TestEqual(TEXT("The placeholder name is unchanged"), Component->PartyInventoryState.ActiveCharacters[0].DisplayName.ToString(), PlaceholderName);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGRejectSecondInitialCharacterCC2Test, "Grimrock.CharacterCreation.CC2.RejectSecondCreation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGRejectSecondInitialCharacterCC2Test::RunTest(const FString& Parameters)
{
	...
	FText Error;
	TestTrue(TEXT("The first creation succeeds"), Component->CreateInitialCharacter(CreateValidCC2Request(TEXT("Elias")), Error));

	const FGuid FirstCharacterId = Component->PartyInventoryState.ActiveCharacters[0].CharacterId;
	Error = FText::GetEmpty();
	TestTrue(TEXT("The second creation is rejected"), !Component->CreateInitialCharacter(CreateValidCC2Request(TEXT("Replacement")), Error));
	TestTrue(TEXT("The second creation returns an error"), !Error.IsEmpty());
	TestTrue(TEXT("The original character id is preserved"), Component->PartyInventoryState.ActiveCharacters[0].CharacterId == FirstCharacterId);
	TestEqual(
		TEXT("The original character name is preserved"), Component->PartyInventoryState.ActiveCharacters[0].DisplayName.ToString(), FString(TEXT("Elias")));
