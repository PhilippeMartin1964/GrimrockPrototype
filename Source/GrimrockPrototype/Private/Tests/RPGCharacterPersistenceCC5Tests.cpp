...
namespace
{
    ...
    UGridPartyInventoryComponent* CreateCC5PartyWithEquippedTorch (FGuid& OutTorchId)
    {
        ...
        if (!Component->AddItemToSelectedCharacterInventory (Torch) ||
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGPartySaveMemoryRoundTripCC5Test,
...
bool FRPGPartySaveMemoryRoundTripCC5Test::RunTest (const FString& Parameters)
{
    ...
    TestTrue (
        TEXT ("The SaveGame serializes to memory"),
    ...
    TestEqual (
        TEXT ("The saved character name survives serialization"),
        LoadedSave->PartyInventoryState.ActiveCharacters[0].DisplayName.ToString (),
        FString (TEXT ("Elias")));
    ...
    const FGridLevelRuntimeState* LoadedLevelState =
        LoadedSave->DungeonRuntimeState.LevelStates.Find (TEXT ("TestLevel"));
    TestNotNull (TEXT ("The dungeon runtime state survives serialization"), LoadedLevelState);
    const FGridRuntimeObjectPresenceState* LoadedPresence =
        LoadedLevelState ? LoadedLevelState->ObjectPresence.Find (TorchId) : nullptr;
    TestNotNull (TEXT ("The picked-up world item presence survives serialization"), LoadedPresence);
    TestTrue (
        TEXT ("The picked-up world item remains removed"),
        LoadedPresence && LoadedPresence->bRemovedFromInitialPlacement);

    UGridPartyInventoryComponent* RestoredComponent = NewObject<UGridPartyInventoryComponent> ();
    FText RestoreError;
    TestTrue (
        TEXT ("The party snapshot restores atomically"),
    ...
    TestTrue (
        ...
        RestoredComponent->GetEquippedItem (0, EGridEquipmentSlot::MainHand, EquippedItem));
    TestTrue (TEXT ("The equipped runtime id is preserved"), EquippedItem.RuntimeObjectId == TorchId);

    FString OwnershipError;
    TestTrue (
        TEXT ("Ownership remains valid after restore"),
        RestoredComponent->ValidateInventoryOwnership (OwnershipError));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGRejectInvalidPartySaveCC5Test,
...
bool FRPGRejectInvalidPartySaveCC5Test::RunTest (const FString& Parameters)
{
    ...
    FText Error;
    TestFalse (
        TEXT ("A snapshot with missing equipment is rejected"),
        Component->RestorePartyInventoryState (InvalidState, Error));
    TestTrue (TEXT ("The invalid snapshot returns an error"), !Error.IsEmpty ());
    TestTrue (
        TEXT ("The previous party is preserved"),
