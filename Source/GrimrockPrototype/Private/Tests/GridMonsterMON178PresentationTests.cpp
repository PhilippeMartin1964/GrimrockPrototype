...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON178BestiaryPresentationBridgeTest,
    "Grimrock.Monsters.MON17.8.BestiaryPresentationBridge",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON178BestiaryPresentationBridgeTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;

    const GridMonsterMON178::FPresentationPair Pairs[] =
    {
        {
            TEXT ("RatGiant"),
        ...
        {
            TEXT ("GoblinThrower"),
            TEXT ("/Game/GrimrockPrototype/Monsters/GoblinThrower/Meshes/SK_GoblinThrower.SK_GoblinThrower"),
    ...
    for (const GridMonsterMON178::FPresentationPair& Pair : Pairs)
    {
        ...
        TestNotNull (
            FString::Printf (TEXT ("%s skeletal mesh loads"), Pair.Label),
            SkeletalMesh);
        TestNotNull (
            FString::Printf (TEXT ("%s Animation Blueprint class loads"), Pair.Label),
            AnimationClass);
        ...
        TestNotNull (
            FString::Printf (TEXT ("%s mesh owns a Skeleton"), Pair.Label),
            MeshSkeleton);

        TestTrue (
            FString::Printf (
                TEXT ("%s AnimBP derives from UGridMonsterAnimInstance"),
                Pair.Label),
        ...
        TestTrue (
            FString::Printf (
                TEXT ("%s AnimBP exposes IAnimClassInterface"),
                Pair.Label),
        ...
        TestNotNull (
            FString::Printf (
                TEXT ("%s AnimBP targets a Skeleton"),
                Pair.Label),
            AnimationSkeleton);
        ...
        TestTrue (
            FString::Printf (
                TEXT ("%s mesh and AnimBP Skeletons are compatible"),
                Pair.Label),
            bCompatibleSkeletons);

        if (bCompatibleSkeletons &&
    ...
    TestEqual (
        TEXT ("RatGiant and GoblinThrower share the generic monster animation bridge"),
        ValidatedPairs,
        2);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON178AnimationStateBridgeContractTest,
    "Grimrock.Monsters.MON17.8.AnimationStateBridgeContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON178AnimationStateBridgeContractTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    TestNotNull (
        TEXT ("Generic monster AnimInstance class exists"),
        AnimInstanceClass);
    ...
    const FName RequiredProperties[] =
    {
        GET_MEMBER_NAME_CHECKED (UGridMonsterAnimInstance, MonsterState),
        GET_MEMBER_NAME_CHECKED (UGridMonsterAnimInstance, bIsMoving),
        GET_MEMBER_NAME_CHECKED (UGridMonsterAnimInstance, bIsTurning),
        GET_MEMBER_NAME_CHECKED (UGridMonsterAnimInstance, bIsDead),
        GET_MEMBER_NAME_CHECKED (UGridMonsterAnimInstance, MoveAlpha),
        GET_MEMBER_NAME_CHECKED (UGridMonsterAnimInstance, TurnDirection),
        GET_MEMBER_NAME_CHECKED (UGridMonsterAnimInstance, CurrentHealth),
        GET_MEMBER_NAME_CHECKED (UGridMonsterAnimInstance, MaxHealth),
        GET_MEMBER_NAME_CHECKED (UGridMonsterAnimInstance, CurrentCell),
    ...
    TestNotNull (
        TEXT ("EGridMonsterState reflection enum exists"),
        MonsterStateEnum);
    if (!MonsterStateEnum)
    {
        return false;
    ...
    const EGridMonsterState RequiredStates[] =
    {
        ...
        EGridMonsterState::Repositioning,
    ...
    for (const EGridMonsterState State : RequiredStates)
    {
        TestTrue (
            FString::Printf (
                TEXT ("Monster presentation state %s remains reflected"),
                *UEnum::GetValueAsString (State)),
            MonsterStateEnum->IsValidEnumValue (
