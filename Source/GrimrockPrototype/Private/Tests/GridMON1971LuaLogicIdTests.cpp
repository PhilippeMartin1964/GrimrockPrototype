...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON1971LogicIdCommandTest,
...
bool FGridMON1971LogicIdCommandTest::RunTest (const FString& Parameters)
{
    ...
    if (!Activation->ReloadLuaRuntime (&Error))
    {
        AddError (FString::Printf (
            TEXT ("MON19.7.1 Lua runtime failed to load: %s"),
            *Error));
        return false;
    }

    TestTrue (
        TEXT ("Lua callback resolves LogicId and applies command"),
        Activation->ExecuteLinksFromObjectForEvent (
            SourceId,
            EGridObjectEvent::Activated));
    TestTrue (
        TEXT ("LogicId target becomes active"),
        Activation->GetActiveObjectIds ().Contains (TargetId));
    return true;
}
