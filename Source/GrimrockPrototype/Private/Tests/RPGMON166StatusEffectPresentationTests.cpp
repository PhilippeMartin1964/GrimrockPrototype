...
namespace
{
    UGridStatusEffectDefinitionAsset* MON166MakeDefinition (
        UObject* Outer,
        FName EffectId,
        ...
        int32 MaxStacks = 1)
    {
        ...
        Definition->Description = FText::FromString (
            FString::Printf (TEXT ("Description %s"), DisplayName));
    ...
    struct FGridMON166Fixture
    {
        ...
        FGridMON166Fixture ()
        {
            ...
            Party->PartyInventoryComponent->PartyInventoryState.ActiveCharacters = {
                MON166MakeCharacter (CharacterId)
            };
            ...
            MonsterDefinition->DisplayName = FText::FromString (
                TEXT ("MON16.6 Monster"));
        ...
        FGridCharacterInventoryState* Character () const
        {
            return Party && Party->PartyInventoryComponent &&
                Party->PartyInventoryComponent->PartyInventoryState
                    .ActiveCharacters.IsValidIndex (0)
                ? &Party->PartyInventoryComponent->PartyInventoryState
                    .ActiveCharacters[0]
                : nullptr;
        }
        ...
        bool IsReady () const
        {
            return TestWorld.World && Runtime && Party && TurnManager &&
                Lifecycle && Character () && Monster && MonsterDefinition;
        }
    };
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON166PresentationProjectionTest,
    "Grimrock.RPG.MON16.6.PresentationProjection",
...
bool FRPGMON166PresentationProjectionTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* Definition = MON166MakeDefinition (
        GetTransientPackage (), TEXT ("MON166_Status"), TEXT ("Venin"),
        EGridStatusEffectDurationUnit::Turns, 3,
        EGridStatusEffectStackPolicy::AddStacks, 3);
    Definition->Disposition = EGridStatusEffectDisposition::Debuff;
    ...
    FString Error;
    TestTrue (TEXT ("Status applies"), Collection.TryApply (
        *Definition, FGuid::NewGuid (), 2, INDEX_NONE, INDEX_NONE,
        ApplyResult, Error));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON166DurationFormattingTest,
    "Grimrock.RPG.MON16.6.DurationFormatting",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON166DurationFormattingTest::RunTest (const FString& Parameters)
{
    (void)Parameters;
    TestEqual (TEXT ("Turns compact"),
        FGridStatusEffectPresentationBuilder::FormatCompactDuration (
            EGridStatusEffectDurationUnit::Turns, 2).ToString (),
        FString (TEXT ("T2")));
    TestEqual (TEXT ("Rounds singular"),
        FGridStatusEffectPresentationBuilder::FormatDuration (
            EGridStatusEffectDurationUnit::Rounds, 1).ToString (),
        FString (TEXT ("1 manche")));
    TestEqual (TEXT ("Permanent compact"),
        FGridStatusEffectPresentationBuilder::FormatCompactDuration (
            EGridStatusEffectDurationUnit::Permanent, 0).ToString (),
        FString (TEXT ("PERM")));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON166DeterministicProjectionTest,
    "Grimrock.RPG.MON16.6.DeterministicProjection",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON166DeterministicProjectionTest::RunTest (const FString& Parameters)
{
    (void)Parameters;
    UGridStatusEffectDefinitionAsset* Z = MON166MakeDefinition (
        GetTransientPackage (), TEXT ("Z_Effect"), TEXT ("Z"));
    UGridStatusEffectDefinitionAsset* A = MON166MakeDefinition (
        GetTransientPackage (), TEXT ("A_Effect"), TEXT ("A"));
    FGridStatusEffectCollection Collection;
    FString Error;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON166FallbackProjectionTest,
    "Grimrock.RPG.MON16.6.FallbackProjection",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON166FallbackProjectionTest::RunTest (const FString& Parameters)
{
    ...
    TestTrue (TEXT ("Valid runtime state still projects"),
        FGridStatusEffectPresentationBuilder::BuildOne (State, View));
    TestEqual (TEXT ("EffectId is fallback display name"),
        View.DisplayName.ToString (), FString (TEXT ("MON166_MissingDefinition")));
    TestTrue (TEXT ("Fallback icon remains null"), View.Icon.IsNull ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON166PartyApplyFeedbackTest,
    "Grimrock.RPG.MON16.6.PartyApplyFeedback",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON166PartyApplyFeedbackTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* Definition = MON166MakeDefinition (
        GetTransientPackage (), TEXT ("MON166_Apply"), TEXT ("Silence"));
    FGridStatusEffectApplyResult Result;
    FString Error;
    TestTrue (TEXT ("Lifecycle applies party status"),
        Fixture.Lifecycle->TryApplyStatusEffectToPartyCharacter (
            0, Definition, FGuid::NewGuid (), Result, Error));
    const FGridCombatLogEntry& Feedback = Fixture.Lifecycle->LastStatusEffectFeedback;
    TestEqual (TEXT ("Feedback type is applied"), Feedback.Type,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON166RefreshFeedbackTest,
    "Grimrock.RPG.MON16.6.RefreshFeedback",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON166RefreshFeedbackTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* Definition = MON166MakeDefinition (
        GetTransientPackage (), TEXT ("MON166_Refresh"), TEXT ("Hâte"),
        EGridStatusEffectDurationUnit::Rounds, 2,
        EGridStatusEffectStackPolicy::RefreshDuration, 1);
    FGridStatusEffectApplyResult Result;
    FString Error;
    TestTrue (TEXT ("Initial apply"),
        Fixture.Lifecycle->TryApplyStatusEffectToPartyCharacter (
            0, Definition, FGuid::NewGuid (), Result, Error));
    TestTrue (TEXT ("Refresh apply"),
        Fixture.Lifecycle->TryApplyStatusEffectToPartyCharacter (
            0, Definition, FGuid::NewGuid (), Result, Error, 1, 4));
    TestEqual (TEXT ("Feedback type is refreshed"),
        Fixture.Lifecycle->LastStatusEffectFeedback.Type,
        EGridCombatLogEntryType::StatusRefreshed);
    TestTrue (TEXT ("Feedback exposes refreshed duration"),
        Fixture.Lifecycle->LastStatusEffectFeedback.StatusEffectDurationText
            .ToString ().Contains (TEXT ("4")));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON166TickAndExpirationFeedbackTest,
    "Grimrock.RPG.MON16.6.TickAndExpirationFeedback",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON166TickAndExpirationFeedbackTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* Definition = MON166MakeDefinition (
        GetTransientPackage (), TEXT ("MON166_Dot"), TEXT ("Brûlure"),
        EGridStatusEffectDurationUnit::Turns, 2);
    Definition->PeriodicDamage.DamageType = EGridDamageType::Fire;
    Definition->PeriodicDamage.DamagePerStack = 2;
    FGridStatusEffectApplyResult Result;
    FString Error;
    TestTrue (TEXT ("DoT applies"),
        Fixture.Lifecycle->TryApplyStatusEffectToPartyCharacter (
            0, Definition, FGuid::NewGuid (), Result, Error));
    ...
    TestEqual (TEXT ("First boundary leaves tick as latest feedback"),
        Fixture.Lifecycle->LastStatusEffectFeedback.Type,
        EGridCombatLogEntryType::StatusTicked);
    TestEqual (TEXT ("Tick dealt two HP"), Character->DerivedStats.CurrentHealth, 8);

    Fixture.TurnManager->OnCombatantStateChanged.Broadcast (Completed);
    TestEqual (TEXT ("Second boundary expires after final tick"),
        Fixture.Lifecycle->LastStatusEffectFeedback.Type,
        EGridCombatLogEntryType::StatusExpired);
    TestFalse (TEXT ("Effect removed after final tick"),
        Character->StatusEffects.Contains (Definition->EffectId));
    TestEqual (TEXT ("Second tick also dealt damage"), Character->DerivedStats.CurrentHealth, 6);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON166MonsterFeedbackParityTest,
    "Grimrock.RPG.MON16.6.MonsterFeedbackParity",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON166MonsterFeedbackParityTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* Definition = MON166MakeDefinition (
        GetTransientPackage (), TEXT ("MON166_MonsterStatus"), TEXT ("Lenteur"));
    FGridStatusEffectApplyResult Result;
    FString Error;
    TestTrue (TEXT ("Lifecycle applies monster status"),
        Fixture.Lifecycle->TryApplyStatusEffectToMonster (
            Fixture.Monster, Definition, FGuid::NewGuid (), Result, Error));
    const FGridCombatLogEntry& Feedback = Fixture.Lifecycle->LastStatusEffectFeedback;
    TestEqual (TEXT ("Monster uses same feedback type"), Feedback.Type,
        EGridCombatLogEntryType::StatusApplied);
    TestEqual (TEXT ("Monster has no party character index"),
        Feedback.TargetCharacterIndex, INDEX_NONE);
    TestEqual (TEXT ("Monster target id is stable"), Feedback.TargetId,
        FName (*Fixture.MonsterId.ToString (EGuidFormats::Digits)));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON166PartyPanelProjectionTest,
    "Grimrock.RPG.MON16.6.PartyPanelProjection",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON166PartyPanelProjectionTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* Definition = MON166MakeDefinition (
        GetTransientPackage (), TEXT ("MON166_Panel"), TEXT ("Immobilisé"),
        EGridStatusEffectDurationUnit::Turns, 2);
    Definition->Control.bBlockTranslation = true;
    FGridStatusEffectApplyResult Result;
    FString Error;
    TestTrue (TEXT ("Panel status applies"),
        Fixture.Lifecycle->TryApplyStatusEffectToPartyCharacter (
            0, Definition, FGuid::NewGuid (), Result, Error));

    UGridCombatActionPanelWidget* Panel =
        NewObject<UGridCombatActionPanelWidget> (Fixture.Party);
    ...
    TestTrue (TEXT ("Panel status summary is visible data"),
        Panel->View.StatusSummary.ToString ().Contains (TEXT ("Immobilisé")));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON166NoParallelSystemTest,
    "Grimrock.RPG.MON16.6.NoParallelSystem",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON166NoParallelSystemTest::RunTest (const FString& Parameters)
{
    ...
    const FExpectation Expectations[] = {
        { TEXT ("Source/GrimrockPrototype/Private/RPG/StatusEffects/GridStatusEffectPresentation.cpp"), TEXT ("DefinitionAsset") },
    ...
    for (const FExpectation& Expectation : Expectations)
    {
        ...
        const FString Path = FPaths::Combine (
            FPaths::ProjectDir (), Expectation.Path);
        TestTrue (*FString::Printf (TEXT ("Source loads: %s"), Expectation.Path),
            FFileHelper::LoadFileToString (Text, *Path));
        TestTrue (*FString::Printf (TEXT ("Source contains: %s"), Expectation.Required),
            Text.Contains (Expectation.Required));
        TestFalse (TEXT ("No WBP dependency introduced"), Text.Contains (TEXT ("WBP_")));
        TestFalse (TEXT ("No hard-coded status identity branch introduced"),
