    ...
    struct FMON194RuntimeFixture
    {
        ...
        bool Initialize(FAutomationTestBase& Test)
        {
            ...
            Level->LevelVariables = {
                MakeBoolVariable194(TEXT("Gate"), false),
                MakeIntVariable194(TEXT("Count"), 1)
            };
        ...
        bool FinalizeLevel(FAutomationTestBase& Test)
        {
            ...
            if (!Activation->ReloadLuaRuntime(&Error))
            {
                Test.AddError(FString::Printf(
                    TEXT("MON19.4 Lua runtime failed to load: %s"),
                    *Error));
                return false;
            }
            return true;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FGridMON194EventContextAndVarsTest,
    "Grimrock.MON19.4.LuaBridge.EventContextAndVariables",
...
bool FGridMON194EventContextAndVarsTest::RunTest(const FString& Parameters)
{
    ...
    TestTrue(
        TEXT("Trigger event invokes Lua callback"),
        Fixture.Activation->ExecuteLinksFromObjectForEvent(
            SourceId,
    ...
    int32 Count = 0;
    TestTrue(
        TEXT("Lua wrote Gate through authoritative store"),
        GridLevelVariableStore::TryGetBool(*Fixture.Level, *State, TEXT("Gate"), bGate, Error));
    TestTrue(TEXT("Lua event context matches source/event"), bGate);
    TestTrue(
        TEXT("Lua wrote Count through authoritative store"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FGridMON194LuaCommandToLogicTest,
    "Grimrock.MON19.4.LuaBridge.CommandToExistingRuntime",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMON194LuaCommandToLogicTest::RunTest(const FString& Parameters)
{
    ...
    TestTrue(
        TEXT("Lua callback can command existing Logic runtime"),
    ...
    int32 Count = 0;
    TestTrue(
        TEXT("Count is readable after grid.command"),
        State && GridLevelVariableStore::TryGetInt32(*Fixture.Level, *State, TEXT("Count"), Count, Error));
    TestEqual(TEXT("grid.command reused AddInt implementation"), Count, 4);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FGridMON194VariableConditionTest,
    "Grimrock.MON19.4.LuaBridge.LinkCondition",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMON194VariableConditionTest::RunTest(const FString& Parameters)
{
    ...
    Fixture.Level->Objects.Add(MakeObject194(SourceId, EGridLevelObjectType::Trigger));
    Fixture.Level->LuaScripts.Add(MakeLuaScript194(
        TEXT("Conditional"),
        TEXT("function on_trigger(event)\n")
        TEXT("  local count, err = grid.vars.get_int('Count')\n")
        TEXT("  assert(err == nil, err)\n")
    ...
    TestFalse(
        TEXT("False level-variable condition blocks Lua callback"),
        Fixture.Activation->ExecuteLinksFromObjectForEvent(SourceId, EGridObjectEvent::Activated));
    ...
    TestTrue(
        TEXT("Gate can be enabled"),
        GridLevelVariableStore::SetBool(*Fixture.Level, *State, TEXT("Gate"), true, Error));
    TestTrue(
        TEXT("True level-variable condition permits Lua callback"),
        Fixture.Activation->ExecuteLinksFromObjectForEvent(SourceId, EGridObjectEvent::Activated));
    GridLevelVariableStore::TryGetInt32(*Fixture.Level, *State, TEXT("Count"), Count, Error);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FGridMON194HostFailureTest,
    "Grimrock.MON19.4.LuaBridge.HostFailureIsProtected",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMON194HostFailureTest::RunTest(const FString& Parameters)
{
    ...
    TestTrue(
        TEXT("Host failure is returned to Lua without aborting callback"),
        Fixture.Activation->ExecuteLinksFromObjectForEvent(SourceId, EGridObjectEvent::Activated));
    ...
    bool bGate = false;
    TestTrue(
        TEXT("Gate is readable after protected host failure"),
        State && GridLevelVariableStore::TryGetBool(*Fixture.Level, *State, TEXT("Gate"), bGate, Error));
    TestTrue(TEXT("Lua observed grid.command failure as data"), bGate);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FGridMON194SharedActionBudgetTest,
...
bool FGridMON194SharedActionBudgetTest::RunTest(const FString& Parameters)
{
    ...
    bool bGate = false;
    TestTrue(
        TEXT("Gate remains readable after budget stop"),
        State && GridLevelVariableStore::TryGetBool(*Fixture.Level, *State, TEXT("Gate"), bGate, Error));
