...
namespace
{
    ...
    struct FGridSAVEFIX2TestSaveSlot
    {
        ...
        FGridSAVEFIX2TestSaveSlot ()
            : SlotName (FString::Printf (
                TEXT ("SAVEFIX2_ContinueFailure_%s"),
    ...
    bool BuildSAVEFIX2LoadablePartyState (
        FGridPartyInventoryState& OutState,
        FGuid& OutCharacterId)
    {
        UGridPartyInventoryComponent* Source =
        ...
        FText CreationError;
        if (!Source->CreateInitialCharacter (Request, CreationError) ||
            Source->PartyInventoryState.ActiveCharacters.IsEmpty ())
        {
            return false;
        }

        UGridItemDefinitionAsset* MissingDefinition =
            NewObject<UGridItemDefinitionAsset> (Source);
        if (!MissingDefinition)
        {
            return false;
        }
        MissingDefinition->ItemDefinitionId = SAVEFIX2MissingItemId;
        MissingDefinition->DisplayName =
            FText::FromString (TEXT ("SAVEFIX2 missing item"));
        ...
        Item.Quantity = 1;
        if (!Source->ApplyItemDefinitionToInstance (Item) ||
            !Source->AddItemToSelectedCharacterInventory (Item))
        {
            return false;
        }
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridSAVEFIX2ContinueFailureIsNonDestructiveTest,
    "Grimrock.Save.SAVEFIX.2.ContinueFailureIsNonDestructive",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridSAVEFIX2ContinueFailureIsNonDestructiveTest::RunTest (
    const FString& Parameters)
{
    ...
    TestTrue (
        TEXT ("A loadable party snapshot can be built"),
        BuildSAVEFIX2LoadablePartyState (SavedState, SavedCharacterId));
    ...
    TestTrue (
        TEXT ("The isolated SAVEFIX.2 save is written"),
        UGameplayStatics::SaveGameToSlot (
            SaveGame,
            SaveSlot.SlotName,
            SaveSlot.UserIndex));
    TestTrue (
        TEXT ("The temporary save is loadable from the main-menu contract"),
        NewObject<UGrimrockGameInstance> ()->HasPartySaveGame (
            SaveSlot.SlotName,
            SaveSlot.UserIndex));
...

    {
        ...
        FNameProperty* MainMenuLevelProperty =
            FindFProperty<FNameProperty> (
                UGrimrockGameInstance::StaticClass (),
                TEXT ("MainMenuLevelName"));
        TestNotNull (
            TEXT ("The main-menu level property is available"),
            MainMenuLevelProperty);
        if (!MainMenuLevelProperty)
        {
            return false;
        }
        MainMenuLevelProperty->SetPropertyValue_InContainer (
            TestWorld.GameInstance,
            NAME_None);
        TestWorld.GameInstance->SetPendingLoadSlot (
            TEXT ("SAVEFIX2_PendingSentinel"),
            0);
        TestWorld.GameInstance->SetPendingStartupMode (
        ...
        AddExpectedError (
            TEXT ("PartySave ContinueAborted Slot="),
            EAutomationExpectedErrorFlags::Contains,
            1);
        AddExpectedError (
            TEXT ("GrimrockGameInstance ReturnToMainMenu Failed Reason=NoMainMenuLevelName"),
            EAutomationExpectedErrorFlags::Contains,
            1);
        ...
        TestTrue (
            TEXT ("Failed Continue disarms this pawn's EndPlay autosave"),
            Party->PartySaveSlotName.IsEmpty ());
        TestTrue (
            TEXT ("Failed Continue does not reset the runtime party"),
            Party->PartyInventoryComponent->PartyInventoryState
                .ActiveCharacters.IsValidIndex (0) &&
            Party->PartyInventoryComponent->PartyInventoryState
                .ActiveCharacters[0].CharacterId == SentinelCharacterId);
        TestTrue (
            TEXT ("Failed Continue never opens Character Creation"),
            !Party->IsCharacterCreationModalActive ());
        TestTrue (
            TEXT ("Return-to-menu request clears the pending load slot"),
            !TestWorld.GameInstance->HasPendingLoadSlot ());
        TestTrue (
            TEXT ("The isolated save still exists after failed Continue"),
    ...
    TestTrue (
        TEXT ("The isolated save survives world teardown"),
        UGameplayStatics::DoesSaveGameExist (
            SaveSlot.SlotName,
            SaveSlot.UserIndex));

    const UGrimrockPartySaveGame* PreservedSave =
        Cast<UGrimrockPartySaveGame> (
            UGameplayStatics::LoadGameFromSlot (
                SaveSlot.SlotName,
                SaveSlot.UserIndex));
    TestNotNull (
        TEXT ("The original save remains readable after the failed Continue"),
        PreservedSave);
    TestTrue (
        TEXT ("The failed Continue did not replace the saved party"),
        PreservedSave &&
        PreservedSave->PartyInventoryState.ActiveCharacters.IsValidIndex (0) &&
        PreservedSave->PartyInventoryState.ActiveCharacters[0].CharacterId ==
            SavedCharacterId);
