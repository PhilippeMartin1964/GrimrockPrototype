...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON2045StoryCompanionPaletteContractTest,
    "Grimrock.MON20.4.RecruitmentUI.PaletteContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON2045StoryCompanionPaletteContractTest::RunTest (
    const FString& Parameters)
{
    ...
    {
        TestNotNull (
            TEXT ("Story companion default is part of the palette entry contract"),
            EntryStruct->FindPropertyByName (
    ...
    UGridObjectArchetypeAsset* TriggerArchetype =
    ...
    TestTrue (
        TEXT ("Non-story palette entries do not require a companion definition"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON2045StoryCompanionPaletteMissingDefinitionTest,
    "Grimrock.MON20.4.RecruitmentUI.PaletteMissingDefinition",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON2045StoryCompanionPaletteMissingDefinitionTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    TArray<FGridArchetypeValidationMessage> Messages;
    TestFalse (
        TEXT ("Story companion palette entry rejects a missing default definition"),
        Palette->ValidatePalette (Messages));
    ...
    for (const FGridArchetypeValidationMessage& Message : Messages)
    {
        if (Message.Severity == EGridArchetypeValidationSeverity::Error &&
            Message.Message.Contains (TEXT ("DefaultStoryCompanionDefinition")))
        {
    ...
    TestTrue (
        TEXT ("Missing story companion definition is reported explicitly"),
        bFoundExpectedError);
