...
namespace RPGMON2092Tests
{
    ...
    const URPGSkillAsset* Resolve (
        const TMap<FName, URPGSkillAsset*>& Definitions,
        FName SkillId)
    {
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2092CaptureActivePoolSparseTest,
    "Grimrock.MON20.9.SkillPersistence.CaptureActivePoolSparse",
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2092CaptureDeterministicOrderTest,
    "Grimrock.MON20.9.SkillPersistence.CaptureDeterministicOrder",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2092CaptureDeterministicOrderTest::RunTest (const FString& Parameters)
{
    ...
    FString Error;
    TestTrue (TEXT ("First ordering captures"),
        FRPGSkillPersistence::CapturePartySkills (PartyA, Resolver, SavedA, Error));
    TestTrue (TEXT ("Second ordering captures"),
        FRPGSkillPersistence::CapturePartySkills (PartyB, Resolver, SavedB, Error));
    ...
    {
        TestTrue (TEXT ("Character order is deterministic"),
            SavedA[Index].CharacterId == SavedB[Index].CharacterId);
        TestEqual (TEXT ("Rank counts are deterministic"),
            SavedA[Index].SkillRanks.Num (),
            SavedB[Index].SkillRanks.Num ());
        for (int32 RankIndex = 0;
            RankIndex < SavedA[Index].SkillRanks.Num () &&
            RankIndex < SavedB[Index].SkillRanks.Num ();
            ++RankIndex)
        {
            TestEqual (TEXT ("SkillId order is deterministic"),
                SavedA[Index].SkillRanks[RankIndex].SkillId,
                SavedB[Index].SkillRanks[RankIndex].SkillId);
            TestEqual (TEXT ("Rank values are deterministic"),
                SavedA[Index].SkillRanks[RankIndex].Rank,
                SavedB[Index].SkillRanks[RankIndex].Rank);
        }
    }
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2092CaptureInvalidCharacterAtomicTest,
    "Grimrock.MON20.9.SkillPersistence.CaptureInvalidCharacterAtomic",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2092CaptureInvalidCharacterAtomicTest::RunTest (const FString& Parameters)
{
    ...
    TestFalse (TEXT ("Duplicate CharacterId is rejected"),
        FRPGSkillPersistence::CapturePartySkills (
            Party,
            [] (FName) -> const URPGSkillAsset* { return nullptr; },
            Saved,
            Error));
    TestEqual (TEXT ("Failed capture leaves output untouched"), Saved.Num (), 1);
    TestTrue (TEXT ("Sentinel snapshot survives failed capture"),
        Saved[0].CharacterId == MakeId (999));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2092CaptureInvalidSkillAtomicTest,
    "Grimrock.MON20.9.SkillPersistence.CaptureInvalidSkillAtomic",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2092CaptureInvalidSkillAtomicTest::RunTest (const FString& Parameters)
{
    ...
    TestFalse (TEXT ("Duplicate runtime SkillId is rejected"),
        FRPGSkillPersistence::CapturePartySkills (
            DuplicateParty, Resolver, Saved, Error));
    ...
    TestFalse (TEXT ("Rank above MaxRank is rejected"),
        FRPGSkillPersistence::CapturePartySkills (
            OverRankParty, Resolver, Saved, Error));
    ...
    TestFalse (TEXT ("Missing canonical definition is rejected"),
        FRPGSkillPersistence::CapturePartySkills (
            MissingParty, Resolver, Saved, Error));
    TestEqual (TEXT ("Missing-definition rejection is atomic"), Saved.Num (), 1);
    TestTrue (TEXT ("Sentinel output remains intact"),
        Saved[0].CharacterId == MakeId (999));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2092RestoreRoundTripTest,
    "Grimrock.MON20.9.SkillPersistence.RestoreRoundTrip",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2092RestoreRoundTripTest::RunTest (const FString& Parameters)
{
    ...
    TArray<FRPGCharacterSkillSaveState> Saved;
    FString Error;
    TestTrue (TEXT ("Source state captures"),
        FRPGSkillPersistence::CapturePartySkills (
            Party, Resolver, Saved, Error));
    ...
    TestTrue (TEXT ("Captured state restores"),
        FRPGSkillPersistence::RestorePartySkills (
            Restored, Saved, Resolver, Error));
    TestEqual (TEXT ("Active Skill rank is restored"),
        FRPGSkillService::GetSkillRank (Restored.ActiveCharacters[0], TEXT ("Skill_A")),
        2);
    TestEqual (TEXT ("Unpersisted active character is cleared"),
        Restored.ActiveCharacters[1].SkillRanks.Num (),
        0);
    TestEqual (TEXT ("Pooled Skill rank is restored"),
        FRPGSkillService::GetSkillRank (Restored.CharacterPool[0], TEXT ("Skill_B")),
        4);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2092RestoreByCharacterIdTest,
    "Grimrock.MON20.9.SkillPersistence.RestoreByCharacterId",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2092RestoreByCharacterIdTest::RunTest (const FString& Parameters)
{
    ...
    FString Error;
    TestTrue (TEXT ("Reversed snapshots restore"),
        FRPGSkillPersistence::RestorePartySkills (
            Party, ReversedSnapshots, Resolver, Error));
    TestEqual (TEXT ("Active character receives its CharacterId-matched Skill"),
        FRPGSkillService::GetSkillRank (Party.ActiveCharacters[0], TEXT ("Skill_B")),
        3);
    TestEqual (TEXT ("Pooled character receives its CharacterId-matched Skill"),
        FRPGSkillService::GetSkillRank (Party.CharacterPool[0], TEXT ("Skill_A")),
        1);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2092RestoreInvalidSnapshotAtomicTest,
    "Grimrock.MON20.9.SkillPersistence.RestoreInvalidSnapshotAtomic",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2092RestoreInvalidSnapshotAtomicTest::RunTest (const FString& Parameters)
{
    ...
    TestFalse (TEXT ("Missing definition rejects restore"),
        FRPGSkillPersistence::RestorePartySkills (
            Party, { MissingDefinition }, Resolver, Error));
    TestEqual (TEXT ("Failed restore preserves original runtime rank"),
        FRPGSkillService::GetSkillRank (Party.ActiveCharacters[0], TEXT ("Skill_A")),
        1);
    ...
    TestFalse (TEXT ("Rank above MaxRank rejects restore"),
        FRPGSkillPersistence::RestorePartySkills (
            Party, { OverRank }, Resolver, Error));
    TestEqual (TEXT ("Second failed restore remains atomic"),
        FRPGSkillService::GetSkillRank (Party.ActiveCharacters[0], TEXT ("Skill_A")),
        1);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2092V7ToV8MigrationTest,
    "Grimrock.MON20.9.SkillPersistence.V7ToV8Migration",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2092V7ToV8MigrationTest::RunTest (const FString& Parameters)
{
    ...
    TestTrue (TEXT ("Version seven migrates to the current contract"),
        FRPGSaveMigrationService::PrepareLoadedSave (
            Save,
            Error,
            &Report));
    TestEqual (TEXT ("Current SaveGame contract is v8"),
    ...
    TestTrue (TEXT ("v7 cannot invent persisted Skill snapshots"),
        Save->CharacterSkillStates.IsEmpty ());
    TestTrue (TEXT ("Any in-memory legacy transient SkillRanks are discarded"),
        Save->PartyInventoryState.CharacterPool[0].SkillRanks.IsEmpty ());
    return true;
}
