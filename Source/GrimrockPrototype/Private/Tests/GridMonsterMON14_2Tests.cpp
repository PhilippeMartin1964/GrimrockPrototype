...
namespace
{
	struct FGridMON142TestWorld
	{
		...
		FGridMON142TestWorld()
		{
			...
			World = UWorld::CreateWorld(EWorldType::Game, false,
				FName(*FString::Printf(TEXT("MON142TestWorld_%s"), *FGuid::NewGuid().ToString(EGuidFormats::Digits))), nullptr, true, ERHIFeatureLevel::Num,
	...
	UGridLevelAsset* MON142MakeFloorLevel(UObject* Outer, int32 Width = 8, int32 Height = 8)
	{
	...
	UGridMonsterDefinitionAsset* MON142MakeDefinition(UObject* Outer, FName MonsterId)
	{
	...
	bool MON142HasErrorContaining(const TArray<FString>& Errors, const TCHAR* Expected)
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMonsterMON142DirectionalSightTest, "Grimrock.Monsters.MON14.2.DirectionalSight",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON142DirectionalSightTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	const FIntPoint Observer(2, 2);
	const auto OpenTraversal = [](const FIntPoint& From, const FIntPoint& To)
	{
		return FGridMonsterPathfinder::ManhattanDistance(From, To) == 1;
	};

	TestTrue(TEXT("North sees only the north axial ray"),
		FGridMonsterPerception::HasDirectionalLineOfSight(Observer, EGridEdge::North, FIntPoint(2, 5), 5, OpenTraversal));
	TestFalse(TEXT("North does not see behind itself"),
		FGridMonsterPerception::HasDirectionalLineOfSight(Observer, EGridEdge::North, FIntPoint(2, 0), 5, OpenTraversal));
	TestFalse(
		TEXT("North does not see sideways"), FGridMonsterPerception::HasDirectionalLineOfSight(Observer, EGridEdge::North, FIntPoint(5, 2), 5, OpenTraversal));

	TestTrue(TEXT("East sees east"), FGridMonsterPerception::HasDirectionalLineOfSight(Observer, EGridEdge::East, FIntPoint(5, 2), 5, OpenTraversal));
	TestTrue(TEXT("South sees south"), FGridMonsterPerception::HasDirectionalLineOfSight(Observer, EGridEdge::South, FIntPoint(2, 0), 5, OpenTraversal));
	TestTrue(TEXT("West sees west"), FGridMonsterPerception::HasDirectionalLineOfSight(Observer, EGridEdge::West, FIntPoint(0, 2), 5, OpenTraversal));
	TestFalse(TEXT("Facing None never creates a visual ray"),
		FGridMonsterPerception::HasDirectionalLineOfSight(Observer, EGridEdge::None, FIntPoint(2, 3), 5, OpenTraversal));
	TestFalse(TEXT("Directional sight still enforces range"),
		FGridMonsterPerception::HasDirectionalLineOfSight(Observer, EGridEdge::North, FIntPoint(2, 7), 4, OpenTraversal));

	const auto BlockNorthEdge = [](const FIntPoint& From, const FIntPoint& To)
	{
		return !(From == FIntPoint(2, 3) && To == FIntPoint(2, 4));
	};
	TestFalse(TEXT("Directional sight still respects blocking edges"),
		FGridMonsterPerception::HasDirectionalLineOfSight(Observer, EGridEdge::North, FIntPoint(2, 5), 5, BlockNorthEdge));

	TestTrue(TEXT("Legacy straight LOS remains geometry-only"), FGridMonsterPerception::HasStraightLineOfSight(Observer, FIntPoint(2, 0), 5, OpenTraversal));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMonsterMON142SpawnModelValidationTest, "Grimrock.Monsters.MON14.2.SpawnModelValidation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON142SpawnModelValidationTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMonsterMON142FreshSpawnConfigurationTest, "Grimrock.Monsters.MON14.2.FreshSpawnConfiguration",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON142FreshSpawnConfigurationTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	...
	AGridMonsterActor* Monster =
		TestWorld.World->SpawnActor<AGridMonsterActor>(AGridMonsterActor::StaticClass(), Runtime->GetCellCenterWorld(3, 3), FRotator::ZeroRotator, Params);
	...
	TestTrue(TEXT("Fresh monster initializes from its MonsterSpawn"),
		Monster->InitializeMonster(Definition, SpawnId, FIntPoint(3, 3), EGridEdge::South, Spawn.EncounterGroupId));
	TestEqual(TEXT("Authored Dormant state reaches the Actor"), Monster->MonsterState, EGridMonsterState::Dormant);
	TestEqual(TEXT("Authored facing remains authoritative"), Monster->Facing, EGridEdge::South);
	TestEqual(TEXT("Encounter group remains intact"), Monster->EncounterGroupId, FName(TEXT("PatrolRoom_A")));
	TestEqual(TEXT("Patrol mode reaches the Actor"), Monster->PatrolMode, EGridMonsterPatrolMode::PingPong);
	TestEqual(TEXT("Both patrol waypoints reach the Actor"), Monster->PatrolWaypoints.Num(), 2);
	if (Monster->PatrolWaypoints.Num() == 2)
	{
		TestTrue(TEXT("First patrol cell is preserved"), Monster->PatrolWaypoints[0].Cell == FIntPoint(3, 3));
		TestEqual(TEXT("Second patrol facing is preserved"), Monster->PatrolWaypoints[1].Facing, EGridEdge::West);
		TestEqual(TEXT("Waypoint wait is preserved"), Monster->PatrolWaypoints[1].WaitSeconds, 0.75f);
	}

	TestFalse(TEXT("MON14.2 route data does not start movement"), Monster->bIsMoving);
	TestFalse(TEXT("MON14.2 route data does not start turning"), Monster->bIsTurning);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMonsterMON142BehaviorFacingIntegrationTest, "Grimrock.Monsters.MON14.2.BehaviorFacingIntegration",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON142BehaviorFacingIntegrationTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	FGridMON142TestWorld TestWorld;
	if (!TestWorld.World)
	...
	Params.Owner = Runtime;
	AGridMonsterActor* Monster =
		TestWorld.World->SpawnActor<AGridMonsterActor>(AGridMonsterActor::StaticClass(), Runtime->GetCellCenterWorld(2, 2), FRotator::ZeroRotator, Params);
	if (!Monster || !Monster->InitializeMonster(Definition, SpawnId, FIntPoint(2, 2), EGridEdge::North))
	{
		return false;
	}

	UGridMonsterBehaviorComponent* Behavior = NewObject<UGridMonsterBehaviorComponent>(Monster, TEXT("MON142Behavior"));
	...
	TestTrue(TEXT("North-facing monster sees party to the north"), Behavior->RefreshPerception());
	TestTrue(TEXT("Behavior records north-facing visual contact"), Behavior->bCanSeeParty);

	Monster->Facing = EGridEdge::South;
	TestFalse(TEXT("Turning away removes all perception when hearing is zero"), Behavior->RefreshPerception());
	TestFalse(TEXT("Behavior no longer sees behind the monster"), Behavior->bCanSeeParty);
	TestFalse(TEXT("Zero-range hearing does not mask the facing test"), Behavior->bCanHearParty);

	Party->CurrentCellX = 2;
	Party->CurrentCellY = 0;
	Party->SetActorLocation(Runtime->GetCellCenterWorld(2, 0));
	TestTrue(TEXT("South-facing monster sees party after it moves south"), Behavior->RefreshPerception());
	TestTrue(TEXT("Behavior records south-facing visual contact"), Behavior->bCanSeeParty);
	return true;
}
