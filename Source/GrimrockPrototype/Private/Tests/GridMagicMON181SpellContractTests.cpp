...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMagicMON181DefinitionValidationTest,
...
bool FGridMagicMON181DefinitionValidationTest::RunTest (const FString& Parameters)
{
    ...
    TestEqual (
        TEXT ("A complete definition is structurally valid"),
        FGridSpellContract::ValidateDefinition (Definition),
        EGridSpellValidationError::None);

    Definition.SpellId = NAME_None;
    TestEqual (
        TEXT ("SpellId is mandatory"),
        FGridSpellContract::ValidateDefinition (Definition),
        EGridSpellValidationError::MissingSpellId);

    Definition = MakeValidDamageSpell ();
    Definition.MaxRangeCells = 0;
    TestEqual (
        TEXT ("Max range cannot be lower than min range"),
        FGridSpellContract::ValidateDefinition (Definition),
        EGridSpellValidationError::InvalidRange);

    Definition = MakeValidDamageSpell ();
    Definition.TargetingPolicy = EGridCombatTargetingPolicy::None;
    TestEqual (
        TEXT ("Targeting policy is mandatory"),
        FGridSpellContract::ValidateDefinition (Definition),
        EGridSpellValidationError::MissingTargetPolicy);

    Definition = MakeValidDamageSpell ();
    Definition.Effects.Reset ();
    TestEqual (
        TEXT ("At least one declarative effect is mandatory"),
        FGridSpellContract::ValidateDefinition (Definition),
        EGridSpellValidationError::MissingEffects);

    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMagicMON181StatusEffectBridgeTest,
...
bool FGridMagicMON181StatusEffectBridgeTest::RunTest (const FString& Parameters)
{
    ...
    TestEqual (
        TEXT ("A spell may reference MON16 through a stable StatusEffectId"),
        FGridSpellContract::ValidateDefinition (Definition),
        EGridSpellValidationError::None);

    Definition.Effects[0].StatusEffectId = NAME_None;
    TestEqual (
        TEXT ("Status-effect spell entries require a stable effect id"),
        FGridSpellContract::ValidateDefinition (Definition),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMagicMON181CastRequestValidationTest,
...
bool FGridMagicMON181CastRequestValidationTest::RunTest (const FString& Parameters)
{
    ...
    TestEqual (
        TEXT ("Stable caster and target identities form a valid axial-target request"),
        FGridSpellContract::ValidateRequest (Definition, Request),
        EGridSpellValidationError::None);

    Request.Target.TargetId.Invalidate ();
    TestEqual (
        TEXT ("Axial-target spells require a stable target identity"),
        FGridSpellContract::ValidateRequest (Definition, Request),
    ...
    TestEqual (
        TEXT ("Cell-target spells use serializable grid coordinates"),
        FGridSpellContract::ValidateRequest (CellDefinition, Request),
        EGridSpellValidationError::None);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMagicMON181NoRuntimePaymentTest,
