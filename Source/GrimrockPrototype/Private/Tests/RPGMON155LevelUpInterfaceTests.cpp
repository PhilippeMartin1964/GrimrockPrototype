...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON155WidgetCancelTest, "Grimrock.RPG.MON15.5.WidgetCancelIsNonMutating", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON155WidgetCancelTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("The level-up widget initializes"), Widget->InitializeLevelUpWidget(Component, 0, 2, 3));
	TestTrue(TEXT("Choice A can be staged"), Widget->StageOrUnstageChoice(TEXT("Choice_A")));
	TestEqual(TEXT("Exactly one pending choice is staged"), Widget->GetPendingChoiceIds().Num(), 1);
	Widget->CancelSelection();

	TArray<FName> Selected;
	TestTrue(TEXT("Committed state remains readable after cancel"), FRPGClassProgressionTransactionService::TryGetSelectedChoiceIds(Component, 0, Selected));
	TestTrue(TEXT("Cancel commits no staged choice"), Selected.IsEmpty());
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON155WidgetConfirmTest, "Grimrock.RPG.MON15.5.WidgetConfirmTransaction", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON155WidgetConfirmTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("The level-up widget initializes"), Widget->InitializeLevelUpWidget(Component, 0, 1, 3));
	TestEqual(TEXT("The widget compares from level one"), Widget->View.PreviousLevel, 1);
	TestEqual(TEXT("The widget compares to level three"), Widget->View.NewLevel, 3);
	TestTrue(TEXT("The maximum health comparison increases"), Widget->View.NewMaxHealth > Widget->View.PreviousMaxHealth);
	TestTrue(TEXT("The maximum mana comparison increases"), Widget->View.NewMaxMana > Widget->View.PreviousMaxMana);
	TestEqual(TEXT("The level-three view exposes two points"), Widget->View.GrantedChoicePoints, 2);

	TestTrue(TEXT("Choice A stages"), Widget->StageOrUnstageChoice(TEXT("Choice_A")));
	TestTrue(TEXT("Choice B can stage after pending prerequisite A"), Widget->StageOrUnstageChoice(TEXT("Choice_B")));
	TestTrue(TEXT("Confirm commits the whole staged selection"), Widget->ConfirmSelection());

	TArray<FName> Selected;
	TestTrue(TEXT("Committed widget state can be read"), FRPGClassProgressionTransactionService::TryGetSelectedChoiceIds(Component, 0, Selected));
	TestEqual(TEXT("Two widget choices committed"), Selected.Num(), 2);
	TestTrue(TEXT("Widget committed Choice A"), Selected.Contains(TEXT("Choice_A")));
	TestTrue(TEXT("Widget committed Choice B"), Selected.Contains(TEXT("Choice_B")));
	return true;
}
