...
namespace
{
	struct FGridMON1751TestWorld
	{
		...
		FGridMON1751TestWorld()
		{
			...
			World = UWorld::CreateWorld(EWorldType::Game, false,
				FName(*FString::Printf(TEXT("MON1751TestWorld_%s"), *FGuid::NewGuid().ToString(EGuidFormats::Digits))), nullptr, true, ERHIFeatureLevel::Num,
				&Values);
	...
	struct FGridMON1751Fixture
	{
		...
		AGridMonsterActor* AddGoblin(UGridMonsterDefinitionAsset* Definition, FIntPoint Cell, EGridEdge Facing, EGridMonsterState InitialState,
			FName EncounterGroupId = NAME_None, EGridMonsterPatrolMode PatrolMode = EGridMonsterPatrolMode::None,
			const TArray<FGridMonsterPatrolWaypoint>& Waypoints = {})
		{
			...
			UGridMonsterMovementComponent* Movement = NewObject<UGridMonsterMovementComponent>(Monster, TEXT("MON1751Movement"));
			Movement->bAutoInitialize = false;
			Movement->bInferCellFromActorLocation = false;
			Monster->AddInstanceComponent(Movement);
			Movement->RegisterComponent();

			UGridMonsterBehaviorComponent* Behavior = NewObject<UGridMonsterBehaviorComponent>(Monster, TEXT("MON1751Behavior"));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMonsterMON1751PatrolRangedKeeperTest, "Grimrock.Monsters.MON17.5.1.PatrolRangedKeeper",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON1751PatrolRangedKeeperTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	...
	UGridMonsterDefinitionAsset* Definition = Fixture.MakeGoblinDefinition(0, 0, false, 0);
	AGridMonsterActor* Goblin = Fixture.AddGoblin(
		Definition, FIntPoint(2, 1), EGridEdge::West, EGridMonsterState::Idle, NAME_None, EGridMonsterPatrolMode::Loop, MakeGoblinPatrolRoute());
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMonsterMON1751DirectionalPerceptionTest, "Grimrock.Monsters.MON17.5.1.DirectionalPerception",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON1751DirectionalPerceptionTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	FGridMON1751Fixture Fixture;
	TestTrue(TEXT("Fixture initializes"), Fixture.Initialize(FIntPoint(1, 4)));

	UGridMonsterDefinitionAsset* Definition = Fixture.MakeGoblinDefinition(8, 4, false, 0);
	AGridMonsterActor* Goblin = Fixture.AddGoblin(Definition, FIntPoint(1, 1), EGridEdge::North, EGridMonsterState::Idle);
	TestNotNull(TEXT("RangedKeeper Goblin exists"), Goblin);
	if (!Goblin)
	{
		return false;
	...
	TestTrue(TEXT("North-facing Goblin perceives party"), Behavior->RefreshPerception());
	TestTrue(TEXT("North-facing Goblin sees party"), Behavior->bCanSeeParty);
	TestTrue(TEXT("Party is also inside hearing range"), Behavior->bCanHearParty);

	Goblin->Facing = EGridEdge::East;
	TestTrue(TEXT("East-facing Goblin still perceives by hearing"), Behavior->RefreshPerception());
	TestFalse(TEXT("East-facing Goblin no longer sees party"), Behavior->bCanSeeParty);
	TestTrue(TEXT("Hearing remains omnidirectional"), Behavior->bCanHearParty);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMonsterMON1751HearingAlarmTest, "Grimrock.Monsters.MON17.5.1.HearingAlarm", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON1751HearingAlarmTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	FGridMON1751Fixture Fixture;
	TestTrue(TEXT("Fixture initializes"), Fixture.Initialize(FIntPoint(1, 4)));
	...
	UGridMonsterDefinitionAsset* SourceDefinition = Fixture.MakeGoblinDefinition(8, 4, true, 2);
	UGridMonsterDefinitionAsset* AllyDefinition = Fixture.MakeGoblinDefinition(8, 0, false, 0);

	AGridMonsterActor* Source = Fixture.AddGoblin(SourceDefinition, FIntPoint(1, 1), EGridEdge::East, EGridMonsterState::Idle, TEXT("GoblinAlarm_A"));
	AGridMonsterActor* Ally = Fixture.AddGoblin(AllyDefinition, FIntPoint(2, 1), EGridEdge::West, EGridMonsterState::Dormant, TEXT("GoblinAlarm_A"));
	...
	TestFalse(TEXT("Hearing/alarm alone does not start combat"), Fixture.TurnManager->bCombatActive);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMonsterMON1751VisionEngagementHandoffTest, "Grimrock.Monsters.MON17.5.1.VisionEngagementHandoff",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON1751VisionEngagementHandoffTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	FGridMON1751Fixture Fixture;
	TestTrue(TEXT("Fixture initializes"), Fixture.Initialize(FIntPoint(1, 4)));
	...
	UGridMonsterDefinitionAsset* Definition = Fixture.MakeGoblinDefinition(8, 4, false, 0);
	AGridMonsterActor* Goblin = Fixture.AddGoblin(
		Definition, FIntPoint(1, 1), EGridEdge::North, EGridMonsterState::Idle, NAME_None, EGridMonsterPatrolMode::Loop, MakeGoblinPatrolRoute());
