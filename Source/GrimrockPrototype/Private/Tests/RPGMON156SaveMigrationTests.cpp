...
namespace
{
    ...
    UGrimrockPartySaveGame* MakeMON156Save (
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON156PersistentChoiceRoundTripTest,
    "Grimrock.RPG.MON15.6.PersistentChoiceRoundTrip",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON156PersistentChoiceRoundTripTest::RunTest (
    const FString& Parameters)
{
    ...
    TestTrue (TEXT ("MON15.6 save serializes to memory"),
    ...
    TestEqual (TEXT ("One character progression snapshot is restored"),
        LoadedSave->ClassProgressionStates.Num (),
        1);
    TestTrue (TEXT ("Choice A survives serialization"),
        LoadedSave->ClassProgressionStates[0].SelectedChoiceIds.Contains (
            TEXT ("Choice_A")));
    TestTrue (TEXT ("Choice B survives serialization"),
        LoadedSave->ClassProgressionStates[0].SelectedChoiceIds.Contains (
            TEXT ("Choice_B")));
    ...
    TestTrue (TEXT ("Choice A requirement is restored before live cache binding"),
        RestoredRequirements.Contains (TEXT ("Choice_A")));
    TestTrue (TEXT ("Choice B requirement is restored before live cache binding"),
        RestoredRequirements.Contains (TEXT ("Choice_B")));
    TestTrue (TEXT ("Choice A granted feature is restored"),
        RestoredRequirements.Contains (TEXT ("Feature_A")));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON156LegacyExperienceAheadMigrationTest,
    "Grimrock.RPG.MON15.6.LegacyExperienceAheadMigration",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON156LegacyExperienceAheadMigrationTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    FMON156StateGuard StateGuard;
    ...
    URPGClassAsset* ClassDefinition = nullptr;
    UGridPartyInventoryComponent* Component =
        MakeMON155Inventory (1, 6000, ClassDefinition);
    ...
    Character.DerivedStats.CurrentHealth =
        Character.DerivedStats.MaxHealth - HealthDeficit;
    Character.DerivedStats.CurrentMana =
        Character.DerivedStats.MaxMana - ManaDeficit;
    ...
    TestEqual (TEXT ("Migrated character reaches level four"),
        MigratedCharacter.Level,
        4);
    TestEqual (TEXT ("Migrated XP remains 6000"),
        MigratedCharacter.Experience,
        6000);
    TestEqual (TEXT ("Health deficit is preserved"),
        MigratedCharacter.DerivedStats.CurrentHealth,
        MigratedCharacter.DerivedStats.MaxHealth - HealthDeficit);
    TestEqual (TEXT ("Mana deficit is preserved"),
        MigratedCharacter.DerivedStats.CurrentMana,
        MigratedCharacter.DerivedStats.MaxMana - ManaDeficit);
    TestEqual (TEXT ("Legacy migration creates one empty choice snapshot"),
        Save->ClassProgressionStates.Num (),
        1);
    TestTrue (TEXT ("Legacy character starts with no confirmed MON15.5 choice"),
        Save->ClassProgressionStates[0].SelectedChoiceIds.IsEmpty ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON156LegacyStoredLevelAheadMigrationTest,
    "Grimrock.RPG.MON15.6.LegacyStoredLevelAheadMigration",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON156LegacyStoredLevelAheadMigrationTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    FMON156StateGuard StateGuard;
    ...
    URPGClassAsset* ClassDefinition = nullptr;
    UGridPartyInventoryComponent* Component =
        MakeMON155Inventory (3, 1000, ClassDefinition);
    UGrimrockPartySaveGame* Save = MakeMON156Save (Component);
    Save->SaveVersion = 3;

    FText Error;
    FRPGSaveMigrationReport Report;
    TestTrue (TEXT ("Version three with stored level ahead migrates"),
        FRPGSaveMigrationService::PrepareLoadedSave (
            Save,
            Error,
            &Report));
    const FGridCharacterInventoryState& MigratedCharacter =
        Save->PartyInventoryState.ActiveCharacters[0];
    TestEqual (TEXT ("Stored level three is preserved"),
        MigratedCharacter.Level,
        3);
    TestEqual (TEXT ("XP is raised to the level-three floor"),
        MigratedCharacter.Experience,
        3000);
    TestEqual (TEXT ("One character was reconciled"),
        Report.ReconciledCharacterCount,
        1);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON156RejectCurrentMismatchTest,
    "Grimrock.RPG.MON15.6.RejectCurrentLevelExperienceMismatch",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON156RejectCurrentMismatchTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    FMON156StateGuard StateGuard;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON156RejectInvalidChoiceSnapshotTest,
    "Grimrock.RPG.MON15.6.RejectInvalidChoiceSnapshot",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON156RejectInvalidChoiceSnapshotTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    FMON156StateGuard StateGuard;
    ...
    FText Error;
    TestFalse (TEXT ("Unknown persisted class choice is rejected"),
        FRPGSaveMigrationService::ValidateCurrentSave (Save, Error));
    TestTrue (TEXT ("Invalid choice rejection reports an error"),
        !Error.IsEmpty ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON156PendingLevelUpRoundTripTest,
    "Grimrock.RPG.MON15.6.PendingLevelUpRoundTrip",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON156PendingLevelUpRoundTripTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    FMON156StateGuard StateGuard;
    ...
    UGrimrockPartySaveGame* SourceSave = MakeMON156Save (Component);
    TArray<uint8> SaveBytes;
    TestTrue (TEXT ("Save with pending Level Up serializes"),
        UGameplayStatics::SaveGameToMemory (SourceSave, SaveBytes));
    ...
    TestEqual (TEXT ("One pending Level Up survives serialization"),
        LoadedSave->PendingLevelUpNotifications.Num (),
        1);
    TestTrue (TEXT ("Pending Level Up keeps CharacterId"),
        LoadedSave->PendingLevelUpNotifications[0].CharacterId == CharacterId);
    TestEqual (TEXT ("Pending Level Up keeps previous level"),
        LoadedSave->PendingLevelUpNotifications[0].PreviousLevel,
        1);
    TestEqual (TEXT ("Pending Level Up keeps new level"),
        LoadedSave->PendingLevelUpNotifications[0].NewLevel,
        2);
    ...
    TestTrue (TEXT ("Loaded notification is restored into the persistent mirror"),
        URPGLevelUpNotificationSubsystem::CapturePersistentState (
            ...
            Recaptured,
            CaptureError));
    TestEqual (TEXT ("Persistent mirror contains the restored notification"),
        Recaptured.Num (),
        1);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON156RejectInvalidPendingNotificationTest,
    "Grimrock.RPG.MON15.6.RejectInvalidPendingNotification",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON156RejectInvalidPendingNotificationTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    FMON156StateGuard StateGuard;
    ...
    FText Error;
    TestFalse (TEXT ("Pending notification targeting a level other than current is rejected"),
        FRPGSaveMigrationService::ValidateCurrentSave (Save, Error));
    TestTrue (TEXT ("Invalid pending notification reports an error"),
        !Error.IsEmpty ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON156SaveVersionContractTest,
    "Grimrock.RPG.MON15.6.SaveVersionContract",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON156SaveVersionContractTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
