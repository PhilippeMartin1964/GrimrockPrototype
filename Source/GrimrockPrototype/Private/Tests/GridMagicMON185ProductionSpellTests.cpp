...
namespace
{
	UGridStatusEffectDefinitionAsset* MakeStatusDefinition(
		FName EffectId, EGridStatusEffectStackPolicy StackPolicy = EGridStatusEffectStackPolicy::RefreshDuration)
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMON185ProductionDefinitionsTest, "Grimrock.Magic.MON18.5.ProductionDefinitions",
...
bool FGridMON185ProductionDefinitionsTest::RunTest(const FString& Parameters)
{
	...
	{
		TestEqual(*FString::Printf(TEXT("%s is structurally valid"), *Definition.SpellId.ToString()), FGridSpellContract::ValidateDefinition(Definition),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMON185DamageResolutionTest, "Grimrock.Magic.MON18.5.DamageResolution", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMON185DamageResolutionTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Arcane Bolt resolves"),
		FGridSpellEffectResolver::ResolveEffects(Spell, FGuid::NewGuid(), 10, Health, Statuses, ResolveNone, Result, RejectReason, Error));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMON185HealingClampTest, "Grimrock.Magic.MON18.5.HealingClamp", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMON185ApplyStatusBridgeTest, "Grimrock.Magic.MON18.5.ApplyStatusBridge", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMON185ApplyStatusBridgeTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Haste resolves through MON16 collection"),
		FGridSpellEffectResolver::ResolveEffects(Spell, FGuid::NewGuid(), 10, Health, Statuses, Resolver, Result, RejectReason, Error));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMON185RemoveStatusTest, "Grimrock.Magic.MON18.5.RemoveStatus", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMON185RemoveStatusTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Cure Poison resolves"),
		FGridSpellEffectResolver::ResolveEffects(Spell, FGuid::NewGuid(), 10, Health, Statuses, ResolveNone, Result, RejectReason, Error));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMON185AtomicFailureTest, "Grimrock.Magic.MON18.5.AtomicFailureNoMutation", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMON185AtomicFailureTest::RunTest(const FString& Parameters)
{
	...
	TestFalse(TEXT("Missing status definition rejects whole effect batch"),
		FGridSpellEffectResolver::ResolveEffects(Spell, FGuid::NewGuid(), 10, Health, Statuses, ResolveNone, Result, RejectReason, Error));
	TestEqual(TEXT("Damage did not partially apply"), Health, 10);
	TestTrue(TEXT("Statuses unchanged"), Statuses.IsEmpty());
	TestEqual(TEXT("Correct reject reason"), RejectReason, EGridSpellEffectResolutionRejectReason::MissingStatusEffectDefinition);
	return true;
}
