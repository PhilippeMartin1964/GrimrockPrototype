...
namespace
{
    ...
    UGridPlayerAttackPresentationComponent* MakePresentationComponent (
        UGridTurnManagerComponent*& OutTurnManager)
    {
        AGridLevelRuntimeActor* Runtime =
            NewObject<AGridLevelRuntimeActor> ();
        OutTurnManager =
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON114ProfileValidationTest,
    "Grimrock.Monsters.MON11.Presentation.ProfileValidation",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON114ProfileValidationTest::RunTest (
    const FString& Parameters)
{
    ...
    Invalid = Empty;
    Invalid.AttackAudio.Sounds.Add (
    ...
    TestTrue (
        TEXT ("Item helper accepts the presentation"),
        Item->HasValidPlayerAttackPresentation ());
    Item->PlayerAttackPresentationProfile.FeedbackDurationSeconds =
        0.0f;
    TestFalse (
        TEXT ("Item helper rejects invalid presentation"),
        Item->HasValidPlayerAttackPresentation ());
    TestTrue (
        TEXT ("Presentation never invalidates gameplay definition"),
        Item->IsValidDefinition ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON114EventOrderTest,
    "Grimrock.Monsters.MON11.Presentation.EventOrder",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON114EventOrderTest::RunTest (
    const FString& Parameters)
{
    UGridTurnManagerComponent* TurnManager = nullptr;
    UGridPlayerAttackPresentationComponent* Presentation =
        MakePresentationComponent (TurnManager);
    ...
    TestEqual (
        TEXT ("Exactly one Attack"),
        Presentation->PresentationAttackCount,
        1);
    TestEqual (
        TEXT ("No impact before resolution"),
        Presentation->PresentationImpactHitCount,
        0);
    TurnManager->OnPlayerAttackResolved.Broadcast (
        ...
        Result);
    TestEqual (
        TEXT ("Exactly one hit impact"),
        Presentation->PresentationImpactHitCount,
        1);
    TestEqual (
        TEXT ("Exactly one feedback"),
        Presentation->FeedbackCount,
        1);
    TestEqual (
        TEXT ("Impact follows Attack sequence"),
        Presentation->LastPresentationRequest.SequenceNumber,
        2);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON114AudioVFXMappingTest,
    "Grimrock.Monsters.MON11.Presentation.AudioVFXMapping",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON114AudioVFXMappingTest::RunTest (
    const FString& Parameters)
{
    ...
    TestTrue (
        TEXT ("Pitch lower bound retained"),
        Profile.AttackAudio.PitchMin <= 1.0f);
    TestTrue (
        TEXT ("Pitch upper bound retained"),
        Profile.AttackAudio.PitchMax >= 1.0f);
    TestNotEqual (
        TEXT ("Attack and hit audio are exclusive"),
        Profile.AttackAudio.Sounds[0].Get (),
        Profile.ImpactHitAudio.Sounds[0].Get ());
    TestNotEqual (
        TEXT ("Hit and miss VFX are exclusive"),
        Profile.ImpactHitVFX.Systems[0].Get (),
        Profile.ImpactMissVFX.Systems[0].Get ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON114HeldItemMotionTest,
    "Grimrock.Monsters.MON11.Presentation.HeldItemMotion",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON114HeldItemMotionTest::RunTest (
    const FString& Parameters)
{
    UGridTurnManagerComponent* TurnManager = nullptr;
    UGridPlayerAttackPresentationComponent* Presentation =
    ...
    TurnManager->OnPlayerAttackRequested.Broadcast (Request);
    TestFalse (
        TEXT ("Unarmed attack never moves an item"),
        Presentation->IsHeldItemMotionActive ());
    TestFalse (
        TEXT ("Unarmed attack reports no started movement"),
        Presentation->bHeldItemMotionStarted);
    Presentation->ResetTransientPresentationState ();
    TestFalse (
        TEXT ("Reset leaves Tick disabled"),
        Presentation->IsComponentTickEnabled ());
    TestEqual (
        TEXT ("Reset does not destroy presentation component"),
        Presentation->IsValidLowLevel (),
        true);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON114FeedbackAndRejectionsTest,
    "Grimrock.Monsters.MON11.Presentation.FeedbackAndRejections",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON114FeedbackAndRejectionsTest::RunTest (
    const FString& Parameters)
{
    const UEnum* RejectEnum =
        StaticEnum<EGridPlayerAttackRejectReason> ();
    for (int32 Index = 0; Index < RejectEnum->NumEnums () - 1; ++Index)
    {
        const auto Reason =
            static_cast<EGridPlayerAttackRejectReason> (
                RejectEnum->GetValueByIndex (Index));
        TestFalse (
            *FString::Printf (
                TEXT ("Reject text %d is non-empty"),
                Index),
            FGridPlayerAttackFeedbackFormatter::
                FormatRejectReason (Reason).IsEmpty ());
    }
    ...
    const FGridPlayerAttackFeedbackRequest Critical =
        FGridPlayerAttackFeedbackFormatter::FormatResolved (
            MakePresentationRequest (),
            Result,
            FText::FromString (TEXT ("Aëlric")),
            FText::FromString (TEXT ("Rat géant")),
            1.25f);
    TestEqual (
        TEXT ("Critical outcome"),
        Critical.Outcome,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON114TargetReactionExclusivityTest,
    "Grimrock.Monsters.MON11.Presentation.TargetReactionExclusivity",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMON114TargetReactionExclusivityTest::RunTest (
    const FString& Parameters)
{
