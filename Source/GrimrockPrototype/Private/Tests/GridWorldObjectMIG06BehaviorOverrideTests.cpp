...
bool FGridWorldObjectMIG06SparseBehaviorResolutionTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Relocation destination remains instance-owned X"), Resolved.Relocation.TargetCellX, 7);
	TestEqual(TEXT("Relocation destination remains instance-owned Y"), Resolved.Relocation.TargetCellY, 8);
...
bool FGridWorldObjectMIG06SparseStorageContractTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Relocation destination is stored as an instance override"), Sparse.Relocation.TargetCellX, 12);
