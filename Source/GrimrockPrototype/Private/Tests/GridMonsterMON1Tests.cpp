...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterDefinitionMON1ValidationTest,
...
bool FGridMonsterDefinitionMON1ValidationTest::RunTest (const FString& Parameters)
{
    ...
    TestTrue (TEXT ("The DirectMelee profile is present"),
        Rat->HasAIProfile (EGridMonsterAIProfile::DirectMelee));
    TestTrue (TEXT ("The FastHarasser profile is present"),
        Rat->HasAIProfile (EGridMonsterAIProfile::FastHarasser));
    TestFalse (TEXT ("The RangedKeeper profile is absent"),
        Rat->HasAIProfile (EGridMonsterAIProfile::RangedKeeper));
    ...
    TestTrue (TEXT ("Fire vulnerability is 1.50"),
        FMath::IsNearlyEqual (Rat->GetDamageMultiplier (EGridDamageType::Fire), 1.50f));
    TestTrue (TEXT ("Slashing vulnerability is 1.25"),
        FMath::IsNearlyEqual (Rat->GetDamageMultiplier (
            EGridDamageType::Physical,
            EGridPhysicalDamageSubtype::Slashing), 1.25f));
    TestTrue (TEXT ("Piercing damage has no modifier"),
        FMath::IsNearlyEqual (Rat->GetDamageMultiplier (
            EGridDamageType::Physical,
            EGridPhysicalDamageSubtype::Piercing), 1.0f));

    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterDefinitionMON1InvalidDataTest,
