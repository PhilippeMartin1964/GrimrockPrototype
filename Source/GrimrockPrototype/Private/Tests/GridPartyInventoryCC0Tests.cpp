...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FGridPartyInventoryDefaultInitializationTest,
    "Grimrock.CharacterCreation.CC0.DefaultPartyInitialization",
...
bool FGridPartyInventoryDefaultInitializationTest::RunTest(const FString& Parameters)
{
    ...
    TestEqual(
        TEXT("Equipment state stays aligned with active characters"),
        Component->PartyInventoryState.ActiveEquipment.Num(),
    ...
    TestTrue(
        FString::Printf(TEXT("Initial ownership is valid: %s"), *OwnershipError),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FGridPartyInventorySelectedCharacterPickupTest,
    "Grimrock.CharacterCreation.CC0.SelectedCharacterPickup",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridPartyInventorySelectedCharacterPickupTest::RunTest(const FString& Parameters)
{
    ...
    TestTrue(
        TEXT("An item can be added to the selected character"),
        Component->AddItemToSelectedCharacterInventory(Item));
    ...
    TestTrue(
        TEXT("The item is owned by a character inventory"),
        FirstSlot.Item.OwnerType == EGridItemOwnerType::CharacterInventory);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FGridPartyInventoryCarryWeightTest,
    "Grimrock.CharacterCreation.CC0.CarryWeightAndOverload",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridPartyInventoryCarryWeightTest::RunTest(const FString& Parameters)
{
    ...
    TestTrue(
        TEXT("Carry capacity uses Strength x 5"),
        FMath::IsNearlyEqual(Character.MaxCarryWeight, 50.0f));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FGridPartyInventoryExclusiveOwnershipTest,
    "Grimrock.CharacterCreation.CC0.ExclusiveOwnershipFlow",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridPartyInventoryExclusiveOwnershipTest::RunTest(const FString& Parameters)
{
    ...
    FString OwnershipError;
    TestTrue(
        FString::Printf(TEXT("Inventory ownership is valid: %s"), *OwnershipError),
        Component->ValidateInventoryOwnership(OwnershipError));
    ...
    TestTrue(
        FString::Printf(TEXT("Cursor ownership is valid: %s"), *OwnershipError),
        Component->ValidateInventoryOwnership(OwnershipError));

    TestTrue(
        TEXT("The cursor item equips to the selected character main hand"),
    ...
    TestTrue(
        TEXT("The main hand contains an item"),
        Component->GetEquippedItem(0, EGridEquipmentSlot::MainHand, EquippedItem));
    ...
    OwnershipError.Empty();
    TestTrue(
        FString::Printf(TEXT("Equipment ownership is valid: %s"), *OwnershipError),
        Component->ValidateInventoryOwnership(OwnershipError));
