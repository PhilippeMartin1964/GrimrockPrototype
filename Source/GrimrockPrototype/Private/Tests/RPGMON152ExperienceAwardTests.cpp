...
namespace
{
	struct FRPGMON152TestWorld
	{
		...
		FRPGMON152TestWorld()
		{
			...
			World = UWorld::CreateWorld(EWorldType::Game, false,
				FName(*FString::Printf(TEXT("MON152TestWorld_%s"), *FGuid::NewGuid().ToString(EGuidFormats::Digits))), nullptr, true, ERHIFeatureLevel::Num,
	...
	AGrimrockPartyPawn* SpawnMON152Party(UWorld* World, int32 ActiveCharacterCount)
	{
		if (!World)
		{
			return nullptr;
		...
		{
			Party->PartyInventoryComponent->PartyInventoryState.ActiveCharacters.Add(MakeMON152Character(Index + 1));
		}
		return Party;
	}

	UGridMonsterDefinitionAsset* MakeMON152MonsterDefinition(UObject* Outer, FName MonsterId, int32 ExperienceReward)
	{
	...
	AGridMonsterActor* SpawnMON152Monster(UWorld* World, UGridMonsterDefinitionAsset* Definition, FGuid SpawnId, FIntPoint Cell)
	{
		if (!World || !Definition)
		{
			return nullptr;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON152ActivePartyDistributionTest, "Grimrock.RPG.MON15.2.ActivePartyDistribution",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON152ActivePartyDistributionTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	UGridPartyInventoryComponent* Component = NewObject<UGridPartyInventoryComponent>();
	Component->PartyInventoryState.ActiveCharacters = { MakeMON152Character(1), MakeMON152Character(2), MakeMON152Character(3), MakeMON152Character(4) };
	Component->PartyInventoryState.CharacterPool = { MakeMON152Character(99, 1, 100) };

	int32 EventCount = 0;
	int32 EventAppliedTotal = 0;
	const FDelegateHandle EventHandle = FRPGExperienceRewardService::OnCharacterExperienceAwarded().AddLambda(
		[&EventCount, &EventAppliedTotal](int32 CharacterIndex, int32 AwardedExperience, int32 PreviousExperience, int32 NewExperience)
		{
			++EventCount;
	...
	const int32 Applied = FRPGExperienceRewardService::AwardToActiveParty(Component, 10);

	FRPGExperienceRewardService::OnCharacterExperienceAwarded().Remove(EventHandle);

	TestEqual(TEXT("The complete reward is applied"), Applied, 10);
	TestEqual(TEXT("Character zero receives the first remainder point"), Component->PartyInventoryState.ActiveCharacters[0].Experience, 3);
	TestEqual(TEXT("Character one receives the second remainder point"), Component->PartyInventoryState.ActiveCharacters[1].Experience, 3);
	TestEqual(TEXT("Character two receives the base share"), Component->PartyInventoryState.ActiveCharacters[2].Experience, 2);
	TestEqual(TEXT("Character three receives the base share"), Component->PartyInventoryState.ActiveCharacters[3].Experience, 2);
	TestEqual(TEXT("A reserve character receives no XP"), Component->PartyInventoryState.CharacterPool[0].Experience, 100);
	TestEqual(TEXT("One event is emitted per rewarded active character"), EventCount, 4);
	TestEqual(TEXT("Events account for the full applied reward"), EventAppliedTotal, 10);
	...
	TestEqual(TEXT("Zero reward is a no-op"), FRPGExperienceRewardService::AwardToActiveParty(Component, 0), 0);
	TestEqual(TEXT("Negative reward is a no-op"), FRPGExperienceRewardService::AwardToActiveParty(Component, -10), 0);
	TestEqual(TEXT("No-op rewards do not mutate XP"), Component->PartyInventoryState.ActiveCharacters[0].Experience, XPBeforeNoOp);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON152ProgressionBoundariesTest, "Grimrock.RPG.MON15.2.ProgressionBoundaries", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON152ProgressionBoundariesTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	UGridPartyInventoryComponent* Component = NewObject<UGridPartyInventoryComponent>();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON152MonsterDeathExactlyOnceTest, "Grimrock.RPG.MON15.2.MonsterDeathExactlyOnce",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON152MonsterDeathExactlyOnceTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	...
	AGrimrockPartyPawn* Party = SpawnMON152Party(TestWorld.World, 2);
	if (!TestNotNull(TEXT("The MON15.2 party exists"), Party))
	{
		return false;
	}

	UGridMonsterDefinitionAsset* Definition = MakeMON152MonsterDefinition(TestWorld.World, TEXT("MON152_RewardMonster"), 10);
	AGridMonsterActor* Monster = SpawnMON152Monster(TestWorld.World, Definition, FGuid(152, 10, 0, 1), FIntPoint(1, 1));
	if (!TestNotNull(TEXT("The reward monster exists"), Monster) || !Monster->DeathComponent)
	{
		return false;
	}
	...
	TestEqual(TEXT("Repeated MarkDead does not award XP again to character one"),
		Party->PartyInventoryComponent->PartyInventoryState.ActiveCharacters[1].Experience, 5);

	AGridMonsterActor* RestoredDeadMonster = SpawnMON152Monster(TestWorld.World, Definition, FGuid(152, 10, 0, 2), FIntPoint(2, 1));
	if (!TestNotNull(TEXT("The restored-dead monster exists"), RestoredDeadMonster) || !RestoredDeadMonster->DeathComponent)
	{
		return false;
	}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON152LootIndependenceTest, "Grimrock.RPG.MON15.2.LootIndependence", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON152LootIndependenceTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	FRPGMON152TestWorld TestWorld;
	...
	AGrimrockPartyPawn* Party = SpawnMON152Party(TestWorld.World, 1);
	UGridMonsterDefinitionAsset* Definition = MakeMON152MonsterDefinition(TestWorld.World, TEXT("MON152_LootFailureMonster"), 7);
	...
	AGridMonsterActor* Monster = SpawnMON152Monster(TestWorld.World, Definition, FGuid(152, 20, 0, 1), FIntPoint(1, 1));
	if (!TestNotNull(TEXT("The loot-independence party exists"), Party) || !TestNotNull(TEXT("The loot-independence monster exists"), Monster) ||
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON152PersistenceStateTest, "Grimrock.RPG.MON15.2.PersistenceState", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON152PersistenceStateTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
