...
namespace RPGMON2084SkillsPageReadModelTests
{
	...
	FGridCharacterInventoryState MakeCharacter(URPGClassAsset* ClassDefinition, const TCHAR* Name)
	{
	...
	UGridPartyInventoryComponent* MakeParty(URPGClassAsset*& OutClassDefinition)
	{
		...
		Party->PartyInventoryState.ActiveCharacters.Add(MakeCharacter(OutClassDefinition, TEXT("A")));
		Party->PartyInventoryState.ActiveCharacters.Add(MakeCharacter(OutClassDefinition, TEXT("B")));
		Party->PartyInventoryState.ActiveEquipment.SetNum(2);
		Party->PartyInventoryState.SelectedCharacterIndex = 0;
		return Party;
	}

	URPGSkillAsset* MakeSkill(UObject* Outer, FName SkillId, const TCHAR* DisplayName)
	{
	...
	bool SetRank(FGridCharacterInventoryState& Character, URPGSkillAsset* Skill, int32 Rank)
	{
		FRPGSkillMutationResult Result;
		return FRPGSkillService::TrySetSkillRank(Character, Skill, Rank, Result);
	}

	bool CommitTalent(UGridPartyInventoryComponent* Party, int32 CharacterIndex)
	{
		FRPGClassProgressionCommitResult Result;
		return FRPGClassProgressionTransactionService::TryCommitChoices(Party, CharacterIndex, { TalentA }, Result);
	}
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON2084SelectedCharacterIdentityTest, "Grimrock.MON20.8.SkillsPage.SelectedCharacterIdentity",
...
bool FRPGMON2084SelectedCharacterIdentityTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Selected view builds"), FGridSkillsPageService::TryBuildSelectedCharacterView(Party, { Skill }, View));
	TestEqual(TEXT("Selected index is authoritative"), View.CharacterIndex, 0);
	TestTrue(TEXT("Selected CharacterId is preserved"), View.CharacterId == Party->PartyInventoryState.ActiveCharacters[0].CharacterId);
	TestTrue(TEXT("Selected name is preserved"), View.CharacterName.EqualTo(FText::FromString(TEXT("A"))));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON2084SkillRanksTest, "Grimrock.MON20.8.SkillsPage.SkillRanks", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2084SkillRanksTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Trained rank setup succeeds"), SetRank(Party->PartyInventoryState.ActiveCharacters[0], Lockpick, 3));

	FGridSkillsPageView View;
	TestTrue(TEXT("View builds with trained and untrained Skills"), FGridSkillsPageService::TryBuildCharacterView(Party, 0, { Lockpick, Alchemy }, View));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON2084SkillOrderTest, "Grimrock.MON20.8.SkillsPage.DeterministicSkillOrder", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON2084TalentProjectionTest, "Grimrock.MON20.8.SkillsPage.TalentProjection", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON2084TalentBalanceTest, "Grimrock.MON20.8.SkillsPage.TalentPointBalance", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2084TalentBalanceTest::RunTest(const FString& Parameters)
{
	...
	FGridSkillsPageView View;
	TestTrue(TEXT("View builds with Talent balance"), FGridSkillsPageService::TryBuildCharacterView(Party, 0, { Skill }, View));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON2084SelectedAuthorityTest, "Grimrock.MON20.8.SkillsPage.SelectedCharacterAuthority",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2084SelectedAuthorityTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Character one rank setup succeeds"), SetRank(Party->PartyInventoryState.ActiveCharacters[1], Skill, 4));

	FGridSkillsPageView View;
	TestTrue(TEXT("Selected-character facade follows inventory selection"), FGridSkillsPageService::TryBuildSelectedCharacterView(Party, { Skill }, View));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON2084DuplicateDefinitionAtomicTest, "Grimrock.MON20.8.SkillsPage.DuplicateDefinitionAtomic",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2084DuplicateDefinitionAtomicTest::RunTest(const FString& Parameters)
{
	...
	TestFalse(TEXT("Duplicate canonical SkillId is rejected"), FGridSkillsPageService::TryBuildCharacterView(Party, 0, { SkillA, SkillB }, View));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON2084MissingRankDefinitionAtomicTest, "Grimrock.MON20.8.SkillsPage.MissingRankDefinitionAtomic",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON2084MissingRankDefinitionAtomicTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Rank setup succeeds"), SetRank(Party->PartyInventoryState.ActiveCharacters[0], Ranked, 2));
	const TArray<FRPGSkillRank> Before = Party->PartyInventoryState.ActiveCharacters[0].SkillRanks;

	FGridSkillsPageView View;
	TestFalse(TEXT("Missing canonical definition for ranked Skill is rejected"), FGridSkillsPageService::TryBuildCharacterView(Party, 0, { Other }, View));
	TestFalse(TEXT("Failed view remains invalid"), View.IsValid());
	TestEqual(TEXT("Read model never mutates SkillRanks"), Party->PartyInventoryState.ActiveCharacters[0].SkillRanks.Num(), Before.Num());
	if (Before.Num() == 1)
	{
		TestEqual(TEXT("Original SkillId is preserved"), Party->PartyInventoryState.ActiveCharacters[0].SkillRanks[0].SkillId, Before[0].SkillId);
		TestEqual(TEXT("Original rank is preserved"), Party->PartyInventoryState.ActiveCharacters[0].SkillRanks[0].Rank, Before[0].Rank);
	}
	return true;
}
