...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMonsterMON1733ProjectileSourceContractTest, "Grimrock.Monsters.MON17.3.3.ProjectileSourceContract",
...
bool FGridMonsterMON1733ProjectileSourceContractTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Projectile source offset defaults to zero"), Attack.ProjectileSourceOffset, FVector::ZeroVector);

	Attack.ProjectileSourceSocketName = TEXT("ProjectileSource");
	Attack.ProjectileSourceOffset = FVector(5.0f, -2.0f, 8.0f);
	TestTrue(TEXT("An authored source socket and finite local offset remain valid"), Attack.IsValidDefinition());

	FString Error;
	Attack.ProjectileSourceOffset.X = std::numeric_limits<float>::quiet_NaN();
	TestFalse(TEXT("Non-finite projectile source offsets are rejected"), Attack.ValidateDefinition(Error));
	TestTrue(TEXT("Validation identifies ProjectileSourceOffset"), Error.Contains(TEXT("ProjectileSourceOffset")));
