...
namespace
{
    UGridStatusEffectDefinitionAsset* MON165MakeControlDefinition (
        UObject* Outer,
        FName EffectId,
    ...
    FGridCombatActionDefinition MON165MakeSelfClassAction (
        FName ActionId,
    ...
    FGridCharacterInventoryState MON165MakeCharacter (
    ...
    FGridCombatantInitiativeEntry MON165MakePartyEntry (
        const FGuid& CharacterId)
    {
    ...
    struct FGridMON165PartyFixture
    {
        ...
        FGridMON165PartyFixture ()
        {
            ...
            Party->PartyInventoryComponent->PartyInventoryState.ActiveCharacters = {
                MON165MakeCharacter (CharacterId)
            };
        ...
        {
            return Party && Party->PartyInventoryComponent &&
    ...
    const FGridAvailableCombatAction* MON165FindAction (
        const TArray<FGridAvailableCombatAction>& Actions,
        FName ActionId)
    {
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165ControlAggregationTest,
    "Grimrock.RPG.MON16.5.ControlAggregation",
...
bool FRPGMON165ControlAggregationTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* First = MON165MakeControlDefinition (
        GetTransientPackage (), TEXT ("MON165_ControlA"), FirstControl);
    UGridStatusEffectDefinitionAsset* Second = MON165MakeControlDefinition (
        GetTransientPackage (), TEXT ("MON165_ControlB"), SecondControl);

    FGridStatusEffectCollection Collection;
    FString Error;
    TestTrue (TEXT ("First arbitrary control effect applies"),
        Collection.TryAdd (*First, FGuid::NewGuid (), Error));
    TestTrue (TEXT ("Second arbitrary control effect applies"),
        Collection.TryAdd (*Second, FGuid::NewGuid (), Error));

    const FGridStatusEffectControlProfile Result =
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165PermanentSkipActivationRejectedTest,
    "Grimrock.RPG.MON16.5.PermanentSkipActivationRejected",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON165PermanentSkipActivationRejectedTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* PermanentSkip = MON165MakeControlDefinition (
        GetTransientPackage (),
        TEXT ("MON165_PermanentSkip"),
        Skip,
        EGridStatusEffectDurationUnit::Permanent,
        0);
    FString Error;
    TestFalse (TEXT ("Permanent activation skipping is rejected"),
        PermanentSkip->ValidateDefinition (Error));
    TestTrue (TEXT ("Validation explains the progress-safety rule"),
        Error.Contains (TEXT ("SkipActivation")));
    ...
    UGridStatusEffectDefinitionAsset* NonSkip = MON165MakeControlDefinition (
        GetTransientPackage (),
        TEXT ("MON165_PermanentControl"),
        PermanentControl,
        EGridStatusEffectDurationUnit::Permanent,
        0);
    TestTrue (TEXT ("Permanent silence/immobilize style control remains valid"),
        NonSkip->ValidateDefinition (Error));
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165StackBooleanSemanticsTest,
    "Grimrock.RPG.MON16.5.StackBooleanSemantics",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON165StackBooleanSemanticsTest::RunTest (const FString& Parameters)
{
    ...
    FString Error;
    TestTrue (TEXT ("Three control stacks apply"),
        Collection.TryApply (
            *Definition,
            FGuid::NewGuid (),
            3,
            ...
            Result,
            Error));
    TestEqual (TEXT ("Stack count is retained by the common status model"),
        Collection.ActiveEffects[0].StackCount,
        3);
    const FGridStatusEffectControlProfile Resolved =
        FGridStatusEffectControlResolver::Resolve (Collection);
    TestTrue (TEXT ("Boolean control remains active regardless of stack count"),
        Resolved.bBlockTranslation);
    TestFalse (TEXT ("Unconfigured capabilities remain false"),
        Resolved.bSkipActivation || Resolved.bBlockSpellActions);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165TurnSkipLifecycleTest,
    "Grimrock.RPG.MON16.5.TurnSkipLifecycle",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON165TurnSkipLifecycleTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* Definition = MON165MakeControlDefinition (
        GetTransientPackage (),
        TEXT ("MON165_TurnSkip"),
        Control,
        EGridStatusEffectDurationUnit::Turns,
        1);
    FString Error;
    TestTrue (TEXT ("Turn skip status applies"),
        Character->StatusEffects.TryAdd (*Definition, FGuid::NewGuid (), Error));
    TestTrue (TEXT ("Control resolver requests activation skipping"),
        FGridStatusEffectControlResolver::Resolve (
            Character->StatusEffects).bSkipActivation);

    UGridStatusEffectLifecycleSubsystem* Lifecycle =
        NewObject<UGridStatusEffectLifecycleSubsystem> (Fixture.TestWorld.World);
    ...
    TestFalse (TEXT ("A one-turn skipped activation expires the effect exactly once"),
        Character->StatusEffects.Contains (TEXT ("MON165_TurnSkip")));
    Lifecycle->UnbindFromTurnManager ();
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165RoundSkipLifecycleTest,
    "Grimrock.RPG.MON16.5.RoundSkipLifecycle",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON165RoundSkipLifecycleTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* Definition = MON165MakeControlDefinition (
        GetTransientPackage (),
        TEXT ("MON165_RoundSkip"),
        Control,
        EGridStatusEffectDurationUnit::Rounds,
        1);
    FString Error;
    TestTrue (TEXT ("Round skip status applies"),
        Character->StatusEffects.TryAdd (*Definition, FGuid::NewGuid (), Error));
    ...
    TestFalse (TEXT ("One-round control expires on the next round boundary"),
        Character->StatusEffects.Contains (TEXT ("MON165_RoundSkip")));
    Lifecycle->UnbindFromTurnManager ();
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165SilenceCatalogIsolationTest,
...
bool FRPGMON165SilenceCatalogIsolationTest::RunTest (const FString& Parameters)
{
    ...
    UGridStatusEffectDefinitionAsset* Definition = MON165MakeControlDefinition (
        GetTransientPackage (), TEXT ("MON165_BlockMagic"), Control);
    FString Error;
    TestTrue (TEXT ("Silence-style status applies"),
        Character->StatusEffects.TryAdd (*Definition, FGuid::NewGuid (), Error));

    TArray<FGridAvailableCombatAction> Actions;
    Fixture.TurnManager->GetAvailableCombatActions (0, Actions);
    const FGridAvailableCombatAction* Spell =
        MON165FindAction (Actions, TEXT ("MON165_Spell"));
    const FGridAvailableCombatAction* Ability =
        MON165FindAction (Actions, TEXT ("MON165_Ability"));
    ...
    TestEqual (TEXT ("Silence uses the canonical missing-requirement availability"),
        Spell ? Spell->AvailabilityReason : EGridCombatActionAvailabilityReason::None,
        EGridCombatActionAvailabilityReason::MissingRequirement);
    TestTrue (TEXT ("Disabled spell carries an explanatory reason"),
        Spell && !Spell->DisabledReason.IsEmpty ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165SilenceRequestAtomicTest,
    "Grimrock.RPG.MON16.5.SilenceRequestAtomic",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON165SilenceRequestAtomicTest::RunTest (const FString& Parameters)
{
    ...
    Control.bBlockSpellActions = true;
    UGridStatusEffectDefinitionAsset* Definition = MON165MakeControlDefinition (
        GetTransientPackage (), TEXT ("MON165_RequestBlock"), Control);
    FString Error;
    TestTrue (TEXT ("Request-blocking status applies"),
        Character->StatusEffects.TryAdd (*Definition, FGuid::NewGuid (), Error));

    TArray<FGridAvailableCombatAction> Actions;
    Fixture.TurnManager->GetAvailableCombatActions (0, Actions);
    const FGridAvailableCombatAction* Spell =
        MON165FindAction (Actions, TEXT ("MON165_RequestSpell"));
    TestNotNull (TEXT ("Request fixture exposes the spell"), Spell);
    TestFalse (TEXT ("Request fixture reaches Silence rather than PartyBusy"),
        Spell && Spell->bEnabled);
    ...
    TestEqual (TEXT ("Canonical reject reason is ActionUnavailable"),
        Result.RejectReason,
        EGridCombatActionRequestRejectReason::ActionUnavailable);
    TestEqual (TEXT ("Rejected spell spends no AP"),
        After.RemainingActionPoints,
        Before.RemainingActionPoints);
    TestEqual (TEXT ("Rejected spell spends no mana"),
        Character->DerivedStats.CurrentMana,
        ManaBefore);
    TestEqual (TEXT ("Rejected spell applies no effect"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165PartyImmobilizeTranslationTest,
    "Grimrock.RPG.MON16.5.PartyImmobilizeTranslation",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON165PartyImmobilizeTranslationTest::RunTest (const FString& Parameters)
{
    ...
    Control.bBlockTranslation = true;
    UGridStatusEffectDefinitionAsset* Definition = MON165MakeControlDefinition (
        GetTransientPackage (), TEXT ("MON165_Root"), Control);
    FString Error;
    TestTrue (TEXT ("Immobilize-style status applies"),
        Character->StatusEffects.TryAdd (*Definition, FGuid::NewGuid (), Error));

    FGridPlayerCharacterTurnState Before;
    Fixture.TurnManager->GetPlayerCharacterTurnState (0, Before);
    const FGridPartyMobilityState MobilityBefore =
    ...
    const bool bAccepted = Fixture.TurnManager->RequestPartyTranslation (
        EGridEdge::North,
        TargetCell,
        RejectReason);
    ...
    FGridPlayerCharacterTurnState After;
    Fixture.TurnManager->GetPlayerCharacterTurnState (0, After);
    const FGridPartyMobilityState MobilityAfter =
        Fixture.TurnManager->GetPartyMobilityState ();
    TestFalse (TEXT ("Immobilized party translation is rejected"), bAccepted);
    TestEqual (TEXT ("MON16.5 keeps the temporary public reject reason"),
        RejectReason,
        EGridPartyMovementRejectReason::PartyBusy);
    TestEqual (TEXT ("Rejected translation spends no personal AP"),
        After.RemainingActionPoints,
        Before.RemainingActionPoints);
    TestEqual (TEXT ("Rejected translation spends no shared PAM"),
        MobilityAfter.RemainingMobilityActionPoints,
        MobilityBefore.RemainingMobilityActionPoints);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165PartyImmobilizeRotationTest,
...
bool FRPGMON165PartyImmobilizeRotationTest::RunTest (const FString& Parameters)
{
    ...
    Control.bBlockTranslation = true;
    UGridStatusEffectDefinitionAsset* Definition = MON165MakeControlDefinition (
        GetTransientPackage (), TEXT ("MON165_RootRotate"), Control);
    FString Error;
    TestTrue (TEXT ("Immobilize-style status applies"),
        Character->StatusEffects.TryAdd (*Definition, FGuid::NewGuid (), Error));
    ...
    EGridPartyMovementRejectReason RejectReason =
        EGridPartyMovementRejectReason::None;
    const bool bAccepted = Fixture.TurnManager->RequestPartyRotation (
        EGridEdge::West,
        RejectReason);
    TestTrue (TEXT ("Immobilize does not block a legal 90-degree rotation"),
        bAccepted);
    TestEqual (TEXT ("Accepted rotation has no reject reason"),
        RejectReason,
        EGridPartyMovementRejectReason::None);
    TestTrue (TEXT ("Rotation is tracked as an accepted pending motion"),
        Fixture.TurnManager->IsPartyMotionInProgress ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165TargetParityTest,
    "Grimrock.RPG.MON16.5.TargetParity",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRPGMON165TargetParityTest::RunTest (const FString& Parameters)
{
    FGridMON165TestWorld TestWorld;
    AGridMonsterActor* Monster = TestWorld.World
    ...
    Control.bBlockTranslation = true;
    UGridStatusEffectDefinitionAsset* Definition = MON165MakeControlDefinition (
        GetTransientPackage (), TEXT ("MON165_Parity"), Control);

    FGridStatusEffectCollection PartyCollection;
    FString Error;
    TestTrue (TEXT ("Party-style collection accepts control"),
        PartyCollection.TryAdd (*Definition, FGuid::NewGuid (), Error));
    TestTrue (TEXT ("Monster collection accepts the same control definition"),
        Monster->StatusEffects.TryAdd (*Definition, FGuid::NewGuid (), Error));

    const FGridStatusEffectControlProfile PartyControl =
        FGridStatusEffectControlResolver::Resolve (PartyCollection);
    const FGridStatusEffectControlProfile MonsterControl =
        FGridStatusEffectControlResolver::Resolve (Monster->StatusEffects);
    TestEqual (TEXT ("SkipActivation has party/monster parity"),
        MonsterControl.bSkipActivation,
        PartyControl.bSkipActivation);
    TestEqual (TEXT ("BlockTranslation has party/monster parity"),
        MonsterControl.bBlockTranslation,
        PartyControl.bBlockTranslation);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON165NoParallelSystemTest,
...
bool FRPGMON165NoParallelSystemTest::RunTest (const FString& Parameters)
{
    ...
    const FSourceExpectation Sources[] = {
        { TEXT ("Source/GrimrockPrototype/Private/RPG/StatusEffects/GridStatusEffectControlResolver.cpp"), TEXT ("DefinitionAsset->Control") },
    ...
    for (const FSourceExpectation& Source : Sources)
    {
        ...
        const FString Path = FPaths::Combine (
            ...
            Source.RelativePath);
        TestTrue (
            *FString::Printf (TEXT ("Production source loads: %s"), Source.RelativePath),
            FFileHelper::LoadFileToString (Text, *Path));
        TestTrue (
            *FString::Printf (TEXT ("Production source uses generic capability: %s"), Source.RequiredToken),
            Text.Contains (Source.RequiredToken));
        TestFalse (
            *FString::Printf (TEXT ("No EffectId equality branch in %s"), Source.RelativePath),
            Text.Contains (TEXT ("EffectId ==")) ||
                Text.Contains (TEXT ("EffectId !=")));
    }
    return true;
}
