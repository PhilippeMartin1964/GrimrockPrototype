...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON6CombatResolverTest,
...
bool FGridMonsterMON6CombatResolverTest::RunTest (const FString& Parameters)
{
    ...
    FGridAttackResult Result = FGridCombatResolver::ResolveAttackFromRolls (
        Source,
        Target,
        Attack,
        1,
    ...
    Source.Accuracy = -100;
    Result = FGridCombatResolver::ResolveAttackFromRolls (
        Source,
        Target,
        Attack,
        20,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON6PartyTargetSelectorTest,
...
bool FGridMonsterMON6PartyTargetSelectorTest::RunTest (const FString& Parameters)
{
    ...
    TestEqual (
        TEXT ("A living front-line character is preferred"),
        FGridPartyTargetSelector::SelectTarget (Party, RandomStream, 3),
        1);

    Party.ActiveCharacters[1].DerivedStats.CurrentHealth = 0;
    RandomStream.Initialize (42);
    TestEqual (
        TEXT ("The second line is used when the front line is defeated"),
        FGridPartyTargetSelector::SelectTarget (Party, RandomStream, 3),
        3);
    ...
    RandomStream.Initialize (42);
    TestEqual (
        TEXT ("No living character produces no target"),
        FGridPartyTargetSelector::SelectTarget (Party, RandomStream, 3),
        INDEX_NONE);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON6DirectMeleePlannerTest,
...
bool FGridMonsterMON6DirectMeleePlannerTest::RunTest (const FString& Parameters)
{
    ...
    if (Actions.Num () == 2)
    {
        TestEqual (TEXT ("The first adjacent action is Turn"),
            Actions[0].Type, EGridCombatActionType::Turn);
        TestEqual (TEXT ("The second adjacent action is MeleeAttack"),
            Actions[1].Type, EGridCombatActionType::MeleeAttack);
    ...
    if (Actions.Num () == 2)
    {
        TestEqual (TEXT ("Distance two starts with Move"),
            Actions[0].Type, EGridCombatActionType::Move);
        TestEqual (TEXT ("Distance two ends with MeleeAttack"),
            Actions[1].Type, EGridCombatActionType::MeleeAttack);
    }
    ...
    if (Actions.Num () == 2)
    {
        TestEqual (TEXT ("First distant action is Move"),
            Actions[0].Type, EGridCombatActionType::Move);
        TestEqual (TEXT ("Second distant action is Move"),
