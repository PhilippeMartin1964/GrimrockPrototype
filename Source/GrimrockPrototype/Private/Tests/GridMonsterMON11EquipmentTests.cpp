...
namespace
{
    ...
    FGridOffensiveEquipmentProfile MakeOffensiveProfile (
        FName AttackId,
    ...
    UGridItemDefinitionAsset* MakeEquipmentDefinition (
        UObject* Outer,
        FName ItemDefinitionId,
        EGridItemType ItemType,
        const FGridOffensiveEquipmentProfile& Profile,
        EGridEquipmentSlot CompatibleSlot,
        bool bProvidesAttack = true)
    {
    ...
    int32 CountPlayerAttackEntries (
    ...
    struct FGridMON11EquipmentFixture
    {
        ...
        explicit FGridMON11EquipmentFixture (
            FIntPoint MonsterCell = FIntPoint (2, 2),
            bool bSpawnInitialMonster = true)
        {
            if (!TestWorld.World)
            {
                return;
            ...
            Character.Attributes = FRPGAttributes {
                18, 16, 14, 18, 12, 10
            };
            ...
            MonsterDefinition->MonsterId =
                TEXT ("MON11_EquipmentRat");
            MonsterDefinition->DisplayName =
                FText::FromString (TEXT ("Rat d'équipement"));
            ...
            TurnManager = NewObject<UGridTurnManagerComponent> (
                Runtime,
                TEXT ("MON11EquipmentTurnManager"));
        ...
        AGridMonsterActor* SpawnMonster (FIntPoint Cell)
        {
            ...
            AGridMonsterActor* NewMonster =
                TestWorld.World->SpawnActor<AGridMonsterActor> ();
            NewMonster->InitializeMonster (
                MonsterDefinition,
                FGuid (113, 2, 0, NextMonsterId++),
                Cell,
                EGridEdge::South);
        ...
        void SetOnlyCombatMonsters (
            std::initializer_list<AGridMonsterActor*> Monsters)
        {
        ...
        void Equip (
            ...
            EGridEquipmentSlot Slot,
            bool bRegisterDefinition = true)
        {
            if (bRegisterDefinition)
            {
                Party->PartyInventoryComponent->RegisterItemDefinition (
                    const_cast<UGridItemDefinitionAsset*> (Definition));
            }
            FGridItemInstance* TargetSlot =
                Party->PartyInventoryComponent->PartyInventoryState
                    .ActiveEquipment[0].GetMutableSlot (Slot);
            if (TargetSlot)
            {
                *TargetSlot = MakeEquippedItem (
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON11OffensiveProfileValidationTest,
    "Grimrock.Monsters.MON11.OffensiveProfileValidation",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON11OffensiveProfileValidationTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    const FGridOffensiveEquipmentProfile PhysicalProfile =
        MakeOffensiveProfile (
            TEXT ("Attack_TestPhysical"),
            ...
            1);
    TestTrue (
        TEXT ("A physical offensive profile is valid"),
        PhysicalProfile.IsValid ());

    const FGridOffensiveEquipmentProfile ElementalProfile =
        MakeOffensiveProfile (
            TEXT ("Attack_TestFire"),
            ...
            3);
    TestTrue (
        TEXT ("An elemental profile with no physical subtype is valid"),
        ElementalProfile.IsValid ());
    ...
    TestFalse (
        TEXT ("MinDamage above MaxDamage is rejected"),
        Invalid.IsValid ());
    Invalid = PhysicalProfile;
    Invalid.AttackDefinition.MinDamage = 0;
    Invalid.AttackDefinition.MaxDamage = 0;
    TestFalse (
        TEXT ("A zero MaxDamage is rejected"),
        Invalid.IsValid ());
    Invalid = PhysicalProfile;
    ...
    Invalid.AttackDefinition.PhysicalSubtype =
        EGridPhysicalDamageSubtype::Piercing;
    TestFalse (
        TEXT ("An elemental physical subtype is rejected"),
        Invalid.IsValid ());

    UGridItemDefinitionAsset* IncompatibleDefinition =
        MakeEquipmentDefinition (
            GetTransientPackage (),
            TEXT ("Item_Incompatible"),
            EGridItemType::Weapon,
            PhysicalProfile,
            EGridEquipmentSlot::Head);
    TestFalse (
        TEXT ("An offensive definition incompatible with both hands is invalid"),
        IncompatibleDefinition->IsValidDefinition ());

    FGridMON11EquipmentFixture Fixture;
    if (!TestTrue (
        TEXT ("The rejection fixture is ready"),
        Fixture.IsReady () && Fixture.Monster))
    {
        return false;
    }
    ...
    TestFalse (
        TEXT ("A missing equipped definition is rejected"),
        Fixture.TurnManager->RequestCharacterAttack (
            ...
            RejectReason));
    TestEqual (
        TEXT ("The missing definition reason is explicit"),
        RejectReason,
        EGridPlayerAttackRejectReason::
            EquippedItemDefinitionUnavailable);
    TestEqual (
        TEXT ("The missing definition consumes no random draw"),
    ...
    Fixture.Equip (
        ...
        EGridEquipmentSlot::MainHand);
    TestFalse (
        TEXT ("An invalid offensive profile is rejected"),
        Fixture.TurnManager->RequestCharacterAttack (
            0,
            Request,
            Result,
    ...
    TestEqual (
        ...
        0);
    TestEqual (
        TEXT ("No rejected attack emits Requested"),
        Fixture.TurnManager->PlayerAttackRequestedBroadcastCount,
        0);
    TestEqual (
        TEXT ("No rejected attack emits Resolved"),
        Fixture.TurnManager->PlayerAttackResolvedBroadcastCount,
        0);
    TestEqual (
        TEXT ("Rejected attacks leave health unchanged"),
        Fixture.Monster->CurrentHealth,
        HealthBefore);
    TestEqual (
        TEXT ("Rejected attacks leave armor unchanged"),
        Fixture.Monster->CurrentPhysicalArmor,
        ArmorBefore);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON11EquippedWeaponMappingTest,
    "Grimrock.Monsters.MON11.EquippedWeaponMapping",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON11EquippedWeaponMappingTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    FGridMON11EquipmentFixture Fixture;
    if (!TestTrue (
        TEXT ("The sword fixture is ready"),
        Fixture.IsReady () && Fixture.Monster))
    {
        return false;
    }

    const FGridOffensiveEquipmentProfile SwordProfile =
        MakeOffensiveProfile (
            TEXT ("Attack_Sword"),
            EGridDamageType::Physical,
            EGridPhysicalDamageSubtype::Slashing,
    ...
    SlashingModifier.PhysicalSubtype =
        EGridPhysicalDamageSubtype::Slashing;
    SlashingModifier.DamageMultiplier = 1.25f;
    Fixture.MonsterDefinition->DamageModifiers.Add (
        SlashingModifier);

    FGridInventoryCharacterSummary Summary;
    TestTrue (
        TEXT ("The final sword wielder summary is available"),
    ...
    TestTrue (
        TEXT ("The exact sword inputs are built"),
        Fixture.TurnManager->BuildPlayerAttackResolutionInputs (
            ...
            Attack));
    TestEqual (
        TEXT ("Final derived accuracy is used"),
        Source.Accuracy,
        Summary.DerivedStats.Accuracy);
    TestEqual (
        TEXT ("Final Strength drives sword damage"),
        Source.DamageBonus,
        URPGCharacterRulesLibrary::GetAttributeModifier (
            Summary.Attributes.Strength));
    TestEqual (
        TEXT ("The sword definition is copied exactly"),
        Attack.MaxDamage,
        SwordProfile.AttackDefinition.MaxDamage);
    TestEqual (
        TEXT ("The Slashing multiplier is selected"),
    ...
    EGridPlayerAttackRejectReason RejectReason =
        EGridPlayerAttackRejectReason::None;
    TestTrue (
        TEXT ("The equipped sword attack resolves"),
        Fixture.TurnManager->RequestCharacterAttack (
            0,
            Request,
            Result,
            RejectReason));
    TestEqual (
        TEXT ("The sword AttackId is retained"),
        Request.AttackId,
        FName (TEXT ("Attack_Sword")));
    TestEqual (
        TEXT ("The real sword item id is retained"),
        Request.OffensiveItemDefinitionId,
        SwordDefinition->ItemDefinitionId);
    TestEqual (
        TEXT ("The real MainHand slot is retained"),
        Request.OffensiveEquipmentSlot,
        EGridEquipmentSlot::MainHand);
    TestEqual (
        TEXT ("The result retains Slashing"),
        Result.PhysicalSubtype,
        EGridPhysicalDamageSubtype::Slashing);
    TestEqual (
        TEXT ("Exactly one result delegate is emitted"),
        Fixture.TurnManager->PlayerAttackResolvedBroadcastCount,
        1);
    TestEqual (
        TEXT ("Exactly one attack log is emitted"),
        CountPlayerAttackEntries (Fixture.TurnManager),
        1);
    TestEqual (
        TEXT ("Physical armor is applied exactly once"),
        Fixture.Monster->CurrentPhysicalArmor,
        ArmorBefore - Result.PhysicalArmorDamage);
    TestEqual (
        TEXT ("Health is applied exactly once"),
        Fixture.Monster->CurrentHealth,
        HealthBefore - Result.HealthDamage);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON11HandPriorityAndFallbackTest,
    "Grimrock.Monsters.MON11.HandPriorityAndFallback",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON11HandPriorityAndFallbackTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    FGridMON11EquipmentFixture Fixture;
    if (!TestTrue (
        TEXT ("The hand-priority fixture is ready"),
        Fixture.IsReady () && Fixture.Monster))
    {
        return false;
    }

    const FGridOffensiveEquipmentProfile MainProfile =
        MakeOffensiveProfile (
            TEXT ("Attack_Main"),
            EGridDamageType::Physical,
            EGridPhysicalDamageSubtype::Slashing,
            ...
            EGridAttackScalingAttribute::Strength,
            1);
    const FGridOffensiveEquipmentProfile OffProfile =
        MakeOffensiveProfile (
            TEXT ("Attack_Off"),
            EGridDamageType::Physical,
            EGridPhysicalDamageSubtype::Piercing,
    ...
    UGridItemDefinitionAsset* TorchDefinition =
        MakeEquipmentDefinition (
            ...
            TEXT ("Item_Torch"),
            EGridItemType::Torch,
            FGridOffensiveEquipmentProfile (),
            EGridEquipmentSlot::MainHand,
            false);
    UGridItemDefinitionAsset* MainDefinition =
        MakeEquipmentDefinition (
            Fixture.Runtime,
            TEXT ("Item_MainWeapon"),
            EGridItemType::Weapon,
            MainProfile,
            EGridEquipmentSlot::MainHand);
    ...
    EGridPlayerAttackRejectReason RejectReason =
        EGridPlayerAttackRejectReason::None;
    TestTrue (
        TEXT ("A non-offensive main hand allows the off hand"),
        Fixture.TurnManager->RequestCharacterAttack (
            0,
            Request,
            Result,
            RejectReason));
    TestEqual (
        TEXT ("OffHand is selected behind a non-offensive MainHand"),
        Request.OffensiveEquipmentSlot,
        EGridEquipmentSlot::OffHand);
    TestEqual (
        TEXT ("The off-hand attack is not combined"),
        Request.AttackId,
        OffProfile.AttackId);
    ...
    Fixture.Equip (
        ...
        EGridEquipmentSlot::MainHand);
    TestTrue (
        TEXT ("Two offensive hands still resolve once"),
        Fixture.TurnManager->RequestCharacterAttack (
            0,
            Request,
            Result,
            RejectReason));
    TestEqual (
        TEXT ("MainHand wins over an offensive OffHand"),
        Request.OffensiveEquipmentSlot,
        EGridEquipmentSlot::MainHand);
    TestEqual (
        TEXT ("Only the main-hand attack is selected"),
        Request.AttackId,
    ...
    Fixture.Equip (
        ...
        EGridEquipmentSlot::OffHand);
    TestTrue (
        TEXT ("Two non-offensive hands fall back"),
        Fixture.TurnManager->RequestCharacterAttack (
            0,
            Request,
            Result,
            RejectReason));
    TestEqual (
        TEXT ("The fallback is Attack_Unarmed"),
        Request.AttackId,
    ...
    Fixture.Equip (
        ...
        EGridEquipmentSlot::Head);
    TestTrue (
        TEXT ("An offensive non-hand slot does not become active"),
        Fixture.TurnManager->RequestCharacterAttack (
            0,
            Request,
            Result,
            RejectReason));
    TestEqual (
        TEXT ("The non-hand profile is ignored"),
        Request.AttackId,
        FName (TEXT ("Attack_Unarmed")));
    TestEqual (
        TEXT ("Four commands produce four resolutions only"),
        Fixture.TurnManager->PlayerAttackResolvedBroadcastCount,
        4);
    TestEqual (
        TEXT ("Four commands produce four attack logs only"),
        CountPlayerAttackEntries (Fixture.TurnManager),
        4);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON11RangedWeaponTargetingTest,
    "Grimrock.Monsters.MON11.RangedWeaponTargeting",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON11RangedWeaponTargetingTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    const FGridOffensiveEquipmentProfile RangedProfile =
        MakeOffensiveProfile (
            TEXT ("Attack_RangeThree"),
            EGridDamageType::Physical,
            EGridPhysicalDamageSubtype::Piercing,
            1,
            4,
...

    {
        ...
        EGridPlayerAttackRejectReason RejectReason =
            EGridPlayerAttackRejectReason::None;
        TestTrue (
            TEXT ("A target at three cells is reached"),
            Fixture.TurnManager->RequestCharacterAttack (
                0,
                Request,
                Result,
                RejectReason));
        TestEqual (
            TEXT ("The exact three-cell target is retained"),
            Request.TargetCell,
            FIntPoint (2, 4));
        TestEqual (
            TEXT ("The request retains range three"),
            Request.RangeCells,
            3);
    }

    {
        FGridMON11EquipmentFixture Fixture (
            FIntPoint (2, 3));
        AGridMonsterActor* RearMonster =
            Fixture.SpawnMonster (FIntPoint (2, 4));
        Fixture.SetOnlyCombatMonsters ({
            Fixture.Monster,
            RearMonster
        });
        UGridItemDefinitionAsset* Weapon =
        ...
        EGridPlayerAttackRejectReason RejectReason =
            EGridPlayerAttackRejectReason::None;
        TestTrue (
            TEXT ("The first monster in line is selected"),
            Fixture.TurnManager->RequestCharacterAttack (
                0,
                Request,
                Result,
                RejectReason));
        TestEqual (
            TEXT ("No attack passes through the first monster"),
            Request.TargetMonsterId,
            Fixture.Monster->ResolvePersistenceId ());
        TestEqual (
            TEXT ("The first monster cell is retained"),
            Request.TargetCell,
            FIntPoint (2, 3));
    }

    {
        ...
        UGridItemDefinitionAsset* Weapon =
            MakeEquipmentDefinition (
                Fixture.Runtime,
                TEXT ("Item_BlockedRange"),
                EGridItemType::Weapon,
                RangedProfile,
                EGridEquipmentSlot::MainHand);
        Fixture.Equip (Weapon, EGridEquipmentSlot::MainHand);
        Fixture.LevelAsset->GetCellMutable (2, 2).NorthWall =
            EGridWallType::Solid;
        ...
        EGridPlayerAttackRejectReason RejectReason =
            EGridPlayerAttackRejectReason::None;
        TestFalse (
            TEXT ("A wall blocks the ranged attack"),
            Fixture.TurnManager->RequestCharacterAttack (
                0,
                Request,
                Result,
                RejectReason));
        TestEqual (
            TEXT ("The wall reports PassageBlocked"),
            RejectReason,
            EGridPlayerAttackRejectReason::PassageBlocked);
        TestEqual (
            TEXT ("The blocked request consumes no random draw"),
            Fixture.TurnManager->CombatRandomStream.GetCurrentSeed (),
            BlockedSeed);
    }

    {
        ...
        EGridPlayerAttackRejectReason RejectReason =
            EGridPlayerAttackRejectReason::None;
        TestFalse (
            TEXT ("A target at four cells is rejected"),
            Fixture.TurnManager->RequestCharacterAttack (
                0,
                Request,
                Result,
                RejectReason));
        TestEqual (
            TEXT ("The fourth cell reports TargetOutOfRange"),
            RejectReason,
...

    {
        ...
        EGridPlayerAttackRejectReason RejectReason =
            EGridPlayerAttackRejectReason::None;
        TestFalse (
            TEXT ("An empty line has no target"),
            Fixture.TurnManager->RequestCharacterAttack (
                0,
                Request,
                Result,
                RejectReason));
        TestEqual (
            TEXT ("An empty line reports NoMonsterInFront"),
            RejectReason,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON11ElementalOffensiveEquipmentTest,
    "Grimrock.Monsters.MON11.ElementalOffensiveEquipment",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON11ElementalOffensiveEquipmentTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    FGridMON11EquipmentFixture Fixture (FIntPoint (2, 3));
    if (!TestTrue (
        TEXT ("The elemental fixture is ready"),
        Fixture.IsReady () && Fixture.Monster))
    {
        return false;
    }

    const FGridOffensiveEquipmentProfile FireProfile =
        MakeOffensiveProfile (
            TEXT ("Attack_FireFocus"),
            EGridDamageType::Fire,
            EGridPhysicalDamageSubtype::None,
            1,
            6,
    ...
    Fixture.Equip (
        ...
        EGridEquipmentSlot::MainHand);
    TestTrue (
        TEXT ("A non-Weapon item can provide an attack"),
        FocusDefinition->CanProvideAttackFromSlot (
            EGridEquipmentSlot::MainHand));

    FGridMonsterDamageModifier FireModifier;
    FireModifier.DamageType = EGridDamageType::Fire;
    FireModifier.PhysicalSubtype =
        EGridPhysicalDamageSubtype::None;
    ...
    FGridInventoryCharacterSummary Summary;
    TestTrue (
        TEXT ("The final focus wielder summary is available"),
        Fixture.Party->PartyInventoryComponent->GetCharacterSummary (
            0,
            Summary));
    FGridAttackSourceStats Source;
    ...
    FGridAttackDefinition Attack;
    TestTrue (
        TEXT ("The elemental inputs are built"),
        Fixture.TurnManager->BuildPlayerAttackResolutionInputs (
            Summary,
            Fixture.Monster,
            FireProfile,
            ...
            Attack));
    TestEqual (
        TEXT ("Final Intelligence plus the flat bonus is used"),
        Source.DamageBonus,
        2 + URPGCharacterRulesLibrary::GetAttributeModifier (
            Summary.Attributes.Intelligence));
    TestEqual (
        TEXT ("The Fire multiplier is selected"),
        Target.DamageMultiplier,
        2.0f);

    const int32 NaturalTwentySeed =
        FindSeedForNaturalRoll (20);
    TestTrue (
        TEXT ("A critical seed is available"),
        NaturalTwentySeed != INDEX_NONE);
    Fixture.TurnManager->CombatRandomStream.Initialize (
        NaturalTwentySeed);
    const int32 PhysicalArmorBefore =
        Fixture.Monster->CurrentPhysicalArmor;
    const int32 MagicalArmorBefore =
    ...
    EGridPlayerAttackRejectReason RejectReason =
        EGridPlayerAttackRejectReason::None;
    TestTrue (
        TEXT ("The Fire focus attack resolves"),
        Fixture.TurnManager->RequestCharacterAttack (
            0,
            Request,
            Result,
            RejectReason));
    TestEqual (
        TEXT ("The request retains the focus id"),
        Request.OffensiveItemDefinitionId,
        FocusDefinition->ItemDefinitionId);
    TestEqual (
        TEXT ("The request retains MainHand"),
        Request.OffensiveEquipmentSlot,
        EGridEquipmentSlot::MainHand);
    TestEqual (
        TEXT ("The request retains Attack_FireFocus"),
        Request.AttackId,
        FireProfile.AttackId);
    TestEqual (
        TEXT ("Physical armor is unchanged by Fire"),
        Fixture.Monster->CurrentPhysicalArmor,
        PhysicalArmorBefore);
    TestEqual (
        TEXT ("Magical armor absorbs Fire first"),
        Fixture.Monster->CurrentMagicalArmor,
        MagicalArmorBefore - Result.MagicalArmorDamage);
    TestEqual (
        TEXT ("Only remaining Fire damage reaches health"),
        Fixture.Monster->CurrentHealth,
        HealthBefore - Result.HealthDamage);
    TestTrue (
        TEXT ("The structured log retains the equipment identity"),
        !Fixture.TurnManager->CombatLogEntries.IsEmpty () &&
            Fixture.TurnManager->CombatLogEntries.Last ()
                .OffensiveItemDefinitionId ==
                FocusDefinition->ItemDefinitionId);
    TestEqual (
        TEXT ("The structured log retains the slot"),
        Fixture.TurnManager->CombatLogEntries.Last ()
