...
namespace
{
	struct FGridMON141TestWorld
	{
		...
		FGridMON141TestWorld()
		{
			...
			World = UWorld::CreateWorld(EWorldType::Game, false,
				FName(*FString::Printf(TEXT("MON141TestWorld_%s"), *FGuid::NewGuid().ToString(EGuidFormats::Digits))), nullptr, true, ERHIFeatureLevel::Num,
	...
	struct FGridMON141Fixture
	{
		...
		bool Initialize(FIntPoint PartyCell = FIntPoint(1, 1))
		{
			...
			TurnManager = NewObject<UGridTurnManagerComponent>(Runtime, TEXT("MON141TurnManager"));
			...
			if (UGridDoorSystemComponent* Doors = Runtime->FindComponentByClass<UGridDoorSystemComponent>())
			{
				Doors->Initialize(Runtime);
				Doors->RebuildIndexes();
			}
			if (UGridMonsterEncounterComponent* Encounters = Runtime->FindComponentByClass<UGridMonsterEncounterComponent>())
			{
				Encounters->Initialize(Runtime);
			}

			Engagement = World->GetSubsystem<UGridAutomaticPerceptionEngagementSubsystem>();
			return Engagement != nullptr;
		}

		UGridMonsterDefinitionAsset* MakeDefinition(FName MonsterId, int32 SightRange, int32 HearingRange, bool bSharesAggro = false, int32 AggroRange = 0)
		{
		...
		AGridMonsterActor* AddMonster(UGridMonsterDefinitionAsset* Definition, FIntPoint Cell, FName GroupId = NAME_None, FGuid Id = FGuid())
		{
			...
			if (!Monster->InitializeMonster(Definition, StableId, Cell, EGridEdge::South, GroupId))
			{
				return nullptr;
			}

			UGridMonsterMovementComponent* Movement = NewObject<UGridMonsterMovementComponent>(Monster, TEXT("MON141Movement"));
			Movement->bAutoInitialize = false;
			Movement->bInferCellFromActorLocation = false;
			Monster->AddInstanceComponent(Movement);
			Movement->RegisterComponent();

			UGridMonsterBehaviorComponent* Behavior = NewObject<UGridMonsterBehaviorComponent>(Monster, TEXT("MON141Behavior"));
			Behavior->bAutoInitialize = false;
			Monster->AddInstanceComponent(Behavior);
			Behavior->RegisterComponent();

			if (!Movement->InitializeMovement(Runtime) || !Behavior->InitializeBehavior(Runtime, Party) || !Monster->CombatComponent ||
	...
	int32 CountParticipant(const UGridTurnManagerComponent* TurnManager, const AGridMonsterActor* Monster)
	{
		...
		int32 Count = 0;
		for (const TObjectPtr<AGridMonsterActor>& Candidate : TurnManager->CombatMonsters)
		{
	...
	UGridMonsterDefinitionAsset* MakeAssetBackedDefinition(FAutomationTestBase& Test, UObject* Outer, FName MonsterId, int32 SightRange, int32 HearingRange)
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMonsterMON141DirectSightTest, "Grimrock.Monsters.MON14.1.DirectSight", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMonsterMON141DirectSightTest::RunTest(const FString& Parameters)
{
	...
	UGridMonsterDefinitionAsset* Definition = Fixture.MakeDefinition(TEXT("MON14_Direct"), 5, 0);
	AGridMonsterActor* Monster = Fixture.AddMonster(Definition, FIntPoint(1, 4));
	...
	TestEqual(TEXT("Only the direct monster participates"), Fixture.TurnManager->CombatMonsters.Num(), 1);
	TestEqual(TEXT("Direct monster is deduplicated"), CountParticipant(Fixture.TurnManager, Monster), 1);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMonsterMON141RangeWallDoorTest, "Grimrock.Monsters.MON14.1.RangeWallDoor", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMonsterMON141RangeWallDoorTest::RunTest(const FString& Parameters)
{
...

	{
		...
		if (!Fixture.Initialize())
			return false;
		AGridMonsterActor* Monster = Fixture.AddMonster(Fixture.MakeDefinition(TEXT("MON14_Range"), 3, 0), FIntPoint(1, 7));
...

	{
		...
		AGridMonsterActor* Monster = Fixture.AddMonster(Fixture.MakeDefinition(TEXT("MON14_Wall"), 6, 0), FIntPoint(1, 4));
...

	{
		...
		if (!Fixture.Initialize())
			return false;
		UGridDoorSystemComponent* Doors = Fixture.Runtime->FindComponentByClass<UGridDoorSystemComponent>();
		TestNotNull(TEXT("Door system exists"), Doors);
		if (!Doors)
			return false;
		Doors->SetDoorPassageBlocked(1, 3, EGridEdge::South, true);
		AGridMonsterActor* Monster = Fixture.AddMonster(Fixture.MakeDefinition(TEXT("MON14_Door"), 6, 0), FIntPoint(1, 4));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMonsterMON141HearingDormantManualTest, "Grimrock.Monsters.MON14.1.HearingDormantManual",
...
bool FGridMonsterMON141HearingDormantManualTest::RunTest(const FString& Parameters)
{
...

	{
		...
		if (!Fixture.Initialize())
			return false;
		AGridMonsterActor* Monster = Fixture.AddMonster(Fixture.MakeDefinition(TEXT("MON14_Hearing"), 8, 3), FIntPoint(2, 2));
		...
		if (Behavior && Monster)
		{
			...
			TestTrue(TEXT("Last known cell matches the party"), Behavior->LastKnownPartyCell == FIntPoint(1, 1));
			TestEqual(TEXT("Hearing changes Idle to Alert"), Monster->MonsterState, EGridMonsterState::Alert);
		}
		TestTrue(TEXT("Historical manual perception start still accepts hearing"), Fixture.TurnManager->StartCombatFromPerception());
	}

	{
		...
		if (!Fixture.Initialize())
			return false;
		AGridMonsterActor* Monster = Fixture.AddMonster(Fixture.MakeDefinition(TEXT("MON14_Dormant"), 5, 0), FIntPoint(1, 4));
		...
		TestEqual(TEXT("Dormant monster wakes to Alert before combat behavior"), Monster->MonsterState, EGridMonsterState::Alert);
	}
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMonsterMON141ExclusionTest, "Grimrock.Monsters.MON14.1.Exclusions", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMonsterMON141ExclusionTest::RunTest(const FString& Parameters)
{
	...
	UGridMonsterDefinitionAsset* Definition = Fixture.MakeDefinition(TEXT("MON14_Excluded"), 6, 0);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMonsterMON141CoalescingAggroTest, "Grimrock.Monsters.MON14.1.CoalescingAggro",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMonsterMON141CoalescingAggroTest::RunTest(const FString& Parameters)
{
	...
	UGridMonsterDefinitionAsset* Definition = Fixture.MakeDefinition(TEXT("MON14_GroupRat"), 6, 0, true, 4);
	AGridMonsterActor* Source = Fixture.AddMonster(Definition, FIntPoint(1, 3), TEXT("RoomA"), FGuid(14, 1, 1, 1));
	AGridMonsterActor* Propagated = Fixture.AddMonster(Definition, FIntPoint(4, 3), TEXT("RoomA"), FGuid(14, 1, 1, 2));
	...
	TestEqual(TEXT("All notifications are counted"), Fixture.Engagement->GetQueuedRequestCount(), InitialQueued + 3);
	TestTrue(TEXT("Duplicate notifications are coalesced into one pending evaluation"), Fixture.Engagement->HasPendingEvaluation());
	TestTrue(TEXT("The coalesced visual evaluation starts combat"), Fixture.Engagement->ProcessPendingEvaluationNow());
	TestEqual(TEXT("Only one effective evaluation ran"), Fixture.Engagement->GetEffectiveEvaluationCount(), 1);
	TestEqual(TEXT("Only one automatic start succeeded"), Fixture.Engagement->GetSuccessfulStartCount(), 1);
	TestEqual(TEXT("MON7 adds the propagated group member"), Fixture.TurnManager->CombatMonsters.Num(), 2);
	...
	TestEqual(TEXT("Propagated member becomes Alert"), Propagated->MonsterState, EGridMonsterState::Alert);
	TestFalse(TEXT("No second pending evaluation means no second start"), Fixture.Engagement->ProcessPendingEvaluationNow());
	TestEqual(TEXT("Successful start count remains one"), Fixture.Engagement->GetSuccessfulStartCount(), 1);
	return true;
}
...
bool FGridMonsterMON141TransitionRebuildSafetyTest::RunTest(const FString& Parameters)
{
	...
	if (!RebuildFixture.Initialize())
		return false;
	AGridMonsterActor* RebuildMonster = RebuildFixture.AddMonster(RebuildFixture.MakeDefinition(TEXT("MON14_Rebuild"), 6, 0), FIntPoint(1, 4));
	TestNotNull(TEXT("Rebuild guard monster exists"), RebuildMonster);
	UGridDoorSystemComponent* Doors = RebuildFixture.Runtime->FindComponentByClass<UGridDoorSystemComponent>();
	TestNotNull(TEXT("Rebuild door system exists"), Doors);
	if (!Doors)
		return false;
	const int32 Before = RebuildFixture.Engagement->GetQueuedRequestCount();
	Doors->RebuildIndexes();
	TestTrue(TEXT("Runtime rebuild requests a deferred perception evaluation"), RebuildFixture.Engagement->GetQueuedRequestCount() > Before);
	TestFalse(TEXT("Rebuild notification itself does not synchronously start combat"), RebuildFixture.TurnManager->bCombatActive);
	TestTrue(TEXT("Post-rebuild safe evaluation may engage visible monster"), RebuildFixture.Engagement->ProcessPendingEvaluationNow());
	return true;
}
