...
namespace
{
	struct FTestGrid
	{
		...
		{
			return FGridMonsterPathfinder::GetDirectionBetweenAdjacentCells(From, To) != EGridEdge::None && !BlockedEdges.Contains(MakeEdgeKey(From, To));
		}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMonsterMON4PathfinderTest, "Grimrock.Monsters.MON4.Pathfinder", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMonsterMON4PathfinderTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Direct path is found"), FGridMonsterPathfinder::FindPath(Query, Grid.MakeContext(), Result));
	...
	Result.Reset();
	TestTrue(TEXT("BFS routes around a blocked edge"), FGridMonsterPathfinder::FindPath(Query, Grid.MakeContext(), Result));
	if (Result.Cells.Num() >= 1)
	{
	...
	Result.Reset();
	TestTrue(TEXT("Occupied cell is avoided"), FGridMonsterPathfinder::FindPath(Query, Grid.MakeContext(), Result));
	...
	Result.Reset();
	TestTrue(TEXT("Reserved cell is avoided like an occupied cell"), FGridMonsterPathfinder::FindPath(Query, Grid.MakeContext(), Result));
	...
	Result.Reset();
	TestFalse(TEXT("Blocked goal is rejected by default"), FGridMonsterPathfinder::FindPath(Query, Grid.MakeContext(), Result));

	Query.bAllowBlockedGoal = true;
	Result.Reset();
	TestTrue(TEXT("Explicitly allowed blocked goal can be reached"), FGridMonsterPathfinder::FindPath(Query, Grid.MakeContext(), Result));
	...
	Query.bAllowBlockedGoal = false;
	Result.Reset();
	TestTrue(TEXT("One of multiple goals is found"), FGridMonsterPathfinder::FindPath(Query, Grid.MakeContext(), Result));
	TestTrue(TEXT("N,E,S,W order deterministically selects North goal"), Result.ReachedGoal == FIntPoint(0, 2));
	...
	Query.Goals = { FIntPoint(2, 2) };
	Result.Reset();
	TestFalse(TEXT("No path is returned from a sealed cell"), FGridMonsterPathfinder::FindPath(Query, ClosedGrid.MakeContext(), Result));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMonsterMON4PerceptionTest, "Grimrock.Monsters.MON4.Perception", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMonsterMON4PerceptionTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Straight unobstructed party is visible"), FGridMonsterPerception::HasStraightLineOfSight(FIntPoint(1, 1), FIntPoint(1, 4), 5, CanTraverse));

	Grid.BlockedEdges.Add(FTestGrid::MakeEdgeKey(FIntPoint(1, 2), FIntPoint(1, 3)));
	TestFalse(TEXT("Wall or closed door blocks sight"), FGridMonsterPerception::HasStraightLineOfSight(FIntPoint(1, 1), FIntPoint(1, 4), 5, CanTraverse));
