...
namespace RPGCustomRecruitMON205WizardContextTests
{
    ...
    UGridPartyInventoryComponent* MakeCompletedParty (
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON205WizardContextDefaultTest,
    "Grimrock.MON20.5.CustomRecruit.WizardContextDefault",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FRPGMON205WizardContextDefaultTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;

    URPGCharacterCreationWidget* Widget =
        NewObject<URPGCharacterCreationWidget> ();
    TestTrue (
        TEXT ("Default context remains New Game"),
        Widget->GetCreationContext () ==
            ERPGCharacterCreationContext::NewGameMainHero);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON205ContextGateCompletedPartyTest,
    "Grimrock.MON20.5.CustomRecruit.ContextGateCompletedParty",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FRPGMON205ContextGateCompletedPartyTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    URPGCharacterCreationWidget* Widget =
        ConfigureWidget<URPGCharacterCreationWidget> (
            Inventory,
            MakeRace (),
            MakeClass (),
    ...
    TestTrue (
        TEXT ("Completed party accepts CustomRecruit submit state"),
        Widget->CanSubmitCharacterCreation ());

    Widget->CreationContext =
        ERPGCharacterCreationContext::NewGameMainHero;
    TestFalse (
        TEXT ("Same completed party rejects NewGame submit state"),
        Widget->CanSubmitCharacterCreation ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON205ContextGateIncompletePartyTest,
    "Grimrock.MON20.5.CustomRecruit.ContextGateIncompleteParty",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FRPGMON205ContextGateIncompletePartyTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGCustomRecruitMON205WizardContextTests;
    ...
    TestFalse (
        TEXT ("Custom recruit cannot precede initial hero creation"),
        Widget->CanSubmitCharacterCreation ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON205ContextGatePartyFullTest,
    "Grimrock.MON20.5.CustomRecruit.ContextGatePartyFull",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FRPGMON205ContextGatePartyFullTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGCustomRecruitMON205WizardContextTests;
    ...
    TestFalse (
        TEXT ("Full active party disables CustomRecruit submit"),
        Widget->CanSubmitCharacterCreation ());
    TestEqual (
        TEXT ("Full-party gate does not create a pool candidate"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON205WizardCustomRecruitSubmitTest,
    "Grimrock.MON20.5.CustomRecruit.WizardCustomRecruitSubmit",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FRPGMON205WizardCustomRecruitSubmitTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGCustomRecruitMON205WizardContextTests;
    ...
    Wizard->OnCustomRecruitCommitted ().AddLambda (
        [&CommitCallbackCount, &CommittedCharacterIndex] (
    ...
    TestTrue (
        TEXT ("Wizard custom submit commits recruitment"),
        Wizard->SubmitCharacterCreation ());
    TestEqual (
        TEXT ("Active party gains exactly one recruit"),
        Inventory->PartyInventoryState.ActiveCharacters.Num (),
        2);
    TestTrue (
        TEXT ("Initial hero identity is preserved"),
        Inventory->PartyInventoryState.ActiveCharacters[0].CharacterId ==
            InitialHeroId);
    TestEqual (
        TEXT ("Temporary pool staging is consumed"),
        Inventory->PartyInventoryState.CharacterPool.Num (),
        0);
    TestEqual (
        TEXT ("Custom recruit commit delegate fires once"),
        CommitCallbackCount,
        1);
    TestEqual (
        TEXT ("Delegate reports recruited active index"),
        CommittedCharacterIndex,
        1);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON205WizardCustomCancelNoMutationTest,
    "Grimrock.MON20.5.CustomRecruit.WizardCustomCancelNoMutation",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FRPGMON205WizardCustomCancelNoMutationTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGCustomRecruitMON205WizardContextTests;
    ...
    const int32 ActiveCountBefore =
        Inventory->PartyInventoryState.ActiveCharacters.Num ();
    const int32 PoolCountBefore =
        Inventory->PartyInventoryState.CharacterPool.Num ();

    Wizard->CancelWizard ();

    TestEqual (
        TEXT ("Custom cancel delegate fires once"),
        CancelCallbackCount,
        1);
    TestEqual (
        TEXT ("Custom cancel does not change active party"),
        Inventory->PartyInventoryState.ActiveCharacters.Num (),
        ActiveCountBefore);
    TestEqual (
        TEXT ("Custom cancel does not change candidate pool"),
        Inventory->PartyInventoryState.CharacterPool.Num (),
        PoolCountBefore);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON205WizardNewGameSubmitRegressionTest,
    "Grimrock.MON20.5.CustomRecruit.WizardNewGameSubmitRegression",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FRPGMON205WizardNewGameSubmitRegressionTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGCustomRecruitMON205WizardContextTests;
    ...
    TestTrue (
        TEXT ("New Game submit remains functional"),
        Wizard->SubmitCharacterCreation ());
    TestTrue (
        TEXT ("Initial character creation is marked completed"),
        Inventory->HasCompletedInitialCharacterCreation ());
    TestEqual (
        TEXT ("New Game creates exactly one active hero"),
        Inventory->PartyInventoryState.ActiveCharacters.Num (),
        1);
    TestEqual (
        TEXT ("New Game does not emit custom recruit callback"),
        CustomCommitCallbackCount,
        0);
    return true;
}
