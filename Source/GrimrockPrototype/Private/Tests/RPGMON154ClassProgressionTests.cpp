...
namespace
{
	URPGClassAsset* MakeMON154Class(UObject* Outer)
	{
		...
		ClassDefinition->DisplayName = FText::FromString(TEXT("MON15.4 Fighter"));
	...
	FGridCombatActionDefinition MakeMON154EffectAction(FName ActionId, FName RequirementId)
	{
	...
	FGridCombatActionCatalogContext MakeMON154CatalogContext(const TSet<FName>& SatisfiedRequirements)
	{
	...
	FGridAvailableCombatAction BuildMON154AvailableAction(
		const URPGClassAsset* ClassDefinition, const FGridCombatActionDefinition& Definition, const TSet<FName>& SatisfiedRequirements)
	{
		FGridCombatActionContribution Contribution;
		Contribution.Definition = Definition;
		Contribution.SourceDefinitionId = ClassDefinition ? ClassDefinition->ClassId : NAME_None;
		Contribution.AvailableSourceQuantity = 1;

		TArray<FGridAvailableCombatAction> Actions;
		FGridCombatActionCatalog::Build(MakeMON154CatalogContext(SatisfiedRequirements), { Contribution }, Actions);
		return Actions.Num() == 1 ? Actions[0] : FGridAvailableCombatAction();
	}
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON154DefinitionValidationTest, "Grimrock.RPG.MON15.4.DefinitionValidation", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON154DefinitionValidationTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Valid progression definition is accepted"), Valid->IsValidDefinition());

	URPGClassAsset* DuplicateLevel = MakeMON154Class(GetTransientPackage());
	const FRPGClassProgressionLevelGrant DuplicateGrant = DuplicateLevel->ProgressionLevelGrants[0];
	DuplicateLevel->ProgressionLevelGrants.Add(DuplicateGrant);
	TestFalse(TEXT("Duplicate grant level is rejected"), DuplicateLevel->IsValidDefinition());

	URPGClassAsset* UnknownPrerequisite = MakeMON154Class(GetTransientPackage());
	UnknownPrerequisite->ProgressionChoices[1].PrerequisiteChoiceIds.Add(TEXT("Choice_Missing"));
	TestFalse(TEXT("Unknown prerequisite is rejected"), UnknownPrerequisite->IsValidDefinition());

	URPGClassAsset* Cycle = MakeMON154Class(GetTransientPackage());
	Cycle->ProgressionChoices[0].PrerequisiteChoiceIds.Add(TEXT("Choice_C"));
	TestFalse(TEXT("Choice dependency cycle is rejected"), Cycle->IsValidDefinition());
	...
	TestFalse(TEXT("No-op level grant is rejected"), NoOpGrant->IsValidDefinition());
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON154ChoicePointAccountingTest, "Grimrock.RPG.MON15.4.ChoicePointAccounting", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON154ChoicePointAccountingTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Level one grants zero choice points"), FRPGClassProgressionService::GetTotalChoicePointsGranted(ClassDefinition, 1), 0);
	TestEqual(TEXT("Level two grants one choice point"), FRPGClassProgressionService::GetTotalChoicePointsGranted(ClassDefinition, 2), 1);
	TestEqual(TEXT("Level three grants two cumulative points"), FRPGClassProgressionService::GetTotalChoicePointsGranted(ClassDefinition, 3), 2);
	TestEqual(TEXT("Level four grants four cumulative points"), FRPGClassProgressionService::GetTotalChoicePointsGranted(ClassDefinition, 4), 4);
	...
	TestTrue(TEXT("A valid hypothetical selection has a balance"),
		FRPGClassProgressionService::TryGetChoicePointBalance(ClassDefinition, 4, Selected, Granted, Spent, Remaining));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON154SelectionRulesTest, "Grimrock.RPG.MON15.4.SelectionRules", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON154SelectionRulesTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Choice A is available at level two"),
		FRPGClassProgressionService::GetChoiceAvailability(ClassDefinition, 2, Selected, TEXT("Choice_A")) ==
			ERPGClassProgressionChoiceAvailabilityReason::None);
	TestTrue(TEXT("Choice B is level-gated at level two"),
		FRPGClassProgressionService::GetChoiceAvailability(ClassDefinition, 2, Selected, TEXT("Choice_B")) ==
			ERPGClassProgressionChoiceAvailabilityReason::LevelTooLow);
	TestTrue(TEXT("Choice B requires Choice A at level three"),
		FRPGClassProgressionService::GetChoiceAvailability(ClassDefinition, 3, Selected, TEXT("Choice_B")) ==
	...
	TestTrue(TEXT("Expensive choice is rejected for insufficient points"),
		FRPGClassProgressionService::GetChoiceAvailability(ClassDefinition, 2, Selected, TEXT("Choice_Expensive")) ==
	...
	TestTrue(TEXT("Already-selected choice is rejected"),
		FRPGClassProgressionService::GetChoiceAvailability(ClassDefinition, 3, Selected, TEXT("Choice_A")) ==
	...
	TestFalse(TEXT("A selection missing its prerequisite is invalid"),
		FRPGClassProgressionService::TryGetChoicePointBalance(ClassDefinition, 3, InvalidSelection, Granted, Spent, Remaining));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON154RequirementProjectionTest, "Grimrock.RPG.MON15.4.RequirementProjection", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON154RequirementProjectionTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Hypothetical selected requirements resolve"),
		FRPGClassProgressionService::CollectSatisfiedRequirements(ClassDefinition, 3, Selected, Requirements));
	TestTrue(TEXT("Choice A id is a requirement tag"), Requirements.Contains(TEXT("Choice_A")));
	TestTrue(TEXT("Choice B id is a requirement tag"), Requirements.Contains(TEXT("Choice_B")));
	TestTrue(TEXT("Choice A grants its feature tag"), Requirements.Contains(TEXT("Feature_A")));
	TestTrue(TEXT("Choice B grants its feature tag"), Requirements.Contains(TEXT("Feature_B")));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRPGMON154CombatActionUnlockProjectionTest, "Grimrock.RPG.MON15.4.CombatActionUnlockProjection",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON154CombatActionUnlockProjectionTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	URPGClassAsset* ClassDefinition = MakeMON154Class(GetTransientPackage());
	const FGridCombatActionDefinition LevelAction = MakeMON154EffectAction(TEXT("Ability_Level2"), TEXT("Feature_Level2"));
	...
	const FGridCombatActionDefinition ChoiceAction = MakeMON154EffectAction(TEXT("Ability_ChoiceA"), TEXT("Choice_A"));
	TSet<FName> Selected;
	Selected.Add(TEXT("Choice_A"));
	TestTrue(TEXT("Choice projection succeeds"), FRPGClassProgressionService::CollectSatisfiedRequirements(ClassDefinition, 2, Selected, Requirements));
	Available = BuildMON154AvailableAction(ClassDefinition, ChoiceAction, Requirements);
	TestTrue(TEXT("Choice-gated ability becomes available for selected candidate"), Available.bEnabled);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRPGMON154LevelUpIntegrationTest, "Grimrock.RPG.MON15.4.LevelUpIntegration", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRPGMON154LevelUpIntegrationTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Character reaches level two"), Component->PartyInventoryState.ActiveCharacters[0].Level, 2);
