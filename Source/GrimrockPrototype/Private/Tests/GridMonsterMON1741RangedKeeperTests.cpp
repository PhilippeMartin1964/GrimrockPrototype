...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON1741PreferredFiringCandidatesTest,
    "Grimrock.Monsters.MON17.4.1.PreferredFiringCandidates",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON1741PreferredFiringCandidatesTest::RunTest (
    const FString& Parameters)
{
    ...
    TestEqual (
        TEXT ("Three preferred distances produce twelve cardinal candidates"),
        Candidates.Num (),
        12);
    ...
    for (const FIntPoint& Candidate : Candidates)
    {
        const int32 Distance =
            FGridMonsterPathfinder::ManhattanDistance (Candidate, PartyCell);
        TestTrue (
            TEXT ("Every generated candidate lies inside the preferred band"),
            FGridMonsterRangedKeeperPlanner::IsPreferredDistance (
                Distance,
                3,
                5));
        TestTrue (
            TEXT ("Every generated candidate is axial to the party"),
            Candidate.X == PartyCell.X || Candidate.Y == PartyCell.Y);
    }

    TestFalse (
        TEXT ("Distance two is below the preferred band"),
        FGridMonsterRangedKeeperPlanner::IsPreferredDistance (2, 3, 5));
    TestTrue (
        TEXT ("Distance five is inside the preferred band"),
        FGridMonsterRangedKeeperPlanner::IsPreferredDistance (5, 3, 5));
    TestFalse (
        TEXT ("Distance six is above the preferred band"),
        FGridMonsterRangedKeeperPlanner::IsPreferredDistance (6, 3, 5));

    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON1741RepositionThenAttackTest,
    "Grimrock.Monsters.MON17.4.1.RepositionThenAttack",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON1741RepositionThenAttackTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    TestTrue (
        TEXT ("One reposition cell plus two attack AP builds a complete turn"),
    ...
    if (Actions.Num () == 2)
    {
        ...
        TestEqual (
            TEXT ("Move and attack consume all three AP"),
            Actions[0].ActionPointCost + Actions[1].ActionPointCost,
            3);
    }
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMonsterMON1741MultiTurnApproachTest,
    "Grimrock.Monsters.MON17.4.1.MultiTurnApproach",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FGridMonsterMON1741MultiTurnApproachTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    const TArray<FIntPoint> TwoStepPath = {
        FIntPoint (2, 1),
        FIntPoint (3, 1)
    };
    TestTrue (
        TEXT ("A two-cell approach still produces movement actions"),
        FGridMonsterRangedKeeperPlanner::BuildRepositionTurn (
            GoblinId,
            FIntPoint (1, 1),
            EGridEdge::East,
            ...
            Actions));
    TestEqual (
        TEXT ("Two moves consume two AP and leave too little AP to throw"),
        Actions.Num (),
        2);
    for (const FGridCombatAction& Action : Actions)
    {
        TestEqual (
            TEXT ("No premature attack is appended"),
            Action.Type,
    ...
    TestTrue (
        TEXT ("A cooling-down monster may still reposition"),
        FGridMonsterRangedKeeperPlanner::BuildRepositionTurn (
            GoblinId,
            FIntPoint (1, 1),
            EGridEdge::East,
