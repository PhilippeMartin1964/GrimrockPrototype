...
bool FGridMON182LearnForgetTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("First learn succeeds"), Spellbook->LearnSpell(CharacterId, SpellId), EGridSpellbookMutationResult::Success);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMON182TransientContractTest, "Grimrock.Magic.MON18.2.TransientContract", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGridMON182TransientContractTest::RunTest(const FString& Parameters)
{
	...
	if (StateProperty)
	{
		TestTrue(TEXT("SpellbookState stays transient until MON18.8"), StateProperty->HasAnyPropertyFlags(CPF_Transient));
	}
	return true;
}
