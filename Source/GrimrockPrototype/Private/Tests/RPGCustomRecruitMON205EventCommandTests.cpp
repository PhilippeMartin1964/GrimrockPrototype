...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2055EventCommandContractTest,
    "Grimrock.MON20.5.CustomRecruit.EventCommandContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FRPGMON2055EventCommandContractTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;

    TestEqual (
        TEXT ("Story companion command keeps serialized value 23"),
        static_cast<uint8> (EGridObjectCommand::OfferRecruitment),
        static_cast<uint8> (23));
    TestEqual (
        TEXT ("OpenCustomRecruit appends serialized command value 24"),
        static_cast<uint8> (EGridObjectCommand::OpenCustomRecruit),
    ...
    {
        TestEqual (
            TEXT ("Lua-style enum lookup resolves OpenCustomRecruit"),
            CommandEnum->GetValueByNameString (TEXT ("OpenCustomRecruit")),
    ...
    {
        TestEqual (
            TEXT ("CustomRecruiter is exposed by the object type enum"),
            TypeEnum->GetValueByNameString (TEXT ("CustomRecruiter")),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2055CustomRecruiterArchetypeContractTest,
    "Grimrock.MON20.5.CustomRecruit.CustomRecruiterArchetypeContract",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FRPGMON2055CustomRecruiterArchetypeContractTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;

    UGridObjectArchetypeAsset* Archetype =
        NewObject<UGridObjectArchetypeAsset> ();
    ...
    TestTrue (
        TEXT ("Data-only CustomRecruiter archetype validates without RuntimeActorClass"),
        Archetype->ValidateArchetype (Messages));
    TestFalse (
        TEXT ("CustomRecruiter does not require a runtime actor class"),
        Archetype->RequiresRuntimeActorClass ());
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2055EventCommandMissingPlayerPawnTest,
    "Grimrock.MON20.5.CustomRecruit.EventCommandMissingPlayerPawn",
    EAutomationTestFlags::EditorContext |
        EAutomationTestFlags::EngineFilter)

bool FRPGMON2055EventCommandMissingPlayerPawnTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    ...
    TestFalse (
        TEXT ("OpenCustomRecruit rejects when no player party pawn exists"),
        Activation->ExecuteLinksFromObjectForEvent (
            SourceId,
            EGridObjectEvent::Activated));
    TestFalse (
        TEXT ("Rejected CustomRecruiter command does not create stateful activation"),
        Activation->GetActiveObjectIds ().Contains (RecruiterId));
