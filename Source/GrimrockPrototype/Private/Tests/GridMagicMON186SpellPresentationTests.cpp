...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON186ProductionProfilesTest,
...
{
    const FGridSpellPresentationProfile Bolt =
    ...
    TestTrue (
        TEXT ("Lesser Heal profile valid"),
        FGridProductionSpellLibrary::MakeLesserHealPresentation ().IsValid ());
    TestFalse (
        TEXT ("Lesser Heal is instant"),
        FGridProductionSpellLibrary::MakeLesserHealPresentation ().Projectile.bEnabled);
    TestFalse (
        TEXT ("Haste is instant"),
        FGridProductionSpellLibrary::MakeHastePresentation ().Projectile.bEnabled);
    TestFalse (
        TEXT ("Cure Poison is instant"),
        FGridProductionSpellLibrary::MakeCurePoisonPresentation ().Projectile.bEnabled);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON186ProjectilePlanSequenceTest,
...
bool FGridMON186ProjectilePlanSequenceTest::RunTest (const FString& Parameters)
{
    ...
    TestTrue (
        TEXT ("Plan built"),
        FGridSpellPresentationService::BuildPlan (
            Definition,
            MakeResolvedTarget (),
            Profile,
            FVector (0.0f, 0.0f, 100.0f),
            FVector (400.0f, 0.0f, 100.0f),
            Plan));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON186InstantPlanSequenceTest,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON186ProjectileTrajectoryReuseTest,
...
bool FGridMON186ProjectileTrajectoryReuseTest::RunTest (const FString& Parameters)
{
    ...
    const FVector Actual =
        FGridSpellPresentationService::EvaluateProjectileTrajectory (
            Source, Target, 0.5f);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON186ProjectileTimingReuseTest,
...
bool FGridMON186ProjectileTimingReuseTest::RunTest (const FString& Parameters)
{
    ...
    const float Actual =
        FGridSpellPresentationService::CalculateProjectileLaunchDelay (
            0.25f, Profile);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON186VisualOptionalityTest,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FGridMON186PresentationPurityTest,
...
bool FGridMON186PresentationPurityTest::RunTest (const FString& Parameters)
{
    ...
    TestTrue (
        TEXT ("Runtime component remains an ActorComponent"),
        UGridSpellPresentationComponent::StaticClass ()->IsChildOf (UActorComponent::StaticClass ()));
    return true;
}
