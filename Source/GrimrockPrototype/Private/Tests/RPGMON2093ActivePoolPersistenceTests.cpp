...
namespace RPGMON2093Tests
{
    ...
    const FGridCharacterInventoryState* FindCharacter (
        const FGridPartyInventoryState& State,
        const FGuid& CharacterId)
    {
        if (const FGridCharacterInventoryState* Active =
            State.ActiveCharacters.FindByPredicate (
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2093RecruitmentPreservesPoolSkillTest,
    "Grimrock.MON20.9.ActivePoolPersistence.RecruitmentPreservesPoolSkill",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON2093RecruitmentPreservesPoolSkillTest::RunTest (
    const FString& Parameters)
{
    ...
    TestTrue (TEXT ("Skilled pool candidate recruits"),
        FRPGPartyRecruitmentService::TryRecruitFromPool (
            Inventory,
            RecruitId,
            Result));
    TestEqual (TEXT ("Candidate leaves the pool"),
        Inventory->PartyInventoryState.CharacterPool.Num (), 0);
    TestTrue (TEXT ("Candidate becomes active"),
        Inventory->PartyInventoryState.ActiveCharacters.IsValidIndex (1) &&
        Inventory->PartyInventoryState.ActiveCharacters[1].CharacterId ==
            RecruitId);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2093PoolSnapshotRestoresAfterRecruitmentTest,
    "Grimrock.MON20.9.ActivePoolPersistence.PoolSnapshotRestoresAfterRecruitment",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON2093PoolSnapshotRestoresAfterRecruitmentTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGMON2093Tests;
    ...
    FString Error;
    TestTrue (TEXT ("Pool Skill snapshot captures"),
    ...
    TestTrue (TEXT ("Pool character recruits after capture"),
        FRPGPartyRecruitmentService::TryRecruitFromPool (
            Inventory,
            RecruitId,
            Recruitment));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2093ActiveSnapshotRestoresAfterReserveMoveTest,
    "Grimrock.MON20.9.ActivePoolPersistence.ActiveSnapshotRestoresAfterReserveMove",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON2093ActiveSnapshotRestoresAfterReserveMoveTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGMON2093Tests;
    ...
    FRPGPartyRecruitmentResult Recruitment;
    TestTrue (TEXT ("Candidate recruits before capture"),
        FRPGPartyRecruitmentService::TryRecruitFromPool (
            Inventory,
            RecruitId,
            Recruitment));
    ...
    TArray<FRPGCharacterSkillSaveState> SavedStates;
    FString Error;
    TestTrue (TEXT ("Active Skill snapshot captures"),
        FRPGSkillPersistence::CapturePartySkills (
            Inventory->PartyInventoryState,
            Resolver,
            SavedStates,
    ...
    FGridPartyInventoryState TargetState =
        Inventory->PartyInventoryState;
    FGridCharacterInventoryState MovedCharacter =
        TargetState.ActiveCharacters[1];
    ...
    TestTrue (TEXT ("Active snapshot restores to pooled identity"),
        FRPGSkillPersistence::RestorePartySkills (
            TargetState,
            SavedStates,
            Resolver,
    ...
    const FGridCharacterInventoryState* Restored =
        FindCharacter (TargetState, RecruitId);
    TestNotNull (TEXT ("Moved identity remains resolvable"), Restored);
    if (!Restored)
    {
        return false;
    }
    TestEqual (TEXT ("Pooled identity receives captured rank"),
        FRPGSkillService::GetSkillRank (
            *Restored,
            TEXT ("Skill_Athletics")),
        2);
    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2093MixedActivePoolIsolationTest,
    "Grimrock.MON20.9.ActivePoolPersistence.MixedActivePoolIsolation",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON2093MixedActivePoolIsolationTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGMON2093Tests;
    ...
    TArray<FRPGCharacterSkillSaveState> SavedStates;
    FString Error;
    TestTrue (TEXT ("Mixed active/pool snapshot captures"),
        FRPGSkillPersistence::CapturePartySkills (
            Inventory->PartyInventoryState,
            Resolver,
            SavedStates,
    ...
    TestTrue (TEXT ("Mixed snapshot restores"),
        FRPGSkillPersistence::RestorePartySkills (
            Inventory->PartyInventoryState,
            SavedStates,
            Resolver,
    ...
    TestEqual (TEXT ("Pool rank remains attached to pool CharacterId"),
        FRPGSkillService::GetSkillRank (
            Inventory->PartyInventoryState.CharacterPool[0],
            TEXT ("Skill_Perception")),
        4);
    return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2093SelectedCharacterIndependentTest,
    "Grimrock.MON20.9.ActivePoolPersistence.SelectedCharacterIndependent",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON2093SelectedCharacterIndependentTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGMON2093Tests;
    ...
    FRPGPartyRecruitmentResult Recruitment;
    TestTrue (TEXT ("Second character recruits"),
        FRPGPartyRecruitmentService::TryRecruitFromPool (
            Inventory,
            RecruitId,
            Recruitment));
    ...
    TestTrue (TEXT ("Snapshot restores after selection changes"),
        FRPGSkillPersistence::RestorePartySkills (
            ...
            Resolver,
            Error));
    TestEqual (TEXT ("Restore does not alter SelectedCharacterIndex"),
        Inventory->PartyInventoryState.SelectedCharacterIndex,
        0);
    TestEqual (TEXT ("First active rank restores"),
        FRPGSkillService::GetSkillRank (
            Inventory->PartyInventoryState.ActiveCharacters[0],
            TEXT ("Skill_Survival")),
        1);
    TestEqual (TEXT ("Second active rank restores"),
        FRPGSkillService::GetSkillRank (
            Inventory->PartyInventoryState.ActiveCharacters[1],
            TEXT ("Skill_Survival")),
        3);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2093RecruitmentRejectPreservesPoolSkillTest,
    "Grimrock.MON20.9.ActivePoolPersistence.RecruitmentRejectPreservesPoolSkill",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON2093RecruitmentRejectPreservesPoolSkillTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGMON2093Tests;
    ...
    FRPGPartyRecruitmentResult Result;
    TestFalse (TEXT ("Full party rejects recruitment"),
        FRPGPartyRecruitmentService::TryRecruitFromPool (
            Inventory,
            RecruitId,
            Result));
    TestTrue (TEXT ("Reject reason is PartyFull"),
        Result.RejectReason == ERPGPartyRecruitmentRejectReason::PartyFull);
    TestEqual (TEXT ("Candidate remains in pool"),
        Inventory->PartyInventoryState.CharacterPool.Num (), 1);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2093PoolReorderRestoresByIdentityTest,
    "Grimrock.MON20.9.ActivePoolPersistence.PoolReorderRestoresByIdentity",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON2093PoolReorderRestoresByIdentityTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGMON2093Tests;
    ...
    TArray<FRPGCharacterSkillSaveState> SavedStates;
    FString Error;
    TestTrue (TEXT ("Pool snapshot captures before reorder"),
        FRPGSkillPersistence::CapturePartySkills (
            Inventory->PartyInventoryState,
            Resolver,
            SavedStates,
    ...
    for (FGridCharacterInventoryState& Character :
        Inventory->PartyInventoryState.CharacterPool)
    {
        Character.SkillRanks.Reset ();
    }

    TestTrue (TEXT ("Pool snapshot restores after reorder"),
        FRPGSkillPersistence::RestorePartySkills (
            Inventory->PartyInventoryState,
            SavedStates,
            Resolver,
            Error));
    const FGridCharacterInventoryState* FirstRestored =
        FindCharacter (Inventory->PartyInventoryState, FirstId);
    const FGridCharacterInventoryState* SecondRestored =
        FindCharacter (Inventory->PartyInventoryState, SecondId);
    ...
    TestEqual (TEXT ("First rank follows CharacterId"),
        FRPGSkillService::GetSkillRank (
            *FirstRestored,
            TEXT ("Skill_Perception")),
        1);
    TestEqual (TEXT ("Second rank follows CharacterId"),
        FRPGSkillService::GetSkillRank (
            *SecondRestored,
            TEXT ("Skill_Perception")),
        4);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
    FRPGMON2093SnapshotValidAfterRecruitmentMoveTest,
    "Grimrock.MON20.9.ActivePoolPersistence.SnapshotValidAfterRecruitmentMove",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FRPGMON2093SnapshotValidAfterRecruitmentMoveTest::RunTest (
    const FString& Parameters)
{
    (void)Parameters;
    using namespace RPGMON2093Tests;
    ...
    FRPGPartyRecruitmentResult Recruitment;
    TestTrue (TEXT ("Identity moves from pool to active"),
        FRPGPartyRecruitmentService::TryRecruitFromPool (
            Inventory,
            RecruitId,
            Recruitment));
