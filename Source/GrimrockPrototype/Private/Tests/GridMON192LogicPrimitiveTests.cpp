...
{
	FGridLevelVariableDefinition MakeBoolVariable(FName Id, bool bDefault = false)
	{
	...
	FGridLevelVariableDefinition MakeIntVariable(FName Id, int32 DefaultValue = 0)
	{
		FGridLevelVariableDefinition Definition;
		Definition.VariableId = Id;
	...
	UGridLevelAsset* MakeLogicLevel(UObject* Outer)
	{
		...
		Level->LevelVariables = { MakeBoolVariable(TEXT("Gate"), false), MakeBoolVariable(TEXT("Latch"), false), MakeIntVariable(TEXT("Count"), 2) };
		return Level;
	}
	...
	struct FMON1923TestWorld
	{
		...
		FMON1923TestWorld()
		{
			...
			World = UWorld::CreateWorld(EWorldType::Game, false, FName(*FString::Printf(TEXT("MON1923_%s"), *FGuid::NewGuid().ToString(EGuidFormats::Digits))),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMON1923MutationPrimitivesTest, "Grimrock.MON19.2.Runtime.Logic.MutationPrimitives",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMON1923MutationPrimitivesTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMON1923ComparisonPrimitivesTest, "Grimrock.MON19.2.Runtime.Logic.ComparisonPrimitives",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMON1923ComparisonPrimitivesTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	UGridLevelAsset* Level = MakeLogicLevel(GetTransientPackage());
	FGridLevelRuntimeState State;
	...
	GridLevelVariableStore::SetBool(*Level, State, TEXT("Gate"), true, Error);
	...
	TestEqual(TEXT("Int threshold true emits Activated"), Result.EmittedEvent, EGridObjectEvent::Activated);

	CompareInt.Logic.IntValue = 3;
	TestTrue(TEXT("CompareInt false branch executes"), Execute(*Level, CompareInt, State, Result));
	TestEqual(TEXT("Int threshold false emits Deactivated"), Result.EmittedEvent, EGridObjectEvent::Deactivated);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGridMON1923LatchRelayTest, "Grimrock.MON19.2.Runtime.Logic.LatchAndRelay", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMON1923LatchRelayTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	UGridLevelAsset* Level = MakeLogicLevel(GetTransientPackage());
	FGridLevelRuntimeState State;
	...
	TestTrue(TEXT("Second latch execute succeeds"), Execute(*Level, Latch, State, Result));
	TestFalse(TEXT("Latched node is silent on repeated execute"), Result.bEmitEvent);

	bool bLatched = false;
	GridLevelVariableStore::TryGetBool(*Level, State, TEXT("Latch"), bLatched, Error);
	TestTrue(TEXT("Latch state is stored in persistent Bool"), bLatched);

	TestTrue(TEXT("Logic Reset succeeds for latch"), Execute(*Level, Latch, State, Result, EGridObjectCommand::LogicReset));
	TestTrue(TEXT("Latch reset emits Deactivated"), Result.bEmitEvent && Result.EmittedEvent == EGridObjectEvent::Deactivated);
	GridLevelVariableStore::TryGetBool(*Level, State, TEXT("Latch"), bLatched, Error);
	TestFalse(TEXT("Latch reset clears persistent Bool"), bLatched);

	TestTrue(TEXT("Latch can fire again after reset"), Execute(*Level, Latch, State, Result));
	TestTrue(TEXT("Re-armed latch emits again"), Result.bEmitEvent);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMON1923ValidationOverflowTest, "Grimrock.MON19.2.Runtime.Logic.ValidationAndOverflow",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMON1923ValidationOverflowTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	UGridLevelAsset* Level = MakeLogicLevel(GetTransientPackage());
	FGridLevelRuntimeState State;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGridMON1923EventCommandChainTest, "Grimrock.MON19.2.Runtime.Logic.EventCommandChain",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGridMON1923EventCommandChainTest::RunTest(const FString& Parameters)
{
	(void)Parameters;
	...
	bool bGate = false;
	int32 Count = 0;
	TestTrue(TEXT("Chained Bool is readable"), GridLevelVariableStore::TryGetBool(*Level, *State, TEXT("Gate"), bGate, Error));
	TestTrue(TEXT("First logic node mutated Bool"), bGate);
	TestTrue(TEXT("Chained Int is readable"), GridLevelVariableStore::TryGetInt32(*Level, *State, TEXT("Count"), Count, Error));
