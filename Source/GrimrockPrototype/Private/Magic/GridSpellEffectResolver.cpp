...
bool FGridSpellEffectResolver::ResolveEffects (
    const FGridSpellDefinition& Definition,
    const FGuid& SourceId,
    int32 MaxHealth,
    int32& InOutCurrentHealth,
    FGridStatusEffectCollection& InOutStatusEffects,
    TFunctionRef<const UGridStatusEffectDefinitionAsset* (FName)> StatusDefinitionResolver,
    FGridSpellEffectResolutionResult& OutResult,
    EGridSpellEffectResolutionRejectReason& OutRejectReason,
    FString& OutError)
{
    OutResult.Reset ();
    ...
    if (FGridSpellContract::ValidateDefinition (Definition) !=
    ...
    if (!SourceId.IsValid () || MaxHealth < 1 ||
        InOutCurrentHealth < 0 || InOutCurrentHealth > MaxHealth)
    {
    ...
    for (const FGridSpellEffectDefinition& Effect : Definition.Effects)
    {
        ...
        const UGridStatusEffectDefinitionAsset* StatusDefinition =
            StatusDefinitionResolver (Effect.StatusEffectId);
        if (!IsValid (StatusDefinition))
        {
            OutRejectReason =
                EGridSpellEffectResolutionRejectReason::MissingStatusEffectDefinition;
            OutError = FString::Printf (
                TEXT ("Missing status effect definition '%s'."),
                *Effect.StatusEffectId.ToString ());
            return false;
        }
        if (!StatusDefinition->IsValidDefinition () ||
            StatusDefinition->EffectId != Effect.StatusEffectId)
        {
            OutRejectReason =
                EGridSpellEffectResolutionRejectReason::InvalidStatusEffectDefinition;
            OutError = FString::Printf (
                TEXT ("Status effect definition '%s' is invalid or mismatched."),
                *Effect.StatusEffectId.ToString ());
            return false;
        }
    ...
    for (const FGridSpellEffectDefinition& Effect : Definition.Effects)
    {
        ...
        {
            const UGridStatusEffectDefinitionAsset* const* FoundDefinition =
                ResolvedStatusDefinitions.Find (Effect.StatusEffectId);
            FGridStatusEffectApplyResult ApplyResult;
            FString ApplyError;
            if (!FoundDefinition || !IsValid (*FoundDefinition) ||
                !WorkingStatuses.TryApply (
                    **FoundDefinition,
                    SourceId,
                    ApplyResult,
                    ApplyError))
            {
                OutRejectReason =
                    EGridSpellEffectResolutionRejectReason::StatusEffectApplyFailed;
                OutError = ApplyError.IsEmpty ()
                    ? FString::Printf (
                        TEXT ("Failed to apply status effect '%s'."),
                        *Effect.StatusEffectId.ToString ())
                    : ApplyError;
                return false;
            }
...
bool FGridSpellEffectResolver::ResolveCharacterEffects (
    const FGridSpellDefinition& Definition,
    const FGuid& SourceId,
    FRPGDerivedStats& InOutStats,
    FGridStatusEffectCollection& InOutStatusEffects,
    TFunctionRef<const UGridStatusEffectDefinitionAsset* (FName)> StatusDefinitionResolver,
    FGridSpellEffectResolutionResult& OutResult,
    EGridSpellEffectResolutionRejectReason& OutRejectReason,
    FString& OutError)
{
    return ResolveEffects (
        Definition,
        SourceId,
        InOutStats.MaxHealth,
        InOutStats.CurrentHealth,
        InOutStatusEffects,
