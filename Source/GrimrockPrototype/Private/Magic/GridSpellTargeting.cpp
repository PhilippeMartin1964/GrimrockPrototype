...
namespace
{
    int32 GridSpellTargetingDistance (
        const FIntPoint& From,
        const FIntPoint& To)
    {
        return FMath::Abs (To.X - From.X) +
            FMath::Abs (To.Y - From.Y);
    }

    bool GridSpellTargetingIsAxial (
        const FIntPoint& From,
        const FIntPoint& To)
    {
    ...
    bool GridSpellTargetingIsInRange (
        const FGridSpellDefinition& Definition,
        int32 Distance)
    {
        return Distance >= Definition.MinRangeCells &&
            Distance <= Definition.MaxRangeCells;
    }
}

EGridSpellTargetingRejectReason FGridSpellTargetingService::ValidateAndResolveTarget (
    const FGridSpellDefinition& Definition,
    ...
    FGridSpellResolvedTarget& OutResolvedTarget)
{
    ...
    if (!Request.CasterCharacterId.IsValid () ||
    ...
    switch (Definition.TargetingPolicy)
    {
        ...
        case EGridCombatTargetingPolicy::Ally:
            if (!Request.Target.TargetId.IsValid () ||
                !Context.ResolvedTargetId.IsValid () ||
                !Context.bHasResolvedTargetCell)
            {
    ...
    const int32 Distance = GridSpellTargetingDistance (
        Context.CasterCell,
        TargetCell);
...
bool FGridSpellCastPipelineService::TryValidateTargetAndCommitCosts (
    const FGridSpellDefinition& Definition,
    const FGridSpellCastRequest& Request,
