...
namespace
{
    ...
    ObjectType* ResolveFirstConfigured (
...
void UGridSpellPresentationComponent::EndPlay (
...
bool UGridSpellPresentationComponent::PresentSpell (
    ...
    const FGridSpellPresentationProfile& Profile)
{
    ...
    {
        if (World && bProjectileEnabled &&
            !Profile.Projectile.VisualMesh.IsNull ())
        {
            ...
            if (IsValid (Mesh))
            {
                AGridCombatProjectileActor* Projectile =
                    World->SpawnActor<AGridCombatProjectileActor> (
                        ...
                        Plan.SourceWorldLocation,
                        FRotator::ZeroRotator);
                if (Projectile &&
                    Projectile->InitializeProjectilePresentation (
                        Mesh,
                        Plan.SourceWorldLocation,
                        Plan.TargetWorldLocation,
                        Profile.Projectile.TravelDurationSeconds,
                        Profile.Projectile.VisualScale,
                        Profile.Projectile.RotationOffset))
                {
        ...
        if (World)
        {
            World->GetTimerManager ().SetTimer (
                ImpactTimerHandle,
                this,
                &UGridSpellPresentationComponent::HandleImpact,
                Profile.Projectile.TravelDurationSeconds,
                false);
        }
        else
...
void UGridSpellPresentationComponent::EmitEvent (
    EGridSpellPresentationEvent Event)
{
    ...
    Request.WorldLocation =
        Event == EGridSpellPresentationEvent::CastStarted ||
        Event == EGridSpellPresentationEvent::ProjectileLaunched
        ...
        if (Request.ResolvedSystem && GetWorld ())
        {
            UNiagaraComponent* Niagara =
                UNiagaraFunctionLibrary::SpawnSystemAtLocation (
                    this,
                    Request.ResolvedSystem,
                    Request.WorldLocation + VFX->LocationOffset,
                    VFX->RotationOffset,
                    VFX->Scale,
                    true,
                    true,
