    ...
    return SNew(SBorder)
        ...
        [
            ...
            [
                ...
                [
                    SAssignNew(NativeTitleText, STextBlock).Justification(ETextJustify::Center).Text(CurrentTitle)
                ]
                + SVerticalBox::Slot().AutoHeight().Padding(0.0f, 0.0f, 0.0f, 8.0f)
                [
                    SAssignNew(NativeStatusText, STextBlock).Justification(ETextJustify::Center).Text(CurrentStatusText)
                ]
                + SVerticalBox::Slot().AutoHeight().Padding(0.0f, 0.0f, 0.0f, 8.0f)
                [
                    SAssignNew(NativeProgressBar, SProgressBar).Percent(CurrentProgress)
                ]
                + SVerticalBox::Slot().AutoHeight()
                [
                    SAssignNew(NativePercentText, STextBlock).Justification(ETextJustify::Center).Text(BuildPercentText())
                ]
            ]
        ];
}
...
void UGridDungeonBuildProgressWidget::SynchronizeNativeAndBlueprintWidgets()
{
    ...
    if (ProgressBar_BuildProgress) ProgressBar_BuildProgress->SetPercent(CurrentProgress);
    if (Text_Title) Text_Title->SetText(CurrentTitle);
    if (Text_Status) Text_Status->SetText(CurrentStatusText);
    if (Text_Percent) Text_Percent->SetText(PercentText);
    if (NativeProgressBar.IsValid()) NativeProgressBar->SetPercent(CurrentProgress);
    if (NativeTitleText.IsValid()) NativeTitleText->SetText(CurrentTitle);
    if (NativeStatusText.IsValid()) NativeStatusText->SetText(CurrentStatusText);
    if (NativePercentText.IsValid()) NativePercentText->SetText(PercentText);
}
