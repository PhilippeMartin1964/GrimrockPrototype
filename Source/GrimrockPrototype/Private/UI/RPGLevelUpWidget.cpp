...
namespace
{
    FRPGLevelUpPresentationView MakeTalentPresentationView ()
    {
        ...
        Presentation.TalentSectionTitle = LOCTEXT (
            "TalentSectionTitle",
            "TALENTS DE CLASSE");
        Presentation.TalentPointsLabel = LOCTEXT (
            "TalentPointsLabel",
            "Points de talent");
        Presentation.EmptyTalentsMessage = LOCTEXT (
            "EmptyTalentsMessage",
            "Aucun talent de classe à sélectionner pour ce niveau.");
        Presentation.SelectionPrompt = LOCTEXT (
            "TalentSelectionPrompt",
            "Sélectionnez un talent, ou annulez pour le reporter.");
        return Presentation;
    }

    FText MakeCommitRejectText (
        ERPGClassProgressionCommitRejectReason Reason)
    {
        switch (Reason)
        {
        ...
        case ERPGClassProgressionCommitRejectReason::MissingPrerequisite:
            return LOCTEXT (
                "CommitMissingPrerequisite",
                "Un prérequis de talent manque.");
        case ERPGClassProgressionCommitRejectReason::InsufficientChoicePoints:
            return LOCTEXT (
                "CommitInsufficientPoints",
                "Il n'y a pas assez de points de talent.");
        case ERPGClassProgressionCommitRejectReason::EmptyRequest:
            return LOCTEXT (
                "CommitEmpty",
                "Aucun talent n'est sélectionné.");
    ...
    int32 InNewLevel)
{
    ...
    if (!IsValid (InventoryComponent) ||
        ...
        PreviousLevel < URPGCharacterRulesLibrary::GetMinimumLevel () ||
        NewLevel < PreviousLevel ||
        NewLevel > URPGCharacterRulesLibrary::GetMaximumLevel () ||
        !ResolveClassDefinition ())
    {
        View = FRPGLevelUpView ();
        View.Presentation = MakeTalentPresentationView ();
        View.ValidationMessage = LOCTEXT (
            "InitializeInvalid",
            "Impossible d'ouvrir la progression de ce personnage.");
        RefreshNativeSlate ();
...
bool URPGLevelUpWidget::StageOrUnstageChoice (FName ChoiceId)
{
    ...
    if (PendingChoiceIds.Contains (ChoiceId))
    {
        ...
        {
            View.ValidationMessage = LOCTEXT (
                "CannotRemovePendingPrerequisite",
                "Ce talent est requis par une autre sélection en attente.");
            RefreshNativeSlate ();
            BP_OnLevelUpViewRefreshed ();
            return false;
        }
    ...
    {
        View.ValidationMessage = GetChoiceStatusText (
...
bool URPGLevelUpWidget::BuildCombinedSelection (
    TSet<FName>& OutSelectedChoiceIds,
    TArray<FName>* OutCommittedChoiceIds) const
{
    ...
    if (!FRPGClassProgressionTransactionService::TryGetSelectedChoiceIds (
            InventoryComponent,
            CharacterIndex,
            CommittedChoices))
    {
...
void URPGLevelUpWidget::RefreshView ()
{
    ...
    if (!ClassDefinition ||
        ...
        !InventoryComponent->IsValidCharacterIndex (CharacterIndex))
    {
        NewView.ValidationMessage = LOCTEXT (
            "RefreshInvalid",
            "La progression du personnage n'est plus disponible.");
    ...
    NewView.ClassName = Character.ClassDisplayName.IsEmpty ()
    ...
    TArray<FName> CommittedChoices;
    TSet<FName> CombinedSelection;
    if (!BuildCombinedSelection (
            CombinedSelection,
            &CommittedChoices))
    {
        NewView.ValidationMessage = LOCTEXT (
            "InvalidCombinedSelection",
            "La sélection de talents est incohérente.");
        View = MoveTemp (NewView);
        RefreshNativeSlate ();
        BP_OnLevelUpViewRefreshed ();
        return;
    ...
    if (!FRPGClassProgressionService::TryGetChoicePointBalance (
            ...
            Character.Level,
            CombinedSelection,
            NewView.GrantedChoicePoints,
            NewView.SpentChoicePoints,
            NewView.RemainingChoicePoints))
    {
        NewView.ValidationMessage = LOCTEXT (
            "InvalidPointBalance",
            "Le budget de talents est invalide.");
        View = MoveTemp (NewView);
        RefreshNativeSlate ();
        BP_OnLevelUpViewRefreshed ();
        return;
        ...
        ClassDefinition->ProgressionChoices)
    {
        ...
        else if (ChoiceView.bPending)
        {
            ...
            ChoiceView.StatusText = LOCTEXT (
                "ChoicePending",
                "Sélectionné — en attente de confirmation");
        }
        else
        {
            const ERPGClassProgressionChoiceAvailabilityReason Reason =
                FRPGClassProgressionService::GetChoiceAvailability (
                    ...
                    CombinedSelection,
                    Choice.ChoiceId);
            ChoiceView.bAvailable =
                Reason == ERPGClassProgressionChoiceAvailabilityReason::None;
            ChoiceView.StatusText = ChoiceView.bAvailable
                ? LOCTEXT ("ChoiceAvailable", "Disponible")
                : GetChoiceStatusText (static_cast<int32> (Reason));
    ...
    NewView.bCanConfirm =
        !PendingChoiceIds.IsEmpty () ||
        NewView.RemainingChoicePoints <= 0 ||
        !NewView.bHasSelectableChoices;
...
FText URPGLevelUpWidget::GetChoiceStatusText (
    int32 AvailabilityReasonValue) const
{
    const ERPGClassProgressionChoiceAvailabilityReason Reason =
        static_cast<ERPGClassProgressionChoiceAvailabilityReason> (
            AvailabilityReasonValue);
    switch (Reason)
    {
