...
void URPGCharacterCreationAttributesStepWidget::InitializeAttributesStep(URPGCharacterCreationWizardWidget* InOwningWizard)
{
...
void URPGCharacterCreationAttributesStepWidget::RefreshAttributeAllocationPreview()
{
	...
	CCAttributeStepSetOptionalText(Text_AttributePointsRemaining,
	...
	CCAttributeStepSetOptionalText(
		Text_AttributeHelp, FText::FromString(TEXT("Les valeurs de classe peuvent être ajustées. Les bonus de race sont appliqués ensuite.")));
	...
	auto RefreshRow =
		[&](FName AttributeId, UTextBlock* ClassText, UTextBlock* RaceText, UTextBlock* TotalText, UTextBlock* ModifierText, UTextBlock* EffectsText)
	{
		...
		if (AttributeId == CCAttributeStepStrength)
		{
			EffectsText->SetText(FText::FromString(FString::Printf(TEXT("Charge %d · CàC %+d"), FMath::RoundToInt(Wizard->GetPreviewCarryWeight()), Modifier)));
		}
		else if (AttributeId == CCAttributeStepDexterity)
		{
			EffectsText->SetText(FText::FromString(FString::Printf(
				TEXT("Précision %+d · Esquive %+d"), bHasDerivedStats ? DerivedStats.Accuracy : Modifier, bHasDerivedStats ? DerivedStats.Evasion : Modifier)));
		}
		else if (AttributeId == CCAttributeStepConstitution)
		{
			EffectsText->SetText(
				FText::FromString(FString::Printf(TEXT("Santé %d · Résistance %+d"), bHasDerivedStats ? DerivedStats.MaxHealth : 0, Modifier)));
		}
	...
	RefreshRow(
		CCAttributeStepDexterity, Text_DexterityClassValue, Text_DexterityRaceBonus, Text_DexterityTotalValue, Text_DexterityModifier, Text_DexterityEffects);
	RefreshRow(CCAttributeStepConstitution, Text_ConstitutionClassValue, Text_ConstitutionRaceBonus, Text_ConstitutionTotalValue, Text_ConstitutionModifier,
		Text_ConstitutionEffects);
	RefreshRow(CCAttributeStepIntelligence, Text_IntelligenceClassValue, Text_IntelligenceRaceBonus, Text_IntelligenceTotalValue, Text_IntelligenceModifier,
