...
void UGrimrockDesignSurfaceWidget::NativeTick (
...
void UGrimrockDesignSurfaceWidget::ApplyDesignSurfaceViewportLimit ()
{
    ...
    if (!RootCanvasPanel || !DesignRootScaleBox || !DesignSurfaceSizeBox)
    {
        UE_LOG (LogTemp, Error,
            TEXT ("%s design surface scaling failed: expected CanvasPanel_Root, ScaleBox_DesignRoot, and SizeBox_DesignSurface."),
            *GetNameSafe (this));
        LastAppliedViewportPx = ViewportPx;
        LastAppliedViewportScale = ViewportScale;
        return;
    ...
    const FVector2D AvailablePx (
        FMath::Max (1.0f, ViewportPx.X - ClampedSafeMarginPx * 2.0f),
        FMath::Max (1.0f, ViewportPx.Y - ClampedSafeMarginPx * 2.0f));
    ...
    const FVector2D FinalPhysicalSize (
        ClampedDesignWidth * PhysicalFitScale,
        ClampedDesignHeight * PhysicalFitScale);
    ...
    {
        UE_LOG (LogTemp, Error,
            TEXT ("%s design surface scaling failed: %s is not directly under CanvasPanel_Root."),
            *GetNameSafe (this),
    ...
    {
        UE_LOG (LogTemp, Error,
            TEXT ("%s design surface scaling failed: %s has no CanvasPanelSlot."),
            *GetNameSafe (this),
            *GetNameSafe (DesignRootScaleBox));
        LastAppliedViewportPx = ViewportPx;
        LastAppliedViewportScale = ViewportScale;
    ...
    UE_LOG (LogTemp, Log,
        TEXT ("%s DesignSurface ViewportPx=%.0fx%.0f Dpi=%.2f Fit=%.3f Design=%.0fx%.0f SlotSlate=%.1fx%.1f FinalPhysical=%.0fx%.0f"),
        *GetNameSafe (this),
        ViewportPx.X,
        ViewportPx.Y,
        ViewportScale,
        ...
        ClampedDesignWidth,
        ClampedDesignHeight,
        FinalSlateSlotSize.X,
        FinalSlateSlotSize.Y,
        FinalPhysicalSize.X,
        FinalPhysicalSize.Y);
}
