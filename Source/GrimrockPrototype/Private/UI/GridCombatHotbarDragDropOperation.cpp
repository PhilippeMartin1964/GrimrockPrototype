...
void UGridCombatHotbarDragDropOperation::InitializeFromSpellbookEntry (
    int32 InCharacterIndex,
    const FGridSpellbookEntryView& InEntry)
{
    ...
    bFromActionPalette = false;
    bFromSpellbook =
        InEntry.bCanAssignToHotbar && !InEntry.SpellId.IsNone ();
    Binding = FGridCombatHotbarBinding ();
    if (bFromSpellbook)
    {
    ...
    int32 TargetSlotIndex) const
{
    ...
    return UGridSpellbookUILibrary::AssignKnownSpellToHotbar (
        InventoryComponent,
        CharacterIndex,
        Spellbook,
