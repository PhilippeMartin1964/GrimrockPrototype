...
namespace
{
	...
	{
		return PortraitVariant.DisplayName.IsEmpty() ? FText::FromName(PortraitVariant.VariantId) : PortraitVariant.DisplayName;
	}

	FText GetGenderDisplayName(ERPGCharacterPortraitGender Gender)
	{
		return Gender == ERPGCharacterPortraitGender::Female ? FText::FromString(TEXT("Féminin")) : FText::FromString(TEXT("Masculin"));
	}

	bool AreSamePortraitTexture(const TSoftObjectPtr<UTexture2D>& Left, const TSoftObjectPtr<UTexture2D>& Right)
	{
		return !Left.IsNull() && !Right.IsNull() && Left.ToSoftObjectPath() == Right.ToSoftObjectPath();
	}
...
void URPGCharacterCreationWidget::BindWidgetEvents()
{
	...
	{
		Button_CreateCharacter->OnClicked.RemoveDynamic(this, &URPGCharacterCreationWidget::HandleCreateCharacterClicked);
		Button_CreateCharacter->OnClicked.AddDynamic(this, &URPGCharacterCreationWidget::HandleCreateCharacterClicked);
	}

	if (Button_GenderMale)
	{
		Button_GenderMale->OnClicked.RemoveDynamic(this, &URPGCharacterCreationWidget::HandleGenderMaleClicked);
		Button_GenderMale->OnClicked.AddDynamic(this, &URPGCharacterCreationWidget::HandleGenderMaleClicked);
	}

	if (Button_GenderFemale)
	{
		Button_GenderFemale->OnClicked.RemoveDynamic(this, &URPGCharacterCreationWidget::HandleGenderFemaleClicked);
		Button_GenderFemale->OnClicked.AddDynamic(this, &URPGCharacterCreationWidget::HandleGenderFemaleClicked);
	}

	if (ComboBox_Race)
	{
		ComboBox_Race->OnSelectionChanged.RemoveDynamic(this, &URPGCharacterCreationWidget::HandleRaceSelectionChanged);
		ComboBox_Race->OnSelectionChanged.AddDynamic(this, &URPGCharacterCreationWidget::HandleRaceSelectionChanged);
	}

	if (ComboBox_Class)
	{
		ComboBox_Class->OnSelectionChanged.RemoveDynamic(this, &URPGCharacterCreationWidget::HandleClassSelectionChanged);
		ComboBox_Class->OnSelectionChanged.AddDynamic(this, &URPGCharacterCreationWidget::HandleClassSelectionChanged);
	}

	if (ComboBox_Gender)
	{
		ComboBox_Gender->OnSelectionChanged.RemoveDynamic(this, &URPGCharacterCreationWidget::HandleGenderSelectionChanged);
		ComboBox_Gender->OnSelectionChanged.AddDynamic(this, &URPGCharacterCreationWidget::HandleGenderSelectionChanged);
	}

	if (ComboBox_PortraitVariant)
	{
		ComboBox_PortraitVariant->OnSelectionChanged.RemoveDynamic(this, &URPGCharacterCreationWidget::HandlePortraitVariantSelectionChanged);
		ComboBox_PortraitVariant->OnSelectionChanged.AddDynamic(this, &URPGCharacterCreationWidget::HandlePortraitVariantSelectionChanged);
	}

	if (EditableText_Name)
	{
		EditableText_Name->OnTextChanged.RemoveDynamic(this, &URPGCharacterCreationWidget::HandleNameChanged);
		EditableText_Name->OnTextChanged.AddDynamic(this, &URPGCharacterCreationWidget::HandleNameChanged);
		EditableText_Name->OnTextCommitted.RemoveDynamic(this, &URPGCharacterCreationWidget::HandleNameCommitted);
		EditableText_Name->OnTextCommitted.AddDynamic(this, &URPGCharacterCreationWidget::HandleNameCommitted);
	}
}
...
void URPGCharacterCreationWidget::PopulateDefinitionOptions()
{
	if (ComboBox_Race)
	{
		...
		if (RaceDefinition && RaceDefinition->IsValidDefinition())
		{
			ComboBox_Race->SetSelectedOption(GetDefinitionDisplayName(RaceDefinition->DisplayName, RaceDefinition->RaceId).ToString());
		}
	}
	...
	if (ComboBox_Class)
	{
		...
		auto AddClassOption = [this, &AddedOptions](URPGClassAsset* CharacterClass)
		{
			...
			const FString Option = GetDefinitionDisplayName(CharacterClass->DisplayName, CharacterClass->ClassId).ToString();
			if (!AddedOptions.Contains(Option))
			{
				AddedOptions.Add(Option);
		...
		if (ClassDefinition && ClassDefinition->IsValidDefinition())
		{
			ComboBox_Class->SetSelectedOption(GetDefinitionDisplayName(ClassDefinition->DisplayName, ClassDefinition->ClassId).ToString());
		}
	}
...
void URPGCharacterCreationWidget::PopulatePortraitOptions()
{
	...
	if (ComboBox_PortraitVariant)
	{
		...
		TSet<FString> AddedOptions;
		for (const FRPGCharacterPortraitVariant& PortraitVariant : PortraitSet->GetPortraitsForGenderRef(SelectedPortraitGender))
		{
		...
		{
			ComboBox_PortraitVariant->SetSelectedOption(GetPortraitVariantDisplayName(CurrentVariant).ToString());
		}
	}
}
...
{
	InitializeCharacterCreationWidgetForContext(InPartyPawn, ERPGCharacterCreationContext::NewGameMainHero);
}
...
ERPGCharacterCreationContext URPGCharacterCreationWidget::GetCreationContext() const
{
...
bool URPGCharacterCreationWidget::IsCreationContextPartyStateReady() const
{
	...
	if (CreationContext == ERPGCharacterCreationContext::NewGameMainHero)
	{
		return !State.bInitialCharacterCreationCompleted;
	}

	return State.bInitialCharacterCreationCompleted && !State.ActiveCharacters.IsEmpty() && State.ActiveEquipment.Num() == State.ActiveCharacters.Num() &&
...
void URPGCharacterCreationWidget::RefreshPreview()
{
	...
	const FText RaceDisplayValue =
		bHasRaceDefinition ? GetDefinitionDisplayName(RaceDefinition->DisplayName, RaceDefinition->RaceId) : FText::FromString(TEXT("Race non configurée"));
	const FText ClassDisplayValue = bHasClassDefinition ? GetDefinitionDisplayName(ClassDefinition->DisplayName, ClassDefinition->ClassId)
	...
	{
		PortraitDescription = DefaultPortrait.IsNull() ? FText::FromString(TEXT("Aucun portrait disponible pour cette race et ce sexe."))
	...
	SetOptionalText(
		Text_IdentityHelp, FText::FromString(TEXT("Donnez un nom à votre personnage et choisissez le portrait qui sera utilisé dans l'interface de jeu.")));
	...
	SetOptionalText(Text_IdentityRace, FText::Format(FText::FromString(TEXT("Race : {0}")), RaceDisplayValue));
	SetOptionalText(Text_IdentityClass, FText::Format(FText::FromString(TEXT("Classe : {0}")), ClassDisplayValue));
	SetOptionalText(Text_IdentityGender, FText::Format(FText::FromString(TEXT("Sexe : {0}")), GenderDisplayValue));
	...
	SetOptionalText(Text_CarryWeightValue, bHasAttributes ? FText::AsNumber(FMath::RoundToInt(GetPreviewCarryWeight())) : UnavailableValue);
	...
	{
		SetValidationMessage(FText::FromString(TEXT("Ajoutez au moins une race et une classe valides dans Class Defaults du widget.")), true);
	}
	else if (!InventoryComponent)
	{
		SetValidationMessage(FText::FromString(TEXT("Composant d'inventaire indisponible.")), true);
	}
	else if (CreationContext == ERPGCharacterCreationContext::NewGameMainHero && InventoryComponent->HasCompletedInitialCharacterCreation())
	{
		SetValidationMessage(FText::FromString(TEXT("La création initiale du personnage est déjà terminée.")), true);
	}
	else if (CreationContext == ERPGCharacterCreationContext::CustomRecruit && !InventoryComponent->HasCompletedInitialCharacterCreation())
	{
		SetValidationMessage(FText::FromString(TEXT("Créez d'abord le héros principal avant d'engager une recrue.")), true);
	}
	else if (CreationContext == ERPGCharacterCreationContext::CustomRecruit &&
		InventoryComponent->GetActiveCharacterCount() >= InventoryComponent->GetMaxActiveCharacters())
	{
		SetValidationMessage(FText::FromString(TEXT("Le groupe actif est complet.")), true);
	}
...
{
	if (!IsCreationContextPartyStateReady() || !RaceDefinition || !RaceDefinition->IsValidDefinition() || !ClassDefinition ||
	...
	FRPGAttributes Attributes;
	return NormalizedName.Len() >= 1 && NormalizedName.Len() <= 24 && GetPreviewAttributes(Attributes) &&
...
bool URPGCharacterCreationWidget::GetPreviewAttributes(FRPGAttributes& OutAttributes) const
{
	...
	OutAttributes = URPGCharacterRulesLibrary::AddAttributes(ClassDefinition->BaseAttributes, RaceDefinition->AttributeBonuses);
	return true;
}
...
bool URPGCharacterCreationWidget::SubmitCharacterCreation()
{
	...
	if (CreationContext == ERPGCharacterCreationContext::CustomRecruit)
	{
		...
		if (!FRPGCustomRecruitService::TryCreateAndRecruit(InventoryComponent, Request, Result))
		{
			const FString ErrorMessage = Result.Error.IsEmpty() ? FString(TEXT("Recrutement impossible.")) : Result.Error;
			SetValidationMessage(FText::FromString(ErrorMessage), true);
			RefreshPreview();
			return false;
		}
...
void URPGCharacterCreationWidget::NotifyCustomRecruitCommitted(int32 CharacterIndex)
{
...
{
	(void)NewText;
	if (CreationContext == ERPGCharacterCreationContext::NewGameMainHero && InventoryComponent && InventoryComponent->HasCompletedInitialCharacterCreation())
	{
		return;
	}
...
void URPGCharacterCreationWidget::HandleNameCommitted(const FText& NewText, ETextCommit::Type CommitMethod)
{
	(void)NewText;
	if (CreationContext == ERPGCharacterCreationContext::NewGameMainHero && InventoryComponent && InventoryComponent->HasCompletedInitialCharacterCreation())
	{
...
void URPGCharacterCreationWidget::HandleRaceSelectionChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
	(void)SelectionType;

	for (URPGRaceAsset* Race : AvailableRaceDefinitions)
	{
		if (Race && Race->IsValidDefinition() && GetDefinitionDisplayName(Race->DisplayName, Race->RaceId).ToString() == SelectedItem)
		{
			RaceDefinition = Race;
			PopulatePortraitOptions();
			SetValidationMessage(FText::GetEmpty(), false);
...
void URPGCharacterCreationWidget::HandleClassSelectionChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
	(void)SelectionType;
	...
	{
		if (CharacterClass && CharacterClass->IsValidDefinition() &&
			GetDefinitionDisplayName(CharacterClass->DisplayName, CharacterClass->ClassId).ToString() == SelectedItem)
		{
			ClassDefinition = CharacterClass;
			SetValidationMessage(FText::GetEmpty(), false);
			RefreshPreview();
...
void URPGCharacterCreationWidget::HandleGenderSelectionChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
	(void)SelectionType;
	...
	SelectPortraitGender(SelectedItem == GetGenderDisplayName(ERPGCharacterPortraitGender::Female).ToString() ? ERPGCharacterPortraitGender::Female
																											  : ERPGCharacterPortraitGender::Male);
}

void URPGCharacterCreationWidget::HandlePortraitVariantSelectionChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
	(void)SelectionType;
	...
	for (const FRPGCharacterPortraitVariant& PortraitVariant : PortraitSet->GetPortraitsForGenderRef(SelectedPortraitGender))
	{
		if (PortraitVariant.IsValidDefinition() && GetPortraitVariantDisplayName(PortraitVariant).ToString() == SelectedItem)
		{
...
bool URPGCharacterCreationWidget::TryResolveSelectedPortraitVariant(FRPGCharacterPortraitVariant& OutVariant) const
{
	...
	if (!SelectedPortraitVariantId.IsNone() && PortraitSet->FindPortraitVariant(SelectedPortraitGender, SelectedPortraitVariantId, OutVariant))
	{
		return true;
	}
	...
	for (const FRPGCharacterPortraitVariant& PortraitVariant : PortraitSet->GetPortraitsForGenderRef(SelectedPortraitGender))
	{
		if (PortraitVariant.IsValidDefinition() && AreSamePortraitTexture(DefaultPortrait, PortraitVariant.Portrait))
		{
...
void URPGCharacterCreationWidget::SelectPortraitVariant(const FRPGCharacterPortraitVariant& PortraitVariant)
{
	if (!PortraitVariant.IsValidDefinition())
	{
		return;
...
void URPGCharacterCreationWidget::RefreshGenderButtonVisualState()
{
	...
	if (Button_GenderMale)
	{
		Button_GenderMale->SetBackgroundColor(SelectedPortraitGender == ERPGCharacterPortraitGender::Male ? SelectedButtonColor : UnselectedButtonColor);
	}

	if (Button_GenderFemale)
	{
		Button_GenderFemale->SetBackgroundColor(SelectedPortraitGender == ERPGCharacterPortraitGender::Female ? SelectedButtonColor : UnselectedButtonColor);
	}
...
void URPGCharacterCreationWidget::SetValidationMessage(const FText& Message, bool bIsError)
{
	...
	Text_ValidationMessage->SetColorAndOpacity(bIsError ? FSlateColor(FLinearColor(0.85f, 0.12f, 0.08f)) : FSlateColor(FLinearColor::White));
	Text_ValidationMessage->SetVisibility(Message.IsEmpty() ? ESlateVisibility::Collapsed : ESlateVisibility::HitTestInvisible);
}
