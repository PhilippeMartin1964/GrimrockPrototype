	...
	const TArray<const URPGSkillAsset*>& SkillDefinitions, FGridSkillsPageView& OutView)
{
	...
	if (!Character.CharacterId.IsValid() || !FRPGSkillService::ValidateSkillState(Character))
	{
		return false;
	}

	TArray<const URPGSkillAsset*> SortedDefinitions;
	if (!ValidateAndSortDefinitions(SkillDefinitions, SortedDefinitions))
	{
		return false;
	}
	...
	if (!FRPGTalentRuntimeService::TryGetSelectedTalents(PartyInventoryComponent, CharacterIndex, TalentViews))
	{
		return false;
	}
	TalentViews.Sort(
		[](const FRPGTalentRuntimeView& Left, const FRPGTalentRuntimeView& Right)
		{
	...
	if (!FRPGTalentRuntimeService::TryGetTalentPointBalance(PartyInventoryComponent, CharacterIndex, Balance))
	{
		return false;
	}
...
void FGridSkillsPageService::ResolveCanonicalSkillDefinitions(TArray<const URPGSkillAsset*>& OutDefinitions)
{
	...
	for (const FPrimaryAssetId& AssetId : AssetIds)
	{
		...
		if (!IsValid(Definition) || !Definition->IsValidDefinition() || Definition->GetPrimaryAssetId() != AssetId ||
