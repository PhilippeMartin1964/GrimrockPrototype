...
TSharedRef<SWidget> URPGStoryCompanionRecruitmentWidget::RebuildWidget()
{
	...
	return SNew(SBorder)
		...
		.VAlign(VAlign_Center)[SNew(SBox).WidthOverride(720.0f)[SNew(SVerticalBox) +
			SVerticalBox::Slot().AutoHeight().Padding(
				0.0f, 0.0f, 0.0f, 12.0f)[SAssignNew(NativeNameText, STextBlock).Justification(ETextJustify::Center).Text(View.DisplayName)] +
			SVerticalBox::Slot().AutoHeight().Padding(
				0.0f, 0.0f, 0.0f, 12.0f)[SAssignNew(NativeIdentityText, STextBlock).Justification(ETextJustify::Center).Text(BuildIdentityLine())] +
			SVerticalBox::Slot().AutoHeight().Padding(
				0.0f, 0.0f, 0.0f, 16.0f)[SAssignNew(NativeDescriptionText, STextBlock).AutoWrapText(true).Text(View.ShortDescription)] +
			SVerticalBox::Slot().AutoHeight().Padding(0.0f, 0.0f, 0.0f, 12.0f)[SAssignNew(NativeDetailsButton, SButton)
					.HAlign(HAlign_Center)
					.Text(LOCTEXT("ShowDetails", "Voir la fiche"))
					.OnClicked(FOnClicked::CreateUObject(this, &URPGStoryCompanionRecruitmentWidget::HandleNativeDetailsClicked))] +
			SVerticalBox::Slot().AutoHeight().Padding(0.0f, 0.0f, 0.0f, 16.0f)[SAssignNew(NativeDetailsText, STextBlock)
					.AutoWrapText(true)
					.Text(BuildDetailsText())
					.Visibility(View.bDetailsVisible ? EVisibility::Visible : EVisibility::Collapsed)] +
			SVerticalBox::Slot().AutoHeight().Padding(0.0f, 0.0f, 0.0f,
				14.0f)[SAssignNew(NativeStatusText, STextBlock).Justification(ETextJustify::Center).AutoWrapText(true).Text(View.StatusText)] +
			SVerticalBox::Slot().AutoHeight()[SNew(SHorizontalBox) +
				SHorizontalBox::Slot().FillWidth(1.0f).Padding(0.0f, 0.0f, 8.0f, 0.0f)[SAssignNew(NativeRecruitButton, SButton)
						.HAlign(HAlign_Center)
						.IsEnabled(View.bCanRecruit)
						.Text(LOCTEXT("Recruit", "Recruter"))
						.OnClicked(FOnClicked::CreateUObject(this, &URPGStoryCompanionRecruitmentWidget::HandleNativeRecruitClicked))] +
				SHorizontalBox::Slot().FillWidth(1.0f).Padding(8.0f, 0.0f, 0.0f, 0.0f)[SNew(SButton)
						.HAlign(HAlign_Center)
						.Text(LOCTEXT("Decline", "Refuser"))
						.OnClicked(FOnClicked::CreateUObject(this, &URPGStoryCompanionRecruitmentWidget::HandleNativeDeclineClicked))]]]];
}

void URPGStoryCompanionRecruitmentWidget::ReleaseSlateResources(bool bReleaseChildren)
{
...
void URPGStoryCompanionRecruitmentWidget::RefreshBoundWidgets()
{
	...
	{
		Text_ShowDetailsAction->SetText(View.bDetailsVisible ? LOCTEXT("HideDetails", "Masquer la fiche") : LOCTEXT("ShowDetails", "Voir la fiche"));
	}
	if (Button_Recruit)
	{
	...
	if (Panel_Details)
	{
		Panel_Details->SetVisibility(View.bDetailsVisible ? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	}
	if (Image_Portrait)
	{
...
void URPGStoryCompanionRecruitmentWidget::RefreshNativeSlate()
{
	...
	if (NativeDetailsText.IsValid())
	{
		...
		NativeDetailsText->SetVisibility(View.bDetailsVisible ? EVisibility::Visible : EVisibility::Collapsed);
	}
	...
	if (NativeDetailsButton.IsValid())
	{
		NativeDetailsButton->SetContent(
			SNew(STextBlock).Text(View.bDetailsVisible ? LOCTEXT("HideDetailsNative", "Masquer la fiche") : LOCTEXT("ShowDetailsNative", "Voir la fiche")));
	}
}
