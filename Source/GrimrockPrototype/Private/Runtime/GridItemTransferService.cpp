...
namespace
{
    FGridItemTransferResult MakeTransferResult (
        EGridItemTransferResult Result,
    ...
    FGridItemTransferResult LogTransferFailure (
        ...
        EGridItemTransferResult Result,
        const FString& Message,
        const FGridItemInstance* Item = nullptr)
    {
        UE_LOG (LogTemp, Warning,
            TEXT ("GridItemTransfer Failed Operation=%s Result=%s Item=%s RuntimeId=%s Message=%s"),
            Operation,
            *UEnum::GetValueAsString (Result),
            Item ? *Item->ItemDefinitionId.ToString () : TEXT ("None"),
            Item ? *Item->RuntimeObjectId.ToString () : TEXT ("None"),
            *Message);
        return MakeTransferResult (Result, Message);
    }

    FGridItemTransferResult LogTransferSuccess (
        const TCHAR* Operation,
        const FGridItemInstance& Item,
        const FString& Message)
    {
        UE_LOG (LogTemp, Log,
            TEXT ("GridItemTransfer Success Operation=%s Item=%s RuntimeId=%s Message=%s"),
            Operation,
    ...
    bool ValidateReceptacleAcceptance (
        ...
        const FGridItemInstance& Item,
        FGridItemTransferResult& OutFailure,
        const TCHAR* Operation)
    {
        ...
        const FString Message = FString::Printf (
            TEXT ("Destination rejected item with reason %d."),
            static_cast<int32> (AcceptanceResult.RejectReason));
        OutFailure = LogTransferFailure (
            Operation,
    ...
    AGridReceptacleActor* Receptacle)
{
    ...
    if (!IsValid (Receptacle))
    {
        return LogTransferFailure (
            ...
            TEXT ("Receptacle destination is invalid."));
    }
    ...
    if (!Candidate.IsValid ())
    {
        return LogTransferFailure (
            Operation,
            EGridItemTransferResult::InvalidItem,
            TEXT ("Inventory slot contains an invalid item."),
            &Candidate);
    }
    ...
    if (!Receptacle->TryInsertItemInstanceFromCursor (Candidate, AcceptedItem))
    {
        if (!CharacterState.InventorySlots[InventorySlotIndex].IsEmpty ())
        {
            return LogTransferFailure (
                Operation,
                EGridItemTransferResult::RollbackFailed,
                TEXT ("Destination insertion failed and the source slot is no longer available."),
                &Candidate);
        }
        ...
        return LogTransferFailure (
            ...
            TEXT ("Destination insertion failed; inventory source was restored."),
            &Candidate);
    }
    ...
    return LogTransferSuccess (
        Operation,
        AcceptedItem,
        TEXT ("Inventory item transferred to receptacle."));
}
    ...
    AGridReceptacleActor* Receptacle)
{
    ...
    if (!Inventory ||
        !Inventory->IsValidCharacterIndex (CharacterIndex) ||
        !Inventory->PartyInventoryState.ActiveEquipment.IsValidIndex (CharacterIndex))
    {
        return LogTransferFailure (
            Operation,
            EGridItemTransferResult::InvalidSource,
            TEXT ("Inventory or equipment character source is invalid."));
    }
    if (!IsValid (Receptacle))
    {
        return LogTransferFailure (
    ...
    if (!SourceItem)
    {
        return LogTransferFailure (
            Operation,
            EGridItemTransferResult::InvalidSource,
            TEXT ("Equipment slot source is invalid."));
    }
    ...
    if (!SourceSnapshot.IsValid ())
    {
        return LogTransferFailure (
            Operation,
            EGridItemTransferResult::InvalidItem,
            TEXT ("Equipment slot is empty or contains an invalid item."),
            &SourceSnapshot);
    }
    ...
    if (!Receptacle->TryInsertItemInstanceFromCursor (SourceSnapshot, AcceptedItem))
    {
        if (SourceItem->IsValid ())
        {
            return LogTransferFailure (
                Operation,
                EGridItemTransferResult::RollbackFailed,
                TEXT ("Destination insertion failed and the equipment slot is no longer available."),
                &SourceSnapshot);
        }
        ...
        *SourceItem = SourceSnapshot;
        ...
        return LogTransferFailure (
            Operation,
            EGridItemTransferResult::DestinationInsertFailed,
            TEXT ("Destination insertion failed; equipment source was restored."),
            &SourceSnapshot);
    }
    ...
    int32 CharacterIndex)
{
    ...
    if (!Inventory || !Inventory->IsValidCharacterIndex (CharacterIndex))
    {
        return LogTransferFailure (
            Operation,
            EGridItemTransferResult::InvalidDestination,
            TEXT ("Inventory destination or character is invalid."));
    }
    ...
    Candidate.LastWorldTransform = bHadVisualActor
        ? SourceSnapshot.ItemActor->GetActorTransform ()
        : FTransform::Identity;
    ...
    if (!Candidate.IsValid ())
    {
        return LogTransferFailure (
            Operation,
            EGridItemTransferResult::InvalidItem,
            TEXT ("Contained receptacle item is invalid."),
            &Candidate);
    }
    if (!Inventory->CanAddItemToCharacterInventory (CharacterIndex, Candidate))
    {
        return LogTransferFailure (
            Operation,
            EGridItemTransferResult::InventoryFull,
            TEXT ("Destination inventory is full."),
            &Candidate);
    }
    ...
    if (!Receptacle->RemoveContainedItemAtIndex (ContainedItemIndex, RemovedItem))
    {
        return LogTransferFailure (
            Operation,
            EGridItemTransferResult::SourceRemoveFailed,
            TEXT ("Failed to remove item from receptacle."),
            &Candidate);
    }

    if (!Inventory->AddItemToCharacterInventory (CharacterIndex, Candidate))
    {
        const int32 RestoredIndex = Receptacle->AddContainedItem (
            SourceSnapshot.ItemDefinitionId,
            SourceSnapshot.ItemDefinition,
            nullptr,
            SourceSnapshot.bWasInitialItem,
            SourceSnapshot.Quantity,
            SourceSnapshot.RuntimeObjectId);
        if (!Receptacle->ContainedItems.IsValidIndex (RestoredIndex))
        {
            return LogTransferFailure (
                Operation,
                EGridItemTransferResult::RollbackFailed,
                TEXT ("Inventory insertion failed and receptacle rollback failed."),
                &Candidate);
        }
        ...
        {
            RestoredItem.ItemActor->SetActorTransform (
                Candidate.LastWorldTransform,
                false,
                nullptr,
        ...
        return LogTransferFailure (
            Operation,
            EGridItemTransferResult::DestinationInsertFailed,
            TEXT ("Inventory insertion failed; receptacle source was restored."),
            &Candidate);
    }
    ...
    return LogTransferSuccess (
        Operation,
        Candidate,
        TEXT ("Receptacle item transferred to inventory."));
}
