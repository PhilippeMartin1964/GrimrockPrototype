...
UGridTurnManagerComponent* AGrimrockPlayerController::ResolveMON5TurnManager() const
{
	...
	if (!TurnManager)
	{
		UE_LOG(LogGridTurnManagerInput, Warning, TEXT("[GridTurnManagerInput] RuntimeActor=%s has no GridTurnManagerComponent."), *GetNameSafe(RuntimeActor));
	}
	return TurnManager;
}
...
void AGrimrockPlayerController::LogMON5CommandResult(const TCHAR* CommandName, bool bSucceeded) const
{
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(INDEX_NONE, 1.50f, bSucceeded ? FColor::Green : FColor::Red, Message);
	}
}
...
void AGrimrockPlayerController::HandleMON11RequestSelectedCharacterAttack()
{
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(INDEX_NONE, 2.0f, bAccepted ? FColor::Green : FColor::Red, Message);
	}
}
#endif
...
bool AGrimrockPlayerController::TryResolveCombatTargetCellUnderCursor(FIntPoint& OutTargetCell) const
{
...
void AGrimrockPlayerController::DrawCombatTargetingPreview(const AGrimrockPartyPawn& PartyPawn) const
{
	const UGridCombatHudWidget* Hud = PartyPawn.CombatHudWidgetInstance;
	const AGridLevelRuntimeActor* RuntimeActor = PartyPawn.LevelRuntimeActor;
	if (!IsValid(Hud) || !Hud->IsCombatActionTargetingActive() || !IsValid(RuntimeActor) || !IsValid(RuntimeActor->LevelAsset))
	{
		return;
	}
...
void AGrimrockPlayerController::HandleLeftMousePressed()
{
	...
	if (MouseResolution.Intent == EGridMouseInteractionIntent::DismissReadableMessage)
	{
		...
		{
			UE_LOG(LogGridMouse, Log, TEXT("GridMouse Click Priority=ReadableMessage Result=Dismissed"));
		}
		return;
	}
				...
				if (MouseResolution.Intent == EGridMouseInteractionIntent::CursorItemWallLock)
				{
					...
					if (bWasUnlocked || !bAcceptedKey)
					{
						...
						const TCHAR* RejectReason = bWasUnlocked
							...
							: (CursorItemDefinition && CursorItemDefinition->ItemType != EGridItemType::Key ? TEXT("NonKeyItem") : TEXT("IncompatibleKey"));
						UE_LOG(LogGridMouse, Log,
						...
						SetGridInteractionCursor(EGridInteractionCursor::CannotPlaceItem, TEXT("ClickWallLockRejected"));
						return;
					}
					...
					else
					{
						UE_LOG(LogGridMouse, Log,
							TEXT("GridMouse Click Priority=CursorItem Branch=WallLockAttempt Item=%s Target=%s Result=Rejected Reason=%s"),
							*CursorItem.ItemDefinitionId.ToString(), *GetNameSafe(WallLockActor), TEXT("InsertFailed"));
					}
					SetGridInteractionCursor(
						bNowUnlocked ? EGridInteractionCursor::Default : EGridInteractionCursor::CannotPlaceItem, TEXT("ClickWallLockAttemptComplete"));
					return;
				}
	...
	const FGridMouseInteractionResolution& MouseResolution, EGridInteractionCursor& OutCursor, const TCHAR*& OutReason) const
{
	...
	switch (MouseResolution.Intent)
	{
		...
		{
			const bool bCanPlaceKey = MouseResolution.bReceptacleAccessible && MouseResolution.WallLockActor &&
				!MouseResolution.WallLockActor->bIsUnlocked &&
				MouseResolution.WallLockActor->CanAcceptKeyDefinition(MouseResolution.CursorItem.ItemDefinitionId);
			OutCursor = bCanPlaceKey ? EGridInteractionCursor::PlaceItem : EGridInteractionCursor::CannotPlaceItem;
			OutReason = bCanPlaceKey ? TEXT("HoverCursorItemWallLock") : TEXT("HoverCursorItemWallLockRejected");
			return true;
		}
...
bool AGrimrockPlayerController::TryGetReceptacleUnderCursor(FHitResult& OutHitResult, AGridReceptacleActor*& OutReceptacleActor) const
{
