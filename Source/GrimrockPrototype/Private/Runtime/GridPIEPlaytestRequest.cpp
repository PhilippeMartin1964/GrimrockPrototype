...
namespace
{
	...
	bool MatchesRequestIdentity(const AGridLevelRuntimeActor* RuntimeActor)
	{
		if (!RuntimeActor || !RuntimeActor->GetWorld() || RuntimeActor->GetWorld()->WorldType != EWorldType::PIE || !FreshPIERequest.bActive)
		{
			return false;
		}

		return RuntimeActor->GetName() == FreshPIERequest.RuntimeActorName &&
			FSoftObjectPath(RuntimeActor->LevelAsset.Get()) == FreshPIERequest.LevelAssetPath &&
			FSoftObjectPath(RuntimeActor->DungeonAsset.Get()) == FreshPIERequest.DungeonAssetPath &&
			RuntimeActor->CurrentDungeonLevelId == FreshPIERequest.DungeonLevelId;
	}
#endif
}
...
{
	void BeginFreshPlaytest(const AGridLevelRuntimeActor* PreparedEditorActor)
	{
#if WITH_EDITOR
		...
		FreshPIERequest.LevelAssetPath = FSoftObjectPath(PreparedEditorActor->LevelAsset.Get());
	...
	{
#if WITH_EDITOR
		return FreshPIERequest.bActive && World && World->WorldType == EWorldType::PIE;
#else
		return false;
#endif
	}
	...
	AGridLevelRuntimeActor* ResolveMatchingRuntimeActor(UWorld* World)
	{
		...
		if (IsActiveForWorld(World))
		{
			for (TActorIterator<AGridLevelRuntimeActor> It(World); It; ++It)
			{
				...
				++RuntimeActorCount;
				const bool bMatches = MatchesRequestIdentity(RuntimeActor);
