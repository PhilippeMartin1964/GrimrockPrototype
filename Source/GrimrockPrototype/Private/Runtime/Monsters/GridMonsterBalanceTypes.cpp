...
bool FGridMonsterBalanceAnalyzer::BuildSnapshot(const UGridMonsterDefinitionAsset* Definition, FGridMonsterBalanceSnapshot& OutSnapshot)
{
	...
	OutSnapshot.ActionPointsPerTurn = Definition->ActionPointsPerTurn;
	...
	{
		const int32 MinimumDamage = Attack.MinDamage + Attack.DamageBonus;
		const int32 MaximumDamage = Attack.MaxDamage + Attack.DamageBonus;

		OutSnapshot.MinimumBaseDamage = FMath::Min(OutSnapshot.MinimumBaseDamage, MinimumDamage);
		OutSnapshot.MaximumBaseDamage = FMath::Max(OutSnapshot.MaximumBaseDamage, MaximumDamage);
		OutSnapshot.MinimumAttackActionPointCost = FMath::Min(OutSnapshot.MinimumAttackActionPointCost, Attack.ActionPointCost);
		OutSnapshot.MaximumAttackActionPointCost = FMath::Max(OutSnapshot.MaximumAttackActionPointCost, Attack.ActionPointCost);
		MidpointSumTimesTwo += static_cast<int64>(MinimumDamage) + MaximumDamage;
	}

	OutSnapshot.AverageBaseDamage = static_cast<float>(MidpointSumTimesTwo) / (2.0f * Definition->Attacks.Num());
	return true;
}
