...
int32 FGridFastHarasserPlanner::ScoreCandidate (
    ...
    int32 CurrentDistanceToParty)
{
    ...
    return DistanceDelta * 100 +
        Candidate.ExitCount * 10 +
        (Candidate.bCanContinuePursuit ? 5 : 0) -
        (Candidate.bIsCulDeSac ? 60 : 0);
}
    ...
    FGridRetreatDecision& OutDecision)
{
    ...
    for (const FGridRetreatCandidate& Candidate : Candidates)
    {
        ...
        if (!OutDecision.bHasRetreatCell ||
            CandidateScore > OutDecision.Score ||
            (CandidateScore == OutDecision.Score &&
                Candidate.StableDirectionOrder < BestStableOrder))
        {
...
bool FGridFastHarasserPlanner::ShouldRetreat (
    float RetreatChance,
...
void FGridFastHarasserPlanner::SelectAggroTargets (
...
{
    OutTargetIds.Reset ();
    if (!SourceObjectId.IsValid () ||
        SourceMonsterId.IsNone () ||
        EncounterGroupId.IsNone () ||
        AggroPropagationRange < 0)
    {
        return;
    }
    ...
    {
        if (!Candidate.SpawnObjectId.IsValid () ||
            Candidate.SpawnObjectId == SourceObjectId ||
            Candidate.MonsterId != SourceMonsterId ||
            Candidate.EncounterGroupId != EncounterGroupId ||
            !Candidate.bIsAlive ||
            !Candidate.bIsEnabled ||
            FGridMonsterPathfinder::ManhattanDistance (SourceCell, Candidate.Cell) >
    ...
    OrderedCandidates.Sort ([] (
        const FGridMonsterAggroCandidate& Left,
        const FGridMonsterAggroCandidate& Right)
    {
        return Left.SpawnObjectId.ToString (EGuidFormats::Digits) <
