...
bool FGridMonsterPathfinder::FindPath (
    ...
    FGridMonsterPathResult& OutResult)
{
    ...
    if (!Context.IsComplete () || Query.Goals.IsEmpty () || Query.MaxVisitedCells <= 0 ||
        !Context.IsValidCell (Query.Start) || !Context.IsWalkableCell (Query.Start))
    {
        return false;
    }
    ...
    while (Queue.IsValidIndex (QueueIndex) && Visited.Num () <= Query.MaxVisitedCells)
    {
        ...
        for (const EGridEdge Direction : GetOrderedDirections ())
        {
            ...
            if (Visited.Contains (Neighbor) || !Context.IsValidCell (Neighbor) ||
                !Context.IsWalkableCell (Neighbor) || !Context.CanTraverse (Current, Neighbor))
            {
                continue;
            }
...
{
    static const TArray<EGridEdge> Directions = {
        EGridEdge::North,
        EGridEdge::East,
        EGridEdge::South,
        EGridEdge::West
...
FIntPoint FGridMonsterPathfinder::GetNeighborCell (const FIntPoint& Cell, EGridEdge Direction)
{
    switch (Direction)
    {
        case EGridEdge::North: return FIntPoint (Cell.X, Cell.Y + 1);
        case EGridEdge::East:  return FIntPoint (Cell.X + 1, Cell.Y);
        case EGridEdge::South: return FIntPoint (Cell.X, Cell.Y - 1);
        case EGridEdge::West:  return FIntPoint (Cell.X - 1, Cell.Y);
        default:               return Cell;
    }
}

EGridEdge FGridMonsterPathfinder::GetDirectionBetweenAdjacentCells (
    ...
    const TFunction<bool (const FIntPoint&, const FIntPoint&)>& CanTraverse)
{
    ...
    const FIntPoint Step (
        FMath::Clamp (TargetCell.X - ObserverCell.X, -1, 1),
        FMath::Clamp (TargetCell.Y - ObserverCell.Y, -1, 1));
...
{
    return HearingRangeCells >= 0 &&
        FGridMonsterPathfinder::ManhattanDistance (ObserverCell, TargetCell) <= HearingRangeCells;
}
