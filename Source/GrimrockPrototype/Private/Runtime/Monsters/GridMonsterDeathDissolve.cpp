...
void UGridMonsterDeathComponent::BeginPlay ()
{
    ...
    {
        OwnerMonster->OnMonsterDied.AddUniqueDynamic (
            this,
            &UGridMonsterDeathComponent::HandleOwnerMonsterDied);
    }
}

void UGridMonsterDeathComponent::EndPlay (
    const EEndPlayReason::Type EndPlayReason)
{
    if (OwnerMonster)
    {
        OwnerMonster->OnMonsterDied.RemoveDynamic (
            this,
            &UGridMonsterDeathComponent::HandleOwnerMonsterDied);
    }
...
void UGridMonsterDeathComponent::HandleOwnerMonsterDied (
...
void UGridMonsterDeathComponent::ScheduleDeathDissolve ()
{
    if (!OwnerMonster ||
        ...
        !OwnerMonster->MonsterDefinition->bEnableDeathDissolve)
    {
        return;
    }
    ...
    const UGridMonsterDefinitionAsset* Definition =
        OwnerMonster->MonsterDefinition;
    const float PresentationDuration = Definition->DeathMontage.IsNull ()
        ? 0.0f
        : FMath::Max (0.01f, Definition->DeathExpectedDuration);
    const float HoldDelay = FMath::Max (0.0f, Definition->DeathDissolveDelay);
    const float StartDelay = PresentationDuration + HoldDelay;

    UE_LOG (
        LogGridMonsterDeathDissolve,
        Log,
        TEXT ("[GridMonsterDeathDissolve] Scheduled Monster=%s Presentation=%.3f Hold=%.3f StartAfter=%.3f"),
    ...
    World->GetTimerManager ().SetTimer (
        DeathDissolveDelayTimerHandle,
        this,
        &UGridMonsterDeathComponent::StartDeathDissolve,
        StartDelay,
        false);
}
...
void UGridMonsterDeathComponent::StartDeathDissolve ()
{
    ...
    const FName ParameterName =
        OwnerMonster->MonsterDefinition->DeathDissolveParameterName;
        ...
        ++MaterialIndex)
    {
        ...
        UMaterialInstanceDynamic* DynamicMaterial =
            Mesh->CreateDynamicMaterialInstance (
                MaterialIndex,
                SourceMaterial);
    ...
    UE_LOG (
        LogGridMonsterDeathDissolve,
        Log,
        TEXT ("[GridMonsterDeathDissolve] Started Monster=%s Materials=%d Parameter=%s Duration=%.3f"),
        *GetNameSafe (OwnerMonster),
        MaterialCount,
        *ParameterName.ToString (),
        OwnerMonster->MonsterDefinition->DeathDissolveDuration);

    World->GetTimerManager ().SetTimer (
        DeathDissolveStepTimerHandle,
        this,
        &UGridMonsterDeathComponent::UpdateDeathDissolve,
...
void UGridMonsterDeathComponent::UpdateDeathDissolve ()
{
    ...
    const float Duration = FMath::Max (
        0.01f,
        OwnerMonster->MonsterDefinition->DeathDissolveDuration);
    DeathDissolveAlpha = FMath::Clamp (
        (World->GetTimeSeconds () - DeathDissolveStartWorldTime) /
            Duration,
        0.0f,
        1.0f);
...
void UGridMonsterDeathComponent::FinishDeathDissolve ()
{
    ...
    UE_LOG (
        LogGridMonsterDeathDissolve,
        Log,
        TEXT ("[GridMonsterDeathDissolve] Completed Monster=%s MeshHidden=true ActorRetained=true"),
        *GetNameSafe (OwnerMonster));
}

void UGridMonsterDeathComponent::ResetDeathDissolvePresentation (
    bool bRestoreOriginalMaterials,
    bool bRestoreVisibility)
{
    ClearDeathDissolveTimers ();
        ...
        OwnerMonster->SkeletalMeshComponent)
    {
        USkeletalMeshComponent* Mesh =
            OwnerMonster->SkeletalMeshComponent;
        for (int32 MaterialIndex = 0;
            MaterialIndex < DeathOriginalMaterials.Num ();
            ++MaterialIndex)
        {
            Mesh->SetMaterial (
                MaterialIndex,
                DeathOriginalMaterials[MaterialIndex]);
        }
    }
