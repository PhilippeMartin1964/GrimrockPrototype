...
bool UGridMonsterBehaviorComponent::InitializeBehavior (
    ...
    AGrimrockPartyPawn* InPartyPawn)
{
    ...
    UGridMonsterOccupancySubsystem* CandidateOccupancy =
...
{
    AGridMonsterActor* Monster = GetMonsterOwner ();
    if (!bInitialized || !IsValid (Monster) || !IsValid (Monster->MonsterDefinition) ||
        !IsValid (RuntimeActor) || !IsValid (PartyPawn))
    {
        return false;
    }
    ...
    bCanHearParty = FGridMonsterPerception::CanHear (
        Monster->CurrentCell,
        PartyCell,
...
bool UGridMonsterBehaviorComponent::GetNextPathDirection (EGridEdge& OutDirection) const
{
    ...
    OutDirection = FGridMonsterPathfinder::GetDirectionBetweenAdjacentCells (
        Monster->CurrentCell,
        LastPath[0]);
...
void UGridMonsterBehaviorComponent::DrawDebugPath (float Duration) const
{
    ...
    FVector Previous = RuntimeActor->GetCellCenterWorld (
    ...
    for (int32 Index = 0; Index < LastPath.Num (); ++Index)
    {
        ...
        DrawDebugBox (
            GetWorld (),
            Current,
            FVector (20.0f, 20.0f, 8.0f),
            Index == LastPath.Num () - 1 ? FColor::Green : FColor::Cyan,
            false,
            Duration,
            0,
            3.0f);
...
void UGridMonsterBehaviorComponent::LogDebugState () const
{
    ...
    UE_LOG (LogTemp, Log,
        ...
        *GetNameSafe (Monster),
        ...
        LastKnownPartyCell.X,
        LastKnownPartyCell.Y,
        bLastPathFound ? TEXT ("true") : TEXT ("false"),
        LastPathGoal.X,
        LastPathGoal.Y,
        LastPath.Num (),
...
bool UGridMonsterBehaviorComponent::BuildAttackGoals (
    const FIntPoint& TargetPartyCell,
    TArray<FIntPoint>& OutGoals) const
{
    OutGoals.Reset ();
    const AGridMonsterActor* Monster = GetMonsterOwner ();
    ...
    for (const EGridEdge FromPartyDirection : FGridMonsterPathfinder::GetOrderedDirections ())
    {
        ...
        const EGridEdge DirectionToParty =
            FGridMonsterPathfinder::GetDirectionBetweenAdjacentCells (Candidate, TargetPartyCell);

        if (!RuntimeActor->IsValidCell (Candidate.X, Candidate.Y) ||
            !RuntimeActor->IsWalkableCell (Candidate.X, Candidate.Y) ||
            DirectionToParty == EGridEdge::None ||
            !RuntimeActor->CanMove (Candidate.X, Candidate.Y, DirectionToParty) ||
            IsCellBlockedForOwner (Candidate))
        {
            continue;
        }
...
bool UGridMonsterBehaviorComponent::CanTraverseCells (
    ...
    const FIntPoint& To) const
{
    ...
    const EGridEdge Direction =
        FGridMonsterPathfinder::GetDirectionBetweenAdjacentCells (From, To);
    return Direction != EGridEdge::None &&
...
void UGridMonsterBehaviorComponent::UpdateOwnerStateFromPerception ()
{
    ...
    if (HasPartyPerception () &&
        (Monster->MonsterState == EGridMonsterState::Dormant || Monster->MonsterState == EGridMonsterState::Idle))
    {
