...
int32 FGridMonsterIdleVariationSelector::BuildPresentationSeed (
...
int32 FGridMonsterIdleVariationSelector::SelectVariationIndex (
    ...
    int32 PreviousVariationIndex,
    bool bAvoidImmediateRepeat)
{
    if (VariationCount <= 0)
    {
        return INDEX_NONE;
    ...
    FRandomStream Stream (
        static_cast<int32> (HashCombine (
            static_cast<uint32> (PresentationSeed),
            MON10IdleVariationSalt)));
    if (bAvoidImmediateRepeat &&
        PreviousVariationIndex >= 0 &&
        PreviousVariationIndex < VariationCount)
    {
        const int32 CompactIndex =
            Stream.RandRange (0, VariationCount - 2);
        return CompactIndex >= PreviousVariationIndex
            ? CompactIndex + 1
            : CompactIndex;
    }
...
float FGridMonsterIdleVariationSelector::SelectDelay (
    int32 PresentationSeed,
    float MinimumDelay,
    float MaximumDelay)
{
    if (!FMath::IsFinite (MinimumDelay) ||
        !FMath::IsFinite (MaximumDelay) ||
        MinimumDelay <= 0.0f ||
...
void UGridMonsterIdleVariationComponent::EndPlay (
...
void UGridMonsterIdleVariationComponent::
RefreshIdleVariationScheduling ()
{
    ...
    if (bIdleVariationActive ||
        IsIdleVariationDelayScheduled ())
    {
        return;
    }
    ...
    ScheduledOccurrenceNumber =
        CompletedOccurrenceNumber + 1;
    const int32 Seed =
        FGridMonsterIdleVariationSelector::BuildPresentationSeed (
    ...
    LastScheduledDelay =
        FGridMonsterIdleVariationSelector::SelectDelay (
            Seed,
            Definition->IdleVariationMinDelay,
            Definition->IdleVariationMaxDelay);

    if (UWorld* World = GetWorld ())
    {
        World->GetTimerManager ().SetTimer (
            IdleVariationDelayTimerHandle,
            this,
            &UGridMonsterIdleVariationComponent::
                HandleIdleVariationDelayTimer,
            LastScheduledDelay,
            false);
    }
}
...
bool UGridMonsterIdleVariationComponent::PlayIdleVariationNow ()
{
    ...
    const int32 OccurrenceNumber =
        ScheduledOccurrenceNumber > CompletedOccurrenceNumber
            ? ScheduledOccurrenceNumber
            : CompletedOccurrenceNumber + 1;
    const int32 Seed =
        FGridMonsterIdleVariationSelector::BuildPresentationSeed (
    ...
    const int32 VariationIndex =
        FGridMonsterIdleVariationSelector::SelectVariationIndex (
            Seed,
            Definition->IdleVariations.Num (),
            PreviousVariationIndex,
            Definition->bAvoidImmediateIdleVariationRepeat);
    if (!Definition->IdleVariations.IsValidIndex (
            VariationIndex))
    {
    ...
    const FGridMonsterIdleVariationDefinition& Variation =
        Definition->IdleVariations[VariationIndex];
    UAnimSequenceBase* Animation =
        Variation.Animation.LoadSynchronous ();
    if (!IsValid (Animation))
    {
        UE_LOG (
            LogGridMonsterIdleVariation,
            Warning,
            TEXT ("[GridMonsterIdle] Monster=%s Variation=%s LoadFailed=%s"),
            *GetNameSafe (OwnerMonster),
            *Variation.VariationId.ToString (),
            *Variation.Animation.ToSoftObjectPath ().ToString ());
        ScheduledOccurrenceNumber = 0;
        RefreshIdleVariationScheduling ();
        return false;
    }
    ...
    float EffectiveDuration =
        Animation->GetPlayLength () / Variation.PlayRate;
    if (!FMath::IsFinite (EffectiveDuration) ||
    ...
    if (bNativePlaybackEnabled)
    {
        ...
        if (AnimInstance)
        {
            ActiveIdleDynamicMontage =
                AnimInstance->PlaySlotAnimationAsDynamicMontage (
                    Animation,
                    Request.SlotName,
                    Request.BlendInTime,
                    Request.BlendOutTime,
                    Request.PlayRate,
                    1,
                    -1.0f,
                    0.0f);
        ...
        if (!ActiveIdleDynamicMontage)
        {
            UE_LOG (
                LogGridMonsterIdleVariation,
                Warning,
                TEXT ("[GridMonsterIdle] Monster=%s Variation=%s NativePlaybackFailed"),
                *GetNameSafe (OwnerMonster),
                *Request.VariationId.ToString ());
        }
    }
    ...
    {
        World->GetTimerManager ().SetTimer (
            IdleVariationDurationTimerHandle,
            this,
            &UGridMonsterIdleVariationComponent::
                HandleIdleVariationDurationTimer,
            Request.EffectiveDuration,
            false);
    }
    return true;
...
bool UGridMonsterIdleVariationComponent::
IsIdleVariationDelayScheduled () const
{
...
LogIdleVariationState () const
{
    UE_LOG (
        ...
        *GetNameSafe (OwnerMonster),
        ...
        bIdleVariationsEnabled ? TEXT ("true") : TEXT ("false"),
        bNativePlaybackEnabled ? TEXT ("true") : TEXT ("false"),
        PlaybackRequestCount,
        IsIdleVariationDelayScheduled ()
            ? TEXT ("true")
            : TEXT ("false"),
        bIdleVariationActive ? TEXT ("true") : TEXT ("false"),
        CurrentVariationIndex,
        PreviousVariationIndex,
        CompletedOccurrenceNumber);
}

bool UGridMonsterIdleVariationComponent::
CanUseIdleVariations () const
{
    if (!bIdleVariationsEnabled ||
        !bInitialized ||
        !IsValid (OwnerMonster) ||
        !IsValid (OwnerMonster->MonsterDefinition) ||
        !OwnerMonster->MonsterDefinition->IsValidDefinition () ||
        !OwnerMonster->MonsterDefinition->bEnableIdleVariations ||
        OwnerMonster->MonsterDefinition->IdleVariations.IsEmpty () ||
        OwnerMonster->IsDead () ||
        !OwnerMonster->bMonsterEnabled ||
        !OwnerMonster->IsRuntimeLevelActive () ||
        OwnerMonster->IsHidden () ||
        (OwnerMonster->MonsterState != EGridMonsterState::Idle &&
            OwnerMonster->MonsterState !=
                EGridMonsterState::Dormant) ||
        OwnerMonster->bIsMoving ||
    ...
    if (OwnerMonster->CombatComponent &&
        OwnerMonster->CombatComponent->
            bAttackPresentationActive)
    {
        return false;
    }
    if (OwnerMonster->DeathComponent &&
        (OwnerMonster->DeathComponent->bDeathCommitted ||
            OwnerMonster->DeathComponent->
