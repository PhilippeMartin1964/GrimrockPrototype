...
namespace
{
	...
	bool IsPartyBetweenGridCells(const AGridLevelRuntimeActor* RuntimeActor, const AGrimrockPartyPawn* PartyPawn)
	{
		...
		const FVector CellCenter = RuntimeActor->GetCellCenterWorld(PartyPawn->CurrentCellX, PartyPawn->CurrentCellY, CurrentLocation.Z);
		const FVector2D DeltaXY(CurrentLocation.X - CellCenter.X, CurrentLocation.Y - CellCenter.Y);
		return !DeltaXY.IsNearlyZero(0.5f);
	}

	bool HasImmediateVisualCombatSource(AGridLevelRuntimeActor* RuntimeActor, UGridTurnManagerComponent* TurnManager)
	{
		UWorld* World = IsValid(RuntimeActor) ? RuntimeActor->GetWorld() : nullptr;
...
namespace GridAutomaticPerceptionEngagement
{
	...
	void Request(AGridLevelRuntimeActor* RuntimeActor, FName Reason)
	{
...
bool UGridAutomaticPerceptionEngagementSubsystem::ProcessPendingEvaluationNow()
{
	...
	if (!TurnManager)
	{
		UE_LOG(LogGridAutomaticEngagement, Verbose, TEXT("[MON14.1] Evaluation skipped Runtime=%s Reason=%s Cause=MissingTurnManager"),
			*GetNameSafe(RuntimeActor), *Reason.ToString());
		return false;
	}

	if (!TurnManager->IsInitialized() && !TurnManager->InitializeTurnManager(RuntimeActor, nullptr))
	{
		UE_LOG(LogGridAutomaticEngagement, Verbose, TEXT("[MON14.1] Evaluation skipped Runtime=%s Reason=%s Cause=TurnManagerNotReady"),
			*GetNameSafe(RuntimeActor), *Reason.ToString());
		return false;
	}
	...
	if (TurnManager->bCombatActive || !bExplorationCompatiblePhase || TurnManager->IsExecutingAction() || TurnManager->IsPartyMotionInProgress())
	{
		UE_LOG(LogGridAutomaticEngagement, Verbose,
			...
			*GetNameSafe(RuntimeActor), *Reason.ToString(), static_cast<int32>(TurnManager->CurrentPhase),
			TurnManager->bCombatActive ? TEXT("true") : TEXT("false"), TurnManager->IsExecutingAction() ? TEXT("true") : TEXT("false"),
	...
	if (UGridMonsterPatrolSubsystem* PatrolSubsystem = World->GetSubsystem<UGridMonsterPatrolSubsystem>())
	{
		PatrolSubsystem->HandlePerceptionEvaluation(RuntimeActor, bStartedCombat, Reason);
	}
