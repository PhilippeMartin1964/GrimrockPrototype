...
{
    if (IsDead () ||
        !IsValid (MonsterDefinition) ||
        MonsterDefinition->HurtMontage.IsNull () ||
    ...
    if ((CombatComponent && CombatComponent->bAttackPresentationActive) ||
        (DeathComponent &&
            (DeathComponent->bDeathCommitted ||
                DeathComponent->bDeathPresentationActive)))
    {
        return false;
    }

    UAnimMontage* HurtMontage =
        MonsterDefinition->HurtMontage.LoadSynchronous ();
