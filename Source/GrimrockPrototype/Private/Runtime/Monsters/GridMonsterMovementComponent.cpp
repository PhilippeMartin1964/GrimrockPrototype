...
void UGridMonsterMovementComponent::TickComponent (
    float DeltaTime,
    ELevelTick TickType,
    FActorComponentTickFunction* ThisTickFunction)
{
    ...
    const float LinearAlpha = MotionDuration <= KINDA_SMALL_NUMBER
        ? 1.0f
        : FMath::Clamp (MotionElapsed / MotionDuration, 0.0f, 1.0f);
    const float VisualAlpha = bUseEaseInOut
...
{
    AGridMonsterActor* Monster = GetMonsterOwner ();
    if (!bInitialized || !IsValid (Monster) || !RuntimeActor || !OccupancySubsystem ||
        Monster->IsDead () || IsBusy () || !IsCardinalDirection (Direction))
    {
        return false;
    }
    ...
    if (!RuntimeActor->CanMove (Monster->CurrentCell.X, Monster->CurrentCell.Y, Direction) ||
        !RuntimeActor->TryGetNeighborCell (
            Monster->CurrentCell.X,
            Monster->CurrentCell.Y,
            Direction,
            TargetX,
    ...
    if (!RuntimeActor->IsValidCell (TargetX, TargetY) ||
        !RuntimeActor->IsWalkableCell (TargetX, TargetY) ||
        RuntimeActor->IsPartyOnCell (TargetX, TargetY) ||
...
{
    AGridMonsterActor* Monster = GetMonsterOwner ();
    if (!bInitialized || !IsValid (Monster) || !RuntimeActor || !OccupancySubsystem || IsBusy () ||
        !RuntimeActor->IsValidCell (Cell.X, Cell.Y) || !RuntimeActor->IsWalkableCell (Cell.X, Cell.Y) ||
...
bool UGridMonsterMovementComponent::ValidateInitialization (
    AGridMonsterActor* Monster,
    AGridLevelRuntimeActor* CandidateRuntime) const
{
    ...
    if (Monster->MonsterDefinition->GridFootprint != FIntPoint (1, 1))
    {
        UE_LOG (LogGridMonsterMovement, Error,
            ...
            *GetNameSafe (Monster),
            ...
            Monster->MonsterDefinition->GridFootprint.Y);
        return false;
    }
...
{
    AGridMonsterActor* Monster = GetMonsterOwner ();
    if (!bInitialized || !IsValid (Monster) || Monster->IsDead () || IsBusy () ||
        !IsCardinalDirection (TargetFacing))
    {
        return false;
    }
...
float UGridMonsterMovementComponent::GetMoveDuration () const
{
    ...
    return FMath::Max (
        KINDA_SMALL_NUMBER,
        Monster && Monster->MonsterDefinition ? Monster->MonsterDefinition->MoveDuration : 0.36f);
}
...
float UGridMonsterMovementComponent::GetTurnDuration () const
{
    ...
    return FMath::Max (
        KINDA_SMALL_NUMBER,
        Monster && Monster->MonsterDefinition ? Monster->MonsterDefinition->TurnDuration : 0.12f);
}

bool UGridMonsterMovementComponent::IsCardinalDirection (EGridEdge Direction)
{
    return Direction == EGridEdge::North || Direction == EGridEdge::East ||
        Direction == EGridEdge::South || Direction == EGridEdge::West;
}
