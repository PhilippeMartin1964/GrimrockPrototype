...
int32 FGridMonsterAudioSelector::BuildPresentationSeed (
...
int32 FGridMonsterAudioSelector::SelectVariationIndex (
...
float FGridMonsterAudioSelector::SelectPitch (
    int32 PresentationSeed,
    float PitchMin,
    float PitchMax)
{
    if (!FMath::IsFinite (PitchMin) ||
...
void UGridMonsterAudioComponent::EndPlay (
...
bool UGridMonsterAudioComponent::PlayAlert ()
{
    ...
    const FGridMonsterAudioEventDefinition* Definition =
        GetMonsterDefinition (EGridMonsterAudioEvent::Alert);
...
bool UGridMonsterAudioComponent::PlayAttack (
...
bool UGridMonsterAudioComponent::PlayAttackImpact (
    const FGridMonsterAttackDefinition& Attack,
    const FGridAttackResult& Result,
    FVector ImpactWorldLocation)
{
    const EGridMonsterAudioEvent Event = Result.bHit
        ? EGridMonsterAudioEvent::ImpactHit
        : EGridMonsterAudioEvent::ImpactMiss;
    const FGridMonsterAudioEventDefinition& Definition = Result.bHit
...
bool UGridMonsterAudioComponent::PlayHurt ()
{
    StopIdleAmbience ();
    const FGridMonsterAudioEventDefinition* Definition =
        GetMonsterDefinition (EGridMonsterAudioEvent::Hurt);
    return Definition && PlayDefinition (
...
bool UGridMonsterAudioComponent::PlayDeath ()
{
    StopIdleAmbience ();
    const FGridMonsterAudioEventDefinition* Definition =
        GetMonsterDefinition (EGridMonsterAudioEvent::Death);
    return Definition && PlayDefinition (
...
bool UGridMonsterAudioComponent::PlayIdleAmbienceNow ()
{
    ...
    return PlayDefinition (
        ...
        OwnerMonster->MonsterDefinition->IdleAudio,
        NAME_None,
        OwnerMonster->GetActorLocation ());
}
...
void UGridMonsterAudioComponent::RefreshIdleAmbienceScheduling ()
{
    ...
    const int32 Seed =
        FGridMonsterAudioSelector::BuildPresentationSeed (
            ...
            EGridMonsterAudioEvent::Idle,
            ++IdleScheduleOccurrence);
    FRandomStream DelayStream (Seed);
    const float Delay = FMath::IsNearlyEqual (
            Definition->IdleAudioMinDelay,
            Definition->IdleAudioMaxDelay)
        ? Definition->IdleAudioMinDelay
        : DelayStream.FRandRange (
            Definition->IdleAudioMinDelay,
            Definition->IdleAudioMaxDelay);
    World->GetTimerManager ().SetTimer (
        IdleAmbienceTimerHandle,
        this,
        &UGridMonsterAudioComponent::HandleIdleAmbienceTimer,
        Delay,
        false);
}
...
int32 UGridMonsterAudioComponent::GetPlaybackRequestCountForEvent (
    EGridMonsterAudioEvent Event) const
{
    const int32 EventIndex = GetEventIndex (Event);
    return EventPlaybackRequestCounts.IsValidIndex (EventIndex)
...
void UGridMonsterAudioComponent::LogMonsterAudioState () const
{
    UE_LOG (
        LogGridMonsterAudio,
        Log,
        TEXT ("[GridMonsterAudio] State Monster=%s Initialized=%s Enabled=%s Native=%s Requests=%d IdleScheduled=%s"),
        *GetNameSafe (OwnerMonster),
        bInitialized ? TEXT ("true") : TEXT ("false"),
        bAudioEnabled ? TEXT ("true") : TEXT ("false"),
        bNativePlaybackEnabled ? TEXT ("true") : TEXT ("false"),
        PlaybackRequestCount,
        IsIdleAmbienceScheduled ()
            ? TEXT ("true")
            : TEXT ("false"));
}
...
{
    if (!CanRequestAudio (Event) ||
        !Definition.IsValidDefinition () ||
        !Definition.HasConfiguredSound ())
    {
        return false;
    }
            ...
            static_cast<double> (Definition.CooldownSeconds))
    {
        UE_LOG (
            LogGridMonsterAudio,
            Verbose,
            TEXT ("[GridMonsterAudio] Rejected Monster=%s Event=%s Reason=Cooldown"),
            *GetNameSafe (OwnerMonster),
    ...
    USoundBase* Sound =
        Definition.Sounds[VariationIndex].LoadSynchronous ();
    if (!IsValid (Sound))
    {
        return false;
    }
    ...
    UE_LOG (
        LogGridMonsterAudio,
        Log,
        TEXT ("[GridMonsterAudio] Seq=%d Monster=%s Event=%s Attack=%s Sound=%s Volume=%.2f Pitch=%.2f Location=%s"),
        ...
        *Sound->GetPathName (),
        ...
        Request.PitchMultiplier,
        *Request.WorldLocation.ToCompactString ());
...
const FGridMonsterAudioEventDefinition*
UGridMonsterAudioComponent::GetMonsterDefinition (
    EGridMonsterAudioEvent Event) const
{
...
bool UGridMonsterAudioComponent::CanRequestAudio (
    EGridMonsterAudioEvent Event) const
{
    return bAudioEnabled &&
        ...
        (Event == EGridMonsterAudioEvent::Death ||
            !OwnerMonster->IsDead ());
}

bool UGridMonsterAudioComponent::CanScheduleIdleAmbience () const
{
    return CanRequestAudio (EGridMonsterAudioEvent::Idle) &&
        OwnerMonster->MonsterDefinition->bEnableIdleAudio &&
        OwnerMonster->MonsterDefinition->IdleAudio
            .HasConfiguredSound () &&
        OwnerMonster->MonsterDefinition->IdleAudio
            .IsValidDefinition () &&
...
int32 UGridMonsterAudioComponent::GetEventIndex (
    EGridMonsterAudioEvent Event) const
{
    const int32 EventIndex = static_cast<int32> (Event);
    return EventIndex >= 0 && EventIndex < AudioEventCount
        ? EventIndex
        : INDEX_NONE;
}
