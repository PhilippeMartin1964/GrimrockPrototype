...
namespace
{
	...
	bool GridInventoryCompatibilityDiagnosticsLooksPotentiallyEquippable(const UGridItemDefinitionAsset* Definition)
	{
		if (!Definition)
		{
			return false;
	...
	void AddEquipmentStatBonus(FGridEquipmentStatBonus& InOutTotal, const FGridEquipmentStatBonus& Bonus)
	{
	...
	void SanitizeCombatHotbarBindings(FGridCharacterInventoryState& CharacterState)
	{
		...
		for (int32 SlotIndex = 0; SlotIndex < CharacterState.CombatHotbarSlots.Num(); ++SlotIndex)
		{
			...
			if (Binding.SourcePolicy == EGridCombatActionSourcePolicy::Equipment)
			{
				...
				{
					AssignedEquipmentRuntimeIds.Add(Binding.PreferredSourceRuntimeId);
				}
			}
...
void UGridPartyInventoryComponent::NotifyPartyInventoryChanged(int32 CharacterIndex)
{
...
bool UGridPartyInventoryComponent::RestorePartyInventoryState(const FGridPartyInventoryState& SavedState, FText& OutError)
{
	...
	for (FGridCharacterInventoryState& Character : RestoredState.ActiveCharacters)
	{
		...
		{
			OutError = FText::FromString(FString::Printf(TEXT("La barre de raccourcis d'un personnage sauvegardé est invalide : %s"), *HotbarError));
			return false;
		}
	}
	...
	for (FGridCharacterInventoryState& Character : RestoredState.CharacterPool)
	{
		...
		if (!ValidateCombatHotbar(Character, HotbarError))
		{
			OutError = FText::FromString(FString::Printf(TEXT("La barre de raccourcis d'un personnage en réserve est invalide : %s"), *HotbarError));
			return false;
		}
	}
...
bool UGridPartyInventoryComponent::GetCharacterSummary(int32 CharacterIndex, FGridInventoryCharacterSummary& OutSummary) const
{
	...
	OutSummary.RaceDisplayName = CharacterState.RaceDisplayName.IsEmpty() ? FText::FromName(CharacterState.RaceId) : CharacterState.RaceDisplayName;
...
bool UGridPartyInventoryComponent::SetCharacterCombatHotbarBinding(int32 CharacterIndex, int32 SlotIndex, const FGridCombatHotbarBinding& Binding)
{
	...
	if (NormalizedBinding.SourcePolicy == EGridCombatActionSourcePolicy::QuickItem &&
		!CharacterHasInventoryItemDefinition(Character, NormalizedBinding.SourceDefinitionId))
	{
		return false;
	}
...
bool UGridPartyInventoryComponent::RemoveFirstItemFromSelectedCharacterInventoryByDefinitionId(FName ItemDefinitionId, FGridItemInstance& OutRemovedItem)
{
	return RemoveFirstItemFromCharacterInventoryByDefinitionId(PartyInventoryState.SelectedCharacterIndex, ItemDefinitionId, OutRemovedItem);
}
...
bool UGridPartyInventoryComponent::RehydrateOwnedItemDefinitions(TFunctionRef<UGridItemDefinitionAsset*(FName)> Resolver, FName& OutMissingDefinitionId)
{
	...
	for (const FName DefinitionId : DefinitionIds)
	{
		...
		if (!Definition || Definition->ItemDefinitionId.IsNone() || Definition->ItemDefinitionId != DefinitionId)
		{
...
bool UGridPartyInventoryComponent::EquipItemFromInventorySlot(int32 CharacterIndex, int32 InventorySlotIndex, EGridEquipmentSlot TargetSlot)
{
	...
	if (!CanEquipItemToSlot(CharacterIndex, ItemToEquip, TargetSlot))
	{
		UE_LOG(LogTemp, Warning, TEXT("GridInventory Equip Failed Character=%d Slot=%s Reason=UnsupportedSlot Item=%s"), CharacterIndex,
			GetEquipmentSlotName(TargetSlot), *ItemToEquip.ItemDefinitionId.ToString());
		return false;
	}
	...
	*TargetItem = ItemToEquip;
	...
	UE_LOG(LogTemp, Log, TEXT("GridInventory Equip Character=%d Slot=%s Item=%s RuntimeId=%s Result=true"), CharacterIndex, GetEquipmentSlotName(TargetSlot),
...
bool UGridPartyInventoryComponent::UnequipItemToInventory(int32 CharacterIndex, EGridEquipmentSlot SourceSlot)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("GridInventory Unequip Failed Character=%d Slot=%s Reason=EmptySlot"), CharacterIndex, GetEquipmentSlotName(SourceSlot));
		return false;
	}
	...
	{
		UE_LOG(
			LogTemp, Warning, TEXT("GridInventory Unequip Failed Character=%d Slot=%s Reason=InventoryFull"), CharacterIndex, GetEquipmentSlotName(SourceSlot));
		return false;
	}
	...
	*EquippedItem = FGridItemInstance();
	...
	UE_LOG(LogTemp, Log, TEXT("GridInventory Unequip Character=%d Slot=%s Item=%s RuntimeId=%s Result=true"), CharacterIndex, GetEquipmentSlotName(SourceSlot),
...
bool UGridPartyInventoryComponent::TryTakeEquipmentSlotToCursor(int32 CharacterIndex, EGridEquipmentSlot SourceSlot)
{
	...
	*EquippedItem = FGridItemInstance();
	...
	UE_LOG(LogTemp, Log, TEXT("GridInventory Cursor Take FromEquipment Character=%d Slot=%s Item=%s RuntimeId=%s Result=true"), CharacterIndex,
		GetEquipmentSlotName(SourceSlot), *ItemToCursor.ItemDefinitionId.ToString(), *ItemToCursor.RuntimeObjectId.ToString());
	return true;
}
	...
	int32 CharacterIndex, EGridEquipmentSlot SourceSlot, FName ExpectedItemDefinitionId, FGridItemInstance& OutWorldItem)
{
	...
	EnsureEquipmentCountMatchesActiveCharacters();
	if (!IsValidCharacterIndex(CharacterIndex) || !PartyInventoryState.ActiveEquipment.IsValidIndex(CharacterIndex) || !IsHandEquipmentSlot(SourceSlot) ||
	...
	UE_LOG(LogTemp, Log, TEXT("GridInventory EquipmentWorldTransfer Extract Character=%d Slot=%s Item=%s RuntimeId=%s Quantity=%d->%d Result=true"),
		CharacterIndex, GetEquipmentSlotName(SourceSlot), *OutWorldItem.ItemDefinitionId.ToString(), *OutWorldItem.RuntimeObjectId.ToString(), QuantityBefore,
		QuantityBefore - 1);
	return true;
}
...
bool UGridPartyInventoryComponent::TryRestoreExtractedItemToEquipment(int32 CharacterIndex, EGridEquipmentSlot TargetSlot, const FGridItemInstance& WorldItem)
{
	...
	if (!WorldItem.IsValid() || WorldItem.Quantity != 1 || !IsValidCharacterIndex(CharacterIndex) ||
		!PartyInventoryState.ActiveEquipment.IsValidIndex(CharacterIndex) || !IsHandEquipmentSlot(TargetSlot))
	{
		return false;
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("GridInventory EquipmentWorldTransfer Restored Character=%d Slot=%s Item=%s RuntimeId=%s Result=true"), CharacterIndex,
		GetEquipmentSlotName(TargetSlot), *WorldItem.ItemDefinitionId.ToString(), *WorldItem.RuntimeObjectId.ToString());
	return true;
}
...
FString UGridPartyInventoryComponent::GetEquipmentDiagnosticsForCharacter(int32 CharacterIndex) const
{
	...
	ForEachEquipmentItem(EquipmentState,
		[&OccupiedSlots](EGridEquipmentSlot Slot, const FGridItemInstance& Item)
		{
			if (Item.IsValid())
			{
				OccupiedSlots.Add(FString::Printf(TEXT("%s=%s"), GetEquipmentSlotName(Slot), *Item.ItemDefinitionId.ToString()));
			}
		});
	...
	*TargetItem = ItemToEquip;
	...
	if (bWasOccupied)
	{
		UE_LOG(LogTemp, Log, TEXT("GridInventory Cursor Equip Swap Character=%d Slot=%s NewItem=%s OldItem=%s Result=true"), CharacterIndex,
			GetEquipmentSlotName(TargetSlot), *ItemToEquip.ItemDefinitionId.ToString(), *PreviouslyEquippedItem.ItemDefinitionId.ToString());
	}
	else
	{
		UE_LOG(LogTemp, Log, TEXT("GridInventory Cursor Equip Character=%d Slot=%s Item=%s RuntimeId=%s Result=true"), CharacterIndex,
			GetEquipmentSlotName(TargetSlot), *ItemToEquip.ItemDefinitionId.ToString(), *ItemToEquip.RuntimeObjectId.ToString());
	}
...
FString UGridPartyInventoryComponent::GetPartyInventoryDiagnostics() const
{
	...
	for (int32 CharacterIndex = 0; CharacterIndex < PartyInventoryState.ActiveCharacters.Num(); ++CharacterIndex)
	{
		...
		Result += FString::Printf(TEXT("[%d] Name=%s Class=%s Level=%d Slots=%d/%d Weight=%.1f/%.1f Overloaded=%s\n"), CharacterIndex,
			*CharacterState.DisplayName.ToString(), *CharacterState.ClassId.ToString(), CharacterState.Level, CountOccupiedSlots(CharacterState),
			CharacterState.InventorySlots.Num(), CharacterState.CurrentWeight, CharacterState.MaxCarryWeight,
		...
		for (int32 SlotIndex = 0; SlotIndex < CharacterState.InventorySlots.Num(); ++SlotIndex)
		{
			...
			Result += FString::Printf(TEXT("      [%d] Item=%s Qty=%d Weight=%.1f Owner=%s\n"), SlotIndex, *Slot.Item.ItemDefinitionId.ToString(),
				Slot.Item.Quantity, Slot.Item.Weight, GetOwnerTypeName(Slot.Item.OwnerType));
		}
	}
...
FString UGridPartyInventoryComponent::GetItemDefinitionDiagnostics() const
{
	...
	for (int32 Index = 0; Index < DefinitionIds.Num(); ++Index)
	{
		...
		Result +=
			FString::Printf(TEXT("[%d] Asset=%s Id=%s Type=%s Weight=%.1f Slots=%s\n"), Index, *Definition->GetName(), *Definition->ItemDefinitionId.ToString(),
				GetItemTypeName(Definition->ItemType), Definition->Weight, *GetEquipmentSlotsText(Definition->CompatibleEquipmentSlots));
	}
...
void UGridPartyInventoryComponent::LogEquipmentCompatibilityDiagnostics() const
{
	...
	for (const FName DefinitionId : DefinitionIds)
	{
		...
		if (Definition->CompatibleEquipmentSlots.Num() == 0 && GridInventoryCompatibilityDiagnosticsLooksPotentiallyEquippable(Definition))
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("GridEquipmentCompatibility Item=%s Warning=PotentiallyEquippableWithoutSlots Type=%s Slots=%s"),
				*Definition->ItemDefinitionId.ToString(), GetItemTypeName(Definition->ItemType), *SlotsText);
		}
		...
		for (const EGridEquipmentSlot Slot : Definition->CompatibleEquipmentSlots)
		{
			if (GridInventoryCompatibilityDiagnosticsIsExcludedPaperDollSlot(Slot))
			{
				...
				UE_LOG(LogTemp, Warning, TEXT("GridEquipmentCompatibility Item=%s Warning=PaperDollExcludedSlot Slot=%s Slots=%s"),
					*Definition->ItemDefinitionId.ToString(), GetEquipmentSlotName(Slot), *SlotsText);
			}
			else if (GridInventoryCompatibilityDiagnosticsIsNewPaperDollSlot(Slot))
			{
				...
				UE_LOG(LogTemp, Log, TEXT("GridEquipmentCompatibility Item=%s UsesNewPaperDollSlot=%s Slots=%s"), *Definition->ItemDefinitionId.ToString(),
					GetEquipmentSlotName(Slot), *SlotsText);
			}
		}
	}
...
void UGridPartyInventoryComponent::LogSelectedCharacterEquipmentStatBonusDiagnostics() const
{
	...
	UE_LOG(LogTemp, Log, TEXT("GridEquipmentStatBonus Diagnostics Character=%d Total=%s"), CharacterIndex, *GetEquipmentStatBonusText(TotalBonus));
	...
	ForEachEquipmentItem(PartyInventoryState.ActiveEquipment[CharacterIndex],
		[this](EGridEquipmentSlot Slot, const FGridItemInstance& Item)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("GridEquipmentStatBonus Item=%s Slot=%s Bonus=%s"), *Definition->ItemDefinitionId.ToString(), GetEquipmentSlotName(Slot),
...
void UGridPartyInventoryComponent::LogSelectedCharacterResistanceDiagnostics() const
{
	...
	UE_LOG(LogTemp, Log, TEXT("GridDamageResistance Diagnostics Character=%d Total=%s"), CharacterIndex, *GetDamageResistanceSetText(TotalResistances));
	...
	ForEachEquipmentItem(PartyInventoryState.ActiveEquipment[CharacterIndex],
		[this](EGridEquipmentSlot Slot, const FGridItemInstance& Item)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("GridDamageResistance Item=%s Slot=%s Resistances=%s"), *Definition->ItemDefinitionId.ToString(),
				GetEquipmentSlotName(Slot), *GetDamageResistanceSetText(Definition->EquipmentResistanceBonus));
		});
}
...
bool UGridPartyInventoryComponent::ValidateCombatHotbar(const FGridCharacterInventoryState& CharacterState, FString& OutError) const
{
	...
	for (int32 SlotIndex = 0; SlotIndex < CharacterState.CombatHotbarSlots.Num(); ++SlotIndex)
	{
		const FGridCombatHotbarBinding& Binding = CharacterState.CombatHotbarSlots[SlotIndex];
		if (Binding.SlotIndex != SlotIndex || !Binding.IsValid())
		{
			OutError = FString::Printf(TEXT("InvalidSlot=%d StoredIndex=%d Action=%s SourcePolicy=%s"), SlotIndex, Binding.SlotIndex,
		...
		if (Binding.SourcePolicy == EGridCombatActionSourcePolicy::Equipment)
		{
			if (AssignedEquipmentRuntimeIds.Contains(Binding.PreferredSourceRuntimeId))
			{
				OutError = FString::Printf(TEXT("DuplicateEquipmentSource Slot=%d RuntimeId=%s"), SlotIndex, *Binding.PreferredSourceRuntimeId.ToString());
				return false;
			}
			AssignedEquipmentRuntimeIds.Add(Binding.PreferredSourceRuntimeId);
		}
		else if (Binding.SourcePolicy == EGridCombatActionSourcePolicy::QuickItem)
		{
			...
			if (AssignedQuickItemDefinitionIds.Contains(Binding.SourceDefinitionId))
			{
				OutError = FString::Printf(TEXT("DuplicateQuickItemSource Slot=%d Definition=%s"), SlotIndex, *Binding.SourceDefinitionId.ToString());
				return false;
			}
			AssignedQuickItemDefinitionIds.Add(Binding.SourceDefinitionId);
		}
