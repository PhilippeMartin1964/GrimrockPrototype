...
namespace
{
	...
	bool IsCardinalMonsterSpawnFacing(EGridEdge Facing)
	{
	...
	if (Entry.Cell == PartyCell)
	{
		...
		if (!bFacesItemEdge && bLogRejection)
		{
			UE_LOG(LogTemp, Warning,
				...
				PartyCell.X, PartyCell.Y, *GetRuntimeEdgeText(PartyPawn->Facing), *GetRuntimeEdgeText(Entry.Edge));
		}
	...
	if (PartyPawn->Facing == EGridEdge::None)
	{
		if (bLogRejection)
		{
			UE_LOG(LogTemp, Warning, TEXT("Grid item pickup rejected: party facing is None. PartyCell=(%d,%d) ItemCell=(%d,%d) ItemEdge=%s."), PartyCell.X,
				PartyCell.Y, Entry.Cell.X, Entry.Cell.Y, *GetRuntimeEdgeText(Entry.Edge));
		}
		return false;
	}
	...
	if (!TryGetNeighborCell(PartyCell.X, PartyCell.Y, PartyPawn->Facing, FrontCellX, FrontCellY) || Entry.Cell != FIntPoint(FrontCellX, FrontCellY))
	{
		if (bLogRejection)
		{
			UE_LOG(LogTemp, Warning,
				...
				PartyCell.X, PartyCell.Y, *GetRuntimeEdgeText(PartyPawn->Facing), Entry.Cell.X, Entry.Cell.Y, *GetRuntimeEdgeText(Entry.Edge));
		}
		return false;
	}
...
void AGridLevelRuntimeActor::ApplyMonsterPlacementMetadata(AGridMonsterActor* Monster) const
{
	...
	if (Placement)
	{
		...
		{
			Monster->HomeDungeonLevelId = ResolveRuntimeStateLevelId(DungeonAsset, CurrentDungeonLevelId);
		}
	}
}
	...
	const FGridLevelObjectData& ObjectData, UGridMonsterDefinitionAsset*& OutDefinition, TSubclassOf<AGridMonsterActor>& OutActorClass, FString& OutError) const
{
	...
	if (!IsCardinalMonsterSpawnFacing(ObjectData.InitialFacing))
	{
...
bool AGridLevelRuntimeActor::GetMonsterSpawnTransform(const FGridLevelObjectData& ObjectData, FTransform& OutTransform) const
{
	...
	if (!LevelAsset || ObjectData.Type != EGridLevelObjectType::MonsterSpawn || !LevelAsset->IsValidCoord(ObjectData.CellX, ObjectData.CellY) ||
		!IsCardinalMonsterSpawnFacing(ObjectData.InitialFacing))
	{
		return false;
	}
...
bool AGridLevelRuntimeActor::TeleportSpawnedMonster(FGuid SpawnId, int32 TargetCellX, int32 TargetCellY, EGridEdge TargetFacing)
{
	...
	if (!ObjectData || !Monster || (Movement && !Movement->IsInitialized()) || Monster->IsDead() || !IsCardinalMonsterSpawnFacing(TargetFacing) ||
		!IsValidCell(TargetCellX, TargetCellY) || !IsWalkableCell(TargetCellX, TargetCellY) || IsPartyOnCell(TargetCellX, TargetCellY) ||
		bGeneratedMonsterOccupiesTarget || !Occupancy || Occupancy->IsCellBlocked(TargetCell, Monster))
	{
		UE_LOG(LogGridMonsterState, Log, TEXT("[GridMonsterLifecycle] TeleportRejected SpawnId=%s Target=(%d,%d) Facing=%s Reason=InvalidOrOccupiedTarget"),
			*SpawnId.ToString(EGuidFormats::DigitsWithHyphens), TargetCellX, TargetCellY, *GetRuntimeEdgeText(TargetFacing));
		return false;
	}
	...
	UE_LOG(LogGridMonsterState, Log, TEXT("[GridMonsterLifecycle] Teleported SpawnId=%s From=(%d,%d) To=(%d,%d) Facing=%s Encounter=%s"),
		*SpawnId.ToString(EGuidFormats::DigitsWithHyphens), PreviousCell.X, PreviousCell.Y, TargetCellX, TargetCellY, *GetRuntimeEdgeText(TargetFacing),
...
AGridMonsterActor* AGridLevelRuntimeActor::AddMonsterSpawnActor(const FGridLevelObjectData& ObjectData, const FGridRuntimeMonsterState* RestoreState)
{
	...
	if (RestoreState)
	{
		...
		SpawnData.InitialFacing = IsCardinalMonsterSpawnFacing(RestoreState->Facing) ? RestoreState->Facing : ObjectData.InitialFacing;
	...
	UE_LOG(LogGridMonsterState, Log, TEXT("[GridMonsterSpawn] Spawned SpawnId=%s DefinitionId=%s Class=%s Cell=(%d,%d) Facing=%s Encounter=%s RuntimeLevel=%s"),
		...
		*GetRuntimeEdgeText(SpawnData.InitialFacing), *SpawnData.EncounterGroupId.ToString(), *Monster->HomeDungeonLevelId.ToString());
	return Monster;
}
...
FString AGridLevelRuntimeActor::GetPIEReadinessDiagnostics() const
{
	...
	if (LevelAsset)
	{
		Result += FString::Printf(TEXT("Start: Cell=(%d,%d) Facing=%s Valid=%s\n"), LevelAsset->StartCellX, LevelAsset->StartCellY,
			*GetRuntimeEdgeText(LevelAsset->StartFacing), *GetRuntimeBoolText(bHasValidStart));
		Result += FString::Printf(TEXT("Asset Stats: Cells=%d Objects=%d Links=%d TransitionObjects=%d\n"), LevelAsset->Cells.Num(), LevelAsset->Objects.Num(),
			LevelAsset->Links.Num(), CountRuntimeTransitionObjects(LevelAsset));
	}
	else
	{
	...
	Result += FString::Printf(TEXT("Components: Activation=%s Doors=%s EditorPreview=%s\n"), *GetRuntimeBoolText(ActivationComponent != nullptr),
		*GetRuntimeBoolText(DoorSystemComponent != nullptr), *GetRuntimeBoolText(EditorPreviewComponent != nullptr));
