	...
	const FGridItemActionContext& ItemContext, FGridFacingTargetContext& OutFacingTarget, TArray<FGridItemContextAction>& OutActions)
{
	...
	{
		Definition = ItemContext.PartyPawn->PartyInventoryComponent->FindItemDefinition(ItemContext.Item.ItemDefinitionId);
	}

	ResolveFacingTarget(ItemContext.PartyPawn, ItemContext.Item, Definition, OutFacingTarget);

	AddAction(OutActions, EGridItemActionType::Examine, NSLOCTEXT("GridItemActions", "Examine", "Examiner"));

	if (ItemContext.EquipmentSlot != EGridEquipmentSlot::None || ItemContext.Item.OwnerType == EGridItemOwnerType::EquipmentSlot)
	{
		...
		AddAction(OutActions, EGridItemActionType::Unequip, UnequipLabel, nullptr, SourceEquipmentSlot);
	}
	else if (Definition)
	{
		if (Definition->CanEquipToSlot(EGridEquipmentSlot::MainHand))
		{
			AddAction(OutActions, EGridItemActionType::Equip,
				FText::Format(NSLOCTEXT("GridItemActions", "EquipSpecificSlot", "S'equiper : {0}"), GetEquipmentSlotActionLabel(EGridEquipmentSlot::MainHand)),
				nullptr, EGridEquipmentSlot::MainHand);
		}
		if (Definition->CanEquipToSlot(EGridEquipmentSlot::OffHand))
		{
			AddAction(OutActions, EGridItemActionType::Equip,
				FText::Format(NSLOCTEXT("GridItemActions", "EquipSpecificSlot", "S'equiper : {0}"), GetEquipmentSlotActionLabel(EGridEquipmentSlot::OffHand)),
				nullptr, EGridEquipmentSlot::OffHand);
		}
		for (const EGridEquipmentSlot CompatibleSlot : Definition->CompatibleEquipmentSlots)
		{
			if (CompatibleSlot == EGridEquipmentSlot::None || CompatibleSlot == EGridEquipmentSlot::MainHand || CompatibleSlot == EGridEquipmentSlot::OffHand)
			{
				continue;
			}
			...
			AddAction(OutActions, EGridItemActionType::Equip, FText::Format(NSLOCTEXT("GridItemActions", "EquipSpecificSlot", "S'equiper : {0}"), SlotLabel),
				nullptr, CompatibleSlot);
		}
	}

	if (Definition && (Definition->ItemType == EGridItemType::Potion || Definition->ItemType == EGridItemType::Food))
	{
	...
	{
		AddAction(
			OutActions, EGridItemActionType::InsertIntoTarget, NSLOCTEXT("GridItemActions", "InsertIntoLock", "Insérer dans la serrure"), &OutFacingTarget);
	}
	else if ((bIsInventorySlotSource || bIsEquipmentSlotSource) && OutFacingTarget.bIsValid && OutFacingTarget.bAcceptsCurrentItem &&
		(OutFacingTarget.TargetType == EGridFacingTargetType::Receptacle || OutFacingTarget.TargetType == EGridFacingTargetType::TorchHolder))
	{
		const bool bLooksLikeAlcove = OutFacingTarget.TargetActor && OutFacingTarget.TargetActor->GetName().Contains(TEXT("Alcove"));
		const FText Label = OutFacingTarget.TargetType == EGridFacingTargetType::TorchHolder
			...
			: (bLooksLikeAlcove ? NSLOCTEXT("GridItemActions", "PlaceInAlcove", "Placer dans l'alcôve")
	...
	AddAction(OutActions, EGridItemActionType::DropToGround, NSLOCTEXT("GridItemActions", "DropToGround", "Déposer au sol"), nullptr, ItemContext.EquipmentSlot,
		bCanDrop, bCanDrop ? FText::GetEmpty() : NSLOCTEXT("GridItemActions", "NoActiveLevel", "Aucun niveau actif"));

	UE_LOG(LogGridItemActions, Log, TEXT("GridItemActions Build Item=%s Target=%s"), *ItemContext.Item.ItemDefinitionId.ToString(),
		OutFacingTarget.bIsValid ? *GetNameSafe(OutFacingTarget.TargetActor) : TEXT("None"));
	for (const FGridItemContextAction& Action : OutActions)
	{
		UE_LOG(LogGridItemActions, Log, TEXT("GridItemActions Action=%s Enabled=%s Reason=%s"), ToActionTypeText(Action.ActionType),
			Action.bEnabled ? TEXT("true") : TEXT("false"), Action.DisabledReason.IsEmpty() ? TEXT("None") : *Action.DisabledReason.ToString());
	}

	return OutActions.Num() > 0;
}

bool UGridItemContextActionLibrary::ResolveFacingTarget(AGrimrockPartyPawn* PartyPawn, const FGridItemInstance& CurrentItem,
	UGridItemDefinitionAsset* ItemDefinition, FGridFacingTargetContext& OutFacingTarget, float TraceDistance)
{
	OutFacingTarget = FGridFacingTargetContext();
	...
	const FVector TraceStart = PartyPawn->Camera ? PartyPawn->Camera->GetComponentLocation() : PartyPawn->GetActorLocation();
	const FVector TraceEnd = TraceStart + PartyPawn->GetActorForwardVector() * FMath::Max(0.f, TraceDistance);
	...
	const bool bHasTraceHit = PartyPawn->GetWorld()->LineTraceSingleByChannel(HitResult, TraceStart, TraceEnd, ECC_Visibility, QueryParams);
	UE_LOG(LogGridItemActions, Log, TEXT("GridFacingTarget TraceHit Actor=%s Component=%s"), bHasTraceHit ? *GetNameSafe(HitResult.GetActor()) : TEXT("None"),
		bHasTraceHit ? *GetNameSafe(HitResult.GetComponent()) : TEXT("None"));

	AActor* TargetActor = bHasTraceHit ? ResolveTargetActor(HitResult.GetActor()) : nullptr;
	if (TargetActor)
	{
	...
	{
		OutFacingTarget.TargetType = EGridFacingTargetType::WallLock;
		OutFacingTarget.bAcceptsCurrentItem = !WallLockActor->bIsUnlocked && WallLockActor->CanAcceptKeyDefinition(CurrentItem.ItemDefinitionId);
		if (!OutFacingTarget.bAcceptsCurrentItem)
		{
			OutFacingTarget.IncompatibilityReason = NSLOCTEXT("GridItemActions", "WrongKey", "Cet objet ne peut pas être utilisé ici");
		}
	}
	else if (const AGridReceptacleActor* ReceptacleActor = Cast<AGridReceptacleActor>(TargetActor))
	{
		...
		OutFacingTarget.bAcceptsCurrentItem = ReceptacleActor->EvaluateItemAcceptance(CurrentItem, AcceptanceResult, false);
		const bool bLooksLikeTorchHolder = TargetActor->GetName().Contains(TEXT("TorchHolder")) ||
			(ItemDefinition && ItemDefinition->ItemType == EGridItemType::Torch && OutFacingTarget.bAcceptsCurrentItem && !ReceptacleActor->bAcceptAnyItem);
		OutFacingTarget.TargetType = bLooksLikeTorchHolder ? EGridFacingTargetType::TorchHolder : EGridFacingTargetType::Receptacle;
		if (!OutFacingTarget.bAcceptsCurrentItem)
		{
			OutFacingTarget.IncompatibilityReason = NSLOCTEXT("GridItemActions", "TargetRejectsItem", "Cet objet ne peut pas être utilisé ici");
		}
	}
	else if (const AGridGenericObjectActor* GenericActor = Cast<AGridGenericObjectActor>(TargetActor); GenericActor && GenericActor->HasReadableText())
	{
	...
	else if (TargetActor && TargetActor->GetClass()->ImplementsInterface(UGridInteractableInterface::StaticClass()))
	{
		OutFacingTarget.TargetType = EGridFacingTargetType::Mechanism;
	}

	if (OutFacingTarget.TargetType == EGridFacingTargetType::None && PartyPawn->LevelRuntimeActor)
	{
		if (AGridWallLockActor* WallLockActor =
				PartyPawn->LevelRuntimeActor->FindWallLockAtEdge(PartyPawn->CurrentCellX, PartyPawn->CurrentCellY, PartyPawn->Facing))
		{
			OutFacingTarget.bIsValid = true;
		...
		{
			UE_LOG(LogGridItemActions, Log, TEXT("GridFacingTarget NoWallLockFound Cell=(%d,%d) Edge=%d"), PartyPawn->CurrentCellX, PartyPawn->CurrentCellY,
	...
	{
		UE_LOG(LogGridItemActions, Log, TEXT("GridFacingTarget Resolved WallLock ObjectId=%s"), *OutFacingTarget.TargetObjectId.ToString());
	}

	if (OutFacingTarget.TargetType == EGridFacingTargetType::None)
	{
		OutFacingTarget = FGridFacingTargetContext();
		UE_LOG(LogGridItemActions, Log, TEXT("GridFacingTarget TargetActor=None ObjectId=None Type=None"));
		return false;
	}

	UE_LOG(LogGridItemActions, Log, TEXT("GridFacingTarget TargetActor=%s ObjectId=%s Type=%s"), *GetNameSafe(OutFacingTarget.TargetActor),
		OutFacingTarget.TargetObjectId.IsValid() ? *OutFacingTarget.TargetObjectId.ToString() : TEXT("None"), ToTargetTypeText(OutFacingTarget.TargetType));
	return true;
}
