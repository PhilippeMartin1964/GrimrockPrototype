...
{
	return IsDoorOpenOnEdge(X, Y, Edge) ? CloseDoorOnEdge(X, Y, Edge) : OpenDoorOnEdge(X, Y, Edge);
}
...
{
	return FString::Printf(TEXT("Doors | Indexed=%d Actors=%d Blocked=%d"), DoorIndexByEdge.Num(), DoorActorByEdge.Num(), RuntimeBlockedDoorEdges.Num());
}
