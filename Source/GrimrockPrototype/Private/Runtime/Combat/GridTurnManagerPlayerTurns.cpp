...
bool UGridTurnManagerComponent::GetPlayerCharacterTurnState (
    ...
    FGridPlayerCharacterTurnState& OutTurnState) const
{
    ...
    else
    {
            ...
            CurrentPhase == EGridCombatPhase::PlayerPhase)
        {
            ...
            (CurrentPhase == EGridCombatPhase::EnemyPhase ||
                CurrentPhase == EGridCombatPhase::EndingRound))
        {
    ...
    int32 CharacterIndex) const
{
    ...
    return GetPlayerCharacterTurnState (CharacterIndex, TurnState) &&
        TurnState.State == EGridCombatantTurnState::Active &&
        TurnState.RemainingActionPoints > 0;
}

bool UGridTurnManagerComponent::CanCharacterSpendActionPoints (
    int32 CharacterIndex,
    int32 ActionPointCost) const
{
    ...
    FGridPlayerCharacterTurnState TurnState;
    return GetPlayerCharacterTurnState (CharacterIndex, TurnState) &&
        TurnState.CanSpend (ActionPointCost);
}
    ...
    int32 CharacterIndex) const
{
    FGridPlayerCharacterTurnState TurnState;
    return GetPlayerCharacterTurnState (CharacterIndex, TurnState) &&
        TurnState.MaximumActionPoints > 0 &&
...
void UGridTurnManagerComponent::BeginPlayerCharacterPhase ()
{
    ...
    const int32 MaximumActionPoints =
        ClampPlayerActionPoints (BasePlayerActionPointsPerTurn);
    ...
    for (const FGridPlayerCharacterTurnState& TurnState :
...
void UGridTurnManagerComponent::CompletePlayerCharacterPhase ()
{
    ...
    const int32 CharacterCount =
        PartyPawn->PartyInventoryComponent->PartyInventoryState
            .ActiveCharacters.Num ();
    for (int32 CharacterIndex = 0;
        CharacterIndex < CharacterCount;
        ++CharacterIndex)
    {
        FGridPlayerCharacterTurnState* TurnState =
            EnsurePlayerCharacterTurnState (CharacterIndex);
        if (!TurnState)
        {
            continue;
        }
...
bool UGridTurnManagerComponent::SpendPlayerCharacterActionPoints (
    int32 CharacterIndex,
    int32 ActionPointCost)
{
    ...
    UE_LOG (
        LogGridTurnManager,
        Log,
        TEXT ("[GridPlayerTurn] Character=%d Cost=%d AP=%d/%d State=%s"),
        CharacterIndex,
        ActionPointCost,
...
void UGridTurnManagerComponent::RefreshPlayerCharacterVitalState (
    int32 CharacterIndex)
{
    if (!IsValid (PartyPawn) ||
        !IsValid (PartyPawn->PartyInventoryComponent))
    ...
    const bool bDefeated =
        Characters[CharacterIndex].DerivedStats.CurrentHealth <= 0;
    if (bDefeated &&
        TurnState->State != EGridCombatantTurnState::Defeated)
    {
        TurnState->State = EGridCombatantTurnState::Defeated;
...
UGridTurnManagerComponent::EnsurePlayerCharacterTurnState (
    int32 CharacterIndex)
{
    ...
    if (FGridPlayerCharacterTurnState* Existing =
        FindPlayerCharacterTurnState (DerivedState.CharacterId))
    {
...
const FGridPlayerCharacterTurnState*
UGridTurnManagerComponent::FindPlayerCharacterTurnState (
...
void UGridTurnManagerComponent::BroadcastPlayerCharacterTurnState (
