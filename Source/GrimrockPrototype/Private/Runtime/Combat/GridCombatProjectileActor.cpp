...
void AGridCombatProjectileActor::Tick (float DeltaSeconds)
{
    ...
    const float Alpha = TravelDuration > KINDA_SMALL_NUMBER
        ? FMath::Clamp (ElapsedTravelTime / TravelDuration, 0.0f, 1.0f)
        : 1.0f;

    SetActorLocation (EvaluateTrajectoryLocation (
        SourceWorldLocation,
        TargetWorldLocation,
        Alpha));
...
bool AGridCombatProjectileActor::InitializeProjectilePresentation (
    UStaticMesh* InMesh,
...
{
    if (!IsValid (InMesh) ||
        InSourceWorldLocation.ContainsNaN () ||
        InTargetWorldLocation.ContainsNaN () ||
        !FMath::IsFinite (InTravelDuration) ||
        InTravelDuration <= 0.0f ||
        InVisualScale.ContainsNaN () ||
        InVisualScale.X <= 0.0f ||
        InVisualScale.Y <= 0.0f ||
        InVisualScale.Z <= 0.0f)
    {
        return false;
    }
...
float AGridCombatProjectileActor::CalculateLaunchDelay (
...
FVector AGridCombatProjectileActor::EvaluateTrajectoryLocation (
