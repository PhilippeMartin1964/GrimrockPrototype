...
void UGridTurnManagerComponent::ResolveSuggestedCombatActionTarget(FGridAvailableCombatAction& Action) const
{
	...
	if (!IsValid(RuntimeActor) || !IsValid(PartyPawn) || Action.Definition.TargetingPolicy != EGridCombatTargetingPolicy::FirstAxialTarget ||
	...
	const FGridAvailableCombatAction& Action, const FIntPoint& TargetCell, FGridCombatActionTargetingPreview& OutPreview) const
{
	...
	if (!IsMON1286TargetedSource(Action.Definition.SourcePolicy) || Action.Definition.ResolutionProfile != EGridCombatActionResolutionProfile::Attack ||
		!IsMON1286ExplicitTargetingPolicy(Action.Definition.TargetingPolicy) || !Action.Definition.OffensiveProfile.IsValid())
	{
...
{
	UGridPartyInventoryComponent* Inventory = IsValid(PartyPawn) ? PartyPawn->PartyInventoryComponent.Get() : nullptr;
	if (!IsValid(Inventory) || !Preview.bValid || Preview.TargetMonsterIds.IsEmpty() ||
		!Inventory->PartyInventoryState.ActiveCharacters.IsValidIndex(Action.CharacterIndex))
	{
		return false;
	}
	...
	OutResult.TargetedActionResult.TargetMonsterIds = Preview.TargetMonsterIds;
	if (bQuickItem)
	{
