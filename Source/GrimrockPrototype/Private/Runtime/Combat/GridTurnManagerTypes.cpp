...
namespace
{
	...
	FGridCombatAction MakeMoveAction(const FGuid& SourceActorId, const FIntPoint& TargetCell)
	{
		FGridCombatAction Action;
		Action.ActionId = FGuid::NewGuid();
	...
	FGridCombatAction MakeMeleeAttackAction(const FGuid& SourceActorId, const FIntPoint& PartyCell, FName AttackId, int32 ActionPointCost)
	{
		FGridCombatAction Action;
		Action.ActionId = FGuid::NewGuid();
	...
	FGridCombatAction MakeWaitAction(const FGuid& SourceActorId, const FIntPoint& CurrentCell)
	{
		FGridCombatAction Action;
		Action.ActionId = FGuid::NewGuid();
...
void FGridInitiativeOrderBuilder::RollAndSort(TArray<FGridCombatantInitiativeEntry>& Entries, FRandomStream& RandomStream)
{
	for (FGridCombatantInitiativeEntry& Entry : Entries)
	{
		Entry.InitiativeRoll = RandomStream.RandRange(1, 20);
		Entry.InitiativeTotal = Entry.InitiativeBase + Entry.InitiativeRoll;
	}

	Sort(Entries);
}

void FGridInitiativeOrderBuilder::Sort(TArray<FGridCombatantInitiativeEntry>& Entries)
{
	Entries.Sort(
		[](const FGridCombatantInitiativeEntry& Left, const FGridCombatantInitiativeEntry& Right)
		{
			return FGridInitiativeOrderBuilder::Precedes(Left, Right);
		});
}

bool FGridInitiativeOrderBuilder::Precedes(const FGridCombatantInitiativeEntry& Left, const FGridCombatantInitiativeEntry& Right)
{
	const int32 LeftEffectiveTotal = Left.GetEffectiveInitiativeTotal();
	const int32 RightEffectiveTotal = Right.GetEffectiveInitiativeTotal();
	...
	const FString LeftId = Left.CombatantId.ToString(EGuidFormats::Digits);
	const FString RightId = Right.CombatantId.ToString(EGuidFormats::Digits);
	if (LeftId != RightId)
	{
...
{
	if (Phase != EGridCombatPhase::Exploration && Phase != EGridCombatPhase::Victory && Phase != EGridCombatPhase::Defeat)
	{
		return false;
	}
...
bool FGridTurnPhaseStateMachine::BeginCombatantTurn(EGridCombatantSide Side)
{
	if (Phase != EGridCombatPhase::PlayerPhase && Phase != EGridCombatPhase::EnemyPhase)
	{
		return false;
	}

	Phase = Side == EGridCombatantSide::Party ? EGridCombatPhase::PlayerPhase : EGridCombatPhase::EnemyPhase;
	return true;
}
...
{
	if (!IsCombatActive() || (ResultPhase != EGridCombatPhase::Victory && ResultPhase != EGridCombatPhase::Defeat))
	{
		return false;
	}
...
{
	return Phase == EGridCombatPhase::StartingCombat || Phase == EGridCombatPhase::PlayerPhase || Phase == EGridCombatPhase::EnemyPhase ||
		Phase == EGridCombatPhase::EndingRound;
}

void FGridMonsterTurnPlanner::BuildMovementTurn(const FGuid& SourceActorId, const FIntPoint& StartCell, EGridEdge StartFacing, const TArray<FIntPoint>& Path,
	int32 AvailableActionPoints, TArray<FGridCombatAction>& OutActions)
{
	...
	for (const FIntPoint& Step : Path)
	{
		...
		const EGridEdge DesiredFacing = FGridMonsterPathfinder::GetDirectionBetweenAdjacentCells(SimulatedCell, Step);
		if (!AppendTurnsToward(SourceActorId, SimulatedCell, DesiredFacing, SimulatedFacing, OutActions))
		{
			break;
		}
...
void FGridMonsterTurnPlanner::BuildDirectMeleeTurn(const FGuid& SourceActorId, const FIntPoint& StartCell, EGridEdge StartFacing, const FIntPoint& PartyCell,
	const TArray<FIntPoint>& Path, int32 AvailableActionPoints, FName AttackId, int32 AttackActionPointCost, TArray<FGridCombatAction>& OutActions)
{
	...
	while (true)
	{
		const int32 DistanceToParty = FMath::Abs(SimulatedCell.X - PartyCell.X) + FMath::Abs(SimulatedCell.Y - PartyCell.Y);
		if (DistanceToParty == 1 && !AttackId.IsNone() && RemainingActionPoints >= SafeAttackCost)
		{
			const EGridEdge AttackFacing = FGridMonsterPathfinder::GetDirectionBetweenAdjacentCells(SimulatedCell, PartyCell);
			if (AppendTurnsToward(SourceActorId, SimulatedCell, AttackFacing, SimulatedFacing, OutActions))
			{
				OutActions.Add(MakeMeleeAttackAction(SourceActorId, PartyCell, AttackId, SafeAttackCost));
			}
			break;
		}
...
void FGridMonsterTurnPlanner::BuildFastHarasserTurn(const FGuid& SourceActorId, const FIntPoint& StartCell, EGridEdge StartFacing, const FIntPoint& PartyCell,
...
{
	BuildDirectMeleeTurn(SourceActorId, StartCell, StartFacing, PartyCell, Path, AvailableActionPoints, AttackId, AttackActionPointCost, OutActions);
	...
	for (const FGridCombatAction& Action : OutActions)
	{
		...
		{
			const EGridEdge TurnFacing = FGridMonsterPathfinder::GetDirectionBetweenAdjacentCells(SimulatedCell, Action.TargetCell);
		...
		{
			SimulatedFacing = FGridMonsterPathfinder::GetDirectionBetweenAdjacentCells(SimulatedCell, Action.TargetCell);
	...
	if (!bMeleeAttackPlanned || RemainingActionPoints < 1 ||
		FGridMonsterPathfinder::GetNeighborCell(SimulatedCell, RetreatDecision.RetreatDirection) != RetreatDecision.RetreatCell)
	{
		return;
	}

	const int32 FirstRetreatActionIndex = OutActions.Num();
	if (!AppendTurnsToward(SourceActorId, SimulatedCell, RetreatDecision.RetreatDirection, SimulatedFacing, OutActions))
	{
		return;
	}
	...
	FGridCombatAction RetreatMove = MakeMoveAction(SourceActorId, RetreatDecision.RetreatCell);
