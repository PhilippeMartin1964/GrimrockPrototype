...
namespace
{
	...
	{
		return !Value.ContainsNaN() && FMath::IsFinite(Value.Pitch) && FMath::IsFinite(Value.Yaw) && FMath::IsFinite(Value.Roll);
	}

	template <typename ObjectType> bool HasNoExplicitNulls(const TArray<TSoftObjectPtr<ObjectType>>& References)
	{
...
{
	return FMath::IsFinite(VolumeMultiplier) && VolumeMultiplier >= 0.0f && FMath::IsFinite(PitchMin) && FMath::IsFinite(PitchMax) && PitchMin > 0.0f &&
		PitchMax > 0.0f && PitchMin <= PitchMax && HasNoExplicitNulls(Sounds);
}

bool FGridPlayerAttackVFXDefinition::IsValid() const
{
	return HasNoExplicitNulls(Systems) && IsFiniteVector(LocationOffset) && IsFiniteRotator(RotationOffset) && IsFiniteVector(Scale) && Scale.X > 0.0f &&
		Scale.Y > 0.0f && Scale.Z > 0.0f;
}

bool FGridPlayerAttackPresentationProfile::IsValid() const
{
	const bool bMotionValid = !bAnimateHeldItem ||
		(FMath::IsFinite(MotionDurationSeconds) && MotionDurationSeconds >= 0.01f && MotionDurationSeconds <= 2.0f && IsFiniteVector(PeakLocationOffset) &&
			IsFiniteRotator(PeakRotationOffset));
	return bMotionValid && FMath::IsFinite(FeedbackDurationSeconds) && FeedbackDurationSeconds >= 0.1f && FeedbackDurationSeconds <= 10.0f &&
		AttackAudio.IsValid() && ImpactHitAudio.IsValid() && ImpactMissAudio.IsValid() && AttackVFX.IsValid() && ImpactHitVFX.IsValid() &&
		ImpactMissVFX.IsValid();
}

FText FGridPlayerAttackFeedbackFormatter::FormatRejectReason(EGridPlayerAttackRejectReason RejectReason)
{
	switch (RejectReason)
	{
		...
		case EGridPlayerAttackRejectReason::NotActiveCombatant:
			return LOCTEXT("RejectNotActiveCombatant", "Ce n’est pas le tour de ce personnage.");
...
FText FGridPlayerAttackFeedbackFormatter::FormatDamageDetail(const FGridAttackResult& Result)
{
	TArray<FText> Parts;
	if (Result.PhysicalArmorDamage > 0)
	{
		Parts.Add(FText::Format(LOCTEXT("PhysicalArmorDetail", "Armure physique : -{0}"), FText::AsNumber(Result.PhysicalArmorDamage)));
	}
	if (Result.MagicalArmorDamage > 0)
	{
		Parts.Add(FText::Format(LOCTEXT("MagicalArmorDetail", "Armure magique : -{0}"), FText::AsNumber(Result.MagicalArmorDamage)));
	}
	if (Result.HealthDamage > 0)
	{
		Parts.Add(FText::Format(LOCTEXT("HealthDetail", "Points de vie : -{0} ({1} → {2})"), FText::AsNumber(Result.HealthDamage),
			FText::AsNumber(Result.TargetHealthBefore), FText::AsNumber(Result.TargetHealthAfter)));
	}
	return FText::Join(LOCTEXT("DetailSeparator", " · "), Parts);
}

FGridPlayerAttackFeedbackRequest FGridPlayerAttackFeedbackFormatter::FormatResolved(const FGridPlayerAttackRequest& Request, const FGridAttackResult& Result,
	const FText& SourceDisplayName, const FText& TargetDisplayName, float DurationSeconds)
{
	...
	const bool bDefeated = Result.bHit && Result.TargetHealthBefore > 0 && Result.TargetHealthAfter <= 0;
	if (!Result.bHit)
	{
		Feedback.Outcome = EGridPlayerAttackFeedbackOutcome::Miss;
		Feedback.PrimaryText = FText::Format(LOCTEXT("Miss", "{0} rate {1}."), SourceDisplayName, TargetDisplayName);
		Feedback.SuggestedColor = FLinearColor(0.75f, 0.75f, 0.75f);
	}
	else if (bDefeated)
	{
		Feedback.Outcome = EGridPlayerAttackFeedbackOutcome::TargetDefeated;
		Feedback.PrimaryText = FText::Format(LOCTEXT("Defeated", "{0} vaincu : {1} dégâts."), TargetDisplayName, FText::AsNumber(Damage));
		Feedback.SuggestedColor = FLinearColor(0.95f, 0.55f, 0.15f);
	}
	else if (Result.bCriticalHit)
	{
		Feedback.Outcome = EGridPlayerAttackFeedbackOutcome::CriticalHit;
		Feedback.PrimaryText = FText::Format(LOCTEXT("Critical", "Coup critique : {0} dégâts à {1}."), FText::AsNumber(Damage), TargetDisplayName);
		Feedback.SuggestedColor = FLinearColor(1.0f, 0.8f, 0.1f);
	}
	else
	{
		Feedback.Outcome = EGridPlayerAttackFeedbackOutcome::Hit;
		Feedback.PrimaryText = FText::Format(LOCTEXT("Hit", "{0} inflige {1} dégâts à {2}."), SourceDisplayName, FText::AsNumber(Damage), TargetDisplayName);
